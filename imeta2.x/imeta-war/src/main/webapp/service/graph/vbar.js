...
* 描画
* ---------------------------------------------------------------- */
panet.graph.vbar.prototype.draw = function(items, inparams) {
	if( ! this.ctx ) {return;}
	this.items = items;
	/* パラメータの初期化 */
	var params = {
...
データラベルを描画
* ---------------------------------------------------------------- */
panet.graph.vbar.prototype._draw_data_label = function(labels) {
	var p = this.params;
...
凡例を描画
* ---------------------------------------------------------------- */
panet.graph.vbar.prototype._draw_legend = function() {
	var p = this.params;
...
panet.graph.vbar.prototype._make_path_legend_mark = function(x,y,w,h) {
	this.ctx.beginPath();
	this.ctx.moveTo(x, y);
...
補助線の位置を自動算出
* ---------------------------------------------------------------- */
panet.graph.vbar.prototype._aline_positions_auto_calc = function(min, max) {
	var range = max - min;
...
y軸目盛とキャプションを表示
* ---------------------------------------------------------------- */
panet.graph.vbar.prototype._draw_y_scale_label = function(pos) {
	var p = this.params;
	if( p.y.length > 0 ) {
		/* y軸キャプションテキスト */
...
x軸目盛とキャプションを表示
* ---------------------------------------------------------------- */
panet.graph.vbar.prototype._draw_x_scale_label = function(pos) {
	var p = this.params;
	if( p.x.length > 0 ) {
		/* x軸キャプションテキスト */
...
グラフの背景を描画
* ---------------------------------------------------------------- */
panet.graph.vbar.prototype._draw_graph_background = function() {
	var p = this.params;
	var d = p.cpos.r / 3;
...
panet.graph.vbar.prototype._draw_graph_axis = function(x, y, w, h) {
	this.ctx.save();
	var p = this.params;
	/* x軸 */
	this.ctx.beginPath();
...
panet.graph.vbar.prototype._draw_graph_background_back = function(x, y, w, h) {
	var p = this.params;
	/* 背景 */
	this.ctx.save();
	this.ctx.fillStyle = p.gBackgroundColor;
...
CANVASの背景を塗る
* ---------------------------------------------------------------- */
panet.graph.vbar.prototype._draw_canvas_background = function() {
...
文字列を描画
* ---------------------------------------------------------------- */
panet.graph.vbar.prototype._drawText = function(x, y, text, font_size, font_family, color) {
...
文字列表示領域のサイズを取得
* ---------------------------------------------------------------- */
panet.graph.vbar.prototype._getTextBoxSize = function(text, font_size, font_family) {
...
* ---------------------------------------------------------------- */
panet.graph.vbar.prototype._getElementAbsPos = function(elm) {
	var obj = new Object();
...
* CSS色文字列をRGBに変換
* ---------------------------------------------------------------- */
panet.graph.vbar.prototype._csscolor2rgb = function (c) {
	if( ! c ) { return null; }
	var color_map = {
		black: "#000000",
		gray: "#808080",
...
* 垂直棒フラフ（角柱）を描画
* ---------------------------------------------------------------- */
panet.graph.vbar.prototype._draw_vertical_bar_square = function(ctx, x, y, h, xr, yr, color, alpha, balpha, bgradation) {
...
* 垂直棒フラフ（線）を描画
* ---------------------------------------------------------------- */
panet.graph.vbar.prototype._draw_vertical_bar_line = function(ctx, x, y, h, xr, color, alpha) {
	if( typeof(alpha) != "number" || alpha < 0 || alpha > 1 ) {
		alpha = 1;
	}
...
* 垂直棒フラフ（平坦）を描画
* ---------------------------------------------------------------- */
panet.graph.vbar.prototype._draw_vertical_bar_flat = function(ctx, x, y, h, xr, color, alpha, balpha, bgradation) {
	if( typeof(alpha) != "number" || alpha < 0 || alpha > 1 ) {
		alpha = 1;
	}
	if( typeof(balpha) != "number" || balpha < 0 || balpha > 1 ) {
...
* 垂直棒フラフ（円柱）を描画
* ---------------------------------------------------------------- */
panet.graph.vbar.prototype._draw_vertical_bar_cylinder = function(ctx, x, y, h, xr, yr, color, alpha, balpha, bgradation) {
	if( typeof(alpha) != "number" || alpha < 0 || alpha > 1 ) {
		alpha = 1;
	}
	if( typeof(balpha) != "number" || balpha < 0 || balpha > 1 ) {
