...
* 描画
* ---------------------------------------------------------------- */
panet.graph.radar.prototype.draw = function(items, inparams) {
	if( ! this.ctx ) {return;}
	/* パラメータの初期化 */
	var params = {
		aCap: [],
...
補助線の位置を自動算出
* ---------------------------------------------------------------- */
panet.graph.radar.prototype._aline_positions_auto_calc = function(min, max) {
	var range = max - min;
...
凡例を描画
* ---------------------------------------------------------------- */
panet.graph.radar.prototype._draw_legend = function(items, params, cpos) {
...
panet.graph.radar.prototype._make_path_legend_mark = function(x,y,w,h) {
	this.ctx.beginPath();
	this.ctx.moveTo(x, y);
...
キャプションを描画
* ---------------------------------------------------------------- */
panet.graph.radar.prototype._draw_caption = function(params, cpos, axis_angles) {
...
スケールラベルを描画
* ---------------------------------------------------------------- */
panet.graph.radar.prototype._draw_scale_label = function(params, cpos) {
...
チャートを描画
* ---------------------------------------------------------------- */
panet.graph.radar.prototype._draw_radar_chart = function(params, cpos, axis_angles, values, color) {
	/* チャート面を塗りつぶす */
...
panet.graph.radar.prototype._make_path_for_radar_chart = function(params, cpos, axis_angles, values) {
	var r0 = 0;
...
軸を描画
* ---------------------------------------------------------------- */
panet.graph.radar.prototype._draw_axis = function(params, cpos, axis_angles) {
...
補助線を描画
* ---------------------------------------------------------------- */
panet.graph.radar.prototype._draw_aline = function(params, cpos, axis_angles) {
...
チャートの形状を描画
* ---------------------------------------------------------------- */
panet.graph.radar.prototype._draw_chart_shape = function(params, cpos, axis_angles) {
	/* チャート形状の塗り */
...
panet.graph.radar.prototype._make_path_chart_shape = function(params, cpos, axis_angles) {
	this.ctx.beginPath();
...
文字列を描画
* ---------------------------------------------------------------- */
panet.graph.radar.prototype._drawText = function(x, y, text, font_size, font_family, color) {
...
文字列表示領域のサイズを取得
* ---------------------------------------------------------------- */
panet.graph.radar.prototype._getTextBoxSize = function(text, font_size, font_family) {
...
* ---------------------------------------------------------------- */
panet.graph.radar.prototype._getElementAbsPos = function(elm) {
	var obj = new Object();
...
* CSS色文字列をRGBに変換
* ---------------------------------------------------------------- */
panet.graph.radar.prototype._csscolor2rgb = function (c) {
	if( ! c ) { return null; }
	var color_map = {
		black: "#000000",
		gray: "#808080",
