...
* 描画
* ---------------------------------------------------------------- */
panet.graph.line.prototype.draw = function(items, inparams) {
	if( ! this.ctx ) {return;}
	/* パラメータの初期化 */
	var params = {
		x: [],
...
垂直補助線を描画
* ---------------------------------------------------------------- */
panet.graph.line.prototype._draw_v_aline = function(x0, y0, y1, width, type, color) {
...
水平補助線を描画
* ---------------------------------------------------------------- */
panet.graph.line.prototype._draw_h_aline = function(x0, y0, x1, width, type, color) {
	color = this._csscolor2rgb(color);
	this.ctx.beginPath();
	color = "rgb(" + color.r + "," + color.g + "," + color.b + ")"
	this.ctx.strokeStyle = color;
...
折線のドットを描画
* ---------------------------------------------------------------- */
panet.graph.line.prototype._draw_dot = function(x, y, rad, type, color) {
	this.ctx.beginPath();
...
文字列を描画
* ---------------------------------------------------------------- */
panet.graph.line.prototype._drawText = function(x, y, text, font_size, font_family, color) {
...
文字列表示領域のサイズを取得
* ---------------------------------------------------------------- */
panet.graph.line.prototype._getTextBoxSize = function(text, font_size, font_family) {
...
* ---------------------------------------------------------------- */
panet.graph.line.prototype._getElementAbsPos = function(elm) {
	var obj = new Object();
...
* CSS色文字列をRGBに変換
* ---------------------------------------------------------------- */
panet.graph.line.prototype._csscolor2rgb = function (c) {
	if( ! c ) { return null; }
	var color_map = {
		black: "#000000",
		gray: "#808080",
