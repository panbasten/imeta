...
import com.panet.imeta.core.DBCacheEntry;
...
import com.panet.imeta.core.exception.KettleDatabaseBatchException;
...
public class Database implements VariableSpace {
	...
	public Connection getConnection() {
		try {
			...
			if (connection == null) {
				connect(this.connectionGroup, this.partitionId);
			}
		} catch (SQLException ex) {
			log.logError(toString(), "Error disconnecting from database:"
					+ Const.CR + ex.getMessage());
		} catch (KettleDatabaseException ex) {
			log.logError(toString(), "Error database:" + Const.CR
					+ ex.getMessage());
		}
		return connection;
	}
	...
	public void normalConnect(String partitionId)
		...
		try {
			...
			// does pooling on it's own.
			) {
				try {
					this.connection = ConnectionPoolUtil.getConnection(
				...
				if (!Const.isEmpty(sql) && !Const.onlySpaces(sql)) {
					...
					if (log.isDetailed())
						log.logDetailed(toString(),
								"Executed connect time SQL statements:"
										+ Const.CR + sql);
				}
			}
		} catch (Exception e) {
		...
		try {
			...
			if (databaseMeta.supportsOptionsInURL()) {
				if (!Const.isEmpty(username) || !Const.isEmpty(password)) {
					...
					connection = DriverManager.getConnection(url, Const.NVL(
							username, " "), Const.NVL(password, ""));
				} else {
	...
	public void cancelStatement(Statement statement)
		...
		try {
			...
			if (log.isDebug())
				log.logDebug(toString(), "Statement canceled!");
		} catch (SQLException ex) {
	...
	public void commit(boolean force) throws KettleDatabaseException {
		try {
			...
			if (getDatabaseMetaData().supportsTransactions()) {
				if (log.isDebug())
					log.logDebug(toString(), "Commit on database connection ["
							+ toString() + "]");
				connection.commit();
			} else {
				if (log.isDetailed())
					log.logDetailed(toString(),
							"No commit possible on database connection ["
									+ toString() + "]");
			}
		} catch (Exception e) {
	...
	public void rollback(boolean force) throws KettleDatabaseException {
		try {
			...
			if (getDatabaseMetaData().supportsTransactions()) {
				if (connection != null) {
					if (log.isDebug())
						log.logDebug(toString(),
								"Rollback on database connection ["
										+ toString() + "]");
					connection.rollback();
				}
			} else {
				if (log.isDetailed())
					log.logDetailed(toString(),
							"No rollback possible on database connection ["
									+ toString() + "]");
			}
	...
	public void prepareInsert(RowMetaInterface rowMeta, String schemaName,
		...
		if (log.isDetailed())
			log.logDetailed(toString(), "Preparing statement: " + Const.CR
					+ ins);
	...
	public RowMetaAndData getGeneratedKeys(PreparedStatement ps)
		...
		try {
			...
			RowMetaInterface rowMeta = getRowInfo(resultSetMetaData, false,
					false);
		...
		try {
			...
			if (!isAutoCommit()) {
				if (useBatchInsert) {
					...
					ps.addBatch(); // Add the batch, but don't forget to run
					// the batch
				} else {
			...
			// System.out.println("Unexpected exception in ["+debug+"] :
			// "+e.getMessage());
			throw new KettleDatabaseException(
	...
	public Result execStatement(String sql, RowMetaInterface params,
			Object[] data) throws KettleDatabaseException {
		...
		try {
			...
			if (params != null) {
				...
				// log.logDetailed(toString(), "Executing SQL Statement:
				// ["+sqlStripped+"]");
			...
			if (resultSet) {
				...
				// log.logDetailed(toString(), "What to do with ResultSet???
				// (count="+count+")");
			} else {
				if (count > 0) {
	...
	public Result execStatements(String script) throws KettleDatabaseException {
		...
		while (to < length) {
			...
			} else if (all.substring(to).startsWith("--")) // -- means: ignore
			// comment until end
			// of line...
			{
				while (to < length && c != '\n' && c != '\r') {
			...
			if (c == ';' || to >= length - 1) // end of statement
			{
				...
				String stat;
				if (to <= length)
					stat = all.substring(from, to);
				else
					stat = all.substring(from);
				...
				if (!Const.onlySpaces(stat)) {
					...
					if (sql.toUpperCase().startsWith("SELECT")) {
						...
						if (log.isDetailed())
							log.logDetailed(toString(),
									"launch SELECT statement: " + Const.CR
											+ sql);
						...
						try {
							...
							if (rs != null) {
								...
								while (row != null) {
									result.setNrLinesRead(result
											.getNrLinesRead() + 1);
									if (log.isDetailed())
										log.logDetailed(toString(), rowMeta
												.getString(row));
									row = getRow(rs);
								}
							...
							throw new KettleDatabaseException(e); // just pass
							// the error
							// upwards.
						} finally {
							try {
					...
					} else // any kind of statement
					{
						if (log.isDetailed())
							log.logDetailed(toString(),
									"launch DDL statement: " + Const.CR + sql);
		...
		if (log.isDetailed())
			log.logDetailed(toString(), nrstats + " statement"
					+ (nrstats == 1 ? "" : "s") + " executed");
	...
	public ResultSet openQuery(String sql, RowMetaInterface params,
		...
		try {
			...
			rowMeta = getRowInfo(res.getMetaData(), databaseMeta
					.getDatabaseType() == DatabaseMeta.TYPE_DATABASE_MYSQL,
					lazyConversion);
		} catch (SQLException ex) {
	...
	private boolean canWeSetFetchSize(Statement statement) throws SQLException {
		return databaseMeta.isFetchSizeSupported()
				&& (statement.getMaxRows() > 0
						|| databaseMeta.getDatabaseType() == DatabaseMeta.TYPE_DATABASE_POSTGRES || (databaseMeta
						.getDatabaseType() == DatabaseMeta.TYPE_DATABASE_MYSQL && databaseMeta
						.isStreamingResults()));
	}
	...
	public ResultSet openQuery(PreparedStatement ps, RowMetaInterface params,
		...
		try {
			...
			throw new KettleDatabaseException("ERROR executing query in part["
					+ debug + "]", ex);
		} catch (Exception e) {
			throw new KettleDatabaseException("ERROR executing query in part["
					+ debug + "]", e);
	...
	public boolean checkTableExists(String tablename)
		...
		try {
			if (log.isDebug())
				log.logDebug(toString(), "Checking if table [" + tablename
						+ "] exists!");
			 ...
			 * found=true; } } alltables.close();
			 * 
			 * return found; } else { throw new KettleDatabaseException("Unable
			 * to read table-names from the database meta-data."); } } else {
			 * throw new KettleDatabaseException("Unable to get database
	...
	public boolean checkColumnExists(String columnname, String tablename)
		...
		try {
			if (log.isDebug())
				log.logDebug(toString(), "Checking if column [" + columnname
	...
	public boolean checkIndexExists(String schemaName, String tableName,
		...
		if (log.isDebug())
			log.logDebug(toString(), "CheckIndexExists() tablename = "
					+ tablename + " type = "
					+ databaseMeta.getDatabaseTypeDesc());
		...
		try {
			...
			case DatabaseMeta.TYPE_DATABASE_MSSQL: {
				...
				sql
						.append("from     sysindexes i, sysindexkeys k, syscolumns c ");
				...
				try {
					...
					if (res != null) {
						Object[] row = getRow(res);
						while (row != null) {
							String column = rowMeta.getString(row,
									"column_name", "");
			...
			case DatabaseMeta.TYPE_DATABASE_ORACLE: {
				...
				// Get the info from the data dictionary...
				//
				String sql = "SELECT * FROM USER_IND_COLUMNS WHERE TABLE_NAME = '"
						+ tableName + "'";
				ResultSet res = null;
				try {
					res = openQuery(sql);
			...
			case DatabaseMeta.TYPE_DATABASE_ACCESS: {
				...
				try {
					indexList = getDatabaseMetaData().getIndexInfo(null, null,
	...
	public String getCreateIndexStatement(String schemaname, String tablename,
		...
		if (unique
				|| (tk && databaseMeta.getDatabaseType() == DatabaseMeta.TYPE_DATABASE_SYBASE))
			cr_index += "UNIQUE ";
		...
		if (databaseMeta.getDatabaseType() == DatabaseMeta.TYPE_DATABASE_ORACLE
				&& databaseMeta.getIndexTablespace() != null
				&& databaseMeta.getIndexTablespace().length() > 0) {
			cr_index += "TABLESPACE "
					+ databaseMeta
							.quoteField(databaseMeta.getIndexTablespace());
		}
	...
	public String getCreateSequenceStatement(String schemaName,
		...
		if (databaseMeta.supportsSequences()) {
			...
			if (max_value != null) {
				...
				// DB2 / AS400
				if ((databaseMeta.getDatabaseType() == DatabaseMeta.TYPE_DATABASE_DB2 || databaseMeta
						.getDatabaseType() == DatabaseMeta.TYPE_DATABASE_AS400)
	...
	private ValueMetaInterface getValueFromSQLType(String name,
		...
		case java.sql.Types.NUMERIC:
			...
			if (type == java.sql.Types.DOUBLE || type == java.sql.Types.FLOAT
				...
				if (precision == 0 && length < 18 && length > 0) // Among
				// others
				// Oracle is
				// affected
				// here.
			...
			if (databaseMeta.getDatabaseType() == DatabaseMeta.TYPE_DATABASE_ORACLE) {
				...
				if (precision <= 0 && length <= 0) // undefined size:
				// BIGNUMBER, precision on
				// Oracle can be 38, too big
				// for a Number type
				{
					valtype = ValueMetaInterface.TYPE_BIGNUMBER;
					length = -1;
		...
		case java.sql.Types.LONGVARBINARY:
			...
			if (databaseMeta.getDatabaseType() == DatabaseMeta.TYPE_DATABASE_DB2
				...
				length = rm.getPrecision(index);
			} else if (databaseMeta.getDatabaseType() == DatabaseMeta.TYPE_DATABASE_ORACLE
		...
		try {
			...
			if (rs.next()) {
				for (int i = 0; i < nrcols; i++) {
					...
					switch (val.getType()) {
					case ValueMetaInterface.TYPE_BOOLEAN:
						data[i] = Boolean.valueOf(rs.getBoolean(i + 1));
						break;
					case ValueMetaInterface.TYPE_NUMBER:
					...
					case ValueMetaInterface.TYPE_BIGNUMBER:
						...
						break;
					case ValueMetaInterface.TYPE_INTEGER:
						data[i] = Long.valueOf(rs.getLong(i + 1));
						break;
					case ValueMetaInterface.TYPE_STRING: {
					...
					case ValueMetaInterface.TYPE_DATE:
						if (databaseMeta.getDatabaseType() == DatabaseMeta.TYPE_DATABASE_NEOVIEW
								&& val.getOriginalColumnType() == java.sql.Types.TIME) {
							// Neoview can not handle getDate / getTimestamp for
							// a Time column
							data[i] = rs.getTime(i + 1);
							break; // Time is a subclass of java.util.Date, the
							// default date will be 1970-01-01
						} else if (val.getPrecision() != 1
								&& databaseMeta
										.supportsTimeStampToDateConversion()) {
					...
					if (rs.wasNull())
						data[i] = null; // null value, it's the default but we
					// want it just to make sure we handle
					// this case too.
				}
			} else {
				data = null;
	...
	public void setLookup(String schemaName, String tableName, String codes[],
			String condition[], String gets[], String rename[], String orderby,
			boolean checkForMultipleResults) throws KettleDatabaseException {
		String table = databaseMeta.getQuotedSchemaTableCombination(schemaName,
				tableName);
		...
		try {
			if (log.isDetailed())
				log.logDetailed(toString(), "Setting preparedStatement to ["
						+ sql + "]");
			...
			throw new KettleDatabaseException(
					"Unable to prepare statement for update [" + sql + "]", ex);
		}
	}
	...
	public boolean prepareUpdate(String schemaName, String tableName,
		...
		try {
			...
			if (log.isDetailed())
				log.logDetailed(toString(),
						"Setting update preparedStatement to [" + s + "]");
	...
	public boolean prepareDelete(String schemaName, String tableName,
		...
		try {
			if (log.isDetailed())
				log.logDetailed(toString(),
						"Setting update preparedStatement to [" + sql + "]");
			prepStatementUpdate = connection.prepareStatement(databaseMeta
					.stripCR(sql));
		} catch (SQLException ex) {
			printSQLException(ex);
	...
	public void setProcLookup(String proc, String arg[], String argdir[],
		...
		try {
			if (log.isDetailed())
				log.logDetailed(toString(),
						"DBA setting callableStatement to [" + sql + "]");
	...
	public DatabaseMetaData getDatabaseMetaData()
		...
		try {
			if (dbmd == null)
				dbmd = connection.getMetaData(); // Only get the metadata
			// once!
		} catch (Exception e) {
			throw new KettleDatabaseException(
	...
	public String getCreateTableStatement(String tableName,
		...
		if (pk == null
				&& tk == null
				&& databaseMeta.getDatabaseType() == DatabaseMeta.TYPE_DATABASE_NEOVIEW) {
			retval += "NO PARTITION"; // use this as a default when no pk/tk
			// is there, otherwise you get an error
		}
	...
	public String getAlterTableStatement(String tableName,
		...
		//
		// OK, see if there are fields for wich we need to modify the type...
		// (length, precision)
		//
		...
		for (int i = 0; i < fields.size(); i++) {
			...
			if (currentField != null) {
				...
				mod |= (currentField.getLength() < desiredField.getLength())
						&& desiredField.getLength() > 0;
				mod |= (currentField.getPrecision() < desiredField
						.getPrecision())
						&& desiredField.getPrecision() > 0;

				// Numeric values...
				mod |= (currentField.getType() != desiredField.getType())
						&& (currentField.isNumber() ^ desiredField.isNumeric());

				// TODO: this is not an optimal way of finding out changes.
				// Perhaps we should just generate the data types strings for
				// existing and new data type and see if anything changed.
				// 

				if (mod) {
					// System.out.println("Desired field:
					// ["+desiredField.toStringMeta()+"], current field:
					// ["+currentField.toStringMeta()+"]");
	...
	public static final RowMetaInterface getTransLogrecordFields(
			boolean update, boolean use_batchid, boolean use_logfield) {
		RowMetaInterface r = new RowMeta();
		ValueMetaInterface v;

		if (use_batchid && !update) {
			v = new ValueMeta("ID_BATCH", ValueMetaInterface.TYPE_INTEGER, 8, 0);
			r.addValueMeta(v);
		}

		if (!update) {
			v = new ValueMeta("TRANSNAME", ValueMetaInterface.TYPE_STRING, 255,
					0);
			r.addValueMeta(v);
		}
		v = new ValueMeta("STATUS", ValueMetaInterface.TYPE_STRING, 15, 0);
		r.addValueMeta(v);
		v = new ValueMeta("LINES_READ", ValueMetaInterface.TYPE_INTEGER, 10, 0);
		r.addValueMeta(v);
		v = new ValueMeta("LINES_WRITTEN", ValueMetaInterface.TYPE_INTEGER, 10,
				0);
		r.addValueMeta(v);
		v = new ValueMeta("LINES_UPDATED", ValueMetaInterface.TYPE_INTEGER, 10,
				0);
		r.addValueMeta(v);
		v = new ValueMeta("LINES_INPUT", ValueMetaInterface.TYPE_INTEGER, 10, 0);
		r.addValueMeta(v);
		v = new ValueMeta("LINES_OUTPUT", ValueMetaInterface.TYPE_INTEGER, 10,
				0);
		r.addValueMeta(v);
		v = new ValueMeta("ERRORS", ValueMetaInterface.TYPE_INTEGER, 10, 0);
		r.addValueMeta(v);
		v = new ValueMeta("STARTDATE", ValueMetaInterface.TYPE_DATE);
		r.addValueMeta(v);
		v = new ValueMeta("ENDDATE", ValueMetaInterface.TYPE_DATE);
		r.addValueMeta(v);
		v = new ValueMeta("LOGDATE", ValueMetaInterface.TYPE_DATE);
		r.addValueMeta(v);
		v = new ValueMeta("DEPDATE", ValueMetaInterface.TYPE_DATE);
		r.addValueMeta(v);
		v = new ValueMeta("REPLAYDATE", ValueMetaInterface.TYPE_DATE);
		r.addValueMeta(v);
		v = new ValueMeta("OPERATEUSER", ValueMetaInterface.TYPE_STRING, 255, 0);
		r.addValueMeta(v);
		v = new ValueMeta("IVOKETYPE", ValueMetaInterface.TYPE_STRING, 15, 0);
		r.addValueMeta(v);

		if (use_logfield) {
			v = new ValueMeta("LOG_FIELD", ValueMetaInterface.TYPE_STRING,
	...
	public static final RowMetaInterface getJobLogrecordFields(boolean update,
			boolean use_jobid, boolean use_logfield) {
		RowMetaInterface r = new RowMeta();
		ValueMetaInterface v;

		if (use_jobid && !update) {
			v = new ValueMeta("ID_JOB", ValueMetaInterface.TYPE_INTEGER, 8, 0);
			r.addValueMeta(v);
		}

		if (!update) {
			v = new ValueMeta("JOBNAME", ValueMetaInterface.TYPE_STRING, 255, 0);
			r.addValueMeta(v);
		}
		v = new ValueMeta("STATUS", ValueMetaInterface.TYPE_STRING, 15, 0);
		r.addValueMeta(v);
	...
	public void writeLogRecord(String logtable, boolean use_id, long id,
			...
			java.util.Date replayDate, String operateUser, String ivokeType,
			String log_string) throws KettleDatabaseException {
		...
		if (update) {
			...
			Object[] data = new Object[] { status, Long.valueOf(read),
					Long.valueOf(written), Long.valueOf(input),
					Long.valueOf(output), Long.valueOf(updated),
					Long.valueOf(errors), startdate, enddate, logdate, depdate,
					replayDate, operateUser, ivokeType, log_string,
					Long.valueOf(id), };
	...
	public static final RowMetaInterface getStepPerformanceLogrecordFields() {
		...
		v = new ValueMeta("ID_BATCH", ValueMetaInterface.TYPE_INTEGER, 8, 0);
		r.addValueMeta(v);
		v = new ValueMeta("SEQ_NR", ValueMetaInterface.TYPE_INTEGER, 8, 0);
		r.addValueMeta(v);
		v = new ValueMeta("LOGDATE", ValueMetaInterface.TYPE_DATE);
		r.addValueMeta(v);
		v = new ValueMeta("TRANSNAME", ValueMetaInterface.TYPE_STRING, 255, 0);
		r.addValueMeta(v);
		v = new ValueMeta("STEPNAME", ValueMetaInterface.TYPE_STRING, 255, 0);
		r.addValueMeta(v);
		v = new ValueMeta("STEP_COPY", ValueMetaInterface.TYPE_INTEGER, 3, 0);
		r.addValueMeta(v);
		v = new ValueMeta("LINES_READ", ValueMetaInterface.TYPE_INTEGER, 10, 0);
		r.addValueMeta(v);
		v = new ValueMeta("LINES_WRITTEN", ValueMetaInterface.TYPE_INTEGER, 10,
		...
		v = new ValueMeta("LINES_OUTPUT", ValueMetaInterface.TYPE_INTEGER, 10,
				0);
		r.addValueMeta(v);
		v = new ValueMeta("LINES_REJECTED", ValueMetaInterface.TYPE_INTEGER,
				10, 0);
		r.addValueMeta(v);
		v = new ValueMeta("ERRORS", ValueMetaInterface.TYPE_INTEGER, 10, 0);
		r.addValueMeta(v);
		v = new ValueMeta("INPUT_BUFFER_ROWS", ValueMetaInterface.TYPE_INTEGER,
				10, 0);
		r.addValueMeta(v);
		v = new ValueMeta("OUTPUT_BUFFER_ROWS",
	...
	public Object[] getLastLogDate(String logtable, String name, boolean job,
			String status) throws KettleDatabaseException {
		Object[] row = null;
	...
	public boolean isSystemTable(String table_name) {
		if (databaseMeta.getDatabaseType() == DatabaseMeta.TYPE_DATABASE_MSSQL) {
			...
			if (table_name.equals("dtproperties"))
				return true;
		} else if (databaseMeta.getDatabaseType() == DatabaseMeta.TYPE_DATABASE_GUPTA) {
			if (table_name.startsWith("SYS"))
				return true;
		}
		return false;
	...
	public String[] getTablenames(boolean includeSchema)
		...
		try {
			...
			while (alltables.next()) {
				...
				try {
					...
					if (log.isDebug())
						log.logDebug(toString(),
								"Error getting tables for field TABLE_CAT (ignored): "
										+ e.toString());
				}
				...
				try {
					...
					if (log.isDebug())
						log.logDebug(toString(),
								"Error getting tables for field TABLE_SCHEM (ignored): "
										+ e.toString());
				}
				...
				String schemaTable;
				if (includeSchema)
					schemaTable = databaseMeta.getQuotedSchemaTableCombination(
							schema, table);
				else
					schemaTable = table;
			...
			log.logError(toString(), "Error getting tablenames from schema ["
					+ schemaname + "]");
		} finally {
			try {
				if (alltables != null)
		...
		if (log.isDetailed())
			log.logDetailed(toString(), "read :" + names.size()
	...
	public String[] getViews(boolean includeSchema)
		...
		ResultSet alltables = null;
		try {
			alltables = dbmd.getTables(null, schemaname, null, databaseMeta
					.getViewTypes());
			while (alltables.next()) {
				// due to PDI-743 with ODBC and MS SQL Server the order is
				// changed and try/catch included for safety
				...
				try {
					...
					if (log.isDebug())
						log.logDebug(toString(),
								"Error getting views for field TABLE_CAT (ignored): "
										+ e.toString());
				}
				...
				try {
					...
					if (log.isDebug())
						log.logDebug(toString(),
								"Error getting views for field TABLE_SCHEM (ignored): "
										+ e.toString());
				}
		...
		try {
			...
			while (alltables.next()) {
				...
				try {
					...
					if (log.isDebug())
						log.logDebug(toString(),
								"Error getting synonyms for field TABLE_CAT (ignored): "
										+ e.toString());
				}
				...
				try {
					...
					if (log.isDebug())
						log.logDebug(toString(),
								"Error getting synonyms for field TABLE_SCHEM (ignored): "
										+ e.toString());
				}
	...
	public Savepoint setSavepoint() throws KettleDatabaseException {
		try {
			...
			throw new KettleDatabaseException(Messages
					.getString("Database.Exception.UnableToSetSavepoint"), e);
		}
	}
	...
	public void releaseSavepoint(Savepoint savepoint)
		...
		try {
			...
			throw new KettleDatabaseException(Messages
					.getString("Database.Exception.UnableToReleaseSavepoint"),
					e);
		}
	}
	...
	public void rollback(Savepoint savepoint) throws KettleDatabaseException {
		try {
			...
			throw new KettleDatabaseException(
					Messages
							.getString("Database.Exception.UnableToRollbackToSavepoint"),
					e);
		}
	}
