...
/**
 * This class contains the basic information on a database connection. It is not
 * intended to be used other than the inheriting classes such as
 * OracleDatabaseInfo, ...
 * 
 * @author Matt
 * @since 11-mrt-2005
...
public abstract class BaseDatabaseMeta implements Cloneable {
	...
	public static final DatabaseConnectionPoolParameter[] poolingParameters = new DatabaseConnectionPoolParameter[] {
			new DatabaseConnectionPoolParameter(
					...
					"true",
					Messages
							.getString("BaseDatabaseMeta.PoolParameter.DefaultAutoCommit.Desc")),
			new DatabaseConnectionPoolParameter(
					"defaultReadOnly",
					null,
					Messages
							.getString("BaseDatabaseMeta.PoolParameter.DefaultReadOnly.Desc")),
			new DatabaseConnectionPoolParameter(
					"defaultTransactionIsolation",
					null,
					Messages
							.getString("BaseDatabaseMeta.PoolParameter.DefaultTransactionIsolation.Desc")),
			new DatabaseConnectionPoolParameter(
					"defaultCatalog",
					null,
					Messages
							.getString("BaseDatabaseMeta.PoolParameter.DefaultCatalog.Desc")),

			new DatabaseConnectionPoolParameter(
					"initialSize",
					"0",
					Messages
							.getString("BaseDatabaseMeta.PoolParameter.InitialSize.Desc")),
			new DatabaseConnectionPoolParameter("maxActive", "8", Messages
					.getString("BaseDatabaseMeta.PoolParameter.MaxActive.Desc")),
			new DatabaseConnectionPoolParameter("maxIdle", "8", Messages
					.getString("BaseDatabaseMeta.PoolParameter.MaxIdle.Desc")),
			new DatabaseConnectionPoolParameter("minIdle", "0", Messages
					.getString("BaseDatabaseMeta.PoolParameter.MinIdle.Desc")),
			new DatabaseConnectionPoolParameter("maxWait", "-1", Messages
					.getString("BaseDatabaseMeta.PoolParameter.MaxWait.Desc")),

			new DatabaseConnectionPoolParameter(
					"validationQuery",
					null,
					Messages
							.getString("BaseDatabaseMeta.PoolParameter.ValidationQuery.Desc")),
			new DatabaseConnectionPoolParameter(
					"testOnBorrow",
					"true",
					Messages
							.getString("BaseDatabaseMeta.PoolParameter.TestOnBorrow.Desc")),
			new DatabaseConnectionPoolParameter(
					"testOnReturn",
					"false",
					Messages
							.getString("BaseDatabaseMeta.PoolParameter.TestOnReturn.Desc")),
			new DatabaseConnectionPoolParameter(
					"testWhileIdle",
					"false",
					Messages
							.getString("BaseDatabaseMeta.PoolParameter.TestWhileIdle.Desc")),
			new DatabaseConnectionPoolParameter(
					"timeBetweenEvictionRunsMillis",
					null,
					Messages
							.getString("BaseDatabaseMeta.PoolParameter.TimeBetweenEvictionRunsMillis.Desc")),

			new DatabaseConnectionPoolParameter(
					"poolPreparedStatements",
					"false",
					Messages
							.getString("BaseDatabaseMeta.PoolParameter.PoolPreparedStatements.Desc")),
			new DatabaseConnectionPoolParameter(
					"maxOpenPreparedStatements",
					"-1",
					Messages
							.getString("BaseDatabaseMeta.PoolParameter.MaxOpenPreparedStatements.Desc")),
			new DatabaseConnectionPoolParameter(
					"accessToUnderlyingConnectionAllowed",
					"false",
					Messages
							.getString("BaseDatabaseMeta.PoolParameter.AccessToUnderlyingConnectionAllowed.Desc")),
			new DatabaseConnectionPoolParameter(
					"removeAbandoned",
					"false",
					Messages
							.getString("BaseDatabaseMeta.PoolParameter.RemoveAbandoned.Desc")),
			new DatabaseConnectionPoolParameter(
					"removeAbandonedTimeout",
					"300",
					Messages
							.getString("BaseDatabaseMeta.PoolParameter.RemoveAbandonedTimeout.Desc")),
			new DatabaseConnectionPoolParameter(
					"logAbandoned",
					"false",
					Messages
							.getString("BaseDatabaseMeta.PoolParameter.LogAbandoned.Desc")), };
	...
	private String dataTablespace; // data storage location, For Oracle &
	// perhaps others
	private String indexTablespace; // index storage location, For Oracle &
	// perhaps others
	...
	/**
	 * Constructs a new database connections. Note that not all these parameters
	 * are not allways mandatory.
	 * 
	 * @param name
	 *            The database name
	 * @param access
	 ...
	 *            The password
	 */
	public BaseDatabaseMeta(String name, String access, String host, String db,
	 ...
	 * @param sequenceName
	 *            The sequence to check
	 * @return The SQL to get the name of the sequence back from the databases
	 *         data dictionary
	 */
	 ...
	 *         example Informix does not on a non-ANSI database type!)
	 */
	...
	/**
	 * Get the schema-table combination to query the right table. Usually that
	 * is SCHEMA.TABLENAME, however there are exceptions to this rule...
	 * 
	 * @param schema_name
	 *            The schema name
	 * @param table_part
	...
	public void setMaximumPoolSize(int maximumPoolSize) {
		attributes.setProperty(ATTRIBUTE_MAXIMUM_POOL_SIZE, Integer
	...
	public void setInitialPoolSize(int initialPoolSize) {
		attributes.setProperty(ATTRIBUTE_INITIAL_POOL_SIZE, Integer
				.toString(initialPoolSize));
	}
	...
	public PartitionDatabaseMeta[] getPartitioningInformation() {
		// find the maximum number of attributes starting with
		...
		for (nr = 0; nr < clusterInfo.length; nr++) {
			String partitionId = attributes
					.getProperty(ATTRIBUTE_CLUSTER_PARTITION_PREFIX + nr);
			String hostname = attributes
					.getProperty(ATTRIBUTE_CLUSTER_HOSTNAME_PREFIX + nr);
			String port = attributes.getProperty(ATTRIBUTE_CLUSTER_PORT_PREFIX
					+ nr);
			String dbName = attributes
					.getProperty(ATTRIBUTE_CLUSTER_DBNAME_PREFIX + nr);
			String username = attributes
					.getProperty(ATTRIBUTE_CLUSTER_USERNAME_PREFIX + nr);
			String password = attributes
					.getProperty(ATTRIBUTE_CLUSTER_PASSWORD_PREFIX + nr);
			clusterInfo[nr] = new PartitionDatabaseMeta(partitionId, hostname,
					port, dbName);
			clusterInfo[nr].setUsername(username);
			clusterInfo[nr].setPassword(Encr
					.decryptPasswordOptionallyEncrypted(password));
		}
	...
	public void setPartitioningInformation(PartitionDatabaseMeta[] clusterInfo) {
		for (int nr = 0; nr < clusterInfo.length; nr++) {
			...
			attributes.put(ATTRIBUTE_CLUSTER_PARTITION_PREFIX + nr, Const.NVL(
					meta.getPartitionId(), ""));
			attributes.put(ATTRIBUTE_CLUSTER_HOSTNAME_PREFIX + nr, Const.NVL(
					meta.getHostname(), ""));
			attributes.put(ATTRIBUTE_CLUSTER_PORT_PREFIX + nr, Const.NVL(meta
					.getPort(), ""));
			attributes.put(ATTRIBUTE_CLUSTER_DBNAME_PREFIX + nr, Const.NVL(meta
					.getDatabaseName(), ""));
			attributes.put(ATTRIBUTE_CLUSTER_USERNAME_PREFIX + nr, Const.NVL(
					meta.getUsername(), ""));
			attributes.put(ATTRIBUTE_CLUSTER_PASSWORD_PREFIX + nr, Const
					.NVL(Encr.encryptPasswordIfNotUsingVariables(meta
							.getPassword()), ""));
		}
	}

	/**
	 * @return The set of properties (newly created object) that contains the
	 *         connection pooling parameters All environment variables will be
	 *         replaced here.
	 */
	...
	/**
	 * You can use this method to supply an alternate factory for the test
	 * method in the dialogs. This is useful for plugins like SAP/R3 and PALO.
	 * 
