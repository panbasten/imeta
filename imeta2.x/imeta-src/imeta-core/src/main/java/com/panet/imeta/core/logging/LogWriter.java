...
public class LogWriter implements LogWriterInterface {
	...
	public static final String logLevelDescription[] = { "Nothing", "Error",
			"Minimal", "Basic", "Detailed", "Debug", "Rowlevel", };
	...
	public static final String log_level_index[] = {
		"0","1","2","3","4","5","6",
	};
	...
	// Default: screen --> out
	public LogWriter(int lvl) {
		this();

		// Check if there already is a console appender (ConsoleAppender) (in
		// the app server for example)
		// 
		boolean found = false;
		Enumeration<?> appenders = rootLogger.getAllAppenders();

		// Set the logging level for the Jackcess stuff to INFO...
		// TODO: make some kind of general configuration possible for this, load
		// from file, etc.
		Logger.getLogger("com.healthmarketscience.jackcess").setLevel(
				Level.INFO);
		Logger.getLogger("com.panet.imeta").setLevel(Level.INFO);
		...
		System
				.setProperty(
						"org.apache.commons.logging.simplelog.log.org.apache.commons.httpclient.auth",
						"warn");

		System.setProperty(
				"org.apache.commons.logging.simplelog.log.com.panet.imeta",
				"info");

		while (appenders.hasMoreElements()) {
			Object appender = appenders.nextElement();
			if (appender instanceof ConsoleAppender
					|| appender instanceof Log4jConsoleAppender)
				found = true;
		}
	 ...
	 * @param exact
	 *            is this an exact filename (false: prefix of name in temp
	 *            directory)
	 * @param level
	 *            The log level
	...
	public static final LogWriter getInstance(String filename, boolean exact,
			int level) throws KettleException {
		if (logWriter != null) {
			...
			//
			if (logWriter.rootLogger.getAppender(LogWriter
					.createFileAppenderName(filename, exact)) == null) {
	...
	/**
	 * Closes the file appender opened by the getInstance(filename, exact,
	 * level) method
	 */
	...
	public LogWriter(String filename, boolean exact, int level) {
		this();
	 ...
	 * 
	 * @param filename
	 *            The (VFS) filename (URL) to write to.
	 * @param exact
	 *            is this an exact filename of a filename to be stored in
	 *            "java.io.tmp"
	 * @return A new file appender
	 * @throws KettleFileException
	 *             In case there is a problem opening the file.
	 */
	public static final Log4jFileAppender createFileAppender(String filename,
			boolean exact) throws KettleFileException {
		try {
			FileObject file;
			if (!exact) {
				file = KettleVFS.createTempFile(filename, ".log", System
			...
			throw new KettleFileException(
					"Unable to add Kettle file appender to Log4J", e);
		}
	}
	 ...
	 */
	public static final Log4jFileAppender createFileAppender(String filename,
			boolean exact, boolean append) throws KettleFileException {
		try {
			FileObject file;
			if (!exact) {
				file = KettleVFS.createTempFile(filename, ".log", System
	 ...
	 * @deprecated : get the layout and use that object
	 * @param tim
	 *            true is the time has to be is added
	 */
	...
	public void println(int lvl, String subj, String msg, Object... args) {
		println(level, lvl, subj, msg, args);
	}

	public void println(int tlvl, int lvl, String subj, String msg,
			Object... args) {
		// do cheapest filtering checks first
		if (tlvl == LOG_LEVEL_NOTHING)
			return; // Nothing, not even errors...
		if (tlvl < lvl)
		...
		// ensure all messages get logged in this logger since we filtered it
		// above
		// we do not set the level in the rootLogger so the rootLogger can
		// decide by itself (e.g. in the plattform)
	...
	public static final int getLogLevel(String lvl) {
		...
		for (int i = 0; i < logLevelDescription.length; i++) {
			if (logLevelDescription[i].equalsIgnoreCase(lvl))
				return i;
		}
		for (int i = 0; i < log_level_desc_long.length; i++) {
			if (log_level_desc_long[i].equalsIgnoreCase(lvl))
				return i;
		}
		for (int i = 0; i< log_level_index.length; i++) {
			if (log_level_index[i].equalsIgnoreCase(lvl))
				return i;
		}
	...
	public static final String getLogLevelDesc(int l) {
		if (l < 0 || l >= logLevelDescription.length)
			return logLevelDescription[LOG_LEVEL_BASIC];
	 ...
	 * up using the filename
	 * 
	 * @param filename
	 *            The exact filename (with path: c:\temp\logfile.txt) or just a
	 *            filename (spoon.log)
	 * @param exact
	 *            true if this is the exact filename or just the last part of
	 *            the complete path.
	 * @return The file input stream of the appender
	 * @throws IOException
	 *             in case the appender ocan't be found
	 */
	public FileInputStream getFileInputStream(String filename, boolean exact)
			throws IOException {
		Logger logger = Logger.getRootLogger();
		Appender appender = logger.getAppender(createFileAppenderName(filename,
				exact));
		if (appender == null) {
			throw new IOException("Unable to find appender for file: "
					+ filename + " (exact=" + exact + ")");
		}
		return KettleVFS.getFileInputStream(((Log4jFileAppender) appender)
				.getFile());
	}
	...
	/**
	 * @deprecated please create your own StringAppender, otherwise this is not
	 *             thread safe!
	 * @return The logging text from since startStringCapture() is called until
	 *         endStringCapture().
	 */
	 ...
	 *             thread safe!
	 * @param string
	 *            the string to set on the string appender buffer
	 */
	...
	public static void setLayout(Layout layout) {
		...
		Enumeration<?> appenders = getInstance().getRootLogger()
				.getAllAppenders();
		while (appenders.hasMoreElements()) {
