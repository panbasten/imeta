 ...
 */
package com.panet.imeta.core.util;
...
public class EnvUtil {
	...
	public static Properties readProperties(String fileName) {
		...
		try {
			Enumeration<URL> resources = EnvUtil.class.getClassLoader()
					.getResources(fileName);
			while (resources.hasMoreElements()) {
				URL url = resources.nextElement();
				Object content = url.getContent();
				if (content instanceof InputStream) {
					props.load((InputStream) content);
				}
				((InputStream) content).close();
			}
		} catch (IOException ioe) {
			log.logDetailed("Kettle Environment", "Unable to read file '"
	...
	public static void environmentInit() {
		// Workaround for a Mac OS/X Leopard issue where getContextClassLoader()
		// is returning
		// null when run from the eclipse IDE
		// http://lists.apple.com/archives/java-dev/2007/Nov/msg00385.html - DM
		// Moving this hack to the first place where the NPE is triggered so all
		// entrypoints can be debugged in Mac Eclipse
		if (Thread.currentThread().getContextClassLoader() == null) {
			Thread.currentThread().setContextClassLoader(
					ClassLoader.getSystemClassLoader());
		}

		Map imetaProperties = EnvUtil.readProperties(Const.IMETA_PROPERTIES);
		System.getProperties().putAll(imetaProperties);

		// Also put some default values for obscure environment variables in
		// there...
		// Place-holders if you will.
		// 
		System.getProperties().put(Const.INTERNAL_VARIABLE_CLUSTER_SIZE, "1");
		System.getProperties().put(Const.INTERNAL_VARIABLE_SLAVE_SERVER_NUMBER,
				"0");
		System.getProperties().put(Const.INTERNAL_VARIABLE_SLAVE_SERVER_NAME,
				"slave-trans-name");

		System.getProperties().put(Const.INTERNAL_VARIABLE_STEP_COPYNR, "0");
		System.getProperties().put(Const.INTERNAL_VARIABLE_STEP_NAME,
				"step-name");
		System.getProperties().put(Const.INTERNAL_VARIABLE_STEP_PARTITION_ID,
				"partition-id");
		System.getProperties().put(Const.INTERNAL_VARIABLE_STEP_PARTITION_NR,
				"0");
		System.getProperties().put(Const.INTERNAL_VARIABLE_STEP_UNIQUE_COUNT,
				"1");
		System.getProperties().put(Const.INTERNAL_VARIABLE_STEP_UNIQUE_NUMBER,
				"0");
	}
	...
	public static void addInternalVariables(Properties prop) {
		...
		prop.put(Const.INTERNAL_VARIABLE_KETTLE_BUILD_VERSION, BuildVersion
				.getInstance().getVersion());

		// The Kettle build date
		prop.put(Const.INTERNAL_VARIABLE_KETTLE_BUILD_DATE, BuildVersion
				.getInstance().getBuildDate());
	}

	/**
	 * Get System.getenv() in a reflection kind of way. The problem is that
	 * System.getenv() was deprecated in Java 1.4 while reinstated in 1.5 This
	 * method will get at getenv() using reflection and will return empty
	 * properties when used in 1.4
	 * 
	 * @return Properties containing the environment. You're not meant to change
	 *         any value in the returned Properties!
	 * 
	 */
	@SuppressWarnings( { "unchecked" })
	private static final Properties getEnv() {
		...
		if (env == null) {
			...
			if (returnMap != null) {
				...
				ArrayList<String> list = new ArrayList<String>(returnMap
						.keySet());
