...
import com.panet.imeta.laf.BasePropertyHandler;
...
public class Const {
	...
	public static final String USER_HOME_DIRECTORY = NVL(System
			.getProperty("KETTLE_HOME"), System.getProperty("user.home"));
	...
	/**
	 * Public directory containing external nodes plugins
	 */
	public static final String PLUGIN_ENTITYNODES_DIRECTORY_PUBLIC = "plugins"
			+ FILE_SEPARATOR + "nodes";

	/**
	 * Private directory containing external nodes plugins
	 */
	public static final String PLUGIN_ENTITYNODES_DIRECTORY_PRIVATE = getKettleDirectory()
			+ FILE_SEPARATOR + "plugins" + FILE_SEPARATOR + "nodes";

	/**
	 * Public directory containing external steps plugins
	 */
	public static final String PLUGIN_STEPS_DIRECTORY_PUBLIC = "plugins"
			+ FILE_SEPARATOR + "steps";

	/**
	 * Private directory containing external steps plugins
	 */
	public static final String PLUGIN_STEPS_DIRECTORY_PRIVATE = getKettleDirectory()
			+ FILE_SEPARATOR + "plugins" + FILE_SEPARATOR + "steps";

	/**
	 * Public directory containing external job entries plugins
	 */
	public static final String PLUGIN_JOBENTRIES_DIRECTORY_PUBLIC = "plugins"
			+ FILE_SEPARATOR + "jobentries";

	/**
	 * Private directory containing external job entries plugins
	 */
	public static final String PLUGIN_JOBENTRIES_DIRECTORY_PRIVATE = getKettleDirectory()
			+ FILE_SEPARATOR + "plugins" + FILE_SEPARATOR + "jobentries";
	 ...
	 */
	public static final String GENERALIZED_DATE_TIME_SHOW_FORMAT = "yyyy-MM-dd HH:mm:ss";
	...
	public static final String STRING_TRANS_DEFAULT_EXT = "ptr";
	...
	public static final String STRING_JOB_DEFAULT_EXT = "pjb";
	...
	public static final String IMETA_PROPERTIES = "imeta.properties";
	...
	/**
	 * You can set this environment variable to make Kettle scan additional
	 * packages for annotated plugins. The packages are comma (,) delimited and
	 * the base specification is in the kettle-config.xml file
	 */
	 ...
	 * <code>yyyy/MM/dd HH:mm:ss.SSS</code>. If the conversion fails, assign
	 * a default value.
	 * 
	 * @param str
	...
	public static final String getKettleDirectory() {
		return USER_HOME_DIRECTORY + FILE_SEPARATOR
				+ BasePropertyHandler.getProperty("userBaseDir", ".kettle");
	}
	...
	public static final String getKettleLocalPropsFile() {
		return "/imeta.properties";
	}
	...
	public static final String[] splitPath(String path, String separator) {
		...
		for (int i = from; i < path.length(); i += sepLen) {
			if (path.substring(i, i + sepLen).equalsIgnoreCase(separator)) {
				...
				// System.out.println(nr+" --> ["+spath[nr]+"],
				// (from,to)=("+from+", "+i+")");
				nr++;
		...
		if (nr < spath.length) {
			...
			// System.out.println(nr+" --> ["+spath[nr]+"], (from,to)=("+from+",
			// "+path.length()+")");
		}
	 ...
	 * returns <code>catalina.out</code>. The method works with the
	 * Environment variable <i>System.getProperty("file.separator")</i>, so on
	 * linux/Unix it will check for the last occurrence of a frontslash, on
	 * windows for the last occurrence of a backslash.
	 * </p>
	 * 
	 * <p>
	...
	public static String[] getTips() {
		if (tips == null) {
			int tipsOfDayCount = toInt(Messages
					.getString("Const.TipOfDay.Count"), 0);
			tips = new String[tipsOfDayCount];
			for (int i = 1; i <= tipsOfDayCount; i++)
				tips[i - 1] = Messages.getString("Const.TipOfDay"
						+ Integer.toString(i));
		}
	...
	public static String[] getDateFormats() {
		if (dateFormats == null) {
			int dateFormatsCount = toInt(Messages
					.getString("Const.DateFormat.Count"), 0);
			dateFormats = new String[dateFormatsCount];
			for (int i = 1; i <= dateFormatsCount; i++)
				dateFormats[i - 1] = Messages.getString("Const.DateFormat"
						+ Integer.toString(i));
		}
		return dateFormats;
	}
	...
	public static String[] getNumberFormats() {
		if (numberFormats == null) {
			int numberFormatsCount = toInt(Messages
					.getString("Const.NumberFormat.Count"), 0);
			numberFormats = new String[numberFormatsCount + 1];
			numberFormats[0] = DEFAULT_NUMBER_FORMAT;
			for (int i = 1; i <= numberFormatsCount; i++)
				numberFormats[i] = Messages.getString("Const.NumberFormat"
						+ Integer.toString(i));
		}
