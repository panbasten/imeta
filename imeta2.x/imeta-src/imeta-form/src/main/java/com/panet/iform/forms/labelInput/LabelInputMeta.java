 ...
 * into with Sinosoft.
 */
package com.panet.iform.forms.labelInput;
...
import com.panet.iform.core.base.InputMeta;
...
*@since 2009/06/22
*/ 
public class LabelInputMeta extends ComplexFormMeta {
	 ...
	 *            Input验证
	 */
	public LabelInputMeta(String id, String label, String clazz,
			Map<String, String> style, String title, String value, String type,
			ValidateForm validate) {
		LabelDataMeta labelDataMeta;
		if (InputDataMeta.INPUT_TYPE_RADIO.equals(type)) {
			labelDataMeta = new LabelDataMeta(id + value + ".label", id + value
					+ ".label", label, id);
		} else {
			labelDataMeta = new LabelDataMeta(id + ".label", id + ".label",
					label, id);
		}
		InputDataMeta inputDataMeta = new InputDataMeta(id, id, null, null,
				title, value, type);
		...
		init(labelDataMeta, inputDataMeta, validate);
	}
	 ...
	 *            Input验证
	 */
	public LabelInputMeta(LabelDataMeta labelDataMeta,
			InputDataMeta inputDataMeta, ValidateForm validate) {
		init(labelDataMeta, inputDataMeta, validate);
	}
	 ...
	 *            Input验证
	 */
	public void init(LabelDataMeta labelDataMeta, InputDataMeta inputDataMeta,
			ValidateForm validate) {
		super.setMultiRoot(true);
		this.label = new LabelMeta(labelDataMeta, false);
		this.input = new InputMeta(inputDataMeta, validate, false);
	...
	public void addListener(String key, String value) {
		this.input.addListener(key, value);
	}

	@Override
	public void addClick(String click) {
		this.input.addClick(click);
	}
	...
	public void setWidth(int width) {
		if (LAYOUT_COLUMN.equals(this.layout)) {
			if (this.labelAfter) {
				...
				if (this.input != null) {
					this.input.setWidth(width - getButtonWidth());
				}
			}
			return;
		}
		...
		if (this.isSingle()) {
			l = 0.3;
			i = 0.65;
			if (InputDataMeta.INPUT_TYPE_CHECKBOX.equals(this.input.getType())
					|| InputDataMeta.INPUT_TYPE_RADIO.equals(this.input
							.getType())) {
				i = 0.1;
				l = 0.85;
			}
		...
		if (this.labelAfter) {
			if (this.input != null) {
				this.input.setWidth((int) (width * i));
			}
			if (this.label != null) {
			...
			if (this.input != null) {
				...
				this.input.setWidth(inputWidth - getButtonWidth());
			}
		}
	}
	...
	public void setDisplay(boolean display) {
		this.label.setDisplay(display);
		this.input.setDisplay(display);
		this.star.setDisplay(display);
		for (ButtonMeta b : this.buttons) {
