...
import com.panet.imeta.compatibility.Row;
...
import com.panet.imeta.trans.KettleURLClassLoader;
 ...
 * 
 */
public class ScriptValuesMetaMod extends BaseStepMeta implements
		StepMetaInterface {
	...
	private void readData(Node stepnode) throws KettleXMLException {
		try {
			...
			String strCompatible = XMLHandler.getTagValue(stepnode,
					"compatible");
			...
			String strVariableReplacementActive = XMLHandler.getTagValue(
					stepnode, "variableReplacementActive");
			if (strVariableReplacementActive == null) {
				variableReplacementActive = true;
			} else {
				variableReplacementActive = "Y"
						.equalsIgnoreCase(strVariableReplacementActive);
			}

			// When in compatibility mode, we load the script, not the other
			// tabs...
			//
			if (!Const.isEmpty(script)) {
				jsScripts = new ScriptValuesScript[1];
				jsScripts[0] = new ScriptValuesScript(
						ScriptValuesScript.TRANSFORM_SCRIPT, "ScriptValue",
						script);
			} else {
				...
				for (int i = 0; i < nrscripts; i++) {
					Node fnode = XMLHandler.getSubNodeByNr(scripts,
							"jsScript", i); //$NON-NLS-1$

					jsScripts[i] = new ScriptValuesScript(Integer
							.parseInt(XMLHandler.getTagValue(fnode,
									JSSCRIPT_TAG_TYPE)), XMLHandler
							.getTagValue(fnode, JSSCRIPT_TAG_NAME), XMLHandler
							.getTagValue(fnode, JSSCRIPT_TAG_SCRIPT));
				}
			}
			...
			for (int i = 0; i < nrfields; i++) {
				...
				type[i] = ValueMeta.getType(XMLHandler.getTagValue(fnode,
						"type")); //$NON-NLS-1$
			...
			throw new KettleXMLException(
					Messages
							.getString("ScriptValuesMetaMod.Exception.UnableToLoadStepInfoFromXML"), e); //$NON-NLS-1$
		}
	}
	...
	public void setDefault() {
		jsScripts = new ScriptValuesScript[1];
		jsScripts[0] = new ScriptValuesScript(
				ScriptValuesScript.TRANSFORM_SCRIPT, Messages
						.getString("ScriptValuesMod.Script1"), "//"
						+ Messages.getString("ScriptValuesMod.ScriptHere")
						+ Const.CR + Const.CR);
	...
	public String getXML() {
		...
		retval
				.append("    ").append(XMLHandler.addTagValue("compatible", compatible)); //$NON-NLS-1$ //$NON-NLS-2$
		retval.append("    ").append(
				XMLHandler.addTagValue("variableReplacementActive",
		...
		for (int i = 0; i < jsScripts.length; i++) {
			...
			retval.append("        ").append(
					XMLHandler.addTagValue(JSSCRIPT_TAG_TYPE, jsScripts[i]
							.getScriptType()));
			retval.append("        ").append(
					XMLHandler.addTagValue(JSSCRIPT_TAG_NAME, jsScripts[i]
							.getScriptName()));
			retval.append("        ").append(
					XMLHandler.addTagValue(JSSCRIPT_TAG_SCRIPT, jsScripts[i]
							.getScript()));
			retval.append("      </jsScript>"); //$NON-NLS-1$
		}
		retval.append("    </jsScripts>");
		...
		for (int i = 0; i < name.length; i++) {
			...
			retval
					.append("        ").append(XMLHandler.addTagValue("name", name[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval
					.append("        ").append(XMLHandler.addTagValue("rename", rename[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval
					.append("        ").append(XMLHandler.addTagValue("type", ValueMeta.getTypeDesc(type[i]))); //$NON-NLS-1$ //$NON-NLS-2$
			retval
					.append("        ").append(XMLHandler.addTagValue("length", length[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval
					.append("        ").append(XMLHandler.addTagValue("precision", precision[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval.append("      </field>"); //$NON-NLS-1$
		}
		retval.append("    </fields>"); //$NON-NLS-1$
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		this.variableReplacementActive = BaseStepMeta.parameterToBoolean(p
				.get(id + ".variableReplacementActive"));
		this.jsScripts = new ScriptValuesScript[] { new ScriptValuesScript(
				ScriptValuesScript.TRANSFORM_SCRIPT, "Script 1", BaseStepMeta
						.parameterToString(p.get(id + ".js"))) };
		String[] name = (p.get(id + "_fields.name") != null) ? p.get(id
				+ "_fields.name") : new String[0];
		String[] rename = (p.get(id + "_fields.rename") != null) ? p.get(id
				+ "_fields.rename") : new String[0];
		String[] type = (p.get(id + "_fields.type") != null) ? p.get(id
				+ "_fields.type") : new String[0];
		String[] length = (p.get(id + "_fields.length") != null) ? p.get(id
				+ "_fields.length") : new String[0];
		String[] precision = (p.get(id + "_fields.precision") != null) ? p
				.get(id + "_fields.precision") : new String[0];

		this.name = (name != null) ? name : (new String[0]);
		this.rename = (rename != null) ? rename : (new String[0]);
	...
	public void readRep(Repository rep, long id_step,
		...
		try {
			...
			compatible = rep.getStepAttributeBoolean(id_step, 0,
					"compatible", true); //$NON-NLS-1$
			variableReplacementActive = rep.getStepAttributeBoolean(
					id_step, 0, "variableReplacementActive", false);
			...
			if (!Const.isEmpty(script)) {
				...
				int nrScripts = rep.countNrStepAttributes(id_step,
						JSSCRIPT_TAG_NAME); //$NON-NLS-1$
				jsScripts = new ScriptValuesScript[nrScripts];
				for (int i = 0; i < nrScripts; i++) {
					jsScripts[i] = new ScriptValuesScript((int) rep
							.getStepAttributeInteger(id_step, i,
									JSSCRIPT_TAG_TYPE), rep
							.getStepAttributeString(id_step, i,
									JSSCRIPT_TAG_NAME), rep
							.getStepAttributeString(id_step, i,
									JSSCRIPT_TAG_SCRIPT));
			...
			for (int i = 0; i < nrfields; i++) {
				...
				rename[i] = rep.getStepAttributeString(id_step, i,
						"field_rename"); //$NON-NLS-1$
			...
			throw new KettleException(
					Messages
							.getString("ScriptValuesMetaMod.Exception.UnexpectedErrorInReadingStepInfo"), e); //$NON-NLS-1$
		}
	}
	...
	public void saveRep(Repository rep, long id_transformation, long id_step)
		...
		try {
			...
			rep.saveStepAttribute(id_transformation, id_step,
					"variableReplacementActive", variableReplacementActive);

			for (int i = 0; i < jsScripts.length; i++) {
				rep.saveStepAttribute(id_transformation, id_step, i,
						JSSCRIPT_TAG_NAME, jsScripts[i].getScriptName());
				rep.saveStepAttribute(id_transformation, id_step, i,
						JSSCRIPT_TAG_SCRIPT, jsScripts[i].getScript());
				rep.saveStepAttribute(id_transformation, id_step, i,
						JSSCRIPT_TAG_TYPE, jsScripts[i].getScriptType());
			}
			...
			for (int i = 0; i < name.length; i++) {
				...
				rep.saveStepAttribute(id_transformation, id_step, i,
						"field_rename", rename[i]); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i,
			...
			throw new KettleException(
					Messages
							.getString("ScriptValuesMetaMod.Exception.UnableToSaveStepInfo") + id_step, e); //$NON-NLS-1$
		}
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (prev != null && strActiveScript.length() > 0) {
			cr = new CheckResult(
					...
					Messages
							.getString(
									"ScriptValuesMetaMod.CheckResult.ConnectedStepOK", String.valueOf(prev.size())), stepinfo); //$NON-NLS-1$ //$NON-NLS-2$
			remarks.add(cr);

			// Adding the existing Scripts to the Context
			for (int i = 0; i < getNumberOfJSScripts(); i++) {
				Scriptable jsR = Context.toObject(jsScripts[i].getScript(),
						jsscope);
			...
			try {
				if (getAddClasses() != null) {
					for (int i = 0; i < getAddClasses().length; i++) {
						Object jsOut = Context.javaToJS(getAddClasses()[i]
								.getAddObject(), jsscope);
						ScriptableObject.putProperty(jsscope,
								getAddClasses()[i].getJSName(), jsOut);
					}
				}
			} catch (Exception e) {
			...
			try {
				...
				error_message = "Couldn't add Default Functions! Error:"
						+ Const.CR + ex.toString();
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR,
						error_message, stepinfo);
				remarks.add(cr);
			}
			...
			try {
				jsscope.put("SKIP_TRANSFORMATION", jsscope, Integer
						.valueOf(ScriptValuesMod.SKIP_TRANSFORMATION));
				jsscope.put("ABORT_TRANSFORMATION", jsscope, Integer
						.valueOf(ScriptValuesMod.ABORT_TRANSFORMATION));
				jsscope.put("ERROR_TRANSFORMATION", jsscope, Integer
						.valueOf(ScriptValuesMod.ERROR_TRANSFORMATION));
				jsscope.put("CONTINUE_TRANSFORMATION", jsscope, Integer
						.valueOf(ScriptValuesMod.CONTINUE_TRANSFORMATION));
			} catch (Exception ex) {
			...
			try {
				ScriptValuesModDummy dummyStep = new ScriptValuesModDummy(prev,
						transMeta.getStepFields(stepinfo));
				...
				for (int i = 0; i < prev.size(); i++) {
					...
					// Set date and string values to something to simulate real
					// thing
					//
					if (valueMeta.isDate())
						valueData = new Date();
					if (valueMeta.isString())
						valueData = "test value test value test value test value test value test value test value test value test value test value"; //$NON-NLS-1$
			...
			try {
				...
				if (strActiveStartScript != null
					...
					/* Object startScript = */jscx.evaluateString(jsscope,
							strActiveStartScript, "trans_Start", 1, null);
					error_message = "Found Start Script. "
							+ strActiveStartScriptName + " Processing OK";
					cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK,
							error_message, stepinfo); //$NON-NLS-1$
					remarks.add(cr);
				}
			} catch (Exception e) {
			...
			try {
				jsscript = jscx.compileString(strActiveScript,
						"script", 1, null); //$NON-NLS-1$

				cr = new CheckResult(
						CheckResultInterface.TYPE_RESULT_OK,
						Messages
								.getString("ScriptValuesMetaMod.CheckResult.ScriptCompiledOK"), stepinfo); //$NON-NLS-1$
				remarks.add(cr);
				...
				try {
					...
					cr = new CheckResult(
							CheckResultInterface.TYPE_RESULT_OK,
							Messages
									.getString("ScriptValuesMetaMod.CheckResult.ScriptCompiledOK2"), stepinfo); //$NON-NLS-1$
					remarks.add(cr);
					...
					if (name.length > 0) {
						StringBuffer message = new StringBuffer(
								Messages
										.getString(
												"ScriptValuesMetaMod.CheckResult.FailedToGetValues", String.valueOf(name.length)) + Const.CR + Const.CR); //$NON-NLS-1$ //$NON-NLS-2$

						if (error_found) {
							cr = new CheckResult(
									CheckResultInterface.TYPE_RESULT_ERROR,
									message.toString(), stepinfo);
						} else {
							cr = new CheckResult(
									CheckResultInterface.TYPE_RESULT_OK,
									message.toString(), stepinfo);
					...
					error_message = Messages
							.getString("ScriptValuesMetaMod.CheckResult.CouldNotExecuteScript") + Const.CR + jse.toString(); //$NON-NLS-1$
					cr = new CheckResult(
							CheckResultInterface.TYPE_RESULT_ERROR,
							error_message, stepinfo);
					remarks.add(cr);
					...
					Context.exit();
					error_message = Messages
							.getString("ScriptValuesMetaMod.CheckResult.CouldNotExecuteScript2") + Const.CR + e.toString(); //$NON-NLS-1$
					cr = new CheckResult(
							CheckResultInterface.TYPE_RESULT_ERROR,
							error_message, stepinfo);
					remarks.add(cr);
				...
				// Checking End Script
				try {
					if (strActiveEndScript != null
							&& strActiveEndScript.length() > 0) {
						/* Object endScript = */jscx.evaluateString(jsscope,
								strActiveEndScript, "trans_End", 1, null);
						error_message = "Found End Script. "
								+ strActiveEndScriptName + " Processing OK";
						cr = new CheckResult(
								CheckResultInterface.TYPE_RESULT_OK,
								error_message, stepinfo); //$NON-NLS-1$
						remarks.add(cr);
				...
				Context.exit();
				error_message = Messages
						.getString("ScriptValuesMetaMod.CheckResult.CouldNotCompileScript") + Const.CR + e.toString(); //$NON-NLS-1$
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR,
						error_message, stepinfo);
				remarks.add(cr);
			}
			...
			Context.exit();
			error_message = Messages
					.getString("ScriptValuesMetaMod.CheckResult.CouldNotGetFieldsFromPreviousStep"); //$NON-NLS-1$
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR,
					error_message, stepinfo);
			remarks.add(cr);
		}
		...
		if (input.length > 0) {
			cr = new CheckResult(
					CheckResultInterface.TYPE_RESULT_OK,
					Messages
							.getString("ScriptValuesMetaMod.CheckResult.ConnectedStepOK2"), stepinfo); //$NON-NLS-1$
			remarks.add(cr);
		} else {
			cr = new CheckResult(
					CheckResultInterface.TYPE_RESULT_ERROR,
					Messages
							.getString("ScriptValuesMetaMod.CheckResult.NoInputReceived"), stepinfo); //$NON-NLS-1$
			remarks.add(cr);
		}
	}
	...
	public boolean getValue(Scriptable scope, int i, Value res,
		...
		if (name[i] != null && name[i].length() > 0) {
			...
			try {
				...
				if (result != null) {
					...
					case ValueMetaInterface.TYPE_NUMBER:
						if (classname
								.equalsIgnoreCase("org.mozilla.javascript.Undefined")) {
							res.setNull();
						} else if (classname
								.equalsIgnoreCase("org.mozilla.javascript.NativeJavaObject")) {
							// Is it a java Value class ?
							Value v = (Value) Context.jsToJava(result,
									Value.class);
					...
					case ValueMetaInterface.TYPE_STRING:
						if (classname
								.equalsIgnoreCase("org.mozilla.javascript.NativeJavaObject") || //$NON-NLS-1$
								classname
										.equalsIgnoreCase("org.mozilla.javascript.Undefined")) //$NON-NLS-1$
						{
							// Is it a java Value class ?
							try {
								Value v = (Value) Context.jsToJava(result,
								...
								// A String perhaps?
								String s = (String) Context.jsToJava(result,
										String.class);
								res.setValue(s);
							}
						} else {
					...
					case ValueMetaInterface.TYPE_DATE:
								...
								.equalsIgnoreCase("org.mozilla.javascript.Undefined")) //$NON-NLS-1$
						{
							...
							if (classname
									.equalsIgnoreCase("org.mozilla.javascript.NativeDate")) //$NON-NLS-1$
							{
									...
									.equalsIgnoreCase("org.mozilla.javascript.NativeJavaObject")) //$NON-NLS-1$
							{
								...
								try {
									Date dat = (Date) Context.jsToJava(result,
				...
				message
						.append(Messages
								.getString(
										"ScriptValuesMetaMod.CheckResult.ErrorRetrievingValue", name[i]) + " : " + e.toString()); //$NON-NLS-1$ //$NON-NLS-2$
				error_found = true;
			}
			...
			message
					.append(Messages
							.getString(
									"ScriptValuesMetaMod.CheckResult.RetrievedValue", name[i], res.toStringMeta())); //$NON-NLS-1$ //$NON-NLS-2$
		} else {
			message
					.append(Messages
							.getString(
									"ScriptValuesMetaMod.CheckResult.ValueIsEmpty", String.valueOf(i))); //$NON-NLS-1$ //$NON-NLS-2$
			error_found = true;
		}
	...
	public StepInterface getStep(StepMeta stepMeta,
			...
			Trans trans) {
		return new ScriptValuesMod(stepMeta, stepDataInterface, cnr, transMeta,
				trans);
	}
	...
	public void parseXmlForAdditionalClasses() throws KettleException {
		try {
			...
			Document dom = XMLHandler.loadXMLFile(strActPath
					+ "/plugins/steps/ScriptValues_mod/plugin.xml");
			...
			for (int i = 0; i < nbOfLibs; i++) {
				...
				String strClassName = XMLHandler.getTagAttribute(fnode,
						"classname");
				String strJSName = XMLHandler.getTagAttribute(fnode, "js_name");

				Class<?> addClass = LoadAdditionalClass(strActPath
						+ "/plugins/steps/ScriptValues_mod/" + strJarName,
						strClassName);
				Object addObject = addClass.newInstance();
				additionalClasses[i] = new ScriptValuesAddClasses(addClass,
						addObject, strJSName);
			}
		} catch (Exception e) {
			throw new KettleException(
					Messages
							.getString("ScriptValuesMetaMod.Exception.UnableToParseXMLforAdditionalClasses"),
					e);
		}
	}

	private static Class<?> LoadAdditionalClass(String strJar,
			String strClassName) throws KettleException {
		try {
			Thread t = Thread.currentThread();
			ClassLoader cl = t.getContextClassLoader();
			URL u = new URL("jar:file:" + strJar + "!/");
			KettleURLClassLoader kl = new KettleURLClassLoader(new URL[] { u },
					cl);
			...
			throw new KettleException(
					Messages
							.getString("ScriptValuesMetaMod.Exception.UnableToLoadAdditionalClass"), e); //$NON-NLS-1$
		}
	}
	...
	public String getDialogClassName() {
		return "com.panet.imeta.trans.steps.scriptvalues_mod.ScriptValuesModDialog";
	}
