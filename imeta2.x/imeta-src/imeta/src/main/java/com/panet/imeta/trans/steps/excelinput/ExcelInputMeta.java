...
public class ExcelInputMeta extends BaseStepMeta implements StepMetaInterface {
	...
	public static final String STEP_ATTRIBUTE_STOPONEMPTY = "stoponempty";
	public static final String STEP_ATTRIBUTE_FILEFIELD = "filefield";
	public static final String STEP_ATTRIBUTE_SHEETFIELD = "sheetfield";
	public static final String STEP_ATTRIBUTE_SHEETROWNUMFIELD = "sheetrownumfield";
	public static final String STEP_ATTRIBUTE_ROWNUMFIELD = "rownumfield";
	...
	public static final String STEP_ATTRIBUTE_SHEET_NAME = "sheet_name";
	...
	// public static final String STEP_ATTRIBUTE_SHEET_NAME = "sheet_name";
	public static final String STEP_ATTRIBUTE_SHEET_STARTROW = "sheet_startrow";
	public static final String STEP_ATTRIBUTE_SHEET_STARTCOL = "sheet_startcol";
	// public static final String STEP_ATTRIBUTE_FIELD_NAME = "field_name";
	...
	public static final String STEP_ATTRIBUTE_STRICT_TYPES = "strict_types";
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		startsWithHeader = BaseStepMeta.parameterToBoolean(p.get(id
				+ ".startsWithHeader"));
		ignoreEmptyRows = BaseStepMeta.parameterToBoolean(p.get(id
				+ ".ignoreEmptyRows"));
		stopOnEmpty = BaseStepMeta.parameterToBoolean(p
				.get(id + ".stopOnEmpty"));
		sheetRowNumberField = BaseStepMeta.parameterToString(p.get(id
				+ ".sheetRowNumberField"));
		...
		sheetField = BaseStepMeta.parameterToString(p.get(id + ".sheetField"));
		fileField = BaseStepMeta.parameterToString(p.get(id + ".fileField"));
		...
		acceptingStepName = BaseStepMeta.parameterToString(p.get(id
				+ ".stepFieldFilesName"));
		strictTypes = BaseStepMeta.parameterToBoolean(p
				.get(id + ".strictTypes"));
		...
		String[] sheetName = p.get(id + "_selectedSheets.sheetName");
		String[] startRow = p.get(id + "_selectedSheets.startRow");
		String[] startColumn = p.get(id + "_selectedSheets.startColumn");
		this.sheetName = (sheetName != null) ? sheetName : (new String[0]);
		this.startRow = (startRow != null) ? stringArrayToIntArray(startRow, 0)
				: (new int[0]);
		this.startColumn = (startColumn != null) ? stringArrayToIntArray(
				startColumn, 0) : (new int[0]);
		...
		if (fieldName != null && fieldName.length > 0) {
			ExcelInputField[] excelInputField = new ExcelInputField[fieldName.length];
			for (int i = 0; i < excelInputField.length; i++) {
				excelInputField[i] = new ExcelInputField();
				excelInputField[i].setName(fieldName[i]);
				excelInputField[i].setType(Const.toInt(fieldType[i], 0));
				excelInputField[i].setLength(Const.toInt(fieldLength[i], -1));
				excelInputField[i].setPrecision(Const.toInt(fieldPrecision[i],
						-1));
				excelInputField[i]
						.setTrimType(Const.toInt(fieldTrimType[i], 0));
				excelInputField[i].setRepeated(Boolean
						.parseBoolean(fieldRepeat[i]));
				excelInputField[i].setFormat(fieldFormat[i]);
				excelInputField[i].setCurrencySymbol(fieldCurrency[i]);
				excelInputField[i].setDecimalSymbol(fieldDecimal[i]);
				excelInputField[i].setGroupSymbol(fieldGroup[i]);
			}
			this.field = excelInputField;
		} else {
	...
	private void readData(Node stepnode) throws KettleXMLException {
		try {
			startsWithHeader = YES.equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "header"));
			...
			sheetRowNumberField = XMLHandler.getTagValue(stepnode,
					"sheetrownumfield");
			...
			for (int i = 0; i < nrfields; i++) {
				...
				field[i].setType(ValueMeta.getType(XMLHandler.getTagValue(
						fnode, "type")));
				field[i].setLength(Const.toInt(XMLHandler.getTagValue(fnode,
						"length"), -1));
				field[i].setPrecision(Const.toInt(XMLHandler.getTagValue(fnode,
						"precision"), -1));
				String srepeat = XMLHandler.getTagValue(fnode, "repeat");
				field[i].setTrimType(getTrimTypeByCode(XMLHandler.getTagValue(
						fnode, "trim_type")));
				...
				field[i].setCurrencySymbol(XMLHandler.getTagValue(fnode,
						"currency"));
				field[i].setDecimalSymbol(XMLHandler.getTagValue(fnode,
						"decimal"));
			...
			for (int i = 0; i < nrsheets; i++) {
				...
				startRow[i] = Const.toInt(XMLHandler.getTagValue(snode,
						"startrow"), 0);
				startColumn[i] = Const.toInt(XMLHandler.getTagValue(snode,
						"startcol"), 0);
			}

			strictTypes = YES.equalsIgnoreCase(XMLHandler.getTagValue(stepnode,
					"strict_types"));
	...
	public void getFields(RowMetaInterface row, String name,
		...
		if (fileField != null && fileField.length() > 0) {
			ValueMetaInterface v = new ValueMeta(fileField,
		...
		if (sheetField != null && sheetField.length() > 0) {
			ValueMetaInterface v = new ValueMeta(sheetField,
					ValueMetaInterface.TYPE_STRING);
			v.setLength(250);
			v.setPrecision(-1);
			v.setOrigin(name);
		...
		if (sheetRowNumberField != null && sheetRowNumberField.length() > 0) {
			ValueMetaInterface v = new ValueMeta(sheetRowNumberField,
	...
	public String getXML() {
		...
		retval.append("    ").append(
				XMLHandler.addTagValue("header", startsWithHeader));
		retval.append("    ").append(
				XMLHandler.addTagValue("noempty", ignoreEmptyRows));
		retval.append("    ").append(
				XMLHandler.addTagValue("stoponempty", stopOnEmpty));
		retval.append("    ").append(
				XMLHandler.addTagValue("filefield", fileField));
		retval.append("    ").append(
				XMLHandler.addTagValue("sheetfield", sheetField));
		retval.append("    ")
				.append(
						XMLHandler.addTagValue("sheetrownumfield",
								sheetRowNumberField));
		retval.append("    ").append(
				XMLHandler.addTagValue("rownumfield", rowNumberField));
		retval.append("    ").append(
				XMLHandler.addTagValue("sheetfield", sheetField));
		retval.append("    ").append(
				XMLHandler.addTagValue("filefield", fileField));
		...
		for (int i = 0; i < field.length; i++) {
			...
			retval.append("        ").append(
					XMLHandler.addTagValue("name", field[i].getName()));
			retval.append("        ").append(
					XMLHandler.addTagValue("type", field[i].getTypeDesc()));
			retval.append("        ").append(
					XMLHandler.addTagValue("length", field[i].getLength()));
			retval.append("        ").append(
					XMLHandler
							.addTagValue("precision", field[i].getPrecision()));
			retval.append("        ").append(
					XMLHandler.addTagValue("trim_type", field[i]
							.getTrimTypeCode()));
			retval.append("        ").append(
					XMLHandler.addTagValue("repeat", field[i].isRepeated()));

			retval.append("        ").append(
					XMLHandler.addTagValue("format", field[i].getFormat()));
			retval.append("        ").append(
					XMLHandler.addTagValue("currency", field[i]
							.getCurrencySymbol()));
			retval.append("        ").append(
					XMLHandler.addTagValue("decimal", field[i]
							.getDecimalSymbol()));
			retval.append("        ").append(
					XMLHandler.addTagValue("group", field[i].getGroupSymbol()));
		...
		for (int i = 0; i < sheetName.length; i++) {
			...
			retval.append("        ").append(
					XMLHandler.addTagValue("name", sheetName[i]));
			retval.append("        ").append(
					XMLHandler.addTagValue("startrow", startRow[i]));
			retval.append("        ").append(
					XMLHandler.addTagValue("startcol", startColumn[i]));
			retval.append("        </sheet>").append(Const.CR);
		}
		retval.append("    </sheets>").append(Const.CR);
		...
		// ERROR HANDLING
		retval.append("    ").append(
				XMLHandler.addTagValue("strict_types", strictTypes));
		retval.append("    ").append(
	...
	public void readRep(Repository rep, long id_step,
		...
		try {
			startsWithHeader = rep.getStepAttributeBoolean(id_step,
					STEP_ATTRIBUTE_HEADER);
			ignoreEmptyRows = rep.getStepAttributeBoolean(id_step,
					STEP_ATTRIBUTE_NOEMPTY);
			stopOnEmpty = rep.getStepAttributeBoolean(id_step,
					STEP_ATTRIBUTE_STOPONEMPTY);
			fileField = rep.getStepAttributeString(id_step,
					STEP_ATTRIBUTE_FILEFIELD);
			sheetField = rep.getStepAttributeString(id_step,
					STEP_ATTRIBUTE_SHEETFIELD);
			sheetRowNumberField = rep.getStepAttributeString(id_step,
					STEP_ATTRIBUTE_SHEETROWNUMFIELD);
			rowNumberField = rep.getStepAttributeString(id_step,
					STEP_ATTRIBUTE_ROWNUMFIELD);
			...
			int nrsheets = rep.countNrStepAttributes(id_step,
					STEP_ATTRIBUTE_SHEET_NAME);
			...
			for (int i = 0; i < nrsheets; i++) {
				sheetName[i] = rep.getStepAttributeString(id_step, i,
						STEP_ATTRIBUTE_SHEET_NAME);
				startRow[i] = (int) rep.getStepAttributeInteger(id_step, i,
						STEP_ATTRIBUTE_SHEET_STARTROW);
				startColumn[i] = (int) rep.getStepAttributeInteger(id_step, i,
						STEP_ATTRIBUTE_SHEET_STARTCOL);
			}

			for (int i = 0; i < nrfields; i++) {
				field[i] = new ExcelInputField();

				field[i].setName(rep.getStepAttributeString(id_step, i,
						STEP_ATTRIBUTE_FIELD_NAME));
				field[i].setType(ValueMeta.getType(rep.getStepAttributeString(
						id_step, i, STEP_ATTRIBUTE_FIELD_TYPE)));
				field[i].setLength((int) rep.getStepAttributeInteger(id_step,
						i, STEP_ATTRIBUTE_FIELD_LENGTH));
				field[i].setPrecision((int) rep.getStepAttributeInteger(
						id_step, i, STEP_ATTRIBUTE_FIELD_PRECISION));
				field[i].setTrimType(getTrimTypeByCode(rep
						.getStepAttributeString(id_step, i,
								STEP_ATTRIBUTE_FIELD_TRIM_TYPE)));
				field[i].setRepeated(rep.getStepAttributeBoolean(id_step, i,
						STEP_ATTRIBUTE_FIELD_REPEAT));

				field[i].setFormat(rep.getStepAttributeString(id_step, i,
						STEP_ATTRIBUTE_FIELD_FORMAT));
				field[i].setCurrencySymbol(rep.getStepAttributeString(id_step,
						i, STEP_ATTRIBUTE_FIELD_CURRENCY));
				field[i].setDecimalSymbol(rep.getStepAttributeString(id_step,
						i, STEP_ATTRIBUTE_FIELD_DECIMAL));
				field[i].setGroupSymbol(rep.getStepAttributeString(id_step, i,
						STEP_ATTRIBUTE_FIELD_GROUP));
			}

			strictTypes = rep.getStepAttributeBoolean(id_step, 0,
					STEP_ATTRIBUTE_STRICT_TYPES, false);
			errorIgnored = rep.getStepAttributeBoolean(id_step, 0,
					STEP_ATTRIBUTE_ERROR_IGNORED, false);
			errorLineSkipped = rep.getStepAttributeBoolean(id_step, 0,
					STEP_ATTRIBUTE_ERROR_LINE_SKIPPED, false);
	...
	public void saveRep(Repository rep, long id_transformation, long id_step)
		...
		try {
			...
			rep.saveStepAttribute(id_transformation, id_step, "noempty",
					ignoreEmptyRows);
			rep.saveStepAttribute(id_transformation, id_step, "stoponempty",
					stopOnEmpty);
			rep.saveStepAttribute(id_transformation, id_step, "filefield",
					fileField);
			rep.saveStepAttribute(id_transformation, id_step, "sheetfield",
					sheetField);
			rep.saveStepAttribute(id_transformation, id_step,
					"sheetrownumfield", sheetRowNumberField);
			rep.saveStepAttribute(id_transformation, id_step, "rownumfield",
					rowNumberField);
			rep
			...
			for (int i = 0; i < sheetName.length; i++) {
				rep.saveStepAttribute(id_transformation, id_step, i,
						"sheet_name", sheetName[i]);
				rep.saveStepAttribute(id_transformation, id_step, i,
						"sheet_startrow", startRow[i]);
				rep.saveStepAttribute(id_transformation, id_step, i,
						"sheet_startcol", startColumn[i]);
			}

			for (int i = 0; i < field.length; i++) {
				rep.saveStepAttribute(id_transformation, id_step, i,
						"field_name", field[i].getName());
				rep.saveStepAttribute(id_transformation, id_step, i,
						"field_type", field[i].getTypeDesc());
				rep.saveStepAttribute(id_transformation, id_step, i,
						"field_length", field[i].getLength());
				rep.saveStepAttribute(id_transformation, id_step, i,
						"field_precision", field[i].getPrecision());
				rep.saveStepAttribute(id_transformation, id_step, i,
						"field_trim_type", field[i].getTrimTypeCode());
				rep.saveStepAttribute(id_transformation, id_step, i,
						"field_repeat", field[i].isRepeated());

				rep.saveStepAttribute(id_transformation, id_step, i,
						"field_format", field[i].getFormat());
				rep.saveStepAttribute(id_transformation, id_step, i,
						"field_currency", field[i].getCurrencySymbol());
				rep.saveStepAttribute(id_transformation, id_step, i,
						"field_decimal", field[i].getDecimalSymbol());
				rep.saveStepAttribute(id_transformation, id_step, i,
						"field_group", field[i].getGroupSymbol());

			}

			rep.saveStepAttribute(id_transformation, id_step, "strict_types",
					strictTypes);
		...
		if (input.length > 0) {
			if (!isAcceptingFilenames()) {
				cr = new CheckResult(
						...
						Messages
								.getString("ExcelInputMeta.CheckResult.NoInputError"),
						stepMeta);
				remarks.add(cr);
			} else {
				cr = new CheckResult(
						...
						Messages
								.getString("ExcelInputMeta.CheckResult.AcceptFilenamesOk"),
						stepMeta);
				remarks.add(cr);
			}
		} else {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, Messages
					.getString("ExcelInputMeta.CheckResult.NoInputOk"),
					stepMeta);
			remarks.add(cr);
		}
		...
		if (fileList.nrOfFiles() == 0) {
			if (!isAcceptingFilenames()) {
				cr = new CheckResult(
						CheckResultInterface.TYPE_RESULT_ERROR,
						Messages
								.getString("ExcelInputMeta.CheckResult.ExpectedFilesError"),
						stepMeta);
				remarks.add(cr);
			}
		} else {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, Messages
					.getString("ExcelInputMeta.CheckResult.ExpectedFilesOk", ""
							+ fileList.nrOfFiles()), stepMeta);
			remarks.add(cr);
		}
	}
	...
	public RowMetaInterface getEmptyFields() {
		...
		for (int i = 0; i < field.length; i++) {
			ValueMetaInterface v = new ValueMeta(field[i].getName(), field[i]
					.getType());
			row.addValueMeta(v);
		}
	...
	public StepInterface getStep(StepMeta stepMeta,
			...
			Trans trans) {
		return new ExcelInput(stepMeta, stepDataInterface, cnr, transMeta,
				trans);
	}
	...
	public boolean readAllSheets() {
		return Const.isEmpty(sheetName)
				|| (sheetName.length == 1 && Const.isEmpty(sheetName[0]));
	}
