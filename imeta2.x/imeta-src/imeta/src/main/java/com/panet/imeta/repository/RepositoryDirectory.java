...
/**
 * This class defines the location of a transformation, job or schema in the
 * repository. That means that it's just an extra parameter for recognising a
 * transformation, job or schema. It allows for subdirectories by linking back
 * to itself.
 * 
 * @author Matt
 * @since 09-nov-2004
...
public class RepositoryDirectory {
	 ...
	 * 
	 * @param parent
	 *            The directory to create the subdirectory in
	 * @param directoryname
	...
	public RepositoryDirectory(RepositoryDirectory parent, String directoryname) {
		...
		this.children = new ArrayList<RepositoryDirectory>(); // default: no
		// subdirectories...
		this.id = 0L; // The root directory!
	}

	/**
	 * Create an empty repository directory. With the name and parent set to
	 * empty, this is the root directory.
	 * 
	 */
	...
	/**
	 * Describe the complete path to ( and including) this directory, separated
	 * by the "file.separator" system property.
	 * 
	...
	public String getPath() {
		if (getParent() == null) // Root!
		{
			...
			if (getParent().getParent() == null) {
				...
				return getParent().getPath() + DIRECTORY_SEPARATOR
						+ getDirectoryName();
			}
		}
	}
	...
	/**
	 * Describe the complete path to ( and including) this directory, as an
	 * array of strings.
	 * 
	 * @return The complete path to this directory.
	 */
	 ...
	 * @return The directory if one can be found, null if no directory was
	 *         found.
	 */
	public RepositoryDirectory findDirectory(String path[]) {
		// Is it root itself?
		if (isRoot() && path.length == 1
				&& path[0].equalsIgnoreCase(DIRECTORY_SEPARATOR)) {
			return this;
		}
		...
		// The root directory?
		if (isRoot() && directoryPath.length == 1
				&& directoryPath[0].equalsIgnoreCase(DIRECTORY_SEPARATOR)) {
			return this;
		} else
		// This directory?
		if (directoryPath.length == 1
				&& directoryPath[0].equalsIgnoreCase(getDirectoryName())) {
			return this;
		} else
		...
		if (directoryPath.length >= 1) {
			for (int i = 0; i < getNrSubdirectories(); i++) {
				...
				if (subdir.getDirectoryName()
						.equalsIgnoreCase(directoryPath[0])) {
					if (directoryPath.length == 1)
						return subdir; // we arrived at the destination...

					// Copy the path exception the highest level, we go down
					// one...
	...
	/**
	 * Find a directory using the path to the directory with file.separator
	 * between the dir-names.
	 * 
	 * @param path
	 ...
	 * @return The RepositoryDirectory if the ID was found, null if nothing
	 *         could be found.
	 */
	public RepositoryDirectory findDirectory(long id_directory) {
		if (getID() == id_directory)
			return this;

		for (int i = 0; i < getNrSubdirectories(); i++) {
			RepositoryDirectory rd = getSubdirectory(i).findDirectory(
					id_directory);
			if (rd != null)
				return rd;
		}
	...
	public String getXML(int level) {
		...
		retval.append(spaces).append("  ").append(
				XMLHandler.addTagValue("name", getDirectoryName()));
	 ...
	 * 
	 * @param repdirnode
	 *            The node in which the Repository directory information
	 *            resides.
	...
	public boolean loadXML(Node repdirnode) {
		try {
			...
			if (subdirsnode != null) {
				...
				for (int i = 0; i < n; i++) {
					Node subdirnode = XMLHandler.getSubNodeByNr(subdirsnode,
							"repdir", i);
	...
	private void loadDirectoryTreeFromRepository(Repository rep)
	...
	public boolean addToRep(Repository rep) {
		try {
			...
			if (getParent() != null)
				id_directory_parent = getParent().getID();

			setID(rep.insertDirectory(id_directory_parent, this));

			LogWriter.getInstance().logDetailed(rep.getName(),
					"New id of directory = " + getID());
	...
	public void delFromRep(Repository rep) throws KettleException {
		try {
			...
			throw new KettleException(
					"Unexpected error deleting repository directory", e);
		}
	}
	 ...
	 * 
	 * @return an array of all the directory id's (this directory &
	 *         subdirectories)
	 */
	public long[] getDirectoryIDs() {
		...
		for (int i = 0; i < retval.length; i++)
			retval[i] = ((Long) ids.get(i)).longValue();

		return retval;
	}

	/**
	 * Fill an arraylist with all the ID_DIRECTORY values in the tree below and
	...
	/**
	 * Create a new directory, possibly by creating several subdirecties of / at
	 * the same time.
	 * 
	 * @param directoryPath
	...
	public RepositoryDirectory createDirectory(Repository rep,
			String directoryPath) throws KettleDatabaseException {
		...
		for (int level = 1; level <= path.length; level++) {
			...
			if (rd == null) {
				...
				if (rd.addToRep(rep)) {
					...
					System.out.println("Created directory [" + rd.getPath()
							+ "], id = " + rd.getID());
					parent = rd;
				} else {
					throw new KettleDatabaseException(
							"Unable to create repository directory ["
									+ subPath[level - 1] + "] in directory ["
									+ parent.getPath() + "]");
				}
			} else {
				parent = rd;
	...
	public String getPathObjectCombination(String transName) {
		if (isRoot()) {
			...
			return getPath() + RepositoryDirectory.DIRECTORY_SEPARATOR
					+ transName;
		}
	}

	public ArrayList<RepositoryDirectory> getChildren() {
		return children;
	}

	public void setChildren(ArrayList<RepositoryDirectory> children) {
