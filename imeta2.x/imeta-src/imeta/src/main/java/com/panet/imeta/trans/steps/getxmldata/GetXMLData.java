...
/**
 * Read XML files, parse them and convert them to rows and writes these to one
...
public class GetXMLData extends BaseStep implements StepInterface {
	...
	public GetXMLData(StepMeta stepMeta, StepDataInterface stepDataInterface,
			int copyNr, TransMeta transMeta, Trans trans) {
		super(stepMeta, stepDataInterface, copyNr, transMeta, trans);
	}

	protected boolean setDocument(String StringXML, FileObject file,
			boolean IsInXMLField, boolean readurl) throws KettleException {
		...
		try {
			...
			if (data.prunePath != null) {
				...
				if (log.isDetailed())
					logDetailed(Messages
							.getString("GetXMLData.Log.StreamingMode.Activated"));
				reader.addHandler(data.prunePath, new ElementHandler() {
					...
					public void onEnd(ElementPath path) {
						if (isStopped()) {
							...
							if (log.isBasic())
								logBasic(Messages
										.getString("GetXMLData.Log.StreamingMode.Stopped"));
						...
						if (log.isDebug())
							logDebug(Messages
									.getString("GetXMLData.Log.StreamingMode.StartProcessing"));
						...
						if (log.isDebug())
							logDebug(Messages
									.getString("GetXMLData.Log.StreamingMode.EndProcessing"));
					}
				});
			}
			...
			if (IsInXMLField) {
				...
				data.document = reader.read(KettleVFS.getInputStream(file),
						encoding);
			}
	...
	/**
	 * Process chunk of data in streaming mode. Called only by the handler when
	 * pruning is true. Not allowed in combination with meta.getIsInFields(),
	 * but could be redesigned later on.
	 * 
	 */
	private void processStreaming(Document document) throws KettleException {
		...
		if (log.isDebug())
			logDebug(Messages
					.getString("GetXMLData.Log.StreamingMode.ApplyXPath"));
		if (!applyXPath()) {
			throw new KettleException(Messages
					.getString("GetXMLData.Log.UnableApplyXPath"));
		}
		// main loop through the data until limit is reached or transformation
		// is stopped
		// similar functionality like in BaseStep.runStepThread
		if (log.isDebug())
			logDebug(Messages
					.getString("GetXMLData.Log.StreamingMode.ProcessingRows"));
		boolean cont = true;
		while (data.nodenr < data.nodesize && cont && !isStopped()) {
			...
			cont = putRowOut(r); // false when limit is reached,
			// functionality is there but we can not
			// stop reading the hole file (slow but
			// works)
		}
		if (log.isDebug())
			logDebug(Messages
					.getString("GetXMLData.Log.StreamingMode.FreeMemory"));
	...
	private void handleMissingFiles() throws KettleException {
		...
		if (nonExistantFiles.size() != 0) {
			...
			log.logError(Messages
					.getString("GetXMLData.Log.RequiredFilesTitle"), Messages
					.getString("GetXMLData.Log.RequiredFiles", message));

			throw new KettleException(Messages.getString(
					"GetXMLData.Log.RequiredFilesMissing", message));
		}
		...
		if (nonAccessibleFiles.size() != 0) {
			...
			log.logError(Messages
					.getString("GetXMLData.Log.RequiredFilesTitle"), Messages
					.getString("GetXMLData.Log.RequiredNotAccessibleFiles",
							message));

			throw new KettleException(Messages
					.getString(
							"GetXMLData.Log.RequiredNotAccessibleFilesMissing",
							message));
		}
	}
	...
	private boolean ReadNextString() {
		...
		try {
			...
			if (data.readrow == null) // finished processing!
			{
				if (log.isDetailed())
					logDetailed(Messages
							.getString("GetXMLData.Log.FinishedProcessing"));
				return false;
			}
			...
			if (first) {
				...
				if (meta.getIsInFields()) {
					...
					if (Const.isEmpty(meta.getXMLField())) {
						...
						throw new KettleException(Messages
								.getString("GetXMLData.Log.NoField"));
					}

					// cache the position of the field
					if (data.indexOfXmlField < 0) {
						data.indexOfXmlField = getInputRowMeta().indexOfValue(
								meta.getXMLField());
						if (data.indexOfXmlField < 0) {
							// The field is unreachable !
							logError(Messages
									.getString("GetXMLData.Log.ErrorFindingField") + "[" + meta.getXMLField() + "]"); //$NON-NLS-1$ //$NON-NLS-2$
							throw new KettleException(
									Messages
											.getString(
													"GetXMLData.Exception.CouldnotFindField", meta.getXMLField())); //$NON-NLS-1$ //$NON-NLS-2$
						}
					}
				}
			}
			...
			if (meta.getIsInFields()) {
				...
				if (log.isDetailed())
					log.logDetailed(toString(), Messages.getString(
							"GetXMLData.Log.XMLStream", meta.getXMLField(),
							Fieldvalue));
				...
				if (meta.getIsAFile()) {
					...
					try {
						...
						if (!setDocument(null, file, false, false)) {
							throw new KettleException(
									Messages
											.getString("GetXMLData.Log.UnableCreateDocument"));
						}
			...
			logError(Messages.getString("GetXMLData.Log.UnexpectedError", e
					.toString()));
			stopAll();
	...
	private void addFileToResultFilesname(FileObject file) throws Exception {
		if (meta.addResultFile()) {
			...
			resultFile.setComment(Messages
					.getString("GetXMLData.Log.FileAddedResult"));
		...
		if (data.NSPath.size() > 0) {
			...
			for (int i = 0; i < data.NSPath.size(); i++) {
				if (data.NSPath.get(i).length() > length
						&& fullPath.startsWith(data.NSPath.get(i))) {
					java.util.Arrays.fill(indexs,
							data.NSPath.get(i).split("/").length - 2,
							indexs.length, i);
			...
			for (int i = 0; i < pathStrs.length; i++) {
				...
				if (tmp.length() > 0 && tmp.indexOf(":") == -1
					...
					if (index >= 0) {
						newPath.append("pre").append(index).append(":").append(
								tmp);
					} else {
						newPath.append(tmp);
					}
	...
	private boolean applyXPath() {
		try {
			...
			if (meta.isNamespaceAware()) {
				xpath = data.document.createXPath(addNSPrefix(data.PathValue,
						data.PathValue));
			...
			log.logError(toString(), Messages.getString(
					"GetXMLData.Log.ErrorApplyXPath", e.getMessage()));
			return false;
		}
		return true;
	}
	...
	private boolean openNextFile() {
		try {
			...
			if (meta.isIgnoreEmptyFile() && fileSize == 0) {
				...
				if (log.isDetailed())
					log
							.logDetailed(toString(), Messages.getString(
									"GetXMLData.Log.OpeningFile", data.file
											.toString()));

				// Open the XML document
				if (!setDocument(null, data.file, false, false)) {
					if (data.stopPruning)
						return false; // ignore error when stopped while
					// pruning
					throw new KettleException(Messages
							.getString("GetXMLData.Log.UnableCreateDocument"));
				}
				...
				// Apply XPath and set node list
				if (data.prunePath == null) { // this was already done in
					// processStreaming()
					if (!applyXPath()) {
						throw new KettleException(Messages
								.getString("GetXMLData.Log.UnableApplyXPath"));
				...
				if (log.isDetailed()) {
					logDetailed(Messages.getString("GetXMLData.Log.FileOpened",
							data.file.toString()));
					log.logDetailed(toString(), Messages.getString(
							"GetXMLData.Log.LoopFileOccurences", ""
									+ data.nodesize, data.file.getName()
									.getBaseName()));
				}
			}
		} catch (Exception e) {
			logError(Messages.getString("GetXMLData.Log.UnableToOpenFile", ""
					+ data.filenr, data.file.toString(), e.toString()));
			stopAll();
			setErrors(1);
			return false;
		}
	...
	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi)
		...
		if (first && !meta.getIsInFields()) {
			...
			if (!meta.isdoNotFailIfNoFile() && data.files.nrOfFiles() == 0) {
				throw new KettleException(Messages
						.getString("GetXMLData.Log.NoFiles"));
			}
	...
	private boolean putRowOut(Object[] r) throws KettleException {
		...
		if (log.isRowLevel())
			logRowlevel(Messages.getString("GetXMLData.Log.ReadRow", r
					.toString()));
		incrementLinesInput();
		data.rownr++;
	...
	private Object[] getXMLRowPutRowWithErrorhandling() throws KettleException {
		...
		try {
			...
			if (meta.getIsInFields())
				r = processPutRow(data.readrow, (AbstractNode) data.an
						.get(data.nodenr));
			else
				r = processPutRow(null, (AbstractNode) data.an.get(data.nodenr));
		} catch (Exception e) {
			throw new KettleException(Messages
					.getString("GetXMLData.Error.UnableReadFile"), e);
		}

		return r;
	}

	private Object[] processPutRow(Object[] row, AbstractNode node)
			throws KettleException {
		// Create new row...
		...
		try {
			data.nodenr++;
			if (row != null)
				System.arraycopy(row, 0, outputRowData, 0,
						data.totalpreviousfields);
			// Read fields...
			for (int i = 0; i < data.nrInputFields; i++) {
				// Get field
				GetXMLDataField xmlDataField = meta.getInputFields()[i];
				// Get the Path to look for
				String XPathValue = environmentSubstitute(xmlDataField
						.getXPath());
				// Get the path type
				...
				if (meta.isuseToken()) {
					...
					if (indexvarstart >= 0 && indexvarend >= 0) {
						String NameVarInputField = XPathValue.substring(
								indexvarstart + 2, indexvarend);
						for (int k = 0; k < meta.getInputFields().length; k++) {
							GetXMLDataField Tmp_xmlInputField = meta
									.getInputFields()[k];
							if (Tmp_xmlInputField.getName().equalsIgnoreCase(
									NameVarInputField)) {
								XPathValue = XPathValue
										.replaceAll(
												data.tokenStart
														...
														+ data.tokenEnd,
												"'"
														+ outputRowData[data.totalpreviousfields
																+ k] + "'");
								if (log.isDetailed())
				...
				if (meta.isNamespaceAware()) {
					XPath xpathField = node.createXPath(addNSPrefix(XPathValue,
							data.PathValue));
				...
				if (meta.getInputFields()[i].isRepeated()) {
					if (data.previousRow != null
							&& Const.isEmpty(nodevalue)
							&& data.previousRow != null
							&& data.previousRow.length > data.totalpreviousfields
									+ i) {
			...
			data.previousRow = irow == null ? outputRowData : (Object[]) irow
			...
			if (getStepMeta().isDoingErrorHandling()) {
				...
				putError(data.outputRowMeta, outputRowData, 1, e.toString(),
						null, "GetXMLData001");
	...
	public boolean init(StepMetaInterface smi, StepDataInterface sdi) {
		...
		if (super.init(smi, sdi)) {
			...
			if (!data.PathValue.substring(0, 1).equals("/"))
				data.PathValue = "/" + data.PathValue;
			if (log.isDetailed())
				log.logDetailed(toString(), Messages.getString(
						"GetXMLData.Log.LoopXPath", data.PathValue));
			...
			if (data.prunePath != null) {
				if (Const.isEmpty(data.prunePath.trim())) {
					...
					if (!data.prunePath.startsWith("/"))
						data.prunePath = "/" + data.prunePath;
					// check if other conditions apply that do not allow pruning
					if (meta.getIsInFields())
						data.prunePath = null; // not possible by design, could
					// be changed later on
				}
			}
