 ...
 */
@Step(name = "FieldSplitter", image = "SPL.png", tooltip = "BaseStep.TypeTooltipDesc.SplitFields", description = "BaseStep.TypeLongDesc.SplitFields", category = StepCategory.CATEGORY_TRANSFORM)
public class FieldSplitterMeta extends BaseStepMeta implements
		StepMetaInterface {
	...
	public static final String STEP_ATTRIBUTE_FIELD_ID = "field_id";
	public static final String STEP_ATTRIBUTE_FIELD_IDREM = "field_idrem";
	...
	public static final String STEP_ATTRIBUTE_FIELD_LANGTH = "field_length";
	...
	public static final String STEP_ATTRIBUTE_FIELD_TRIMTYPE = "field_trimtype";
	/** Field to split */
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		String[] fieldID = p.get(id + "_fields.fieldID");
		String[] fieldType = p.get(id + "_fields.fieldType");
		String[] fieldRemoveID = p.get(id + "_fields.fieldRemoveID");
		...
		String[] fieldIfNull = p.get(id + "_fields.fieldIfNull");
		String[] fieldTrimType = p.get(id + "_fields.fieldTrimType");

//		this.fieldName = fieldName;
//		this.fieldID = fieldID;
//		this.fieldRemoveID = stringArrayToBooleanArray(fieldRemoveID, false);
//		this.fieldType =  stringArrayToIntArray(fieldType, 0);
//		this.fieldLength = stringArrayToIntArray(fieldLength, -1);
//		this.fieldPrecision = stringArrayToIntArray(fieldPrecision, -1);
//		this.fieldFormat = fieldFormat;
//		this.fieldGroup = fieldGroup;
//		this.fieldDecimal = fieldDecimal;
//		this.fieldCurrency = fieldCurrency;
//		this.fieldNullIf = fieldNullIf;
//		this.fieldIfNull = fieldIfNull;
//		this.fieldTrimType = stringArrayToIntArray(fieldTrimType, 0);
		this.fieldName = (fieldName != null) ? fieldName : (new String[0]);
		this.fieldID = (fieldID != null) ? fieldID : (new String[0]);
		this.fieldRemoveID = stringArrayToBooleanArray(fieldRemoveID, true);
		this.fieldType = (fieldType != null) ? stringArrayToIntArray(fieldType,
				0) : (new int[0]);
		this.fieldLength = (fieldLength != null) ? stringArrayToIntArray(fieldLength,
				-1) : (new int[0]);
		this.fieldPrecision = (fieldPrecision != null) ? stringArrayToIntArray(fieldPrecision,
				0) : (new int[0]);
		this.fieldFormat = (fieldFormat != null) ? fieldFormat : (new String[0]);
		this.fieldGroup = (fieldGroup != null) ? fieldGroup : (new String[0]);
		this.fieldDecimal = (fieldDecimal != null) ? fieldDecimal : (new String[0]);
		this.fieldCurrency = (fieldCurrency != null) ? fieldCurrency : (new String[0]);
		this.fieldNullIf = (fieldNullIf != null) ? fieldNullIf : (new String[0]);
		this.fieldIfNull = (fieldIfNull != null) ? fieldIfNull : (new String[0]);
		this.fieldTrimType = (fieldTrimType != null) ? stringArrayToIntArray(fieldTrimType,
				0) : (new int[0]);
	...
	private void readData(Node stepnode) throws KettleXMLException {
		try {
			...
			for (int i = 0; i < nrfields; i++) {
				final Node fnode = XMLHandler
						.getSubNodeByNr(fields, "field", i); //$NON-NLS-1$
			...
			throw new KettleXMLException(
					Messages
							.getString("FieldSplitterMeta.Exception.UnableToLoadStepInfoFromXML"), e); //$NON-NLS-1$
		}
	}
	...
	public void getFields(RowMetaInterface r, String name,
		...
		if (idx < 0) // not found
		{
			throw new RuntimeException(Messages.getString(
					"FieldSplitter.Log.CouldNotFindFieldToSplit", splitField)); //$NON-NLS-1$ //$NON-NLS-2$
		}

		// Add the new fields at the place of the index --> replace!
		for (int i = 0; i < fieldName.length; i++) {
			final ValueMetaInterface v = new ValueMeta(fieldName[i],
					fieldType[i]);
	...
	public String getXML() {
		...
		retval
				.append("   ").append(XMLHandler.addTagValue("splitfield", splitField)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("   ").append(XMLHandler.addTagValue("delimiter", delimiter)); //$NON-NLS-1$ //$NON-NLS-2$

		retval.append("    <fields>"); //$NON-NLS-1$
		for (int i = 0; i < fieldName.length; i++) {
			retval.append("      <field>"); //$NON-NLS-1$
			retval
					.append("        ").append(XMLHandler.addTagValue("name", fieldName[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval
					.append("        ").append(XMLHandler.addTagValue("id", fieldID[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval
					.append("        ").append(XMLHandler.addTagValue("idrem", fieldRemoveID[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval
					.append("        ").append(XMLHandler.addTagValue("type", ValueMeta.getTypeDesc(fieldType[i]))); //$NON-NLS-1$ //$NON-NLS-2$
			retval
					.append("        ").append(XMLHandler.addTagValue("format", fieldFormat[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval
	...
	public void readRep(Repository rep, long id_step,
		...
		try {
			...
			for (int i = 0; i < nrfields; i++) {
				...
				fieldRemoveID[i] = rep.getStepAttributeBoolean(id_step, i,
						"field_idrem"); //$NON-NLS-1$
				fieldType[i] = ValueMeta.getType(rep.getStepAttributeString(
						id_step, i, "field_type")); //$NON-NLS-1$
				...
				fieldTrimType[i] = (int) rep.getStepAttributeInteger(id_step,
						i, "field_trimtype"); //$NON-NLS-1$
			}
		} catch (Exception e) {
			throw new KettleException(
					Messages
							.getString("FieldSplitterMeta.Exception.UnexpectedErrorInReadingStepInfo"), e); //$NON-NLS-1$
		}
	}
	...
	public void saveRep(Repository rep, long id_transformation, long id_step)
		...
		try {
			...
			for (int i = 0; i < fieldName.length; i++) {
				...
				rep.saveStepAttribute(id_transformation, id_step, i,
						"field_id", fieldID[i]); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i,
						"field_idrem", fieldRemoveID[i]); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i,
			...
			throw new KettleException(
					Messages
							.getString("FieldSplitterMeta.Exception.UnalbeToSaveStepInfoToRepository") + id_step, e); //$NON-NLS-1$
		}
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transmeta,
		...
		if (prev != null && prev.size() > 0) {
			cr = new CheckResult(
					...
					Messages
							.getString(
									"FieldSplitterMeta.CheckResult.StepReceivingFields", prev.size() + ""), stepMeta); //$NON-NLS-1$ //$NON-NLS-2$
			remarks.add(cr);
			...
			if (i < 0) {
				error_message = Messages
						.getString(
								"FieldSplitterMeta.CheckResult.SplitedFieldNotPresentInInputStream", splitField); //$NON-NLS-1$ //$NON-NLS-2$
				...
				cr = new CheckResult(
						...
						Messages
								.getString(
										"FieldSplitterMeta.CheckResult.SplitedFieldFoundInInputStream", splitField), stepMeta); //$NON-NLS-1$ //$NON-NLS-2$
				remarks.add(cr);
			}
		} else {
			error_message = Messages
					.getString("FieldSplitterMeta.CheckResult.CouldNotReadFieldsFromPreviousStep") + Const.CR; //$NON-NLS-1$
		...
		if (input.length > 0) {
			cr = new CheckResult(
					CheckResult.TYPE_RESULT_OK,
					Messages
							.getString("FieldSplitterMeta.CheckResult.StepReceivingInfoFromOtherStep"), stepMeta); //$NON-NLS-1$
			remarks.add(cr);
		} else {
			cr = new CheckResult(
					CheckResult.TYPE_RESULT_ERROR,
					Messages
							.getString("FieldSplitterMeta.CheckResult.NoInputReceivedFromOtherStep"), stepMeta); //$NON-NLS-1$
			remarks.add(cr);
		}
	}
	...
	public StepInterface getStep(StepMeta stepMeta,
			...
			Trans trans) {
		return new FieldSplitter(stepMeta, stepDataInterface, cnr, transMeta,
				trans);
	}
