...
public class FormulaMeta extends BaseStepMeta implements StepMetaInterface
{
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		if (fieldName != null && fieldName.length > 0) {
			this.formula = new FormulaMetaFunction[fieldName.length];
			for (int i = 0; i < fieldName.length; i++) {
				this.formula[i] = new FormulaMetaFunction();
				this.formula[i].setFieldName(fieldName[i]);
				this.formula[i].setFormula(formula[i]);
				this.formula[i].setValueType(BaseStepMeta
						.parameterToInt(valueType[i]));
				this.formula[i].setValueLength(BaseStepMeta
						.parameterToInt(valueLength[i]));
				this.formula[i].setValuePrecision(BaseStepMeta
						.parameterToInt(valuePrecision[i]));
