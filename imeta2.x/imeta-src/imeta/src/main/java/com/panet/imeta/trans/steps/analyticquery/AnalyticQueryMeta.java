...
public class AnalyticQueryMeta extends BaseStepMeta implements
		StepMetaInterface {
	...
	private void readData(Node stepnode) throws KettleXMLException {
		try {
			...
			throw new KettleXMLException(
					Messages
							.getString("AnalyticQueryMeta.Exception.UnableToLoadStepInfoFromXML"), e); //$NON-NLS-1$
		}
	}
	...
	public void readRep(Repository rep, long id_step,
		...
		try {
			...
			for (int i = 0; i < nrvalues; i++) {
				...
				valueField[i] = (int) rep.getStepAttributeInteger(id_step, i,
			...
			throw new KettleException(
					Messages
							.getString("AnalyticQueryMeta.Exception.UnexpectedErrorInReadingStepInfoFromRepository"), e); //$NON-NLS-1$
		}
	}
	...
	public void saveRep(Repository rep, long id_transformation, long id_step)
		...
		try {
			...
			throw new KettleException(
					Messages
							.getString("AnalyticQueryMeta.Exception.UnableToSaveStepInfoToRepository") + id_step, e); //$NON-NLS-1$
		}
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (input.length > 0) {
			cr = new CheckResult(
					...
					Messages
							.getString("AnalyticQueryMeta.CheckResult.ReceivingInfoOK"), stepMeta); //$NON-NLS-1$
			remarks.add(cr);
		} else {
			cr = new CheckResult(
					CheckResultInterface.TYPE_RESULT_ERROR,
					Messages
							.getString("AnalyticQueryMeta.CheckResult.NoInputError"), stepMeta); //$NON-NLS-1$
			remarks.add(cr);
		}
	}
	...
	public StepInterface getStep(StepMeta stepMeta,
			...
			Trans trans) {
		return new AnalyticQuery(stepMeta, stepDataInterface, cnr, transMeta,
				trans);
	}
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		String[] fieldName = p.get(id + "_group.groupField") == null ? new String[0]
				: p.get(id + "_group.groupField");
		String[] aggregateField = p.get(id + "_analyticFunctions.name") == null ? new String[0]
				: p.get(id + "_analyticFunctions.name");
		String[] subjectField = p.get(id + "_analyticFunctions.subject") == null ? new String[0]
				: p.get(id + "_analyticFunctions.subject");
		String[] aggregateType = p.get(id + "_analyticFunctions.type") == null ? new String[0]
				: p.get(id + "_analyticFunctions.type");
		String[] valueField = p.get(id + "_analyticFunctions.value") == null ? new String[0]
				: p.get(id + "_analyticFunctions.value");
		this.groupField = fieldName;
		this.aggregateField = aggregateField;
		this.subjectField = subjectField;
		this.aggregateType = stringArrayToIntArray(aggregateType, 0);
		this.valueField = stringArrayToIntArray(valueField, 0);
	}
}
