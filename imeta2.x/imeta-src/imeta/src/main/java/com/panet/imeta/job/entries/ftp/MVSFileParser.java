...
public class MVSFileParser extends FTPFileParser {
  ...
  */
  ...
  public boolean isValidFormat(String[] listing) {
    ...
    if (log4j.isDebugEnabled()) {log4j.debug(Messages.getString("MVSFileParser.DEBUG.Checking.Parser"));} //$NON-NLS-1$
    if (listing.length > 0) {
      ...
      if ( (header.length == FOLDER_LISTING_LENGTH_NORMAL) || (header.length == FOLDER_LISTING_LENGTH_ARCIVE) ) {
        if (header[FOLDER_HEADER_TYPE_IDX].equals(HEADER_VOLUME) ) { 
          ...
          if (log4j.isInfoEnabled()) {log4j.debug(Messages.getString("MVSFileParser.INFO.Detected.Dir"));} //$NON-NLS-1$
          ...
          if (log4j.isInfoEnabled()) {log4j.debug(Messages.getString("MVSFileParser.INFO.Detected.PDS"));} //$NON-NLS-1$
  ...
  public void setLocale(Locale arg0) {
    //
    if (log4j.isDebugEnabled()) {log4j.debug(Messages.getString("MVSFileParser.DEBUG.Ignore.Locale"));} //$NON-NLS-1$
  }
  ...
  protected FTPFile parsePDSLine(String[] aLine, String raw) throws ParseException {
    ...
    if (aLine[0].equals(HEADER_NAME)) {
      if (log4j.isDebugEnabled()) {log4j.debug(Messages.getString("MVSFileParser.DEBUG.Skip.Header"));} //$NON-NLS-1$
      return null;
    }
    rtn = new FTPFile(raw);
    rtn.setName(aLine[0]);
  ...
  protected FTPFile parseFolder(String[] aLine, String raw) {
    ...
    if (aLine[0].equals(LINE_TYPE_ARCIVE)) { // It's on tape somewhere
      if (log4j.isDebugEnabled()) {log4j.debug(Messages.getString("MVSFileParser.DEBUG.Skip.ARCIVE"));} //$NON-NLS-1$
      return null;
    }
    if (aLine[0].equals(LINE_TYPE_MIGRATED)) { // It's been moved.
      if (log4j.isDebugEnabled()) {log4j.debug(Messages.getString("MVSFileParser.DEBUG.Skip.Migrated"));} //$NON-NLS-1$
      return null;
    }
    if (aLine[5].charAt(0) != 'F' && aLine[5].charAt(0) != 'V') {
      if (log4j.isDebugEnabled()) {log4j.debug(Messages.getString("MVSFileParser.DEBUG.Skip.recf"));} //$NON-NLS-1$
      return null;
    }
    if (aLine[8].charAt(0) != 'P') { // Only handle PO, PS, or PO-E
      if (log4j.isDebugEnabled()) {log4j.debug(Messages.getString("MVSFileParser.DEBUG.Skip.dso"));} //$NON-NLS-1$
      return null;
    }
    ...
    if (aLine[8].equals(ENTRY_FILE_TYPE)) {
      if (log4j.isDebugEnabled()) {log4j.debug(Messages.getString("MVSFileParser.DEBUG.Found.File", aLine[9]));} //$NON-NLS-1$
      // This is a file...
      rtn.setDir(false);
      long l = -1;
      try {
      ...
      if (log4j.isDebugEnabled()) {log4j.debug(Messages.getString("MVSFileParser.DEBUG.Found.Folder", aLine[9]));} //$NON-NLS-1$
      rtn.setDir(true);
    }
  ...
  protected boolean isValidDirectoryFormat(String[] listing) {
    ...
    for (int i=1; i<listing.length; i++) {
      ...
      if ( (aLine.length == 2) && (aLine[0].equals(LINE_TYPE_MIGRATED))) {
        if (log4j.isDebugEnabled()) {log4j.debug(Messages.getString("MVSFileParser.DEBUG.Detected.Migrated"));} //$NON-NLS-1$
      } else if (aLine.length != 10 && ( !aLine[0].equals(LINE_TYPE_ARCIVE))) { // 10 = regular, ARCIVE=on tape
        log4j.error(Messages.getString("MVSFileParser.ERROR.Invalid.Folder.Line", listing[i])); //$NON-NLS-1$
        return false;
      }
  ...
  protected boolean isValidPDSFormat(String[] listing) {
    ...
    for (int i=1; i<listing.length; i++) {
      ...
      if (aLine.length != 9) { // 9 because there are two fields for changed...
        log4j.error(Messages.getString("MVSFileParser.ERROR.Invalid.PDS.Line", listing[i])); //$NON-NLS-1$
        return false;
      }
      if (dateFormatString != null) {
  ...
  protected boolean checkDateFormat(String dateStr) {
    try {
      ...
      if (log4j.isDebugEnabled()) {
        if (log4j.isDebugEnabled()) {log4j.debug(Messages.getString("MVSFileParser.DEBUG.Date.Parse.Error"));} //$NON-NLS-1$
      }
      if ( (alternateFormatString != null) ) {
        if (log4j.isDebugEnabled()) {
          if (log4j.isDebugEnabled()) {log4j.debug(Messages.getString("MVSFileParser.DEBUG.Date.Parse.Choose.Alt"));} //$NON-NLS-1$
        }
        ...
        log4j.error(Messages.getString("MVSFileParser.ERROR.Date.Parse.Fail", dateStr)); //$NON-NLS-1$
        return false;
      }
    }
    return true;
  ...
  protected void guessDateFormat(String dateStr) {
    if (log4j.isDebugEnabled()) {log4j.debug(Messages.getString("MVSFileParser.DEBUG.Guess.Date"));} //$NON-NLS-1$
    ...
    if (yrFmt != null) {
      ...
      if (dayPos >=0) {
        ...
        if (log4j.isDebugEnabled()) {log4j.debug(Messages.getString("MVSFileParser.DEBUG.Guess.Date.Obvious"));} //$NON-NLS-1$
      } else {
        ...
        if (log4j.isDebugEnabled()) {log4j.debug(Messages.getString("MVSFileParser.DEBUG.Guess.Date.Ambiguous"));} //$NON-NLS-1$
      }
      this.dateFormatString = fmt.toString();
      this.dateFormat = new SimpleDateFormat(dateFormatString);
      if (log4j.isDebugEnabled()) {log4j.debug(Messages.getString("MVSFileParser.DEBUG.Guess.Date.Decided", this.dateFormatString));} //$NON-NLS-1$
      try {
        dateFormat.parse(dateStr);
      } catch (ParseException ex) {
        if (log4j.isDebugEnabled()) {log4j.debug(Messages.getString("MVSFileParser.DEBUG.Guess.Date.Unparsable", dateStr));} //$NON-NLS-1$
      }
    } else {
      // looks ilke something like 01/02/05 - where's the year?
      if (log4j.isDebugEnabled()) {log4j.debug(Messages.getString("MVSFileParser.DEBUG.Guess.Date.Year.Ambiguous"));} //$NON-NLS-1$
      return;
    }
