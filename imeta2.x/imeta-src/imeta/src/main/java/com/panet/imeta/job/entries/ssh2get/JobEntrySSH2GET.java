...
public class JobEntrySSH2GET extends JobEntryBase implements Cloneable, JobEntryInterface
{
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		localDirectory = JobEntryBase.parameterToString(p.get(id + ".localDirectory"));
		wildcard = JobEntryBase.parameterToString(p.get(id + ".wildcard"));
		httpProxyHost = JobEntryBase.parameterToString(p.get(id + ".httpProxyHost"));
		httpproxyport = JobEntryBase.parameterToString(p.get(id + ".httpproxyport"));
		httpproxyusername = JobEntryBase.parameterToString(p.get(id + ".httpproxyusername"));
		httpProxyPassword = JobEntryBase.parameterToString(p.get(id + ".httpProxyPassword"));
		keyFilename = JobEntryBase.parameterToString(p.get(id + ".keyFilename"));
		keyFilePass = JobEntryBase.parameterToString(p.get(id + ".keyFilePass"));
		afterFtpPut = JobEntryBase.parameterToString(p.get(id + ".afterFtpPut"));
		destinationfolder = JobEntryBase.parameterToString(p.get(id + ".destinationfolder"));
		onlyGettingNewFiles = JobEntryBase.parameterToBoolean(p.get(id + ".onlyGettingNewFiles"));	
		usehttpproxy = JobEntryBase.parameterToBoolean(p.get(id + ".usehttpproxy"));	
		publicpublickey = JobEntryBase.parameterToBoolean(p.get(id + ".publicpublickey"));	
		useBasicAuthentication = JobEntryBase.parameterToBoolean(p.get(id + ".useBasicAuthentication"));	
		createdestinationfolder = JobEntryBase.parameterToBoolean(p.get(id + ".createdestinationfolder"));	
		cachehostkey = JobEntryBase.parameterToBoolean(p.get(id + ".cachehostkey"));	
		createtargetfolder = JobEntryBase.parameterToBoolean(p.get(id + ".createtargetfolder"));	
		includeSubFolders = JobEntryBase.parameterToBoolean(p.get(id + ".includeSubFolders"));	
