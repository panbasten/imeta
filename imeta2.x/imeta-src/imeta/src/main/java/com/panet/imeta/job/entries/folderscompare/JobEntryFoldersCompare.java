...
/**
 * This defines a 'folder compare' job entry. It will compare 2 folders, and
 * will either follow the true flow upon the files being the same or the false
 * flow otherwise.
 * 
 ...
 * 
 */
public class JobEntryFoldersCompare extends JobEntryBase implements Cloneable,
		JobEntryInterface {
	...
	public String getXML() {
		...
		retval.append("      ").append(
				XMLHandler.addTagValue("filename1", filename1));
		retval.append("      ").append(
				XMLHandler.addTagValue("filename2", filename2));
	...
	public void loadXML(Node entrynode, List<DatabaseMeta> databases,
		...
		try {
			...
			includesubfolders = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					entrynode, "include_subfolders"));
			comparefilecontent = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					entrynode, "compare_filecontent"));
			comparefilesize = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					entrynode, "compare_filesize"));
			...
			throw new KettleXMLException(Messages.getString(
					"JobFoldersCompare.Meta.UnableLoadXML", xe.getMessage()));
		}
	}
	...
	public void loadRep(Repository rep, long id_jobentry,
		...
		try {
			...
			includesubfolders = rep.getJobEntryAttributeBoolean(id_jobentry,
					"include_subfolders");
			comparefilecontent = rep.getJobEntryAttributeBoolean(id_jobentry,
					"compare_filecontent");
			comparefilesize = rep.getJobEntryAttributeBoolean(id_jobentry,
					"compare_filesize");

			compareonly = rep.getJobEntryAttributeString(id_jobentry,
					"compareonly");
			wildcard = rep.getJobEntryAttributeString(id_jobentry, "wildcard");
			filename1 = rep
					.getJobEntryAttributeString(id_jobentry, "filename1");
			filename2 = rep
					.getJobEntryAttributeString(id_jobentry, "filename2");
		} catch (KettleException dbe) {
			throw new KettleException(Messages.getString(
					"JobFoldersCompare.Meta.UnableLoadRep", "" + id_jobentry,
					dbe.getMessage()));
		}
	}
	...
	public void saveRep(Repository rep, long id_job) throws KettleException {
		try {
			...
			rep.saveJobEntryAttribute(id_job, getID(), "include_subfolders",
					includesubfolders);
			rep.saveJobEntryAttribute(id_job, getID(), "compare_filecontent",
					comparefilecontent);
			rep.saveJobEntryAttribute(id_job, getID(), "compare_filesize",
					comparefilesize);

			rep.saveJobEntryAttribute(id_job, getID(), "compareonly",
					compareonly);
			...
			throw new KettleException(Messages.getString(
					"JobFoldersCompare.Meta.UnableSaveRep", "" + id_job, dbe
							.getMessage()));
		}
	}
		...
		try {
			if (filename1 != null && filename2 != null) {
				...
				if (folder1.exists() && folder2.exists()) {
					if (!folder1.getType().equals(folder2.getType())) {
						...
						log
								.logError(
										toString(),
										Messages
												.getString("JobFoldersCompare.Log.CanNotCompareFilesFolders"));

						if (folder1.getType() == FileType.FILE)
							log.logError(toString(), Messages.getString(
									"JobFoldersCompare.Log.IsAFile",
									realFilename1));
						else if (folder1.getType() == FileType.FOLDER)
							log.logError(toString(), Messages.getString(
									"JobFoldersCompare.Log.IsAFolder",
									realFilename1));
						else
							log.logError(toString(), Messages.getString(
						...
						if (folder1.getType() == FileType.FILE) {
							...
							FileObject list1[] = folder1
									.findFiles(new TextFileSelector(folder1
											.toString()));
							FileObject list2[] = folder2
									.findFiles(new TextFileSelector(folder2
											.toString()));
							...
							if (log.isDetailed()) {
								log.logDetailed(toString(), Messages.getString(
										"JobFoldersCompare.Log.FolderContains",
							...
							if (lenList1 == lenList2) {
								...
								// compare content
								Set<Map.Entry<String, String>> entrees = collection1
										.entrySet();
								Iterator<Map.Entry<String, String>> iterateur = entrees
										.iterator();

								while (iterateur.hasNext()) {
									Map.Entry<String, String> entree = iterateur
											.next();
									if (!collection2.containsKey(entree
											.getKey())) {
										ok = false;
										if (log.isDetailed())
											log
										...
										filefolder1 = KettleVFS
												.getFileObject(entree
														.getValue().toString());
										filefolder2 = KettleVFS
												.getFileObject(collection2.get(
														entree.getKey())
														.toString());
										...
										if (!filefolder2.getType().equals(
											...
											if (filefolder2.getType() == FileType.FILE) {
												...
												if (comparefilesize) {
													long filefolder1_size = filefolder1
															.getContent()
															.getSize();
													long filefolder2_size = filefolder2
															.getContent()
															.getSize();
								...
								if (log.isDetailed())
									log
											.logDetailed(
													...
													Messages
															.getString(
																	"JobFoldersCompare.Log.FoldersDifferentFiles",
																	realFilename1
																			.toString(),
																	realFilename2
																			.toString()));
					...
					if (!folder1.exists())
						log.logError(toString(), Messages.getString(
								"JobFileCompare.Log.FileNotExist",
								realFilename1));
					if (!folder2.exists())
						log.logError(toString(), Messages.getString(
								"JobFileCompare.Log.FileNotExist",
				...
				log.logError(toString(), Messages
						.getString("JobFoldersCompare.Log.Need2Files"));
			}
		} catch (Exception e) {
			result.setResult(false);
			result.setNrErrors(1);
			log.logError(toString(), Messages.getString(
					"JobFoldersCompare.Log.ErrorComparing", realFilename2,
			...
			try {
				if (!info.getFile().toString().equals(source_folder)) {
					...
					if (info.getFile().getParent().equals(info.getBaseFolder())) {
						// In the Base Folder...
						if ((info.getFile().getType() == FileType.FILE && compareonly
								.equals("only_files"))
								|| (info.getFile().getType() == FileType.FOLDER && compareonly
										.equals("only_folders"))
								|| (GetFileWildcard(short_filename) && compareonly
										.equals("specify"))
				...
				log.logError(toString(), "Error while finding files ... in ["
						+ info.getFile().toString() + "]. Exception :"
						+ e.getMessage());
				returncode = false;
			}
			return returncode;
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		this.includesubfolders = BaseStepMeta.parameterToBoolean(p.get(id
				+ ".includesubfolders"));
		this.compareonly = BaseStepMeta.parameterToString(p.get(id
				+ ".compareonly"));
		this.wildcard = BaseStepMeta.parameterToString(p.get(id + ".wildcard"));
		this.comparefilecontent = BaseStepMeta.parameterToBoolean(p.get(id
				+ ".comparefilecontent"));
		this.comparefilesize = BaseStepMeta.parameterToBoolean(p.get(id
				+ ".comparefilesize"));
		this.filename1 = BaseStepMeta.parameterToString(p
				.get(id + ".filename1"));
		this.filename2 = BaseStepMeta.parameterToString(p
				.get(id + ".filename2"));
	}
}
