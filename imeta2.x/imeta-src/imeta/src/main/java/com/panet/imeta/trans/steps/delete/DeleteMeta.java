...
/**
 * This class takes care of deleting values in a table using a certain condition
 * and values for input.
 * 
 * @author Tom, Matt
 * @since 28-March-2006
 */
public class DeleteMeta extends BaseStepMeta implements StepMetaInterface {
	...
	private void readData(Node stepnode,
		...
		try {
			...
			throw new KettleXMLException(
					Messages
							.getString("DeleteMeta.Exception.UnableToReadStepInfoFromXML"), e); //$NON-NLS-1$
		}
	}
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		String[] keyLookup = p.get(id + "_fields.keyLookup");
		String[] keyCondition = p.get(id + "_fields.keyCondition");
		String[] keyStream = p.get(id + "_fields.keyStream");
		String[] keyStream2 = p.get(id + "_fields.keyStream2");

		this.keyLookup = (keyLookup == null) ? new String[0] : keyLookup;
		this.keyCondition = (keyCondition == null) ? new String[0] : keyCondition;
		this.keyStream = (keyStream == null) ? new String[0] : keyStream;
		this.keyStream2 = (keyStream2 == null) ? new String[0] : keyStream2;
	...
	public void readRep(Repository rep, long id_step,
		...
		try {
			...
			throw new KettleException(
					Messages
							.getString("DeleteMeta.Exception.UnexpectedErrorInReadingStepInfo"), e); //$NON-NLS-1$
		}
	}
	...
	public void saveRep(Repository rep, long id_transformation, long id_step)
		...
		try {
			...
			throw new KettleException(
					Messages
							.getString("DeleteMeta.Exception.UnableToSaveStepInfo") + id_step, e); //$NON-NLS-1$
		}
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (databaseMeta != null) {
			...
			try {
				...
				if (!Const.isEmpty(tableName)) {
					cr = new CheckResult(
							...
							Messages
									.getString("DeleteMeta.CheckResult.TablenameOK"), stepinfo); //$NON-NLS-1$
					remarks.add(cr);
					...
					if (r != null) {
						cr = new CheckResult(
								CheckResultInterface.TYPE_RESULT_OK,
								Messages
										.getString("DeleteMeta.CheckResult.VisitTableSuccessfully"), stepinfo); //$NON-NLS-1$
						remarks.add(cr);
							...
							if (v == null) {
								if (first) {
									...
									error_message += Messages
											.getString("DeleteMeta.CheckResult.MissingCompareFieldsInTargetTable") + Const.CR; //$NON-NLS-1$
								}
								error_found = true;
						...
						if (error_found) {
							...
							cr = new CheckResult(
									CheckResultInterface.TYPE_RESULT_OK,
									Messages
											.getString("DeleteMeta.CheckResult.FoundLookupFields"), stepinfo); //$NON-NLS-1$
						}
						remarks.add(cr);
					} else {
						error_message = Messages
								.getString("DeleteMeta.CheckResult.CouldNotReadTableInfo"); //$NON-NLS-1$
						cr = new CheckResult(
								CheckResultInterface.TYPE_RESULT_ERROR,
								error_message, stepinfo);
						remarks.add(cr);
				...
				if (prev != null && prev.size() > 0) {
					cr = new CheckResult(
							...
							Messages
									.getString(
											"DeleteMeta.CheckResult.ConnectedStepSuccessfully", String.valueOf(prev.size())), stepinfo); //$NON-NLS-1$ //$NON-NLS-2$
					remarks.add(cr);
					...
					for (int i = 0; i < keyStream.length; i++) {
						...
						if (v == null) {
							if (first) {
								first = false;
								error_message += Messages
										.getString("DeleteMeta.CheckResult.MissingFields") + Const.CR; //$NON-NLS-1$
							}
							error_found = true;
						...
						if (keyStream2[i] != null && keyStream2[i].length() > 0) {
							...
							if (v == null) {
								if (first) {
									first = false;
									error_message += Messages
											.getString("DeleteMeta.CheckResult.MissingFields2") + Const.CR; //$NON-NLS-1$
								}
								error_found = true;
								error_message += "\t\t" + keyStream[i] + Const.CR; //$NON-NLS-1$
							}
					...
					if (error_found) {
						...
						cr = new CheckResult(
								CheckResultInterface.TYPE_RESULT_OK,
								Messages
										.getString("DeleteMeta.CheckResult.AllFieldsFound"), stepinfo); //$NON-NLS-1$
					}
					remarks.add(cr);
					...
					error_message = Messages
							.getString("DeleteMeta.CheckResult.MissingFields3") + Const.CR; //$NON-NLS-1$
					cr = new CheckResult(
							CheckResultInterface.TYPE_RESULT_ERROR,
							error_message, stepinfo);
					remarks.add(cr);
				...
				error_message = Messages
						.getString("DeleteMeta.CheckResult.DatabaseError") + e.getMessage(); //$NON-NLS-1$
			...
			error_message = Messages
					.getString("DeleteMeta.CheckResult.InvalidConnection"); //$NON-NLS-1$
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR,
					error_message, stepinfo);
			remarks.add(cr);
		}
		...
		if (input.length > 0) {
			cr = new CheckResult(
					CheckResultInterface.TYPE_RESULT_OK,
					Messages
							.getString("DeleteMeta.CheckResult.StepReceivingInfo"), stepinfo); //$NON-NLS-1$
			remarks.add(cr);
		} else {
			cr = new CheckResult(
					CheckResultInterface.TYPE_RESULT_ERROR,
					Messages
							.getString("DeleteMeta.CheckResult.NoInputReceived"), stepinfo); //$NON-NLS-1$
			remarks.add(cr);
		}
	}
				...
				if (!Const.isEmpty(tableName)) {
					...
					try {
						...
						if (keyLookup != null && keyLookup.length > 0) {
							...
							retval
									.setError(Messages
											.getString("DeleteMeta.CheckResult.KeyFieldsRequired")); //$NON-NLS-1$
						}
						...
						retval
								.setError(Messages
										.getString("DeleteMeta.Returnvalue.ErrorOccurred") + e.getMessage()); //$NON-NLS-1$
					}
				} else {
					retval
							.setError(Messages
									.getString("DeleteMeta.Returnvalue.NoTableDefinedOnConnection")); //$NON-NLS-1$
				}
			} else {
				retval
						.setError(Messages
								.getString("DeleteMeta.Returnvalue.NoReceivingAnyFields")); //$NON-NLS-1$
			}
		} else {
			retval.setError(Messages
					.getString("DeleteMeta.Returnvalue.NoConnectionDefined")); //$NON-NLS-1$
		}
