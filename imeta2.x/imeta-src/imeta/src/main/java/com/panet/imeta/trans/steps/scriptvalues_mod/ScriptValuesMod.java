...
import com.panet.imeta.compatibility.ValueUsedListener;
...
public class ScriptValuesMod extends BaseStep implements StepInterface,
			...
			if (strTransformScript.indexOf(valname) >= 0) {
				if (log.isDetailed())
					logDetailed(Messages
							.getString(
									"ScriptValuesMod.Log.UsedValueName", String.valueOf(i), valname)); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	...
	private boolean addValues(RowMetaInterface rowMeta, Object[] row)
			...
			try {
				...
				for (int i = 0; i < data.fields_used.length; i++) {
					ValueMetaInterface valueMeta = rowMeta
							.getValueMeta(data.fields_used[i]);
					Object valueData = row[data.fields_used[i]];

					if (meta.isCompatible()) {
						data.values_used[i] = valueMeta
								.createOriginalValue(valueData);

						Scriptable jsarg = Context.toObject(
								data.values_used[i], data.scope);
						data.scope.put(valueMeta.getName(), data.scope, jsarg);
					} else {
						Scriptable jsarg;
						if (valueData != null) {
							jsarg = Context.toObject(valueMeta
									.convertToNormalStorageType(valueData),
									data.scope);
						} else {
							jsarg = null;
						}
				...
				try {
					if (meta.getAddClasses() != null) {
						for (int i = 0; i < meta.getAddClasses().length; i++) {
							Object jsOut = Context.javaToJS(meta
									.getAddClasses()[i].getAddObject(),
									data.scope);
							ScriptableObject.putProperty(data.scope, meta
					...
					throw new KettleValueException(
							Messages
									.getString("ScriptValuesMod.Log.CouldNotAttachAdditionalScripts"), e); //$NON-NLS-1$
				}
				...
				try {
					...
					throw new KettleValueException(
							Messages
									.getString("ScriptValuesMod.Log.CouldNotAddDefaultFunctions"), ex); //$NON-NLS-1$
				}
				;
				...
				try {
					...
					data.scope.put("SKIP_TRANSFORMATION", data.scope, Integer
							.valueOf(SKIP_TRANSFORMATION));
					data.scope.put("ABORT_TRANSFORMATION", data.scope, Integer
							.valueOf(ABORT_TRANSFORMATION));
					data.scope.put("ERROR_TRANSFORMATION", data.scope, Integer
							.valueOf(ERROR_TRANSFORMATION));
					data.scope.put("CONTINUE_TRANSFORMATION", data.scope,
							Integer.valueOf(CONTINUE_TRANSFORMATION));
					...
					throw new KettleValueException(
							Messages
									.getString("ScriptValuesMod.Log.CouldNotAddDefaultConstants"),
							ex);
				}
				;
				...
				try {
					...
					if (strStartScript != null && strStartScript.length() > 0) {
						Script startScript = data.cx.compileString(
								strStartScript, "trans_Start", 1, null);
					...
					throw new KettleValueException(
							Messages
									.getString("ScriptValuesMod.Log.ErrorProcessingStartScript"),
							es);

				}
				// Now Compile our Script
				data.script = data.cx.compileString(strTransformScript,
						"script", 1, null);
			} catch (Exception e) {
				throw new KettleValueException(
						Messages
								.getString("ScriptValuesMod.Log.CouldNotCompileJavascript"),
						e);
			}
		}
		...
		try {
			try {
				...
				throw new KettleValueException(Messages
						.getString("ScriptValuesMod.Log.UnexpectedeError"), e); //$NON-NLS-1$ //$NON-NLS-2$				
			}
			...
			if (bWithTransStat) {
				iTranStat = (int) Context.toNumber(data.scope.get(
						"trans_Status", data.scope));
			} else {
			...
			if (iTranStat == CONTINUE_TRANSFORMATION) {
				...
				for (int i = 0; i < meta.getName().length; i++) {
					Object result = data.scope.get(meta.getName()[i],
							data.scope);
				...
				if (meta.isCompatible()) {
					for (int i = 0; i < data.values_used.length; i++) {
						ValueMetaInterface valueMeta = rowMeta
								.getValueMeta(data.fields_used[i]);
						outputRow[data.fields_used[i]] = valueMeta
								.getValueData(data.values_used[i]);
					}
			...
			throw new KettleValueException(Messages
					.getString("ScriptValuesMod.Log.JavascriptError"), e); //$NON-NLS-1$
		}
		return bRC;
	}

	public Object getValueFromJScript(Object result, int i)
			throws KettleValueException {
		if (meta.getName()[i] != null && meta.getName()[i].length() > 0) {
				...
				if (result != null) {
					...
					case ValueMetaInterface.TYPE_NUMBER:
						if (classType
							...
							try {
								...
								return new Double(Double.parseDouble(Const
										.trim(string)));
							}
						} else if (classType
								.equalsIgnoreCase("org.mozilla.javascript.NativeNumber")) {
					...
					case ValueMetaInterface.TYPE_INTEGER:
						if (classType.equalsIgnoreCase("java.lang.Byte")) {
							return new Long(((java.lang.Byte) result)
									.longValue());
						} else if (classType
							...
							//
							try {
								Value value = (Value) Context.jsToJava(result,
										Value.class);
								return value.getInteger();
							} catch (Exception e2) {
								String string = Context.toString(result);
								return new Long(Long.parseLong(Const
										.trim(string)));
							}
						} else if (classType
								.equalsIgnoreCase("org.mozilla.javascript.UniqueTag")) {
							return Long
									.valueOf(Long
											.parseLong(((UniqueTag) result)
													.toString()));
						} else {
					...
					case ValueMetaInterface.TYPE_DATE:
						...
						if (classType
							...
							if (classType
									.equalsIgnoreCase("org.mozilla.javascript.NativeDate")) {
								...
								try {
									...
									//
									try {
										Value value = (Value) Context.jsToJava(
												result, Value.class);
								...
								String string = (String) Context.jsToJava(
										result, String.class);
					...
					case ValueMetaInterface.TYPE_BIGNUMBER:
						if (classType
							...
							try {
								BigDecimal bd = (BigDecimal) Context.jsToJava(
							...
							return new BigDecimal(((java.lang.Byte) result)
									.longValue());
						} else if (classType
								.equalsIgnoreCase("java.lang.Short")) {
			...
			throw new KettleValueException(
					"No name was specified for result value #" + (i + 1));
		}
	}
	...
	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi)
			...
			try {
				if (data.cx != null) {
					...
					if (strEndScript != null && strEndScript.length() > 0) {
						Script endScript = data.cx.compileString(strEndScript,
								"trans_End", 1, null);
				...
				logError(Messages
						.getString("ScriptValuesMod.Log.UnexpectedeError") + " : " + e.toString()); //$NON-NLS-1$ //$NON-NLS-2$				
				logError(Messages
						.getString("ScriptValuesMod.Log.ErrorStackTrace") + Const.CR + Const.getStackTracker(e)); //$NON-NLS-1$
				setErrors(1);
				stopAll();
			}
		...
		try {
			...
			if (getStepMeta().isDoingErrorHandling()) {
				putError(getInputRowMeta(), r, 1, e.getMessage() + Const.CR
						+ location, null, "SCR-001");
			} else {
				throw (e);
			}
		}
			...
			for (int j = 0; j < jsScripts.length; j++) {
				...
				case ScriptValuesScript.TRANSFORM_SCRIPT:
					...
					if (meta.isVariableReplacementActive()) {
						strTransformScript = environmentSubstitute(strTransformScript);
					}
					break;
				case ScriptValuesScript.START_SCRIPT:
					strStartScript = jsScripts[j].getScript();
					if (meta.isVariableReplacementActive()) {
						strStartScript = environmentSubstitute(strStartScript);
					}
					break;
				case ScriptValuesScript.END_SCRIPT:
					strEndScript = jsScripts[j].getScript();
					if (meta.isVariableReplacementActive()) {
						strEndScript = environmentSubstitute(strEndScript);
					}
					break;
				}
			}
	...
	public void dispose(StepMetaInterface smi, StepDataInterface sdi) {
		try {
			...
			// Eat this error, it's typically : "Calling Context.exit without
