...
/**
 * Here are the steps that we need to take to make streaming loading possible
 * for LucidDB:<br>
 * <br>
 * Prerequisites:<br>
 * <br>
 ...
 * <br>
 * - In the FIFO directory, create a FIFO file called {tableName}.csv (using
 * mkfifo, LINUX ONLY FOLKS!)<br>
 ...
 * - Execute the following SQL Command to bulk load in a separate SQL thread in
 * the background:<br>
 * 
 * <pre>
 ...
 */
public class LucidDBBulkLoaderMeta extends BaseStepMeta implements
		StepMetaInterface {
	...
	/**
	 * The number of rows to buffer before passing them over to LucidDB. This
	 * number should be non-zero since we need to specify the number of rows we
	 * pass.
	 */
	...
	private void readData(Node stepnode,
		...
		try {
			...
			fifoServerName = XMLHandler.getTagValue(stepnode,
					"fifo_server_name"); //$NON-NLS-1$
			...
			throw new KettleXMLException(
					Messages
							.getString("LucidDBBulkLoaderMeta.Exception.UnableToReadStepInfoFromXML"), e); //$NON-NLS-1$
		}
	}
	...
	public void setDefault() {
		...
		tableName = Messages
				.getString("LucidDBBulkLoaderMeta.DefaultTableName"); //$NON-NLS-1$
		encoding = ""; //$NON-NLS-1$
	...
	public String getXML() {
		...
		retval
				.append("    ").append(XMLHandler.addTagValue("buffer_size", bufferSize)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
		...
		retval
				.append("    ").append(XMLHandler.addTagValue("fifo_directory", fifoDirectory)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("    ").append(XMLHandler.addTagValue("fifo_server_name", fifoServerName)); //$NON-NLS-1$ //$NON-NLS-2$
		...
		for (int i = 0; i < fieldTable.length; i++) {
			...
			retval
					.append("        ").append(XMLHandler.addTagValue("field_format_ok", fieldFormatOk[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval.append("      </mapping>").append(Const.CR); //$NON-NLS-1$
		}
	...
	public void readRep(Repository rep, long id_step,
		...
		try {
			...
			fifoDirectory = rep.getStepAttributeString(id_step,
					"fifo_directory"); //$NON-NLS-1$
			fifoServerName = rep.getStepAttributeString(id_step,
					"fifo_server_name"); //$NON-NLS-1$
			...
			for (int i = 0; i < nrvalues; i++) {
				...
				fieldFormatOk[i] = rep.getStepAttributeBoolean(id_step, i,
						"field_format_ok"); //$NON-NLS-1$
			}
		} catch (Exception e) {
			throw new KettleException(
					Messages
							.getString("LucidDBBulkLoaderMeta.Exception.UnexpectedErrorReadingStepInfoFromRepository"), e); //$NON-NLS-1$
		}
	}
	...
	public void saveRep(Repository rep, long id_transformation, long id_step)
		...
		try {
			...
			rep.saveStepAttribute(id_transformation, id_step,
					"buffer_size", bufferSize); //$NON-NLS-1$
			rep.saveStepAttribute(id_transformation, id_step,
			...
			rep.saveStepAttribute(id_transformation, id_step,
					"fifo_directory", fifoDirectory); //$NON-NLS-1$
			rep.saveStepAttribute(id_transformation, id_step,
					"fifo_server_name", fifoServerName); //$NON-NLS-1$
			...
			for (int i = 0; i < fieldTable.length; i++) {
				...
				rep.saveStepAttribute(id_transformation, id_step, i,
						"field_format_ok", fieldFormatOk[i]); //$NON-NLS-1$
			}
			...
			throw new KettleException(
					Messages
							.getString("LucidDBBulkLoaderMeta.Exception.UnableToSaveStepInfoToRepository") + id_step, e); //$NON-NLS-1$
		}
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (databaseMeta != null) {
			...
			try {
				...
				if (!Const.isEmpty(tableName)) {
					cr = new CheckResult(
							...
							Messages
									.getString("LucidDBBulkLoaderMeta.CheckResult.TableNameOK"), stepMeta); //$NON-NLS-1$
					remarks.add(cr);
					...
					if (r != null) {
						cr = new CheckResult(
								CheckResultInterface.TYPE_RESULT_OK,
								Messages
										.getString("LucidDBBulkLoaderMeta.CheckResult.TableExists"), stepMeta); //$NON-NLS-1$
						remarks.add(cr);
							...
							if (v == null) {
								if (first) {
									...
									error_message += Messages
											.getString("LucidDBBulkLoaderMeta.CheckResult.MissingFieldsToLoadInTargetTable") + Const.CR; //$NON-NLS-1$
								}
								error_found = true;
						...
						if (error_found) {
							...
							cr = new CheckResult(
									CheckResultInterface.TYPE_RESULT_OK,
									Messages
											.getString("LucidDBBulkLoaderMeta.CheckResult.AllFieldsFoundInTargetTable"), stepMeta); //$NON-NLS-1$
						}
						remarks.add(cr);
					} else {
						error_message = Messages
								.getString("LucidDBBulkLoaderMeta.CheckResult.CouldNotReadTableInfo"); //$NON-NLS-1$
						cr = new CheckResult(
								CheckResultInterface.TYPE_RESULT_ERROR,
								error_message, stepMeta);
						remarks.add(cr);
				...
				if (prev != null && prev.size() > 0) {
					cr = new CheckResult(
							...
							Messages
									.getString(
											"LucidDBBulkLoaderMeta.CheckResult.StepReceivingDatas", prev.size() + ""), stepMeta); //$NON-NLS-1$ //$NON-NLS-2$
					remarks.add(cr);
					...
					for (int i = 0; i < fieldStream.length; i++) {
						...
						if (v == null) {
							if (first) {
								first = false;
								error_message += Messages
										.getString("LucidDBBulkLoaderMeta.CheckResult.MissingFieldsInInput") + Const.CR; //$NON-NLS-1$
							}
							error_found = true;
					...
					if (error_found) {
						...
						cr = new CheckResult(
								CheckResultInterface.TYPE_RESULT_OK,
								Messages
										.getString("LucidDBBulkLoaderMeta.CheckResult.AllFieldsFoundInInput"), stepMeta); //$NON-NLS-1$
					}
					remarks.add(cr);
				} else {
					error_message = Messages
							.getString("LucidDBBulkLoaderMeta.CheckResult.MissingFieldsInInput3") + Const.CR; //$NON-NLS-1$
					cr = new CheckResult(
							CheckResultInterface.TYPE_RESULT_ERROR,
							error_message, stepMeta);
					remarks.add(cr);
				...
				error_message = Messages
						.getString("LucidDBBulkLoaderMeta.CheckResult.DatabaseErrorOccurred") + e.getMessage(); //$NON-NLS-1$
			...
			error_message = Messages
					.getString("LucidDBBulkLoaderMeta.CheckResult.InvalidConnection"); //$NON-NLS-1$
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR,
					error_message, stepMeta);
			remarks.add(cr);
		}
		...
		if (input.length > 0) {
			cr = new CheckResult(
					CheckResultInterface.TYPE_RESULT_OK,
					Messages
							.getString("LucidDBBulkLoaderMeta.CheckResult.StepReceivingInfoFromOtherSteps"), stepMeta); //$NON-NLS-1$
			remarks.add(cr);
		} else {
			cr = new CheckResult(
					CheckResultInterface.TYPE_RESULT_ERROR,
					Messages
							.getString("LucidDBBulkLoaderMeta.CheckResult.NoInputError"), stepMeta); //$NON-NLS-1$
			remarks.add(cr);
		}
	}
			...
			if (prev != null && prev.size() > 0) {
				...
				if (!Const.isEmpty(tableName)) {
					...
					try {
						...
						retval
								.setError(Messages
										.getString("LucidDBBulkLoaderMeta.GetSQL.ErrorOccurred") + e.getMessage()); //$NON-NLS-1$
					}
				} else {
					retval
							.setError(Messages
									.getString("LucidDBBulkLoaderMeta.GetSQL.NoTableDefinedOnConnection")); //$NON-NLS-1$
				}
			} else {
				retval
						.setError(Messages
								.getString("LucidDBBulkLoaderMeta.GetSQL.NotReceivingAnyFields")); //$NON-NLS-1$
			}
		} else {
			retval
					.setError(Messages
							.getString("LucidDBBulkLoaderMeta.GetSQL.NoConnectionDefined")); //$NON-NLS-1$
		}
	...
	public StepInterface getStep(StepMeta stepMeta,
			...
			Trans trans) {
		return new LucidDBBulkLoader(stepMeta, stepDataInterface, cnr,
				transMeta, trans);
	}
	...
	public RowMetaInterface getRequiredFields() throws KettleException {
		if (databaseMeta != null) {
			...
			try {
				...
				if (!Const.isEmpty(tableName)) {
					...
					if (db.checkTableExists(schemaTable)) {
						...
						throw new KettleException(
								Messages
										.getString("LucidDBBulkLoaderMeta.Exception.TableNotFound"));
					}
				} else {
					throw new KettleException(
							Messages
									.getString("LucidDBBulkLoaderMeta.Exception.TableNotSpecified"));
				}
			} catch (Exception e) {
				throw new KettleException(
						Messages
								.getString("LucidDBBulkLoaderMeta.Exception.ErrorGettingFields"),
						e);
			} finally {
				db.disconnect();
			}
			...
			throw new KettleException(
					Messages
							.getString("LucidDBBulkLoaderMeta.Exception.ConnectionNotDefined"));
		}
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		this.schemaName = BaseStepMeta.parameterToString(p.get(id
				+ ".targetSchame"));
		this.tableName = BaseStepMeta.parameterToString(p.get(id
				+ ".targetTable"));
		this.maxErrors = BaseStepMeta.parameterToInt(p.get(id
				+ ".maximumErrors"));
		this.fifoDirectory = BaseStepMeta.parameterToString(p.get(id
				+ ".fifoFilePath"));
		this.fifoServerName = BaseStepMeta.parameterToString(p.get(id
				+ ".fifoForeign"));
		this.fieldTable = p.get(id + "_fieldsToLoad.fieldTable") == null ? new String[0]
				: p.get(id + "_fieldsToLoad.fieldTable");
		this.fieldStream = p.get(id + "_fieldsToLoad.fieldStream") == null ? new String[0]
				: p.get(id + "_fieldsToLoad.fieldStream");
		this.fieldFormatOk = p.get(id + "_fieldsToLoad.fieldFormatOk") == null ? new boolean[0]
				: BaseStepMeta.stringArrayToBooleanArray(p.get(id
						+ "_fieldsToLoad.fieldFormatOk"),true);
