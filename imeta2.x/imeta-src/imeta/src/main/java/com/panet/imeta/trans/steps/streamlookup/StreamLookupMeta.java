...
public class StreamLookupMeta extends BaseStepMeta implements StepMetaInterface
{
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		lookupFromStepname = BaseStepMeta.parameterToString(p.get(id + ".lookupFromStepname"));
		inputSorted = BaseStepMeta.parameterToBoolean(p.get(id + ".inputSorted"));
		memoryPreservationActive = BaseStepMeta.parameterToBoolean(p.get(id + ".memoryPreservationActive"));
		usingSortedList = BaseStepMeta.parameterToBoolean(p.get(id + ".usingSortedList"));
		usingIntegerPair = BaseStepMeta.parameterToBoolean(p.get(id + ".usingIntegerPair"));
		
		String[] keystream = p.get(id + "_keys.keystream");
		String[] keylookup = p.get(id + "_keys.keylookup");
		String[] value = p.get(id + "_values.value");
		String[] valueName = p.get(id + "_values.valueName");
		String[] valueDefault = p.get(id + "_values.valueDefault");
		String[] valueDefaultType = p.get(id + "_values.valueDefaultType");
		...
		this.valueName = (valueName != null) ? valueName : (new String[0]);
		this.valueDefault = (valueDefault != null) ? valueDefault : (new String[0]);
		this.valueDefaultType = (valueDefaultType != null) ? stringArrayToIntArray(valueDefaultType,0) : (new int[0]);
