...
public class JobEntryColumnsExist extends JobEntryBase implements Cloneable,
		JobEntryInterface {
	...
	public String getXML() {
		...
		if (arguments != null) {
			for (int i = 0; i < arguments.length; i++) {
				...
				retval.append("          ").append(
						XMLHandler.addTagValue("name", arguments[i]));
				retval.append("        </field>").append(Const.CR); //$NON-NLS-1$
			}
		}
		retval.append("      </fields>").append(Const.CR); //$NON-NLS-1$
	...
	public void loadXML(Node entrynode, List<DatabaseMeta> databases,
		...
		try {
			...
			throw new KettleXMLException(Messages
					.getString("JobEntryColumnsExist.Meta.UnableLoadXml"), e);
		}
	}
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		String[] arguments = p.get(id + "_line.arguments");
	...
	public void loadRep(Repository rep, long id_jobentry,
		...
		try {
			...
			throw new KettleException(Messages
					.getString("JobEntryColumnsExist.Meta.UnableLoadRep", ""
							+ id_jobentry), dbe);
		}
	}
	...
	public void saveRep(Repository rep, long id_job) throws KettleException {
		try {
			...
			if (arguments != null) {
				for (int i = 0; i < arguments.length; i++) {
					rep.saveJobEntryAttribute(id_job, getID(), i, "name",
							arguments[i]);
				}
			}
		} catch (KettleDatabaseException dbe) {
			throw new KettleException(Messages.getString(
					"JobEntryColumnsExist.Meta.UnableSaveRep", "" + id_job),
					dbe);
		}
	}
	...
	public Result execute(Result previousResult, int nr, Repository rep,
		...
		if (Const.isEmpty(tablename)) {
			log.logError(toString(), Messages
					.getString("JobEntryColumnsExist.Error.TablenameEmpty"));
			return result;
		}
		if (arguments == null) {
			log.logError(toString(), Messages
					.getString("JobEntryColumnsExist.Error.ColumnameEmpty"));
			return result;
		}
		if (connection != null) {
							...
							if (log.isDetailed())
								log
										.logDetailed(
												...
												Messages
														.getString(
																"JobEntryColumnsExist.Log.ColumnExists",
																realColumnname,
																realTablename));
							nrexistcolums++;
						} else {
							log.logError(toString(), Messages.getString(
									"JobEntryColumnsExist.Log.ColumnNotExists",
									realColumnname, realTablename));
							nrnotexistcolums++;
						}
					}
				} else {
					log.logError(toString(), Messages.getString(
							"JobEntryColumnsExist.Log.TableNotExists",
							realTablename));
				}
			} catch (KettleDatabaseException dbe) {
				log.logError(toString(), Messages.getString(
						"JobEntryColumnsExist.Error.UnexpectedError", dbe
								.getMessage()));
			} finally {
				if (db != null)
					try {
			...
			log.logError(toString(), Messages
					.getString("JobEntryColumnsExist.Error.NoDbConnection"));
		}
	...
	public void check(List<CheckResultInterface> remarks, JobMeta jobMeta) {
		...
		andValidator().validate(this,
				"columnname", remarks, putValidators(notBlankValidator())); //$NON-NLS-1$
	}
