...
public class GroupByMeta extends BaseStepMeta implements StepMetaInterface {
	...
	public static final String typeGroupLongDesc[] = {
			...
			Messages
					.getString("GroupByMeta.TypeGroupLongDesc.CUMUMALTIVE_AVERAGE"), //$NON-NLS-1$ 
			Messages
					.getString("GroupByMeta.TypeGroupLongDesc.STANDARD_DEVIATION"), //$NON-NLS-1$ 
	...
	private void readData(Node stepnode) throws KettleXMLException {
		try {
			...
			aggregateIgnoredField = XMLHandler.getTagValue(stepnode,
					"field_ignore"); //$NON-NLS-1$

			directory = XMLHandler.getTagValue(stepnode, "directory"); //$NON-NLS-1$
			prefix = XMLHandler.getTagValue(stepnode, "prefix"); //$NON-NLS-1$

			addingLineNrInGroup = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "add_linenr")); // $NON-NLS-1$
			lineNrInGroupField = XMLHandler.getTagValue(stepnode,
					"linenr_fieldname");
			...
			for (int i = 0; i < nrfields; i++) {
				...
				aggregateType[i] = getType(XMLHandler
						.getTagValue(fnode, "type")); //$NON-NLS-1$
			...
			String giveBackRow = XMLHandler.getTagValue(stepnode,
					"give_back_row"); // $NON-NLS-1$
			...
			throw new KettleXMLException(
					Messages
							.getString("GroupByMeta.Exception.UnableToLoadStepInfoFromXML"), e); //$NON-NLS-1$
		}
	}
	...
	public void getFields(RowMetaInterface r, String origin,
		...
		if (passAllRows) {
			...
			if (addingLineNrInGroup && !Const.isEmpty(lineNrInGroupField)) {
				ValueMetaInterface lineNr = new ValueMeta(lineNrInGroupField,
	...
	public String getXML() {
		...
		retval
				.append("      ").append(XMLHandler.addTagValue("all_rows", passAllRows)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("      ").append(XMLHandler.addTagValue("ignore_aggregate", aggregateIgnored)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("      ").append(XMLHandler.addTagValue("field_ignore", aggregateIgnoredField)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
		...
		retval
				.append("      ").append(XMLHandler.addTagValue("add_linenr", addingLineNrInGroup)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("      ").append(XMLHandler.addTagValue("linenr_fieldname", lineNrInGroupField)); //$NON-NLS-1$
		retval
				.append("      ").append(XMLHandler.addTagValue("give_back_row", alwaysGivingBackOneRow)); //$NON-NLS-1$ //$NON-NLS-2$

		retval.append("      <group>").append(Const.CR); //$NON-NLS-1$
		for (int i = 0; i < groupField.length; i++) {
			retval.append("        <field>").append(Const.CR); //$NON-NLS-1$
			retval
					.append("          ").append(XMLHandler.addTagValue("name", groupField[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval.append("        </field>").append(Const.CR); //$NON-NLS-1$
		}
		retval.append("      </group>").append(Const.CR); //$NON-NLS-1$
		...
		for (int i = 0; i < subjectField.length; i++) {
			retval.append("        <field>").append(Const.CR); //$NON-NLS-1$
			retval
					.append("          ").append(XMLHandler.addTagValue("aggregate", aggregateField[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval
					.append("          ").append(XMLHandler.addTagValue("subject", subjectField[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval
					.append("          ").append(XMLHandler.addTagValue("type", getTypeDesc(aggregateType[i]))); //$NON-NLS-1$ //$NON-NLS-2$
			retval.append("          ").append(
					XMLHandler.addTagValue("valuefield", valueField[i]));
			retval.append("        </field>").append(Const.CR); //$NON-NLS-1$
		}
		retval.append("      </fields>").append(Const.CR); //$NON-NLS-1$
	...
	public void readRep(Repository rep, long id_step,
		...
		try {
			...
			aggregateIgnored = rep.getStepAttributeBoolean(id_step,
					"ignore_aggregate"); //$NON-NLS-1$
			aggregateIgnoredField = rep.getStepAttributeString(id_step,
					"field_ignore"); //$NON-NLS-1$
			...
			addingLineNrInGroup = rep.getStepAttributeBoolean(id_step,
					"add_linenr"); // $NON-NLS-1$
			lineNrInGroupField = rep.getStepAttributeString(id_step,
					"linenr_fieldname"); // $NON-NLS-1$
			...
			for (int i = 0; i < groupsize; i++) {
				groupField[i] = rep.getStepAttributeString(id_step, i,
						"group_name"); //$NON-NLS-1$
			}

			boolean hasNumberOfValues = false;
			for (int i = 0; i < nrvalues; i++) {
				aggregateField[i] = rep.getStepAttributeString(id_step, i,
						"aggregate_name"); //$NON-NLS-1$
				subjectField[i] = rep.getStepAttributeString(id_step, i,
						"aggregate_subject"); //$NON-NLS-1$
				aggregateType[i] = getType(rep.getStepAttributeString(id_step,
						i, "aggregate_type")); //$NON-NLS-1$

				if (aggregateType[i] == TYPE_GROUP_COUNT_ALL) {
					hasNumberOfValues = true;
				}
				valueField[i] = rep.getStepAttributeString(id_step, i,
						"aggregate_value_field"); //$NON-NLS-1$
			}

			alwaysGivingBackOneRow = rep.getStepAttributeBoolean(id_step, 0,
					"give_back_row", hasNumberOfValues); // $NON-NLS-1$
		} catch (Exception e) {
			throw new KettleException(
					Messages
							.getString("GroupByMeta.Exception.UnexpectedErrorInReadingStepInfoFromRepository"), e); //$NON-NLS-1$
		}
	}
	...
	public void saveRep(Repository rep, long id_transformation, long id_step)
		...
		try {
			rep.saveStepAttribute(id_transformation, id_step,
					"all_rows", passAllRows); //$NON-NLS-1$
			rep.saveStepAttribute(id_transformation, id_step,
					"ignore_aggregate", aggregateIgnored); //$NON-NLS-1$
			rep.saveStepAttribute(id_transformation, id_step,
					"field_ignore", aggregateIgnoredField); //$NON-NLS-1$
			rep.saveStepAttribute(id_transformation, id_step,
					"directory", directory); //$NON-NLS-1$
			rep.saveStepAttribute(id_transformation, id_step, "prefix", prefix); //$NON-NLS-1$
			rep.saveStepAttribute(id_transformation, id_step, "add_linenr",
					addingLineNrInGroup); // $NON-NLS-1$
			rep.saveStepAttribute(id_transformation, id_step,
					"linenr_fieldname", lineNrInGroupField); // $NON-NLS-1$
			rep.saveStepAttribute(id_transformation, id_step, "give_back_row",
					alwaysGivingBackOneRow); // $NON-NLS-1$

			for (int i = 0; i < groupField.length; i++) {
				rep.saveStepAttribute(id_transformation, id_step, i,
						"group_name", groupField[i]); //$NON-NLS-1$
			}

			for (int i = 0; i < subjectField.length; i++) {
				rep.saveStepAttribute(id_transformation, id_step, i,
						"aggregate_name", aggregateField[i]); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i,
						"aggregate_subject", subjectField[i]); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i,
						"aggregate_type", getTypeDesc(aggregateType[i])); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i,
						"aggregate_value_field", valueField[i]);
			}
		} catch (Exception e) {
			throw new KettleException(
					Messages
							.getString("GroupByMeta.Exception.UnableToSaveStepInfoToRepository") + id_step, e); //$NON-NLS-1$
		}
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (input.length > 0) {
			cr = new CheckResult(
					...
					Messages
							.getString("GroupByMeta.CheckResult.ReceivingInfoOK"), stepMeta); //$NON-NLS-1$
			remarks.add(cr);
		} else {
			cr = new CheckResult(
					CheckResultInterface.TYPE_RESULT_ERROR,
					Messages.getString("GroupByMeta.CheckResult.NoInputError"), stepMeta); //$NON-NLS-1$
			remarks.add(cr);
		}
	}
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		passAllRows = BaseStepMeta.parameterToBoolean(p
				.get(id + ".passAllRows"));
		directory = BaseStepMeta.parameterToString(p.get(id + ".directory"));
		prefix = BaseStepMeta.parameterToString(p.get(id + ".prefix"));
		addingLineNrInGroup = BaseStepMeta.parameterToBoolean(p.get(id
				+ ".addingLineNrInGroup"));
		lineNrInGroupField = BaseStepMeta.parameterToString(p.get(id
				+ ".lineNrInGroupField"));
		alwaysGivingBackOneRow = BaseStepMeta.parameterToBoolean(p.get(id
				+ ".alwaysGivingBackOneRow"));

		String[] fieldName = p.get(id + "_fields.fieldName")==null?new String[0]:p.get(id + "_fields.fieldName");
		String[] aggregateField = p.get(id + "_aggregation.aggregateField")==null?new String[0]:p.get(id + "_aggregation.aggregateField");
		String[] subjectField = p.get(id + "_aggregation.subjectField")==null?new String[0]:p.get(id + "_aggregation.subjectField");
		String[] aggregateType = p.get(id + "_aggregation.aggregateType")==null?new String[0]:p.get(id + "_aggregation.aggregateType");
		String[] valueField = p.get(id + "_aggregation.valueField")==null?new String[0]:p.get(id + "_aggregation.valueField");
		this.groupField = fieldName;
		this.aggregateField = aggregateField;
		this.subjectField = subjectField;
		this.aggregateType = stringArrayToIntArray(aggregateType, 0);
		this.valueField = valueField;
