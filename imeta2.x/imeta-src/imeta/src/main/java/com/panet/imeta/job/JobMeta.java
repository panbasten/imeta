 ...
 * 
 */
public class JobMeta extends ChangedFlag implements Cloneable,
		Comparable<JobMeta>, XMLInterface, UndoInterface,
	...
	public static final String[] jobstatusList = new String[] {
			Messages.getString("JobMeta.JobStatus.Draft"),
	...
	protected boolean changedEntries, changedHops, changedNotes,
			changedDatabases;
	...
	public String getJobMetaTreeSegment(String fileId) {
		...
		ro.append("<li class='closed'><span class='db-folder'>"
				+ Messages.getString("JobMeta.Element.Database") + "</span>");
	...
	public static final JobEntryCopy createDummyEntry() {
		JobEntrySpecial jobEntrySpecial = new JobEntrySpecial(
				STRING_SPECIAL_DUMMY, false, true);
		JobEntryCopy jobEntry = new JobEntryCopy();
		jobEntry.setID(-1L);
		jobEntry.setEntry(jobEntrySpecial);
		jobEntry.setLocation(50, 50);
		jobEntry.setDrawn(false);
		jobEntry.setDescription(Messages
				.getString("JobMeta.DummyJobEntry.Description")); //$NON-NLS-1$
		return jobEntry;
	}
	...
	public Object realClone(boolean doClear) {
		try {
			...
			for (JobHopMeta entry : jobhops) {
				...
				newJobEntryCopy.from_entry = jobMeta
						.getJobEntry(entry.from_entry.getName());
				newJobEntryCopy.to_entry = jobMeta.getJobEntry(entry.to_entry
						.getName());
	...
	protected void saveRepJob(Repository rep) throws KettleException {
		try {
			...
			rep.insertJob(getID(), directory.getID(), getName(),
					logConnection == null ? -1 : logConnection.getID(),
					logTable, modifiedUser, modifiedDate, useBatchId,
					batchIdPassed, logfieldUsed, sharedObjectsFile,
					description, extendedDescription, jobVersion, jobStatus,
					created_user, created_date, guiLocationX, guiLocationY,
					guiScale);
		} catch (KettleDatabaseException dbe) {
			throw new KettleException(
					Messages
							.getString("JobMeta.Exception.UnableToSaveJobToRepository"), dbe); //$NON-NLS-1$
		}
	}
	...
	public boolean isDatabaseConnectionUsed(DatabaseMeta databaseMeta) {
		for (int i = 0; i < nrJobEntries(); i++) {
			...
			DatabaseMeta dbs[] = jobEntry.getEntry()
	...
	public String getXML() {
		...
		retval.append("    ").append(
				XMLHandler.addTagValue("job_version", jobVersion));
		if (jobStatus >= 0) {
			retval.append("    ").append(
					XMLHandler.addTagValue("job_status", jobStatus));
		}

		retval
				.append("  ").append(XMLHandler.addTagValue("directory", directory.getPath())); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("  ").append(XMLHandler.addTagValue("created_user", created_user)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("  ").append(XMLHandler.addTagValue("created_date", XMLHandler.date2string(created_date))); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		retval
				.append("  ").append(XMLHandler.addTagValue("modified_user", modifiedUser)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("  ").append(XMLHandler.addTagValue("modified_date", XMLHandler.date2string(created_date))); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		...
		retval
				.append("  ").append(XMLHandler.addTagValue("logconnection", ci == null ? "" : ci.getName())); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		retval
				.append("  ").append(XMLHandler.addTagValue("logtable", logTable)); //$NON-NLS-1$ //$NON-NLS-2$

		retval
				.append("   ").append(XMLHandler.addTagValue("use_batchid", useBatchId)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("   ").append(XMLHandler.addTagValue("pass_batchid", batchIdPassed)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
	...
	public JobMeta(LogWriterInterface log, String fname, Repository rep)
	 ...
	 * @throws KettleXMLException
	 */
	public JobMeta(LogWriterInterface log, VariableSpace parentSpace, String fname,
			Repository rep, OverwritePrompter prompter)
			throws KettleXMLException {
		this.log = log;
		this.rep = rep;
		...
		try {
			...
			Document doc = XMLHandler.loadXMLFile(KettleVFS
					.getFileObject(fname));
			if (doc != null) {
				// Clear the job
				clear();
				...
				throw new KettleXMLException(
						Messages
								.getString("JobMeta.Exception.ErrorReadingFromXMLFile") + fname); //$NON-NLS-1$
			}
		} catch (Exception e) {
			throw new KettleXMLException(
					Messages
							.getString("JobMeta.Exception.UnableToLoadJobFromXMLFile") + fname + "]", e); //$NON-NLS-1$ //$NON-NLS-2$
		}
	}

	public JobMeta(LogWriterInterface log, Node jobnode, Repository rep,
			OverwritePrompter prompter) throws KettleXMLException {
		this.log = log;
		this.rep = rep;
		loadXML(jobnode, rep, prompter);
	...
	public void loadXML(Node jobnode, Repository rep, OverwritePrompter prompter)
			throws KettleXMLException {
		Props props = null;
		if (Props.isInitialized())
			props = Props.getInstance();
		...
		try {
			...
			//
			if (rep != null) {
				String directoryPath = XMLHandler.getTagValue(jobnode,
						"directory");
			...
			extendedDescription = XMLHandler.getTagValue(jobnode,
					"extended_description");

			// gui location x
			//
			guiLocationX = Const.toInt(XMLHandler.getTagValue(jobnode,
					"gui_location_x"), 0);

			// gui location y
			//
			guiLocationY = Const.toInt(XMLHandler.getTagValue(jobnode,
					"gui_location_y"), 0);

			// gui scale
			//
			guiScale = Const.toDouble(XMLHandler.getTagValue(jobnode,
					"gui_scale"), 1);
			...
			// job status
			jobStatus = Const.toInt(XMLHandler.getTagValue(jobnode,
					"job_status"), -1);
			...
			try {
				sharedObjectsFile = XMLHandler.getTagValue(jobnode,
						"shared_objects_file"); //$NON-NLS-1$ //$NON-NLS-2$
				sharedObjects = readSharedObjects(rep);
			} catch (Exception e) {
				LogWriter.getInstance().logError(
						toString(),
						Messages.getString(
								"JobMeta.ErrorReadingSharedObjects.Message", e
										.toString())); // $NON-NLS-1$
				// //$NON-NLS-1$
			...
			for (int i = 0; i < nr; i++) {
				Node dbnode = XMLHandler.getSubNodeByNr(jobnode,
						"connection", i); //$NON-NLS-1$
						...
						if (askOverwrite && prompter != null) {
							overwrite = prompter
									.overwritePrompt(
											Messages
													.getString(
															"JobMeta.Dialog.ConnectionExistsOverWrite.Message",
															dbcon.getName()),
											Messages
			...
			// 
			Node slaveServersNode = XMLHandler.getSubNode(jobnode,
			...
			for (int i = 0; i < tr; i++) {
				Node entrynode = XMLHandler.getSubNodeByNr(entriesnode,
						"entry", i); //$NON-NLS-1$
			...
			throw new KettleXMLException(
					Messages
							.getString("JobMeta.Exception.UnableToLoadJobFromXMLNode"), e); //$NON-NLS-1$
		} finally {
			setInternalKettleVariables();
		}
	}
	...
	public void readDatabases(Repository rep, boolean overWriteShared)
		...
		try {
			...
			throw new KettleException(
					Messages
							.getString("JobMeta.Log.UnableToReadDatabaseIDSFromRepository"), dbe); //$NON-NLS-1$
		} catch (KettleException ke) {
			throw new KettleException(
					Messages
							.getString("JobMeta.Log.UnableToReadDatabasesFromRepository"), ke); //$NON-NLS-1$
		}
	}
	...
	public void readSlaves(Repository rep, boolean overWriteShared)
		...
		try {
			...
			throw new KettleException(
					Messages
							.getString("JobMeta.Log.UnableToReadSlaveServersFromRepository"), dbe); //$NON-NLS-1$
		}
	}
	...
	public SharedObjects readSharedObjects(Repository rep)
		...
		Map<?, SharedObjectInterface> objectsMap = sharedObjects
				.getObjectsMap();
	...
	public boolean saveSharedObjects() {
		try {
			...
			for (int i = 0; i < shared.size(); i++) {
				SharedObjectInterface sharedObject = (SharedObjectInterface) shared
						.get(i);
	...
	public void setInfo(Map<String, String[]> p, String id)
		...
		directory = directory.findDirectory(BaseStepMeta.parameterToString(p
				.get(id + ".directory")));
		...
		batchIdPassed = BaseStepMeta.parameterToBoolean(p.get(id
				+ ".batchIdPassed"));
	...
	public void saveRep(Repository rep, ProgressMonitorListener monitor)
			throws KettleException {
		try {
			int nrWorks = 2 + nrDatabases() + nrNotes() + nrJobEntries()
					+ nrJobHops();
			if (monitor != null)
				monitor
						.beginTask(
								Messages
										.getString("JobMeta.Monitor.SavingTransformation") + directory + Const.FILE_SEPARATOR + getName(), nrWorks); //$NON-NLS-1$
			...
			if (monitor != null)
				monitor
						.subTask(Messages
								.getString("JobMeta.Monitor.HandlingPreviousVersionOfJob")); //$NON-NLS-1$
			...
			if (log.isDebug())
				log.logDebug(toString(), Messages
						.getString("JobMeta.Log.SavingDatabaseConnections")); //$NON-NLS-1$
			for (int i = 0; i < nrDatabases(); i++) {
				if (monitor != null)
					monitor
							.subTask(Messages
									.getString("JobMeta.Monitor.SavingDatabaseTask.Title") + (i + 1) + "/" + nrDatabases()); //$NON-NLS-1$ //$NON-NLS-2$
			...
			if (monitor != null)
				monitor.subTask(Messages
						.getString("JobMeta.Monitor.SavingJobDetails")); //$NON-NLS-1$
			if (log.isDetailed())
			...
			for (int i = 0; i < nrNotes(); i++) {
				if (monitor != null)
					monitor
							.subTask(Messages
									.getString("JobMeta.Monitor.SavingNoteNr") + (i + 1) + "/" + nrNotes()); //$NON-NLS-1$ //$NON-NLS-2$
				NotePadMeta ni = getNote(i);
			...
			if (log.isDetailed())
				log
						.logDetailed(
								toString(),
								"Saving " + nrJobEntries() + " Job enty copies to repository..."); //$NON-NLS-1$ //$NON-NLS-2$
			rep.updateJobEntryTypes();
			for (int i = 0; i < nrJobEntries(); i++) {
				if (monitor != null)
					monitor
							.subTask(Messages
									.getString("JobMeta.Monitor.SavingJobEntryNr") + (i + 1) + "/" + nrJobEntries()); //$NON-NLS-1$ //$NON-NLS-2$
				JobEntryCopy cge = getJobEntry(i);
			...
			for (int i = 0; i < nrJobHops(); i++) {
				if (monitor != null)
					monitor
							.subTask("Saving job hop #" + (i + 1) + "/" + nrJobHops()); //$NON-NLS-1$ //$NON-NLS-2$
			...
			throw new KettleException(
					Messages
							.getString("JobMeta.Exception.UnableToSaveJobInRepositoryRollbackPerformed"), dbe); //$NON-NLS-1$
		} finally {
	 ...
	 * @throws KettleException
	 */
	public JobMeta(LogWriterInterface log, Repository rep, String jobname,
		...
		synchronized (rep) {
			try {
				...
				if (getID() > 0) {
					...
					int nrWork = 2 + noteids.length + jecids.length
							+ hopid.length;
					if (monitor != null)
						monitor
								.beginTask(
										Messages
												.getString("JobMeta.Monitor.LoadingJob") + repdir + Const.FILE_SEPARATOR + jobname, nrWork); //$NON-NLS-1$
					...
					if (monitor != null)
						monitor
								.subTask(Messages
										.getString("JobMeta.Monitor.ReadingJobInformation")); //$NON-NLS-1$
					...
					extendedDescription = jobRow.getString(
							"EXTENDED_DESCRIPTION", null); //$NON-NLS-1$

					guiLocationX = (int) jobRow
							.getInteger("GUI_LOCATION_X", 0L);
					guiLocationY = (int) jobRow
							.getInteger("GUI_LOCATION_Y", 0L);
					guiScale = jobRow.getNumber("GUI_SCALE", 1);

					jobVersion = jobRow.getString("JOB_VERSION", null); //$NON-NLS-1$
					jobStatus = Const.toInt(jobRow
							.getString("JOB_STATUS", null), -1); //$NON-NLS-1$
					...
					if (id_logdb > 0) {
						...
						logConnection = RepositoryUtil.loadDatabaseMeta(rep,
								id_logdb);
					...
					if (monitor != null)
						monitor
								.subTask(Messages
										.getString("JobMeta.Monitor.ReadingAvailableDatabasesFromRepository")); //$NON-NLS-1$
					// Read objects from the shared XML file & the repository
					try {
						sharedObjectsFile = jobRow.getString("SHARED_FILE",
								null);
						sharedObjects = readSharedObjects(rep);
					} catch (Exception e) {
						LogWriter
								...
								.logError(
										...
										Messages
												.getString(
														"JobMeta.ErrorReadingSharedObjects.Message",
					...
					if (log.isDetailed())
						log.logDetailed(toString(),
								"Loading " + noteids.length + " notes"); //$NON-NLS-1$ //$NON-NLS-2$
					for (int i = 0; i < noteids.length; i++) {
						if (monitor != null)
							monitor
									.subTask(Messages
											.getString("JobMeta.Monitor.ReadingNoteNr") + (i + 1) + "/" + noteids.length); //$NON-NLS-1$ //$NON-NLS-2$
					...
					if (log.isDetailed())
						log.logDetailed(toString(),
								"Loading " + jecids.length + " job entries"); //$NON-NLS-1$ //$NON-NLS-2$
					for (int i = 0; i < jecids.length; i++) {
						if (monitor != null)
							monitor
									.subTask(Messages
											.getString("JobMeta.Monitor.ReadingJobEntryNr") + (i + 1) + "/" + (jecids.length)); //$NON-NLS-1$ //$NON-NLS-2$

						JobEntryCopy jec = new JobEntryCopy(log, rep, getID(),
								jecids[i], jobentries, databases, slaveServers);
					...
					if (log.isDetailed())
						log.logDetailed(toString(),
								"Loading " + hopid.length + " job hops"); //$NON-NLS-1$ //$NON-NLS-2$
					for (int i = 0; i < hopid.length; i++) {
						if (monitor != null)
							monitor
									.subTask(Messages
											.getString("JobMeta.Monitor.ReadingJobHopNr") + (i + 1) + "/" + (jecids.length)); //$NON-NLS-1$ //$NON-NLS-2$
						JobHopMeta hi = new JobHopMeta(rep, hopid[i], this,
								jobcopies);
						jobhops.add(hi);
						if (monitor != null)
							monitor.worked(1);
					...
					if (monitor != null)
						monitor
								.subTask(Messages
										.getString("JobMeta.Monitor.FinishedLoadOfJob")); //$NON-NLS-1$
					if (monitor != null)
						monitor.done();
				} else {
					throw new KettleException(
							Messages
									.getString("JobMeta.Exception.CanNotFindJob") + jobname); //$NON-NLS-1$
				}
			} catch (KettleException dbe) {
				throw new KettleException(Messages.getString(
						"JobMeta.Exception.AnErrorOccuredReadingJob", jobname),
						dbe);
			} finally {
				setInternalKettleVariables();
			}
	...
	public JobHopMeta findJobHopFrom(JobEntryCopy jge) {
		if (jge != null) {
			for (JobHopMeta hi : jobhops) {
				if (hi != null && (hi.from_entry != null)
						&& hi.from_entry.equals(jge)) // return
				// the
				// first
				{
					return hi;
	...
	public int findNrNextJobEntries(JobEntryCopy from) {
		...
		for (JobHopMeta hi : jobhops) // Look at all the hops
		{
			if (hi.isEnabled() && (hi.from_entry != null)
					&& hi.from_entry.equals(from))
				count++;
		}
		return count;
	}
	...
	public List<SQLStatement> getSQLStatements(Repository repository,
		...
		if (monitor != null)
			monitor
					.beginTask(
							Messages
									.getString("JobMeta.Monitor.GettingSQLNeededForThisJob"), nrJobEntries() + 1); //$NON-NLS-1$
		...
		for (int i = 0; i < nrJobEntries(); i++) {
			...
			if (monitor != null)
				monitor
						.subTask(Messages
								.getString("JobMeta.Monitor.GettingSQLForJobEntryCopy") + copy + "]"); //$NON-NLS-1$ //$NON-NLS-2$
			List<SQLStatement> list = copy.getEntry().getSQLStatements(
					repository, this);
			stats.addAll(list);
			if (monitor != null)
				monitor.worked(1);
		...
		if (monitor != null)
			monitor
					.subTask(Messages
							.getString("JobMeta.Monitor.GettingSQLStatementsForJobLogTables")); //$NON-NLS-1$
		if (logConnection != null && logTable != null && logTable.length() > 0) {
			Database db = new Database(logConnection);
			try {
				db.connect();
				RowMetaInterface fields = Database.getJobLogrecordFields(false,
						useBatchId, logfieldUsed);
				String sql = db.getDDL(logTable, fields);
				if (sql != null && sql.length() > 0) {
					SQLStatement stat = new SQLStatement(
							Messages.getString("JobMeta.SQLFeedback.ThisJob"), logConnection, sql); //$NON-NLS-1$
					stats.add(stat);
				}
			} catch (KettleDatabaseException dbe) {
				SQLStatement stat = new SQLStatement(
						Messages.getString("JobMeta.SQLFeedback.ThisJob"), logConnection, null); //$NON-NLS-1$
				stat
						.setError(Messages
								.getString("JobMeta.SQLFeedback.ErrorObtainingJobLogTableInfo") + dbe.getMessage()); //$NON-NLS-1$
				stats.add(stat);
			} finally {
				db.disconnect();
			}
			...
			for (int i = 0; i < nrDatabases(); i++) {
				...
				if (meta.getDatabaseName() != null)
					stringList.add(new StringSearchResult(meta
							.getDatabaseName(), meta, this, Messages
	 ...
	 */
	public static final JobEntryInterface findJobEntry(
			List<JobEntryInterface> jobentries, long id_jobentry) {
		if (jobentries == null)
			return null;
	 ...
	 */
	public static final JobEntryCopy findJobEntryCopy(
			List<JobEntryCopy> jobcopies, long id_jobentry_copy) {
		if (jobcopies == null)
			return null;
	...
	public void checkJobEntries(List<CheckResultInterface> remarks,
		...
		if (monitor != null)
			monitor
					.beginTask(
							Messages
									.getString("JobMeta.Monitor.VerifyingThisJobEntryTask.Title"), jobcopies.size() + 2); //$NON-NLS-1$
				...
				if (entry != null) {
					if (monitor != null)
						monitor
								.subTask(Messages
										.getString(
												"JobMeta.Monitor.VerifyingJobEntry.Title", entry.getName())); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public String exportResources(VariableSpace space,
		...
		try {
			...
			resourceName = namingInterface.nameResource(fileObject.getName()
					.getBaseName(), fileObject.getParent().getName().getPath(),
					"kjb"); //$NON-NLS-1$
			...
			if (definition == null) {
				...
				for (JobEntryCopy jobEntry : jobMeta.jobcopies) {
					jobEntry.getEntry().exportResources(jobMeta, definitions,
							namingInterface);
				}
				...
				definition = new ResourceDefinition(resourceName,
						jobMetaContent);
				definitions.put(fileObject.getName().getPath(), definition);
			}
		} catch (FileSystemException e) {
			throw new KettleException(Messages
					.getString("JobMeta.Exception.AnErrorOccuredReadingJob",
							getFilename()), e);
		} catch (IOException e) {
			throw new KettleException(Messages
					.getString("JobMeta.Exception.AnErrorOccuredReadingJob",
	...
	public void renameJobEntryIfNameCollides(JobEntryCopy je) {
		...
		do {
			found = false;
			for (JobEntryCopy copy : jobcopies) {
				if (copy != je && copy.getName().equalsIgnoreCase(newname)
						&& copy.getNr() == 0)
					found = true;
			}
			if (found) {
