...
import com.panet.imeta.job.JobPluginMeta;
import com.panet.imeta.job.entry.Messages;
 ...
 */public class AnnotatedJobConfigManager<T extends JobPluginMeta> extends BasicConfigManager<T> 
{
	...
	public Collection<T> load() throws KettleConfigException
	{
		...
		for (Class<?> clazz : resolver.getClasses())
		{
			...
			if (Const.isEmpty(packageName)) packageName = com.panet.imeta.job.entry.Messages.class.getPackage().getName();
