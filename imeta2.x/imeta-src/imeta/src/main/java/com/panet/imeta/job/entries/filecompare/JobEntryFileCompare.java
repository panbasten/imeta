...
/**
 * This defines a 'file compare' job entry. It will compare 2 files in a binary
 * way, and will either follow the true flow upon the files being the same or
 * the false flow otherwise.
 * 
 * @author Sven Boden
 * @since 01-02-2007
 * 
 */
public class JobEntryFileCompare extends JobEntryBase implements Cloneable,
		JobEntryInterface {
	...
	public String getXML() {
		...
		retval
				.append("      ").append(XMLHandler.addTagValue("filename1", filename1)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("      ").append(XMLHandler.addTagValue("filename2", filename2)); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public void loadXML(Node entrynode, List<DatabaseMeta> databases,
		...
		try {
			...
			throw new KettleXMLException(
					Messages
							.getString("JobEntryFileCompare.ERROR_0001_Unable_To_Load_From_Xml_Node"), xe); //$NON-NLS-1$
		}
	}
	...
	public void loadRep(Repository rep, long id_jobentry,
		...
		try {
			...
			filename1 = rep
					.getJobEntryAttributeString(id_jobentry, "filename1"); //$NON-NLS-1$
			filename2 = rep
					.getJobEntryAttributeString(id_jobentry, "filename2"); //$NON-NLS-1$
			...
			throw new KettleException(
					Messages
							.getString(
									"JobEntryFileCompare.ERROR_0002_Unable_To_Load_Job_From_Repository", Long.toString(id_jobentry)), dbe); //$NON-NLS-1$
		}
	}
	...
	public void saveRep(Repository rep, long id_job) throws KettleException {
		try {
			...
			throw new KettleException(
					Messages
							.getString(
									"JobEntryFileCompare.ERROR_0003_Unable_To_Save_Job", Long.toString(id_job)), dbe); //$NON-NLS-1$
		}
	}
		...
		try {
			if (filename1 != null && filename2 != null) {
				...
				if (file1.exists() && file2.exists()) {
					...
					if (addFilenameToResult && file1.getType() == FileType.FILE
						...
						ResultFile resultFile = new ResultFile(
								ResultFile.FILE_TYPE_GENERAL, file1, parentJob
										.getJobname(), toString());
						...
						resultFile = new ResultFile(
								ResultFile.FILE_TYPE_GENERAL, file2, parentJob
										.getJobname(), toString());
						resultFile.setComment(Messages
								.getString("JobWaitForFile.FilenameAdded"));
						result.getResultFiles().put(
					...
					if (!file1.exists())
						log
								.logError(
										...
										Messages
												.getString(
														"JobEntryFileCompare.ERROR_0004_File1_Does_Not_Exist", realFilename1)); //$NON-NLS-1$
					if (!file2.exists())
						log
								.logError(
										toString(),
										Messages
												.getString(
														"JobEntryFileCompare.ERROR_0005_File2_Does_Not_Exist", realFilename2)); //$NON-NLS-1$
					result.setResult(false);
				...
				log
						.logError(
								toString(),
								Messages
										.getString("JobEntryFileCompare.ERROR_0006_Need_Two_Filenames")); //$NON-NLS-1$
			}
		} catch (Exception e) {
			result.setResult(false);
			result.setNrErrors(1);
			log
					.logError(
							...
							Messages
									.getString(
											"JobEntryFileCompare.ERROR_0007_Comparing_Files", realFilename2, realFilename2, e.getMessage())); //$NON-NLS-1$
		} finally {
			try {
				if (file1 != null)
					file1.close();
	...
	public List<ResourceReference> getResourceDependencies(JobMeta jobMeta) {
		...
		if ((!Const.isEmpty(filename1)) && (!Const.isEmpty(filename2))) {
			...
			reference.getEntries().add(
					new ResourceEntry(realFilename1, ResourceType.FILE));
			reference.getEntries().add(
					new ResourceEntry(realFilename2, ResourceType.FILE));
