...
public class TransExecutionConfiguration implements Cloneable {
	...
	public Object clone() {
		try {
			TransExecutionConfiguration configuration = (TransExecutionConfiguration) super
					.clone();
	...
	public void getAllVariables(TransMeta transMeta) {
		...
		if (vars != null && vars.length > 0) {
			...
			for (int i = 0; i < vars.length; i++) {
				...
				newVariables.put(varname, Const.NVL(variables.get(varname), sp
						.getProperty(varname, "")));
			}
			// variables.clear();
	...
	public void getUsedArguments(TransMeta transMeta,
		...
		//
		Map<String, String> map = transMeta
				.getUsedArguments(commandLineArguments);
	...
	public String getXML() throws IOException {
		...
		xml.append("    ").append(
				XMLHandler.addTagValue("exec_cluster", executingClustered));
		xml.append("    ").append(
				XMLHandler.addTagValue("cluster_post", clusterPosting));
		xml.append("    ").append(
				XMLHandler.addTagValue("cluster_prepare", clusterPreparing));
		xml.append("    ").append(
				XMLHandler.addTagValue("cluster_start", clusterStarting));
		xml.append("    ").append(
				XMLHandler.addTagValue("cluster_show_trans",
						clusterShowingTransformation));
	...
	public TransExecutionConfiguration(Node trecNode) throws KettleException {
		...
		executingClustered = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
				trecNode, "exec_cluster"));
		clusterPosting = "Y".equalsIgnoreCase(XMLHandler.getTagValue(trecNode,
				"cluster_post"));
		clusterPreparing = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
				trecNode, "cluster_prepare"));
		clusterStarting = "Y".equalsIgnoreCase(XMLHandler.getTagValue(trecNode,
				"cluster_start"));
		clusterShowingTransformation = "Y".equalsIgnoreCase(XMLHandler
				.getTagValue(trecNode, "cluster_show_trans"));
		...
		for (int i = 0; i < nrParams; i++) {
			Node parmNode = XMLHandler
					.getSubNodeByNr(parmsNode, "parameter", i);
	...
	public String[] getArgumentStrings() {
		...
		for (int i = 0; i < argNames.length; i++) {
			if (argNames[i].equalsIgnoreCase(Props.STRING_ARGUMENT_NAME_PREFIX
					+ (i + 1))) {
