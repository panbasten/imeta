 ...
 * @author Matt Casters
 */
public class TransMeta extends ChangedFlag implements XMLInterface,
		Comparator<TransMeta>, Comparable<TransMeta>, Cloneable, UndoInterface,
	...
	public static final String[] transstatusList = new String[] {
			Messages.getString("TransMeta.TransStatus.Draft"),
	...
	private boolean changed_steps, changed_databases, changed_hops,
			changed_notes;
	...
	public TransMeta(Repository rep) {
		...
		try {
			...
			LogWriter.getInstance().logError(
					toString(),
					Messages.getString(
							"TransMeta.ErrorReadingSharedObjects.Message", e
									.toString()));
	...
	public String getTransMetaTreeSegment(String fileId) {
		...
		ro.append("<li class='closed'><span class='db-folder'>"
				+ Messages.getString("TransMeta.Element.Database") + "</span>");
		if (dbs != null && dbs.size() > 0) {
			...
			while (dbIter.hasNext()) {
				db = dbIter.next();
				ro.append("<li><span class='element-file " + fileId
						+ "-setting' elementId='" + db.getID() + "' dbName='"
						+ db.getName() + "' elementType='"
		...
		ro.append("</li>");
		ro.append("<li class='closed'><span class='step-folder'>"
				+ Messages.getString("TransMeta.Element.Step") + "</span>");
		...
		ro.append("</li>");
		ro.append("<li class='closed'><span class='hop-folder'>"
				+ Messages.getString("TransMeta.Element.Hop") + "</span>");
	...
	public Object realClone(boolean doClear) {
		...
		try {
			...
			for (TransHopMeta hop : hops) {
				...
				newTransHopMeta.setFromStep(transMeta.getStep(hop.getFromStep()
						.getName()));
				newTransHopMeta.setToStep(transMeta.getStep(hop.getToStep()
						.getName()));
			...
			for (ClusterSchema schema : clusterSchemas)
				transMeta.getClusterSchemas().add(
						(ClusterSchema) schema.clone());
			for (PartitionSchema schema : partitionSchemas)
				transMeta.getPartitionSchemas().add(
						(PartitionSchema) schema.clone());
	...
	public void setInfo(Map<String, String[]> p, String id)
		...
		extended_description = BaseStepMeta.parameterToString(p.get(id
				+ ".extendedDescription"));
		trans_status = BaseStepMeta.parameterToInt(p.get(id + ".transstatus"));
		trans_version = BaseStepMeta.parameterToString(p.get(id
				+ ".transversion"));
		if (directoryTree == null) {
			directoryTree = new RepositoryDirectory();
		}
		directory = directoryTree.findDirectory(BaseStepMeta
				.parameterToString(p.get(id + ".directory")));
		...
		// 2 日志
		readStep = this.getStep(BaseStepMeta.parameterToString(p.get(id
				+ ".readLogStep")));
		inputStep = this.getStep(BaseStepMeta.parameterToString(p.get(id
				+ ".inputLogStep")));
		writeStep = this.getStep(BaseStepMeta.parameterToString(p.get(id
				+ ".writeLogStep")));
		outputStep = this.getStep(BaseStepMeta.parameterToString(p.get(id
				+ ".outputLogStep")));
		updateStep = this.getStep(BaseStepMeta.parameterToString(p.get(id
				+ ".updateLogStep")));
		rejectedStep = this.getStep(BaseStepMeta.parameterToString(p.get(id
				+ ".rejectedLogStep")));

		int logConnectionId = BaseStepMeta.parameterToInt(p.get(id
				+ ".logConnection"));
		...
		stepPerformanceLogTable = BaseStepMeta.parameterToString(p.get(id
				+ ".stepPerformanceLogTable"));
		useBatchId = BaseStepMeta
				.parameterToBoolean(p.get(id + ".batchIdUsed"));
		logfieldUsed = BaseStepMeta.parameterToBoolean(p.get(id
				+ ".logfieldUsed"));

		// 3日期
		int maxDateConnectionId = BaseStepMeta.parameterToInt(p.get(id
				+ ".maxDateConnection"));
		if (maxDateConnectionId > 0)
			maxDateConnection = this.getDatabase(maxDateConnectionId);
		else
			maxDateConnection = null;
		maxDateTable = BaseStepMeta.parameterToString(p.get(id
				+ ".maxDateTable"));
		maxDateField = BaseStepMeta.parameterToString(p.get(id
				+ ".maxDateField"));
		maxDateOffset = BaseStepMeta.parameterToDouble(p.get(id
				+ ".maxDateOffset"));
		maxDateDifference = BaseStepMeta.parameterToDouble(p.get(id
				+ ".maxDateDifference"));
		...
		if (dbConnIds != null)
			for (int i = 0; i < dbConnIds.length; i++) {
				...
				TransDependency td = new TransDependency(dm, dbtables[i],
						dbfields[i]);
				addDependency(td);
			}
		...
		feedbackShown = BaseStepMeta.parameterToBoolean(p.get(id
				+ ".feedbackShown"));
		feedbackSize = BaseStepMeta.parameterToInt(p.get(id + ".feedbackSize"));
		usingUniqueConnections = BaseStepMeta.parameterToBoolean(p.get(id
				+ ".usingUniqueConnections"));
		sharedObjectsFile = BaseStepMeta.parameterToString(p.get(id
				+ ".sharedObjectsFile"));
		usingThreadPriorityManagment = BaseStepMeta.parameterToBoolean(p.get(id
				+ ".usingThreadPriorityManagment"));
		// 7监控
		capturingStepPerformanceSnapShots = BaseStepMeta.parameterToBoolean(p
				.get(id + ".capturingStepPerformanceSnapShots"));
		stepPerformanceCapturingDelay = BaseStepMeta.parameterToLong(p.get(id
			...
			if (hi == null || hi.getToStep() == null) {
				log
						.logError(
								toString(),
								Messages
										.getString("TransMeta.Log.DestinationOfHopCannotBeNull")); //$NON-NLS-1$
			}
			if (hi != null && hi.getToStep() != null && hi.isEnabled()
					&& hi.getToStep().equals(stepMeta)) {
				// Check if this previous step isn't informative
				// (StreamValueLookup)
				// We don't want fields from this stream to show up!
	...
	public boolean partOfTransHop(StepMeta stepMeta) {
		...
		for (i = 0; i < nrTransHops(); i++) {
			...
			if (hi.getFromStep() == null || hi.getToStep() == null)
				return false;
			if (hi.getFromStep().equals(stepMeta)
					|| hi.getToStep().equals(stepMeta))
				return true;
		}
		return false;
	 ...
	 */
	public RowMetaInterface getStepFields(StepMeta stepMeta,
			StepMeta targetStep, ProgressMonitorListener monitor)
			throws KettleStepException {
		RowMetaInterface row = new RowMeta();
		...
		String fromToCacheEntry = stepMeta.getName()
				+ (targetStep != null ? ("-" + targetStep.getName()) : "");
		...
		for (int i = 0; i < findNrPrevSteps(stepMeta); i++) {
			...
			if (monitor != null) {
				monitor
						.subTask(Messages
								.getString(
										"TransMeta.Monitor.CheckingStepTask.Title", prevStepMeta.getName())); //$NON-NLS-1$ //$NON-NLS-2$
			}
			...
			if (log.isDebug())
				log
						.logDebug(
								toString(),
								Messages
										.getString("TransMeta.Log.FoundFieldsToAdd") + add.toString()); //$NON-NLS-1$
			if (i == 0) {
				row.addRowMeta(add);
			} else {
			...
			if (log.isDebug())
				log
						.logDebug(
								toString(),
								Messages
										.getString("TransMeta.Log.FoundFieldsToAdd2") + add.toString()); //$NON-NLS-1$
	...
	public RowMetaInterface getThisStepFields(String stepname,
			RowMetaInterface row) throws KettleStepException {
	...
	public RowMetaInterface getThisStepFields(StepMeta stepMeta,
			StepMeta nextStep, RowMetaInterface row) throws KettleStepException {
			...
			try {
				...
				log
						.logError(
								toString(),
								Messages
										.getString("TransMeta.Log.DatabaseError") + dbe.getMessage()); //$NON-NLS-1$
				return true;
			}
		}
		return false;
	 ...
	 *             if an error occurrs.
	 */
	public void saveRep(Repository rep, ProgressMonitorListener monitor)
			throws KettleException {
		try {
			if (monitor != null)
				monitor.subTask(Messages
						.getString("TransMeta.Monitor.LockingRepository")); //$NON-NLS-1$
			...
			int nrWorks = 2 + nrDatabases() + nrNotes() + nrSteps()
					+ nrTransHops();
			if (monitor != null)
				monitor
						.beginTask(
								Messages
										.getString("TransMeta.Monitor.SavingTransformationTask.Title") + getPathAndName(), nrWorks); //$NON-NLS-1$
			if (log.isDebug())
				log
						.logDebug(
								toString(),
								Messages
										.getString("TransMeta.Log.SavingOfTransformationStarted")); //$NON-NLS-1$
			...
			if (monitor != null)
				monitor
						.subTask(Messages
								.getString("TransMeta.Monitor.HandlingOldVersionTransformationTask.Title")); //$NON-NLS-1$
			...
			if (getID() <= 0) {
				...
				if (monitor != null)
					monitor
							.subTask(Messages
									.getString("TransMeta.Monitor.DeletingOldVersionTransformationTask.Title")); //$NON-NLS-1$
				if (log.isDebug())
					log
							.logDebug(
									toString(),
									Messages
											.getString("TransMeta.Log.DeletingOldVersionTransformation")); //$NON-NLS-1$
				...
				if (log.isDebug())
					log
							.logDebug(
									toString(),
									Messages
											.getString("TransMeta.Log.OldVersionOfTransformationRemoved")); //$NON-NLS-1$
			}
			...
			if (log.isDebug())
				log.logDebug(toString(), Messages
						.getString("TransMeta.Log.SavingNotes")); //$NON-NLS-1$
			for (int i = 0; i < nrNotes(); i++) {
				if (monitor != null && monitor.isCanceled())
					throw new KettleDatabaseException(Messages
							.getString("TransMeta.Log.UserCancelledTransSave"));

				if (monitor != null)
					monitor
							.subTask(Messages
									.getString("TransMeta.Monitor.SavingNoteTask.Title") + (i + 1) + "/" + nrNotes()); //$NON-NLS-1$ //$NON-NLS-2$
			...
			if (log.isDebug())
				log.logDebug(toString(), Messages
						.getString("TransMeta.Log.SavingDatabaseConnections")); //$NON-NLS-1$
			for (int i = 0; i < nrDatabases(); i++) {
				...
				if (monitor != null)
					monitor
							.subTask(Messages
									.getString("TransMeta.Monitor.SavingDatabaseTask.Title") + (i + 1) + "/" + nrDatabases()); //$NON-NLS-1$ //$NON-NLS-2$
			...
			if (log.isDebug())
				log.logDebug(toString(), Messages
						.getString("TransMeta.Log.CheckingStepTypes")); //$NON-NLS-1$
			// rep.updateStepTypes();

			if (log.isDebug())
				log.logDebug(toString(), Messages
						.getString("TransMeta.Log.SavingSteps")); //$NON-NLS-1$
			for (int i = 0; i < nrSteps(); i++) {
				if (monitor != null && monitor.isCanceled())
					throw new KettleDatabaseException(Messages
							.getString("TransMeta.Log.UserCancelledTransSave"));
				...
				if (monitor != null)
					monitor
							.subTask(Messages
									.getString("TransMeta.Monitor.SavingStepTask.Title") + (i + 1) + "/" + nrSteps()); //$NON-NLS-1$ //$NON-NLS-2$
				StepMeta stepMeta = getStep(i);
				StepMetaInterface sii = stepMeta.getStepMetaInterface();
				if (sii != null)
					sii.searchInfoAndTargetSteps(steps);
			...
			if (log.isDebug())
				log.logDebug(toString(), Messages
						.getString("TransMeta.Log.SavingHops")); //$NON-NLS-1$
			for (int i = 0; i < nrTransHops(); i++) {
				...
				if (monitor != null)
					monitor
							.subTask(Messages
									.getString("TransMeta.Monitor.SavingHopTask.Title") + (i + 1) + "/" + nrTransHops()); //$NON-NLS-1$ //$NON-NLS-2$
				TransHopMeta hi = getTransHop(i);
			...
			if (monitor != null)
				monitor.subTask(Messages
						.getString("TransMeta.Monitor.FinishingTask.Title")); //$NON-NLS-1$
			if (log.isDebug())
				log.logDebug(toString(), Messages
						.getString("TransMeta.Log.SavingTransformationInfo")); //$NON-NLS-1$
			...
			if (log.isDebug())
				log.logDebug(toString(), Messages
						.getString("TransMeta.Log.SavingDependencies")); //$NON-NLS-1$
			...
			if (log.isDebug())
				log.logDebug(toString(), Messages
						.getString("TransMeta.Log.SavingFinished")); //$NON-NLS-1$

			if (monitor != null)
				monitor.subTask(Messages
						.getString("TransMeta.Monitor.UnlockingRepository")); //$NON-NLS-1$
			...
			log
					.logError(
							toString(),
							Messages
									.getString("TransMeta.Log.ErrorSavingTransformationToRepository") + Const.CR + dbe.getMessage()); //$NON-NLS-1$
			throw new KettleException(
					Messages
							.getString("TransMeta.Log.ErrorSavingTransformationToRepository"), dbe); //$NON-NLS-1$
		} finally {
	...
	public boolean isUsingPartitionSchema(PartitionSchema partitionSchema) {
		...
		for (int i = 0; i < nrSteps(); i++) {
			StepPartitioningMeta stepPartitioningMeta = getStep(i)
					.getStepPartitioningMeta();
			if (stepPartitioningMeta != null) {
				PartitionSchema check = stepPartitioningMeta
						.getPartitionSchema();
	...
	public void readDatabases(Repository rep, boolean overWriteShared)
		...
		try {
			...
			for (int i = 0; i < dbids.length; i++) {
				DatabaseMeta databaseMeta = RepositoryUtil.loadDatabaseMeta(
						rep, dbids[i]);
			...
			throw new KettleException(
					Messages
							.getString("TransMeta.Log.UnableToReadDatabaseIDSFromRepository"), dbe); //$NON-NLS-1$
		} catch (KettleException ke) {
			throw new KettleException(
					Messages
							.getString("TransMeta.Log.UnableToReadDatabasesFromRepository"), ke); //$NON-NLS-1$
		}
	}
	...
	public void readPartitionSchemas(Repository rep, boolean overWriteShared)
		...
		try {
			...
			for (int i = 0; i < dbids.length; i++) {
				PartitionSchema partitionSchema = new PartitionSchema(rep,
						dbids[i]);
				PartitionSchema check = findPartitionSchema(partitionSchema
						.getName()); // Check if there already is one in the
				// transformation
			...
			throw new KettleException(
					Messages
							.getString("TransMeta.Log.UnableToReadPartitionSchemaFromRepository"), dbe); //$NON-NLS-1$
		}
	}
	 ...
	 * @throws KettleException
	 */
	public void readSlaves(Repository rep, boolean overWriteShared)
			throws KettleException {
		try {
			...
			throw new KettleException(
					Messages
							.getString("TransMeta.Log.UnableToReadSlaveServersFromRepository"), dbe); //$NON-NLS-1$
		}
	}
	 ...
	 * @throws KettleException
	 */
	public void readClusters(Repository rep, boolean overWriteShared)
			throws KettleException {
		try {
			...
			throw new KettleException(
					Messages
							.getString("TransMeta.Log.UnableToReadClustersFromRepository"), dbe); //$NON-NLS-1$
		}
	}
	...
	public void loadRepTrans(Repository rep) throws KettleException {
		try {
			...
			if (r != null) {
				...
				extended_description = r
						.getString("EXTENDED_DESCRIPTION", null);
				trans_version = r.getString("TRANS_VERSION", null);
				trans_status = (int) r.getInteger("TRANS_STATUS", -1L);

				readStep = StepMeta.findStep(steps, r.getInteger(
						"ID_STEP_READ", -1L)); //$NON-NLS-1$
				writeStep = StepMeta.findStep(steps, r.getInteger(
						"ID_STEP_WRITE", -1L)); //$NON-NLS-1$
				inputStep = StepMeta.findStep(steps, r.getInteger(
						"ID_STEP_INPUT", -1L)); //$NON-NLS-1$
				outputStep = StepMeta.findStep(steps, r.getInteger(
						"ID_STEP_OUTPUT", -1L)); //$NON-NLS-1$
				updateStep = StepMeta.findStep(steps, r.getInteger(
						"ID_STEP_UPDATE", -1L)); //$NON-NLS-1$

				long id_rejected = rep.getTransAttributeInteger(getID(), 0,
						"ID_STEP_REJECTED"); // $NON-NLS-1$
				if (id_rejected > 0) {
					rejectedStep = StepMeta.findStep(steps, id_rejected); //$NON-NLS-1$
				}

				logConnection = DatabaseMeta.findDatabase(databases, r
						.getInteger("ID_DATABASE_LOG", -1L)); //$NON-NLS-1$
				...
				maxDateConnection = DatabaseMeta.findDatabase(databases, r
						.getInteger("ID_DATABASE_MAXDATE", -1L)); //$NON-NLS-1$
				...
				if (id_directory >= 0) {
					if (log.isDetailed())
						log.logDetailed(toString(),
								"ID_DIRECTORY=" + id_directory); //$NON-NLS-1$
					// Set right directory...
					directory = directoryTree.findDirectory(id_directory);
				}

				usingUniqueConnections = rep.getTransAttributeBoolean(getID(),
						0, Repository.TRANS_ATTRIBUTE_UNIQUE_CONNECTIONS);
				feedbackShown = !"N".equalsIgnoreCase(rep
						.getTransAttributeString(getID(), 0,
								Repository.TRANS_ATTRIBUTE_FEEDBACK_SHOWN));
			...
			throw new KettleException(
					Messages
							.getString("TransMeta.Exception.UnableToLoadTransformationInfoFromRepository"), dbe); //$NON-NLS-1$
		} finally {
			initializeVariablesFrom(null);
	 ...
	 */
	public TransMeta(Repository rep, String transname,
			RepositoryDirectory repdir) throws KettleException {
	 ...
	 */
	public TransMeta(Repository rep, String transname,
			RepositoryDirectory repdir, ProgressMonitorListener monitor)
			throws KettleException {
		...
		synchronized (rep) {
			try {
				String pathAndName = repdir.isRoot() ? repdir + transname
						: repdir + RepositoryDirectory.DIRECTORY_SEPARATOR
								+ transname;
				...
				if (log.isDetailed())
					log
							.logDetailed(
									...
									Messages
											.getString(
													"TransMeta.Log.LookingForTransformation", transname, directory.getPath())); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

				if (monitor != null)
					monitor
							.subTask(Messages
									.getString("TransMeta.Monitor.ReadingTransformationInfoTask.Title")); //$NON-NLS-1$
				...
				if (getID() > 0) {
					...
					int nrWork = 3 + noteids.length + stepids.length
							+ hopids.length;
					...
					if (monitor != null)
						monitor
								.beginTask(
										Messages
												.getString("TransMeta.Monitor.LoadingTransformationTask.Title") + pathAndName, nrWork); //$NON-NLS-1$
					...
					if (log.isDetailed())
						log
								.logDetailed(
										...
										Messages
												.getString(
														"TransMeta.Log.LoadingTransformation", getName())); //$NON-NLS-1$ //$NON-NLS-2$

					// Load the common database connections
					if (monitor != null)
						monitor
								.subTask(Messages
										.getString("TransMeta.Monitor.ReadingTheAvailableSharedObjectsTask.Title")); //$NON-NLS-1$
					try {
						sharedObjects = readSharedObjects(rep);
					} catch (Exception e) {
						LogWriter
								...
								.logError(
										...
										Messages
												.getString(
														"TransMeta.ErrorReadingSharedObjects.Message",
														e.toString()));
						LogWriter.getInstance().logError(toString(),
								Const.getStackTracker(e));
					}
					...
					if (monitor != null)
						monitor
								.subTask(Messages
										.getString("TransMeta.Monitor.ReadingNoteTask.Title")); //$NON-NLS-1$
					...
					if (monitor != null)
						monitor
								.subTask(Messages
										.getString("TransMeta.Monitor.ReadingStepsTask.Title")); //$NON-NLS-1$
						...
						if (log.isDetailed())
							log
									.logDetailed(
											toString(),
											Messages
													.getString("TransMeta.Log.LoadingStepWithID") + stepids[i]); //$NON-NLS-1$
						if (monitor != null)
							monitor
									.subTask(Messages
											.getString("TransMeta.Monitor.ReadingStepTask.Title") + (i + 1) + "/" + (stepids.length)); //$NON-NLS-1$ //$NON-NLS-2$
						
						try{
							StepMeta stepMeta = new StepMeta(rep, stepids[i],
									databases, counters, partitionSchemas);
					...
					// source steps...
					for (int i = 0; i < nrSteps(); i++) {
						StepMetaInterface sii = getStep(i)
					...
					if (monitor != null)
						monitor
								.subTask(Messages
										.getString("TransMeta.Monitor.LoadingTransformationDetailsTask.Title")); //$NON-NLS-1$
					loadRepTrans(rep);
					if (monitor != null)
						monitor.worked(1);
					...
					if (monitor != null)
						monitor
								.subTask(Messages
										.getString("TransMeta.Monitor.ReadingHopTask.Title")); //$NON-NLS-1$
					for (int i = 0; i < hopids.length; i++) {
						TransHopMeta hi = new TransHopMeta(rep, hopids[i],
								steps);
						addTransHop(hi);
						if (monitor != null)
							monitor.worked(1);
					...
					for (int i = 0; i < nrSteps(); i++) {
						...
						if (stepPartitioningMeta != null) {
							stepPartitioningMeta
									.setPartitionSchemaAfterLoading(partitionSchemas);
						}
					}
					...
					if (monitor != null)
						monitor
								.subTask(Messages
										.getString("TransMeta.Monitor.ReadingTheDependenciesTask.Title")); //$NON-NLS-1$
					long depids[] = rep.getTransDependencyIDs(getID());
					for (int i = 0; i < depids.length; i++) {
						TransDependency td = new TransDependency(rep,
								depids[i], databases);
						addDependency(td);
					}
					if (monitor != null)
					...
					for (int i = 0; i < nrSteps(); i++) {
						StepMeta stepMeta = getStep(i);
						String sourceStep = rep.getStepAttributeString(stepMeta
								.getID(), "step_error_handling_source_step");
						if (sourceStep != null) {
							StepErrorMeta stepErrorMeta = new StepErrorMeta(
									this, rep, stepMeta, steps);
							stepErrorMeta.getSourceStep().setStepErrorMeta(
									stepErrorMeta); // a bit of a trick, I know.
						}
					}
					...
					if (monitor != null)
						monitor
								.subTask(Messages
										.getString("TransMeta.Monitor.SortingStepsTask.Title")); //$NON-NLS-1$
					sortSteps();
					if (monitor != null)
						monitor.worked(1);
					if (monitor != null)
					...
					throw new KettleException(
							Messages
									.getString("TransMeta.Exception.TransformationDoesNotExist") + name); //$NON-NLS-1$
				}
				if (log.isDetailed()) {
					log
							.logDetailed(
									...
									Messages
											.getString(
													"TransMeta.Log.LoadedTransformation2", transname, String.valueOf(directory == null))); //$NON-NLS-1$ //$NON-NLS-2$
					log
							.logDetailed(
									toString(),
									Messages
											.getString(
													"TransMeta.Log.LoadedTransformation", transname, directory.getPath())); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
				}
				...
				log
						.logError(
								toString(),
								Messages
										.getString("TransMeta.Log.DatabaseErrorOccuredReadingTransformation") + Const.CR + e); //$NON-NLS-1$
				throw new KettleException(
						Messages
								.getString("TransMeta.Exception.DatabaseErrorOccuredReadingTransformation"), e); //$NON-NLS-1$
			} catch (Exception e) {
				log
						.logError(
								toString(),
				...
				throw new KettleException(
						Messages
								.getString("TransMeta.Exception.DatabaseErrorOccuredReadingTransformation2"), e); //$NON-NLS-1$
			} finally {
				initializeVariablesFrom(null);
	...
	public String getXML() throws KettleException {
		...
		retval
				.append("  ").append(XMLHandler.openTag(XML_TAG_INFO)).append(Const.CR); //$NON-NLS-1$

		retval.append("    ").append(XMLHandler.addTagValue("name", name)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("    ").append(XMLHandler.addTagValue("description", description)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("    ").append(XMLHandler.addTagValue("gui_location_x", guiLocationX)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("    ").append(XMLHandler.addTagValue("gui_location_y", guiLocationY)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("    ").append(XMLHandler.addTagValue("gui_scale", guiScale)); //$NON-NLS-1$ //$NON-NLS-2$
		retval.append("    ").append(
				XMLHandler.addTagValue("extended_description",
						extended_description));
		retval.append("    ").append(
				XMLHandler.addTagValue("trans_version", trans_version));

		if (trans_status >= 0) {
			retval.append("    ").append(
					XMLHandler.addTagValue("trans_status", trans_status));
		}
		retval
		...
		retval
				.append("    ").append(XMLHandler.openTag(XML_TAG_PARAMETERS)).append(Const.CR); //$NON-NLS-1$
		String[] parameters = listParameters();
		for (int idx = 0; idx < parameters.length; idx++) {
			retval
					.append("        ").append(XMLHandler.openTag("parameter")).append(Const.CR); //$NON-NLS-1$ //$NON-NLS-2$
			...
			retval
					.append("            ").append(XMLHandler.openTag("description")); //$NON-NLS-1$
			...
			retval
					.append("        ").append(XMLHandler.closeTag("parameter")).append(Const.CR); //$NON-NLS-1$ //$NON-NLS-2$        	
		}
		retval
				.append("    ").append(XMLHandler.closeTag(XML_TAG_PARAMETERS)).append(Const.CR); //$NON-NLS-1$

		retval.append("    <log>").append(Const.CR); //$NON-NLS-1$
		retval
				.append("      ").append(XMLHandler.addTagValue("read", readStep == null ? "" : readStep.getName())); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		retval
				.append("      ").append(XMLHandler.addTagValue("write", writeStep == null ? "" : writeStep.getName())); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		retval
				.append("      ").append(XMLHandler.addTagValue("input", inputStep == null ? "" : inputStep.getName())); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		retval
				.append("      ").append(XMLHandler.addTagValue("output", outputStep == null ? "" : outputStep.getName())); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		retval
				.append("      ").append(XMLHandler.addTagValue("update", updateStep == null ? "" : updateStep.getName())); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		retval
				.append("      ").append(XMLHandler.addTagValue("rejected", rejectedStep == null ? "" : rejectedStep.getName())); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		retval
				.append("      ").append(XMLHandler.addTagValue("connection", logConnection == null ? "" : logConnection.getName())); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		retval
				.append("      ").append(XMLHandler.addTagValue("table", logTable)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("      ").append(XMLHandler.addTagValue("step_performance_table", stepPerformanceLogTable)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("      ").append(XMLHandler.addTagValue("use_batchid", useBatchId)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("      ").append(XMLHandler.addTagValue("use_logfield", logfieldUsed)); //$NON-NLS-1$ //$NON-NLS-2$
		retval.append("    </log>").append(Const.CR); //$NON-NLS-1$
		...
		retval
				.append("      ").append(XMLHandler.addTagValue("table", maxDateTable)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("      ").append(XMLHandler.addTagValue("field", maxDateField)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("      ").append(XMLHandler.addTagValue("offset", maxDateOffset)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("      ").append(XMLHandler.addTagValue("maxdiff", maxDateDifference)); //$NON-NLS-1$ //$NON-NLS-2$
		retval.append("    </maxdate>").append(Const.CR); //$NON-NLS-1$

		retval
				.append("    ").append(XMLHandler.addTagValue("size_rowset", sizeRowset)); //$NON-NLS-1$ //$NON-NLS-2$

		retval
				.append("    ").append(XMLHandler.addTagValue("sleep_time_empty", sleepTimeEmpty)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("    ").append(XMLHandler.addTagValue("sleep_time_full", sleepTimeFull)); //$NON-NLS-1$ //$NON-NLS-2$

		retval
				.append("    ").append(XMLHandler.addTagValue("unique_connections", usingUniqueConnections)); //$NON-NLS-1$ //$NON-NLS-2$

		retval
				.append("    ").append(XMLHandler.addTagValue("feedback_shown", feedbackShown)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("    ").append(XMLHandler.addTagValue("feedback_size", feedbackSize)); //$NON-NLS-1$ //$NON-NLS-2$
		retval.append("    ").append(
				XMLHandler.addTagValue("using_thread_priorities",
						usingThreadPriorityManagment)); // $NON-NLS-1$
		retval.append("    ").append(
				XMLHandler
						.addTagValue("shared_objects_file", sharedObjectsFile)); // $NON-NLS-1$

		// Performance monitoring
		//
		retval.append("    ").append(
				XMLHandler.addTagValue("capture_step_performance",
						capturingStepPerformanceSnapShots)); // $NON-NLS-1$
		retval.append("    ").append(
				XMLHandler.addTagValue("step_performance_capturing_delay",
						stepPerformanceCapturingDelay)); // $NON-NLS-1$

		retval
				.append("    ").append(XMLHandler.openTag(XML_TAG_DEPENDENCIES)).append(Const.CR); //$NON-NLS-1$
		for (int i = 0; i < nrDependencies(); i++) {
			TransDependency td = getDependency(i);
			retval.append(td.getXML());
		}
		retval
				.append("    ").append(XMLHandler.closeTag(XML_TAG_DEPENDENCIES)).append(Const.CR); //$NON-NLS-1$

		// The partitioning schemas...
		//
		retval
				.append("    ").append(XMLHandler.openTag(XML_TAG_PARTITIONSCHEMAS)).append(Const.CR); //$NON-NLS-1$
		for (int i = 0; i < partitionSchemas.size(); i++) {
			PartitionSchema partitionSchema = partitionSchemas.get(i);
			retval.append(partitionSchema.getXML());
		}
		retval
				.append("    ").append(XMLHandler.closeTag(XML_TAG_PARTITIONSCHEMAS)).append(Const.CR); //$NON-NLS-1$

		// The slave servers...
		//
		retval
				.append("    ").append(XMLHandler.openTag(XML_TAG_SLAVESERVERS)).append(Const.CR); //$NON-NLS-1$
		for (int i = 0; i < slaveServers.size(); i++) {
			SlaveServer slaveServer = slaveServers.get(i);
			retval.append("         ").append(slaveServer.getXML()).append(
					Const.CR);
		}
		retval
				.append("    ").append(XMLHandler.closeTag(XML_TAG_SLAVESERVERS)).append(Const.CR); //$NON-NLS-1$

		// The cluster schemas...
		//
		retval
				.append("    ").append(XMLHandler.openTag(XML_TAG_CLUSTERSCHEMAS)).append(Const.CR); //$NON-NLS-1$
		for (int i = 0; i < clusterSchemas.size(); i++) {
			ClusterSchema clusterSchema = clusterSchemas.get(i);
			retval.append(clusterSchema.getXML());
		}
		retval
				.append("    ").append(XMLHandler.closeTag(XML_TAG_CLUSTERSCHEMAS)).append(Const.CR); //$NON-NLS-1$

		retval.append("  ").append(
				XMLHandler.addTagValue("modified_user", modifiedUser));
		retval.append("  ").append(
				XMLHandler.addTagValue("modified_date", modifiedDate));

		retval
				.append("  ").append(XMLHandler.closeTag(XML_TAG_INFO)).append(Const.CR); //$NON-NLS-1$

		retval
				.append("  ").append(XMLHandler.openTag(XML_TAG_NOTEPADS)).append(Const.CR); //$NON-NLS-1$
		if (notes != null)
			for (int i = 0; i < nrNotes(); i++) {
				NotePadMeta ni = getNote(i);
		...
		retval
				.append("  ").append(XMLHandler.closeTag(XML_TAG_NOTEPADS)).append(Const.CR); //$NON-NLS-1$
		...
		retval
				.append("  ").append(XMLHandler.openTag(XML_TAG_ORDER)).append(Const.CR); //$NON-NLS-1$
		for (int i = 0; i < nrTransHops(); i++) {
			TransHopMeta transHopMeta = getTransHop(i);
			retval.append(transHopMeta.getXML());
		}
		retval
				.append("  ").append(XMLHandler.closeTag(XML_TAG_ORDER)).append(Const.CR); //$NON-NLS-1$
		...
		retval.append("  ").append(
				XMLHandler.openTag(XML_TAG_STEP_ERROR_HANDLING)).append(
				Const.CR);
		for (int i = 0; i < nrSteps(); i++) {
			StepMeta stepMeta = getStep(i);
		...
		retval.append("  ").append(
				XMLHandler.closeTag(XML_TAG_STEP_ERROR_HANDLING)).append(
				Const.CR);

		// The slave-step-copy/partition distribution. Only used for slave
		// transformations in a clustering environment.
		retval.append("   ")
				.append(slaveStepCopyPartitionDistribution.getXML());

		// Is this a slave transformation or not?
		retval.append("   ").append(
				XMLHandler.addTagValue("slave_transformation",
						slaveTransformation));
	 ...
	 */
	public TransMeta(String fname, Repository rep,
			boolean setInternalVariables, VariableSpace parentVariableSpace)
			throws KettleXMLException {
	 ...
	 * @throws KettleXMLException
	 */
	public void loadXML(Node transnode, Repository rep,
			boolean setInternalVariables) throws KettleXMLException {
	...
	public void loadXML(Node transnode, Repository rep,
		...
		try {
			...
			try {
				sharedObjectsFile = XMLHandler.getTagValue(transnode,
						"info", "shared_objects_file"); //$NON-NLS-1$ //$NON-NLS-2$
				sharedObjects = readSharedObjects(rep);
			} catch (Exception e) {
				LogWriter.getInstance().logError(
						toString(),
			...
			if (log.isDebug())
				log.logDebug(toString(), Messages.getString(
						"TransMeta.Log.WeHaveConnections", String.valueOf(n))); //$NON-NLS-1$ //$NON-NLS-2$
			for (int i = 0; i < n; i++) {
				if (log.isDebug())
					log
							.logDebug(
									toString(),
									Messages
											.getString("TransMeta.Log.LookingAtConnection") + i); //$NON-NLS-1$
				Node nodecon = XMLHandler.getSubNodeByNr(transnode,
						DatabaseMeta.XML_TAG, i); //$NON-NLS-1$
				...
				if (exist == null) {
					...
					// shared connection.
					{
						boolean askOverwrite = Props.isInitialized() ? props
								.askAboutReplacingDatabaseConnections() : false;
						boolean overwrite = Props.isInitialized() ? props
								.replaceExistingDatabaseConnections() : true;
						if (askOverwrite) {
			...
			// Read the notes...
			Node notepadsnode = XMLHandler.getSubNode(transnode,
					XML_TAG_NOTEPADS); //$NON-NLS-1$
			int nrnotes = XMLHandler.countNodes(notepadsnode,
					NotePadMeta.XML_TAG); //$NON-NLS-1$
			for (int i = 0; i < nrnotes; i++) {
				Node notepadnode = XMLHandler.getSubNodeByNr(notepadsnode,
						NotePadMeta.XML_TAG, i); //$NON-NLS-1$
			...
			if (log.isDebug())
				log
						.logDebug(
								toString(),
								Messages
										.getString("TransMeta.Log.ReadingSteps") + s + " steps..."); //$NON-NLS-1$ //$NON-NLS-2$
			for (int i = 0; i < s; i++) {
				Node stepnode = XMLHandler.getSubNodeByNr(transnode,
						StepMeta.XML_TAG, i); //$NON-NLS-1$

				if (log.isDebug())
					log.logDebug(toString(), Messages
							.getString("TransMeta.Log.LookingAtStep") + i); //$NON-NLS-1$
			...
			for (int i = 0; i < nrErrorHandlers; i++) {
				...
				StepErrorMeta stepErrorMeta = new StepErrorMeta(this,
			...
			if (log.isDebug())
				log
						.logDebug(
								toString(),
								Messages.getString("TransMeta.Log.WeHaveHops") + n + " hops..."); //$NON-NLS-1$ //$NON-NLS-2$
			for (int i = 0; i < n; i++) {
				if (log.isDebug())
					log.logDebug(toString(), Messages
							.getString("TransMeta.Log.LookingAtHop") + i); //$NON-NLS-1$
			...
			// gui location x
			//
			guiLocationX = Const.toInt(XMLHandler.getTagValue(infonode,
					"gui_location_x"), 0);

			// gui location y
			//
			guiLocationY = Const.toInt(XMLHandler.getTagValue(infonode,
					"gui_location_y"), 0);

			// gui scale
			//
			guiScale = Const.toDouble(XMLHandler.getTagValue(infonode,
					"gui_scale"), 1);
			...
			// trans status
			//
			trans_status = Const.toInt(XMLHandler.getTagValue(infonode,
					"trans_status"), -1);

			// Optionally load the repository directory...
			//
			if (rep != null) {
				String directoryPath = XMLHandler.getTagValue(infonode,
						"directory");
				if (directoryPath != null) {
					directory = rep.getDirectoryTree().findDirectory(
							directoryPath);
			...
			writeStep = findStep(XMLHandler.getTagValue(infonode,
					"log", "write")); //$NON-NLS-1$ //$NON-NLS-2$
			inputStep = findStep(XMLHandler.getTagValue(infonode,
					"log", "input")); //$NON-NLS-1$ //$NON-NLS-2$
			outputStep = findStep(XMLHandler.getTagValue(infonode,
					"log", "output")); //$NON-NLS-1$ //$NON-NLS-2$
			updateStep = findStep(XMLHandler.getTagValue(infonode,
					"log", "update")); //$NON-NLS-1$ //$NON-NLS-2$
			rejectedStep = findStep(XMLHandler.getTagValue(infonode,
					"log", "rejected")); //$NON-NLS-1$ //$NON-NLS-2$
			String logcon = XMLHandler.getTagValue(infonode,
					"log", "connection"); //$NON-NLS-1$ //$NON-NLS-2$
			logConnection = findDatabase(logcon);
			logTable = XMLHandler.getTagValue(infonode, "log", "table"); //$NON-NLS-1$ //$NON-NLS-2$
			stepPerformanceLogTable = XMLHandler.getTagValue(infonode,
					"log", "step_performance_table"); //$NON-NLS-1$ //$NON-NLS-2$
			...
			String maxdatcon = XMLHandler.getTagValue(infonode,
					"maxdate", "connection"); //$NON-NLS-1$ //$NON-NLS-2$
			...
			String offset = XMLHandler.getTagValue(infonode,
					"maxdate", "offset"); //$NON-NLS-1$ //$NON-NLS-2$
			maxDateOffset = Const.toDouble(offset, 0.0);
			String mdiff = XMLHandler.getTagValue(infonode,
					"maxdate", "maxdiff"); //$NON-NLS-1$ //$NON-NLS-2$
			...
			//
			Node depsNode = XMLHandler.getSubNode(infonode,
					XML_TAG_DEPENDENCIES);
			int nrDeps = XMLHandler.countNodes(depsNode,
					TransDependency.XML_TAG);

			for (int i = 0; i < nrDeps; i++) {
				Node depNode = XMLHandler.getSubNodeByNr(depsNode,
						TransDependency.XML_TAG, i);

				TransDependency transDependency = new TransDependency(depNode,
						databases);
				if (transDependency.getDatabase() != null
						&& transDependency.getFieldname() != null) {
					addDependency(transDependency);
				}
			}

			// Read the named parameters.
			Node paramsNode = XMLHandler.getSubNode(infonode,
					XML_TAG_PARAMETERS);
			int nrParams = XMLHandler.countNodes(paramsNode, "parameter"); //$NON-NLS-1$

			for (int i = 0; i < nrParams; i++) {
				Node paramNode = XMLHandler.getSubNodeByNr(paramsNode,
			...
			// 
			Node partSchemasNode = XMLHandler.getSubNode(infonode,
					XML_TAG_PARTITIONSCHEMAS); //$NON-NLS-1$
			int nrPartSchemas = XMLHandler.countNodes(partSchemasNode,
					PartitionSchema.XML_TAG); //$NON-NLS-1$
			for (int i = 0; i < nrPartSchemas; i++) {
				Node partSchemaNode = XMLHandler.getSubNodeByNr(
						partSchemasNode, PartitionSchema.XML_TAG, i);
			...
			for (int i = 0; i < nrSteps(); i++) {
				...
				StepPartitioningMeta targetStepPartitioningMeta = getStep(i)
			...
			// 
			Node slaveServersNode = XMLHandler.getSubNode(infonode,
					XML_TAG_SLAVESERVERS); //$NON-NLS-1$
			int nrSlaveServers = XMLHandler.countNodes(slaveServersNode,
					SlaveServer.XML_TAG); //$NON-NLS-1$
			for (int i = 0; i < nrSlaveServers; i++) {
				Node slaveServerNode = XMLHandler.getSubNodeByNr(
						slaveServersNode, SlaveServer.XML_TAG, i);
			...
			// 
			Node clusterSchemasNode = XMLHandler.getSubNode(infonode,
					XML_TAG_CLUSTERSCHEMAS); //$NON-NLS-1$
			int nrClusterSchemas = XMLHandler.countNodes(clusterSchemasNode,
					ClusterSchema.XML_TAG); //$NON-NLS-1$
			for (int i = 0; i < nrClusterSchemas; i++) {
				Node clusterSchemaNode = XMLHandler.getSubNodeByNr(
						clusterSchemasNode, ClusterSchema.XML_TAG, i);
				ClusterSchema clusterSchema = new ClusterSchema(
						clusterSchemaNode, slaveServers);
			...
			sleepTimeEmpty = Const.toInt(XMLHandler.getTagValue(infonode,
					"sleep_time_empty"), Const.TIMEOUT_GET_MILLIS); //$NON-NLS-1$
			sleepTimeFull = Const.toInt(XMLHandler.getTagValue(infonode,
					"sleep_time_full"), Const.TIMEOUT_PUT_MILLIS); //$NON-NLS-1$
			...
			feedbackSize = Const.toInt(XMLHandler.getTagValue(infonode,
					"feedback_size"), Const.ROWS_UPDATE); //$NON-NLS-1$
			usingThreadPriorityManagment = !"N".equalsIgnoreCase(XMLHandler.getTagValue(infonode, "using_thread_priorities")); //$NON-NLS-1$ 

			// Performance monitoring for steps...
			//
			capturingStepPerformanceSnapShots = "Y".equalsIgnoreCase(XMLHandler
					.getTagValue(infonode, "capture_step_performance")); // $NON-NLS-1$
			// $NON-NLS-2$
			stepPerformanceCapturingDelay = Const.toLong(XMLHandler
					.getTagValue(infonode, "step_performance_capturing_delay"),
					1000); // $NON-NLS-1$

			// Created user/date
			createdUser = XMLHandler.getTagValue(infonode, "created_user");
			String createDate = XMLHandler
					.getTagValue(infonode, "created_date");
			if (createDate != null) {
			...
			Node partitionDistNode = XMLHandler.getSubNode(transnode,
					SlaveStepCopyPartitionDistribution.XML_TAG);
			...
			//
			slaveTransformation = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					transnode, "slave_transformation"));
			if (log.isDebug()) {
				log
						.logDebug(
								toString(),
								Messages
										.getString("TransMeta.Log.NumberOfStepsReaded") + nrSteps()); //$NON-NLS-1$
				log
						.logDebug(
								toString(),
								Messages
										.getString("TransMeta.Log.NumberOfHopsReaded") + nrTransHops()); //$NON-NLS-1$
			}
			sortSteps();
		} catch (KettleXMLException xe) {
			throw new KettleXMLException(
					Messages
							.getString("TransMeta.Exception.ErrorReadingTransformation"), xe); //$NON-NLS-1$
	...
	public SharedObjects readSharedObjects(Repository rep)
			throws KettleException {
		if (rep != null) {
			sharedObjectsFile = rep.getTransAttributeString(getId(), 0,
					"SHARED_FILE");
		}
		...
		if (sharedObjects.getObjectsMap().isEmpty()) {
			log.logDetailed(toString(), Messages.getString(
					"TransMeta.Log.EmptySharedObjectsFile", soFile));
		}
		...
		//
		for (SharedObjectInterface object : sharedObjects.getObjectsMap()
				.values()) {
	...
	public List<StepMeta> getTransHopSteps(boolean all) {
		...
		for (int x = 0; x < nrSteps(); x++) {
			...
			if (!stepMeta.getRemoteInputSteps().isEmpty()
					|| !stepMeta.getRemoteOutputSteps().isEmpty()) {
	...
	public boolean haveStepsChanged() {
		...
		for (int i = 0; i < nrSteps(); i++) {
			...
			if (stepMeta.hasChanged())
				return true;
			if (stepMeta.getStepPartitioningMeta() != null
					&& stepMeta.getStepPartitioningMeta().hasChanged())
				return true;
		}
		return false;
	}
	...
	private boolean hasLoop(StepMeta stepMeta, StepMeta lookup, boolean info) {
		String cacheKey = stepMeta.getName() + " - "
				+ (lookup != null ? lookup.getName() : "") + " - "
				+ (info ? "true" : "false");
		...
		for (int i = 0; i < nr && !hasLoop; i++) {
			...
			if (prevStepMeta != null) {
				if (prevStepMeta.equals(stepMeta)) {
					...
					hasLoop = true;
					break; // no need to check more but caching this one below
				} else if (hasLoop(prevStepMeta, lookup == null ? stepMeta
						: lookup, info)) {
					hasLoop = true;
					break; // no need to check more but caching this one below
				}
	 ...
	 *            one.
	 */
	public void addUndo(Object from[], Object to[], int pos[], Point prev[],
			Point curr[], int type_of_change, boolean nextAlso) {
		...
		try {
			...
			LogWriter
					...
					.logError(
							toString(),
							Messages
									.getString("TransMeta.Exception.ErrorOfSortingSteps") + e); //$NON-NLS-1$
			LogWriter.getInstance().logError(toString(),
					Const.getStackTracker(e));
		}
	}
	...
	public Map<StepMeta, Map<StepMeta, Boolean>> sortStepsNatural() {
		...
		for (StepMeta stepMeta : steps) {
			...
			for (StepMeta prev : prevSteps) {
				Map<StepMeta, Boolean> beforePrevMap = updateFillStepMap(
						previousCache, beforeCache, stepMeta, prev);
		...
		log.logBasic(toString(), "Natural sort of steps executed in "
				+ (endTime - startTime) + "ms (" + prevCount
	...
	private Map<StepMeta, Boolean> updateFillStepMap(
		...
		for (StepMeta prev : prevSteps) {
			Map<StepMeta, Boolean> beforePrevMap = updateFillStepMap(
					previousCache, beforeCache, originStepMeta, prev);
	...
	public void analyseImpact(List<DatabaseImpact> impact,
		...
		if (monitor != null) {
			monitor
					.beginTask(
							Messages
									.getString("TransMeta.Monitor.DeterminingImpactTask.Title"), nrSteps()); //$NON-NLS-1$
		}
		boolean stop = false;
		for (int i = 0; i < nrSteps() && !stop; i++) {
			if (monitor != null)
				monitor
						.subTask(Messages
								.getString("TransMeta.Monitor.LookingAtStepTask.Title") + (i + 1) + "/" + nrSteps()); //$NON-NLS-1$ //$NON-NLS-2$
			StepMeta stepMeta = getStep(i);
			...
			stepint.analyseImpact(impact, this, stepMeta, prev, null, null,
					inform);
	...
	public List<SQLStatement> getSQLStatements(ProgressMonitorListener monitor)
		...
		if (monitor != null)
			monitor
					.beginTask(
							Messages
									.getString("TransMeta.Monitor.GettingTheSQLForTransformationTask.Title"), nrSteps() + 1); //$NON-NLS-1$
		...
		for (int i = 0; i < nrSteps(); i++) {
			...
			if (monitor != null)
				monitor
						.subTask(Messages
								.getString(
										"TransMeta.Monitor.GettingTheSQLForStepTask.Title", "" + stepMeta)); //$NON-NLS-1$ //$NON-NLS-2$
			RowMetaInterface prev = getPrevStepFields(stepMeta);
			SQLStatement sql = stepMeta.getStepMetaInterface()
					.getSQLStatements(this, stepMeta, prev);
		...
		if (monitor != null)
			monitor
					.subTask(Messages
							.getString("TransMeta.Monitor.GettingTheSQLForTransformationTask.Title2")); //$NON-NLS-1$
		if (logConnection != null
				&& (!Const.isEmpty(logTable) || !Const
						.isEmpty(stepPerformanceLogTable))) {
			...
			try {
				...
				if (!Const.isEmpty(logTable)) {
					RowMetaInterface fields = Database.getTransLogrecordFields(
							false, useBatchId, logfieldUsed);
					String sql = db.getDDL(logTable, fields);
					if (sql != null && sql.length() > 0) {
						SQLStatement stat = new SQLStatement(
								"<this transformation>", logConnection, sql); //$NON-NLS-1$
						stats.add(stat);
					}
				}
				if (!Const.isEmpty(stepPerformanceLogTable)) {
					RowMetaInterface fields = Database
							.getStepPerformanceLogrecordFields();
					String sql = db.getDDL(logTable, fields);
					if (sql != null && sql.length() > 0) {
						SQLStatement stat = new SQLStatement(
								"<this transformation>", logConnection, sql); //$NON-NLS-1$
				...
				SQLStatement stat = new SQLStatement(
						"<this transformation>", logConnection, null); //$NON-NLS-1$
				stat
						.setError(Messages
								.getString("TransMeta.SQLStatement.ErrorDesc.ErrorObtainingTransformationLogTableInfo") + dbe.getMessage()); //$NON-NLS-1$
				stats.add(stat);
			} finally {
				db.disconnect();
			}
	...
	public void checkSteps(List<CheckResultInterface> remarks,
			boolean only_selected, ProgressMonitorListener monitor) {
		try {
			...
			if (monitor != null)
				monitor
						.beginTask(
								Messages
										.getString("TransMeta.Monitor.VerifyingThisTransformationTask.Title"), steps.length + 2); //$NON-NLS-1$
			...
			for (int i = 0; i < steps.length && !stop_checking; i++) {
				if (monitor != null)
					monitor
							.subTask(Messages
									.getString(
											"TransMeta.Monitor.VerifyingStepTask.Title", stepnames[i])); //$NON-NLS-1$ //$NON-NLS-2$
				...
				if (isStepUsedInTransHops(stepMeta)) {
					...
					if (prev != null) {
							...
							if (name == null)
								values
										.put(
												v,
												Messages
														.getString("TransMeta.Value.CheckingFieldName.FieldNameIsEmpty.Description")); //$NON-NLS-1$
							else if (name.indexOf(' ') >= 0)
								values
										.put(
												v,
												Messages
														.getString("TransMeta.Value.CheckingFieldName.FieldNameContainsSpaces.Description")); //$NON-NLS-1$
							else {
								char list[] = new char[] { '.', ',', '-', '/',
										'+', '*', '\'', '\t', '"', '|', '@',
										'(', ')', '{', '}', '!', '^' };
						...
						CheckResult cr = new CheckResult(
								CheckResultInterface.TYPE_RESULT_ERROR,
								Messages
										.getString("TransMeta.CheckResult.TypeResultError.CannotFindPreviousFields.Description") + stepMeta.getName(), //$NON-NLS-1$
								stepMeta);
						remarks.add(cr);
					}
				} else {
					CheckResult cr = new CheckResult(
							...
							Messages
									.getString("TransMeta.CheckResult.TypeResultWarning.StepIsNotUsed.Description"), stepMeta); //$NON-NLS-1$
					remarks.add(cr);
				}
				...
				try {
					...
					CheckResult cr = new CheckResult(
							CheckResultInterface.TYPE_RESULT_ERROR, e
									.getMessage(), stepMeta);
					remarks.add(cr);
				}
			...
			if (monitor == null || !monitor.isCanceled()) {
				if (monitor != null)
					monitor
							.subTask(Messages
									.getString("TransMeta.Monitor.CheckingTheLoggingTableTask.Title")); //$NON-NLS-1$
				if (getLogConnection() != null) {
					...
					try {
						...
						CheckResult cr = new CheckResult(
								...
								Messages
										.getString("TransMeta.CheckResult.TypeResultOK.ConnectingWorks.Description"), //$NON-NLS-1$
								null);
						remarks.add(cr);
						...
						if (getLogTable() != null) {
							if (logdb.checkTableExists(getLogTable())) {
								cr = new CheckResult(
										...
										Messages
												.getString(
														"TransMeta.CheckResult.TypeResultOK.LoggingTableExists.Description", getLogTable()), null); //$NON-NLS-1$ //$NON-NLS-2$
								remarks.add(cr);

								RowMetaInterface fields = Database
										.getTransLogrecordFields(false,
												isBatchIdUsed(),
												isLogfieldUsed());
								String sql = logdb
										.getDDL(getLogTable(), fields);
								if (sql == null || sql.length() == 0) {
									cr = new CheckResult(
											CheckResultInterface.TYPE_RESULT_OK,
											Messages
													.getString("TransMeta.CheckResult.TypeResultOK.CorrectLayout.Description"), null); //$NON-NLS-1$
									remarks.add(cr);
								} else {
									cr = new CheckResult(
											CheckResultInterface.TYPE_RESULT_ERROR,
											Messages
													.getString("TransMeta.CheckResult.TypeResultError.LoggingTableNeedsAdjustments.Description") + Const.CR + sql, //$NON-NLS-1$
											null);
									remarks.add(cr);
								}
								...
								cr = new CheckResult(
										CheckResultInterface.TYPE_RESULT_ERROR,
										Messages
												.getString("TransMeta.CheckResult.TypeResultError.LoggingTableDoesNotExist.Description"), null); //$NON-NLS-1$
								remarks.add(cr);
							}
						} else {
							cr = new CheckResult(
									CheckResultInterface.TYPE_RESULT_ERROR,
									Messages
											.getString("TransMeta.CheckResult.TypeResultError.LogTableNotSpecified.Description"), null); //$NON-NLS-1$
							remarks.add(cr);
						}
					} catch (KettleDatabaseException dbe) {
			...
			if (monitor != null)
				monitor
						.subTask(Messages
								.getString("TransMeta.Monitor.CheckingForDatabaseUnfriendlyCharactersInFieldNamesTask.Title")); //$NON-NLS-1$
			if (values.size() > 0) {
				...
				CheckResult cr = new CheckResult(
						CheckResultInterface.TYPE_RESULT_OK,
						Messages
								.getString("TransMeta.CheckResult.TypeResultOK.Description"), null); //$NON-NLS-1$
				remarks.add(cr);
			}
			if (monitor != null)
				monitor.worked(1);
	...
	public String getPathAndName() {
		if (getDirectory().isRoot())
			...
			return getDirectory().getPath()
					+ RepositoryDirectory.DIRECTORY_SEPARATOR + getName();
	}
	...
	public Map<String, String> getUsedArguments(String[] arguments) {
		...
		String[] saved = Props.isInitialized() ? Props.getInstance()
				.getLastArguments() : null;
	...
	public boolean isDatabaseConnectionUsed(DatabaseMeta databaseMeta) {
		for (int i = 0; i < nrSteps(); i++) {
			StepMeta stepMeta = getStep(i);
			DatabaseMeta dbs[] = stepMeta.getStepMetaInterface()
	...
	public List<StringSearchResult> getStringList(boolean searchSteps,
		...
		if (searchSteps) {
			...
			for (int i = 0; i < nrSteps(); i++) {
				StepMeta stepMeta = getStep(i);
				stringList.add(new StringSearchResult(stepMeta.getName(),
						stepMeta, this, "Step name"));
				if (stepMeta.getDescription() != null)
					stringList.add(new StringSearchResult(stepMeta
							.getDescription(), stepMeta, this,
							"Step description"));
				StepMetaInterface metaInterface = stepMeta
						.getStepMetaInterface();
				StringSearcher.findMetaData(metaInterface, 1, stringList,
						stepMeta, this);
			}
		}
		...
		if (searchDatabases) {
			for (int i = 0; i < nrDatabases(); i++) {
				...
				stringList.add(new StringSearchResult(meta.getName(), meta,
						this, "Database connection name"));
				if (meta.getHostname() != null)
					stringList.add(new StringSearchResult(meta.getHostname(),
							meta, this, "Database hostname"));
				if (meta.getDatabaseName() != null)
					stringList.add(new StringSearchResult(meta
							.getDatabaseName(), meta, this, "Database name"));
				if (meta.getUsername() != null)
					stringList.add(new StringSearchResult(meta.getUsername(),
							meta, this, "Database Username"));
				if (meta.getDatabaseTypeDesc() != null)
					stringList.add(new StringSearchResult(meta
							.getDatabaseTypeDesc(), meta, this,
							"Database type description"));
				if (meta.getDatabasePortNumberString() != null)
					stringList.add(new StringSearchResult(meta
							.getDatabasePortNumberString(), meta, this,
							"Database port"));
				if (meta.getServername() != null)
					stringList.add(new StringSearchResult(meta.getServername(),
							meta, this, "Database server"));
				if (includePasswords) {
					if (meta.getPassword() != null)
						stringList
								.add(new StringSearchResult(meta.getPassword(),
										meta, this, "Database password"));
				}
			}
		}
		...
		if (searchNotes) {
			for (int i = 0; i < nrNotes(); i++) {
				...
				if (meta.getNote() != null)
					stringList.add(new StringSearchResult(meta.getNote(), meta,
							this, "Notepad text"));
			}
		}
	...
	public boolean saveSharedObjects() {
		try {
			...
			log.logError(toString(), "Unable to save shared ojects: "
					+ e.toString());
			return false;
		}
	}
	...
	public void checkRowMixingStatically(StepMeta stepMeta,
			ProgressMonitorListener monitor) throws KettleRowException {
			...
			for (int i = 0; i < nrPrevious; i++) {
				...
				try {
					...
					if (referenceRow == null) {
						...
						if (!stepMeta.getStepMetaInterface()
	...
	public void setInternalKettleVariables(VariableSpace var) {
		...
		// The name of the transformation
		//
		var.setVariable(Const.INTERNAL_VARIABLE_TRANSFORMATION_NAME, Const.NVL(
				name, ""));
		...
		if (var.getVariable(Const.INTERNAL_VARIABLE_JOB_FILENAME_DIRECTORY) == null) {
			var.setVariable(Const.INTERNAL_VARIABLE_JOB_FILENAME_DIRECTORY,
					"Parent Job File Directory"); //$NON-NLS-1$
		}
		if (var.getVariable(Const.INTERNAL_VARIABLE_JOB_FILENAME_NAME) == null) {
			var.setVariable(Const.INTERNAL_VARIABLE_JOB_FILENAME_NAME,
					"Parent Job Filename"); //$NON-NLS-1$
		}
		if (var.getVariable(Const.INTERNAL_VARIABLE_JOB_NAME) == null) {
			var
					.setVariable(Const.INTERNAL_VARIABLE_JOB_NAME,
							"Parent Job Name"); //$NON-NLS-1$
		}
		...
		if (!Const.isEmpty(stepname)) {
			...
			if (stepMeta == null) {
				throw new KettleStepException(Messages.getString(
						"TransMeta.Exception.StepNameNotFound", stepname));
			}
			return stepMeta;
		} else {
			...
			if (stepMeta == null) {
				throw new KettleStepException(
						Messages
								.getString("TransMeta.Exception.OneMappingInputStepRequired"));
			}
			return stepMeta;
		}
	}
	...
	public StepMeta findMappingOutputStep(String stepname)
		...
		if (!Const.isEmpty(stepname)) {
			...
			if (stepMeta == null) {
				throw new KettleStepException(
						Messages
								.getString("TransMeta.Exception.OneMappingOutputStepRequired"));
			}
			return stepMeta;
		}
	}
	...
	public String exportResources(VariableSpace space,
		...
		try {
			...
			String exportFileName = resourceNamingInterface.nameResource(
					fileObject.getName().getBaseName(), fileObject.getParent()
							.getName().getPath(), "ktr"); //$NON-NLS-1$
			...
			if (definition == null) {
				...
				for (StepMeta stepMeta : transMeta.getSteps()) {
					stepMeta.exportResources(space, definitions,
				...
				definition = new ResourceDefinition(exportFileName,
						transMetaContent);
	 ...
	 */
	public void setCapturingStepPerformanceSnapShots(
			boolean capturingStepPerformanceSnapShots) {
	 ...
	 */
	public void setStepPerformanceCapturingDelay(
			long stepPerformanceCapturingDelay) {
	...
	private void fireFilenameChangedListeners(String oldFilename,
			String newFilename) {
