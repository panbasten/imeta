 ...
 * @since 13-05-2008
 */
public class JobEntryDeleteFolders extends JobEntryBase implements Cloneable,
		JobEntryInterface {
	...
	public String getXML() {
		...
		retval
				.append("      ").append(XMLHandler.addTagValue("success_condition", success_condition)); //$NON-NLS-1$ //$NON-NLS-2$
		retval.append("      ").append(
				XMLHandler.addTagValue("limit_folders", limit_folders));
	...
	public void loadXML(Node entrynode, List<DatabaseMeta> databases,
		...
		try {
			...
			throw new KettleXMLException(Messages
					.getString("JobEntryDeleteFolders.UnableToLoadFromXml"), xe); //$NON-NLS-1$
		}
	}
	...
	public void loadRep(Repository rep, long id_jobentry,
		...
		try {
			...
			limit_folders = rep.getJobEntryAttributeString(id_jobentry,
					"limit_folders");
			...
			throw new KettleException(
					Messages
							.getString(
									"JobEntryDeleteFolders.UnableToLoadFromRepo", String.valueOf(id_jobentry)), dbe); //$NON-NLS-1$
		}
	}
	...
	public void saveRep(Repository rep, long id_job) throws KettleException {
		try {
			...
			rep.saveJobEntryAttribute(id_job, getID(), "limit_folders",
					limit_folders);
			...
			throw new KettleException(
					Messages
							.getString(
									"JobEntryDeleteFolders.UnableToSaveToRepo", String.valueOf(id_job)), dbe); //$NON-NLS-1$
		}
	}
	...
	public Result execute(Result result, int nr, Repository rep, Job parentJob)
		...
		limitFolders = Const
				.toInt(environmentSubstitute(getLimitFolders()), 10);
			...
			if (log.isDetailed())
				log
						.logDetailed(
								...
								Messages
										.getString(
												"JobEntryDeleteFolders.FoundPreviousRows", String.valueOf((rows != null ? rows.size() : 0)))); //$NON-NLS-1$
		}
				...
				if (successConditionBroken) {
					log
							.logError(
									...
									Messages
											.getString(
													"JobEntryDeleteFolders.Error.SuccessConditionbroken",
													"" + NrErrors));
				...
				if (!Const.isEmpty(args_previous)) {
					...
					log
							.logError(
									toString(),
									Messages
											.getString("JobEntryDeleteFolders.Error.EmptyLine"));
				}
			}
		...
		if (log.isDetailed()) {
			...
			log.logDetailed(toString(), Messages.getString(
					"JobEntryDeleteFolders.Log.Info.NrError", "" + NrErrors));
			log.logDetailed(toString(), Messages.getString(
					"JobEntryDeleteFolders.Log.Info.NrDeletedFolders", ""
							+ NrSuccess));
			log.logDetailed(toString(),
					"=======================================");
		}
	...
	private boolean checkIfSuccessConditionBroken() {
		...
		if ((NrErrors > 0 && getSuccessCondition().equals(SUCCESS_IF_NO_ERRORS))
				|| (NrErrors >= limitFolders && getSuccessCondition().equals(
	...
	private boolean getSuccessStatus() {
		...
		if ((NrErrors == 0 && getSuccessCondition()
				...
				|| (NrSuccess >= limitFolders && getSuccessCondition().equals(
						SUCCESS_IF_AT_LEAST_X_FOLDERS_DELETED))
				|| (NrErrors <= limitFolders && getSuccessCondition().equals(
						SUCCESS_IF_ERRORS_LESS))) {
			retval = true;
		}
				...
				if (filefolder.getType() == FileType.FOLDER) {
					...
					if (log.isDetailed())
						log
								.logDetailed(
										...
										Messages
												.getString(
														"JobEntryDeleteFolders.ProcessingFolder", foldername)); //$NON-NLS-1$
					// Delete Files
					...
					if (log.isDetailed())
						log
								.logDetailed(
										...
										Messages
												.getString(
														"JobEntryDeleteFolders.TotalDeleted", foldername, String.valueOf(Nr))); //$NON-NLS-1$
					rcode = true;
				} else {
					...
					log
							.logError(
									toString(),
									Messages
											.getString("JobEntryDeleteFolders.Error.NotFolder"));
				}
			} else {
				...
				if (log.isBasic())
					log
							.logBasic(
									...
									Messages
											.getString(
													"JobEntryDeleteFolders.FolderAlreadyDeleted", foldername)); //$NON-NLS-1$
				rcode = true;
			}
		} catch (IOException e) {
			log
					.logError(
							...
							Messages
									.getString(
											"JobEntryDeleteFolders.CouldNotDelete", foldername, e.getMessage())); //$NON-NLS-1$
		} finally {
			if (filefolder != null) {
				try {
					filefolder.close();
	...
	public void check(List<CheckResultInterface> remarks, JobMeta jobMeta) {
		boolean res = andValidator().validate(this,
				"arguments", remarks, putValidators(notNullValidator())); //$NON-NLS-1$
		...
		if (arguments != null) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				reference.getEntries().add(
						new ResourceEntry(filename, ResourceType.FILE));
			}
		}
		return references;
	}
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		this.success_condition = BaseStepMeta.parameterToString(p.get(id
				+ ".success_condition"));
		this.limit_folders = BaseStepMeta.parameterToString(p.get(id
				+ ".limit_folders"));
		this.arguments = p.get(id + "_foldersToDeleteTable.folderName");
	}
