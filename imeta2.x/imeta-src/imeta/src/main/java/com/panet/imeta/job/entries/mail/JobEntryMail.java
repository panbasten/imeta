...
import static com.panet.imeta.job.entry.validator.JobEntryValidatorUtils.emailValidator;
...
import com.panet.imeta.job.JobEntryResult;
 ...
 * 
 */
public class JobEntryMail extends JobEntryBase implements Cloneable,
		JobEntryInterface {
	...
	public String getXML() {
		...
		retval.append("      ")
				.append(XMLHandler.addTagValue("server", server));
		retval.append("      ").append(XMLHandler.addTagValue("port", port));
		retval.append("      ").append(
				XMLHandler.addTagValue("destination", destination));
		retval.append("      ").append(
				XMLHandler.addTagValue("destinationCc", destinationCc));
		retval.append("      ").append(
				XMLHandler.addTagValue("destinationBCc", destinationBCc));
		retval.append("      ").append(
				XMLHandler.addTagValue("replyto", replyAddress));
		retval.append("      ").append(
				XMLHandler.addTagValue("replytoname", replyName));
		retval.append("      ").append(
				XMLHandler.addTagValue("subject", subject));
		retval.append("      ").append(
				XMLHandler.addTagValue("include_date", includeDate));
		retval.append("      ").append(
				XMLHandler.addTagValue("contact_person", contactPerson));
		retval.append("      ").append(
				XMLHandler.addTagValue("contact_phone", contactPhone));
		retval.append("      ").append(
				XMLHandler.addTagValue("comment", comment));
		retval.append("      ").append(
				XMLHandler.addTagValue("include_files", includingFiles));
		retval.append("      ").append(
				XMLHandler.addTagValue("zip_files", zipFiles));
		retval.append("      ").append(
				XMLHandler.addTagValue("zip_name", zipFilename));
		...
		retval.append("      ").append(
				XMLHandler.addTagValue("only_comment", onlySendComment));
		retval.append("      ").append(
				XMLHandler.addTagValue("use_HTML", useHTML));
		retval.append("      ").append(
				XMLHandler.addTagValue("use_Priority", usePriority));

		retval.append("      ").append(
				XMLHandler.addTagValue("encoding", encoding));
		retval.append("      ").append(
				XMLHandler.addTagValue("priority", priority));
		retval.append("      ").append(
				XMLHandler.addTagValue("importance", importance));

		retval.append("      ").append(
				XMLHandler.addTagValue("secureconnectiontype",
						secureConnectionType));
		retval.append("      ").append(
				XMLHandler.addTagValue("replyToAddresses", replyToAddresses));

		retval.append("      <filetypes>");
		if (fileType != null)
			for (int i = 0; i < fileType.length; i++) {
				retval.append("        ").append(
						XMLHandler.addTagValue("filetype", ResultFile
								.getTypeCode(fileType[i])));
			}
		retval.append("      </filetypes>");
	...
	public void loadXML(Node entrynode, List<DatabaseMeta> databases,
		...
		try {
			...
			setDestinationBCc(XMLHandler.getTagValue(entrynode,
					"destinationBCc"));
			...
			setIncludeDate("Y".equalsIgnoreCase(XMLHandler.getTagValue(
					entrynode, "include_date")));
			setContactPerson(XMLHandler
					.getTagValue(entrynode, "contact_person"));
			setContactPhone(XMLHandler.getTagValue(entrynode, "contact_phone"));
			setComment(XMLHandler.getTagValue(entrynode, "comment"));
			setIncludingFiles("Y".equalsIgnoreCase(XMLHandler.getTagValue(
					entrynode, "include_files")));

			setUsingAuthentication("Y".equalsIgnoreCase(XMLHandler.getTagValue(
					entrynode, "use_auth")));
			setUsingSecureAuthentication("Y".equalsIgnoreCase(XMLHandler
					.getTagValue(entrynode, "use_secure_auth")));
			setAuthenticationUser(XMLHandler
					.getTagValue(entrynode, "auth_user"));
			setAuthenticationPassword(Encr
					.decryptPasswordOptionallyEncrypted(XMLHandler.getTagValue(
							entrynode, "auth_password")));

			setOnlySendComment("Y".equalsIgnoreCase(XMLHandler.getTagValue(
					entrynode, "only_comment")));
			setUseHTML("Y".equalsIgnoreCase(XMLHandler.getTagValue(entrynode,
					"use_HTML")));

			setUsePriority("Y".equalsIgnoreCase(XMLHandler.getTagValue(
					entrynode, "use_Priority")));
			...
			setSecureConnectionType(XMLHandler.getTagValue(entrynode,
					"secureconnectiontype"));
			...
			for (int i = 0; i < nrTypes; i++) {
				...
				fileType[i] = ResultFile.getType(XMLHandler
						.getNodeValue(ftnode));
			}

			setZipFiles("Y".equalsIgnoreCase(XMLHandler.getTagValue(entrynode,
					"zip_files")));
			setZipFilename(XMLHandler.getTagValue(entrynode, "zip_name"));
			setReplyToAddresses(XMLHandler.getTagValue(entrynode,
					"replyToAddresses"));

		} catch (KettleException xe) {
			throw new KettleXMLException(
					"Unable to load job entry of type 'mail' from XML node", xe);
		}
	}
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		server = JobEntryBase.parameterToString(p.get(id + ".server"));
		port = JobEntryBase.parameterToString(p.get(id + ".port"));
		destination = JobEntryBase
				.parameterToString(p.get(id + ".destination"));
		destinationCc = JobEntryBase.parameterToString(p.get(id
				+ ".destinationCc"));
		destinationBCc = JobEntryBase.parameterToString(p.get(id
				+ ".destinationBCc"));
		replyAddress = JobEntryBase.parameterToString(p.get(id
				+ ".replyAddress"));
		replyName = JobEntryBase.parameterToString(p.get(id + ".replyName"));
		subject = JobEntryBase.parameterToString(p.get(id + ".subject"));
		includeDate = JobEntryBase.parameterToBoolean(p
				.get(id + ".includeDate"));
		contactPerson = JobEntryBase.parameterToString(p.get(id
				+ ".contactPerson"));
		contactPhone = JobEntryBase.parameterToString(p.get(id
				+ ".contactPhone"));
		comment = JobEntryBase.parameterToString(p.get(id + ".comment"));
		encoding = JobEntryBase.parameterToString(p.get(id + ".encoding"));
		priority = JobEntryBase.parameterToString(p.get(id + ".priority"));
		importance = JobEntryBase.parameterToString(p.get(id + ".importance"));
		includingFiles = JobEntryBase.parameterToBoolean(p.get(id
				+ ".includingFiles"));
		usingAuthentication = JobEntryBase.parameterToBoolean(p.get(id
				+ ".usingAuthentication"));
		usingSecureAuthentication = JobEntryBase.parameterToBoolean(p.get(id
				+ ".usingSecureAuthentication"));
		authenticationUser = JobEntryBase.parameterToString(p.get(id
				+ ".authenticationUser"));
		authenticationPassword = JobEntryBase.parameterToString(p.get(id
				+ ".authenticationPassword"));
		useHTML = JobEntryBase.parameterToBoolean(p.get(id + ".useHTML"));
		usePriority = JobEntryBase.parameterToBoolean(p
				.get(id + ".usePriority"));
		secureConnectionType = JobEntryBase.parameterToString(p.get(id
				+ ".secureConnectionType"));
		onlySendComment = JobEntryBase.parameterToBoolean(p.get(id
				+ ".onlySendComment"));
		zipFiles = JobEntryBase.parameterToBoolean(p.get(id + ".zipFiles"));
		zipFilename = JobEntryBase
				.parameterToString(p.get(id + ".zipFilename"));
		replyToAddresses = JobEntryBase.parameterToString(p.get(id
				+ ".replyToAddresses"));

		fileType = JobEntryBase.stringArrayToIntArray(p.get(id + ".fileType"));
	}
	...
	public void loadRep(Repository rep, long id_jobentry,
		...
		try {
			...
			// First load the common parts like name & description, then the
			// attributes...
			//
			server = rep.getJobEntryAttributeString(id_jobentry, "server");
			port = rep.getJobEntryAttributeString(id_jobentry, "port");
			destination = rep.getJobEntryAttributeString(id_jobentry,
					"destination");
			destinationCc = rep.getJobEntryAttributeString(id_jobentry,
					"destinationCc");
			destinationBCc = rep.getJobEntryAttributeString(id_jobentry,
					"destinationBCc");
			replyAddress = rep.getJobEntryAttributeString(id_jobentry,
					"replyto");
			replyName = rep.getJobEntryAttributeString(id_jobentry,
					"replytoname");
			subject = rep.getJobEntryAttributeString(id_jobentry, "subject");
			includeDate = rep.getJobEntryAttributeBoolean(id_jobentry,
					"include_date");
			contactPerson = rep.getJobEntryAttributeString(id_jobentry,
					"contact_person");
			contactPhone = rep.getJobEntryAttributeString(id_jobentry,
					"contact_phone");
			...
			importance = rep.getJobEntryAttributeString(id_jobentry,
					"importance");
			includingFiles = rep.getJobEntryAttributeBoolean(id_jobentry,
					"include_files");
			usingAuthentication = rep.getJobEntryAttributeBoolean(id_jobentry,
					"use_auth");
			usingSecureAuthentication = rep.getJobEntryAttributeBoolean(
					id_jobentry, "use_secure_auth");
			authenticationUser = rep.getJobEntryAttributeString(id_jobentry,
					"auth_user");
			authenticationPassword = Encr
					.decryptPasswordOptionallyEncrypted(rep
							.getJobEntryAttributeString(id_jobentry,
									"auth_password"));
			onlySendComment = rep.getJobEntryAttributeBoolean(id_jobentry,
					"only_comment");
			useHTML = rep.getJobEntryAttributeBoolean(id_jobentry, "use_HTML");
			usePriority = rep.getJobEntryAttributeBoolean(id_jobentry,
					"use_Priority");
			secureConnectionType = rep.getJobEntryAttributeString(id_jobentry,
					"secureconnectiontype");

			int nrTypes = rep.countNrJobEntryAttributes(id_jobentry,
					"file_type");
			allocate(nrTypes);

			for (int i = 0; i < nrTypes; i++) {
				String typeCode = rep.getJobEntryAttributeString(id_jobentry,
						i, "file_type");
				fileType[i] = ResultFile.getType(typeCode);
			}

			zipFiles = rep
					.getJobEntryAttributeBoolean(id_jobentry, "zip_files");
			zipFilename = rep.getJobEntryAttributeString(id_jobentry,
					"zip_name");
			replyToAddresses = rep.getJobEntryAttributeString(id_jobentry,
					"replyToAddresses");

		} catch (KettleDatabaseException dbe) {
			throw new KettleException(
					"Unable to load job entry of type 'mail' from the repository with id_jobentry="
							+ id_jobentry, dbe);
		}
	...
	public void saveRep(Repository rep, long id_job) throws KettleException {
		try {
			...
			rep.saveJobEntryAttribute(id_job, getID(), "destination",
					destination);
			rep.saveJobEntryAttribute(id_job, getID(), "destinationCc",
					destinationCc);
			rep.saveJobEntryAttribute(id_job, getID(), "destinationBCc",
					destinationBCc);
			rep.saveJobEntryAttribute(id_job, getID(), "replyto", replyAddress);
			rep
					.saveJobEntryAttribute(id_job, getID(), "replytoname",
							replyName);
			rep.saveJobEntryAttribute(id_job, getID(), "subject", subject);
			rep.saveJobEntryAttribute(id_job, getID(), "include_date",
					includeDate);
			rep.saveJobEntryAttribute(id_job, getID(), "contact_person",
					contactPerson);
			rep.saveJobEntryAttribute(id_job, getID(), "contact_phone",
					contactPhone);
			...
			rep
					.saveJobEntryAttribute(id_job, getID(), "importance",
							importance);

			rep.saveJobEntryAttribute(id_job, getID(), "include_files",
					includingFiles);
			rep.saveJobEntryAttribute(id_job, getID(), "use_auth",
					usingAuthentication);
			rep.saveJobEntryAttribute(id_job, getID(), "use_secure_auth",
					usingSecureAuthentication);
			rep.saveJobEntryAttribute(id_job, getID(), "auth_user",
					authenticationUser);
			rep
			...
			rep.saveJobEntryAttribute(id_job, getID(), "only_comment",
					onlySendComment);
			rep.saveJobEntryAttribute(id_job, getID(), "use_HTML", useHTML);
			rep.saveJobEntryAttribute(id_job, getID(), "use_Priority",
					usePriority);
			rep.saveJobEntryAttribute(id_job, getID(), "secureconnectiontype",
					secureConnectionType);

			if (fileType != null) {
				for (int i = 0; i < fileType.length; i++) {
					rep.saveJobEntryAttribute(id_job, getID(), i, "file_type",
			...
			rep.saveJobEntryAttribute(id_job, getID(), "replyToAddresses",
					replyToAddresses);
	...
	public Result execute(Result result, int nr, Repository rep, Job parentJob) {
		...
		if (Const.isEmpty(server)) {
			log.logError(toString(), Messages
					.getString("JobMail.Error.HostNotSpecified"));
		...
		if (!Const.isEmpty(port))
			props
					.put("mail." + protocol + ".port",
							environmentSubstitute(port));
		...
		try {
			...
			if (!Const.isEmpty(sender_address)) {
				...
				throw new MessagingException(Messages
						.getString("JobMail.Error.ReplyEmailNotFilled"));
			}
			...
			if (!Const.isEmpty(reply_to_address)) {
				...
				String[] reply_Address_List = environmentSubstitute(
						reply_to_address).split(" ");
			...
			if (!Const.isEmpty(destinationCc)) {
				...
				String destinationsCc[] = environmentSubstitute(destinationCc)
						.split(" ");
			...
			if (!Const.isEmpty(destinationBCc)) {
				...
				String destinationsBCc[] = environmentSubstitute(destinationBCc)
						.split(" ");
			...
			if (comment != null) {
				messageText.append(environmentSubstitute(comment)).append(
						Const.CR).append(Const.CR);
			}
			if (!onlySendComment) {

				messageText.append(
						Messages.getString("JobMail.Log.Comment.Job")).append(
						Const.CR);
				messageText.append("-----").append(Const.CR);
				messageText.append(
						Messages.getString("JobMail.Log.Comment.JobName")
								+ "    : ").append(
						parentJob.getJobMeta().getName()).append(Const.CR);
				messageText.append(
						Messages.getString("JobMail.Log.Comment.JobDirectory")
								+ "  : ").append(
						parentJob.getJobMeta().getDirectory()).append(Const.CR);
				messageText.append(
						Messages.getString("JobMail.Log.Comment.JobEntry")
								+ "   : ").append(getName()).append(Const.CR);
				messageText.append(Const.CR);
			}

			if (includeDate) {
				messageText.append(Const.CR).append(
						Messages.getString("JobMail.Log.Comment.MsgDate")
								+ ": ").append(
						XMLHandler.date2string(new Date())).append(Const.CR)
						.append(Const.CR);
			}
			if (!onlySendComment && result != null) {
				messageText
						.append(
								Messages
										.getString("JobMail.Log.Comment.PreviousResult")
										+ ":").append(Const.CR);
				messageText.append("-----------------").append(Const.CR);
				messageText.append(
						Messages.getString("JobMail.Log.Comment.JobEntryNr")
								+ "         : ").append(result.getEntryNr())
						.append(Const.CR);
				messageText.append(
						Messages.getString("JobMail.Log.Comment.Errors")
								+ "               : ").append(
						result.getNrErrors()).append(Const.CR);
				messageText.append(
						Messages.getString("JobMail.Log.Comment.LinesRead")
								+ "           : ").append(
						result.getNrLinesRead()).append(Const.CR);
				messageText.append(
						Messages.getString("JobMail.Log.Comment.LinesWritten")
								+ "        : ").append(
						result.getNrLinesWritten()).append(Const.CR);
				messageText.append(
						Messages.getString("JobMail.Log.Comment.LinesInput")
								+ "          : ").append(
						result.getNrLinesInput()).append(Const.CR);
				messageText.append(
						Messages.getString("JobMail.Log.Comment.LinesOutput")
								+ "         : ").append(
						result.getNrLinesOutput()).append(Const.CR);
				messageText.append(
						Messages.getString("JobMail.Log.Comment.LinesUpdated")
								+ "        : ").append(
						result.getNrLinesUpdated()).append(Const.CR);
				messageText.append(
						Messages.getString("JobMail.Log.Comment.Status")
								+ "  : ").append(result.getExitStatus())
						.append(Const.CR);
				messageText.append(
						Messages.getString("JobMail.Log.Comment.Result")
								+ "               : ").append(
						result.getResult()).append(Const.CR);
				messageText.append(Const.CR);
			}
			...
			if (!onlySendComment
				...
				messageText.append(
						Messages.getString("JobMail.Log.Comment.ContactInfo")
								+ " :").append(Const.CR);
				messageText.append("---------------------").append(Const.CR);
				messageText
						.append(
								Messages
										.getString("JobMail.Log.Comment.PersonToContact")
										+ " : ").append(
								environmentSubstitute(contactPerson)).append(
								Const.CR);
				messageText.append(
						Messages.getString("JobMail.Log.Comment.Tel") + "  : ")
						.append(environmentSubstitute(contactPhone)).append(
								Const.CR);
				messageText.append(Const.CR);
			}
			...
			if (!onlySendComment) {
				...
				if (jobTracker != null) {
					messageText
							.append(
									Messages
											.getString("JobMail.Log.Comment.PathToJobentry")
											+ ":").append(Const.CR);
			...
			if (useHTML) {
				if (!Const.isEmpty(getEncoding())) {
					part1.setContent(messageText.toString(), "text/html; "
							+ "charset=" + getEncoding());
				} else {
					part1.setContent(messageText.toString(), "text/html; "
							+ "charset=ISO-8859-1");
						...
						for (ResultFile resultFile : resultFiles) {
							...
							if (file != null && file.exists()) {
								...
								if (found) {
									...
									URLDataSource fds = new URLDataSource(file
											.getURL());
									...
									log.logBasic(toString(), "Added file '"
											+ fds.getName()
											+ "' to the mail message.");
								}
							}
			...
			try {
				...
				if (usingAuthentication) {
					if (!Const.isEmpty(port)) {
						transport.connect(environmentSubstitute(Const.NVL(
								server, "")), Integer
								.parseInt(environmentSubstitute(Const.NVL(port,
										""))), environmentSubstitute(Const.NVL(
								authenticationUser, "")),
								environmentSubstitute(Const.NVL(
										authenticationPassword, "")));
					} else {
						transport.connect(environmentSubstitute(Const.NVL(
								server, "")), environmentSubstitute(Const.NVL(
								authenticationUser, "")),
								environmentSubstitute(Const.NVL(
										authenticationPassword, "")));
					}
			...
			log.logError(toString(), "Problem while sending message: "
					+ e.toString());
			result.setNrErrors(1);
	...
	private void addBacktracking(JobTracker jobTracker,
			StringBuffer messageText, int level) {
		...
		if (jer != null) {
			messageText.append(" : ");
			if (jer.getJobEntry() != null
					&& jer.getJobEntry().getName() != null) {
				messageText.append(" : ");
			...
			if (jer.getLogDate() != null) {
				messageText.append(" (");
				messageText.append(XMLHandler.date2string(jer.getLogDate())); // $NON
																				// -
																				// NLS
																				// -
																				// 1
	...
	public void check(List<CheckResultInterface> remarks, JobMeta jobMeta) {
		...
		andValidator().validate(this,
				"server", remarks, putValidators(notBlankValidator())); //$NON-NLS-1$
		andValidator()
				.validate(
						this,
						"replyAddress", remarks, putValidators(notBlankValidator(), emailValidator())); //$NON-NLS-1$

		andValidator().validate(this,
				"destination", remarks, putValidators(notBlankValidator())); //$NON-NLS-1$

		if (usingAuthentication) {
			andValidator()
					.validate(
							this,
							"authenticationUser", remarks, putValidators(notBlankValidator())); //$NON-NLS-1$
			andValidator()
					.validate(
							this,
							"authenticationPassword", remarks, putValidators(notNullValidator())); //$NON-NLS-1$
		}

		andValidator().validate(this,
				"port", remarks, putValidators(integerValidator())); //$NON-NLS-1$
