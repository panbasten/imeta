...
public class XMLJoin extends BaseStep implements StepInterface {
	...
	public XMLJoin(StepMeta stepMeta, StepDataInterface stepDataInterface,
	...
	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi)
		...
		// if first row we do some initializing and process the first row of the
		// target XML Step
		if (first) {
			first = false;
			...
			if (data.TargetRowSet != null
					&& data.TargetRowSet.getRowMeta() != null) {
				String[] target_field_names = data.TargetRowSet.getRowMeta()
						.getFieldNames();
				if (target_field_names != null && target_field_names.length > 0) {
					for (int i = 0; i < target_field_names.length; i++) {
						if (meta.getTargetXMLfield().equals(
								target_field_names[i])) {
							target_field_id = i;
						}
					}
				}
			...
			if (target_field_id == -1)
				throw new KettleException(Messages.getString(
						"XMLJoin.Exception.FieldNotFound", meta
								.getTargetXMLfield()));

			data.outputRowMeta = data.TargetRowSet.getRowMeta().clone();
			meta.getFields(data.outputRowMeta, getStepname(),
					new RowMetaInterface[] { data.TargetRowSet.getRowMeta() },
					null, this);
			...
			try {
				...
				if (!meta.isComplexJoin()) {
					data.targetNode = (Node) xpath.evaluate(
							data.XPathStatement, data.targetDOM,
							XPathConstants.NODE);
					if (data.targetNode == null) {
						throw new KettleXMLException(
								"XPath statement returned no reuslt ["
										+ data.XPathStatement + "]");
					}
				}
			} catch (Exception e) {
		...
		Object[] rJoinSource = getRowFrom(data.SourceRowSet); // This also
		// waits for a
		// row to be
		// finished.
		...
		if (rJoinSource == null) // no more input to be expected...
		{
			...
			try {
				...
				putRow(data.outputRowMeta, RowDataUtil.addValueData(
						data.outputRowData, outputIndex, output));
				// finishing up
				setOutputDone();
				return false;
			} catch (Exception e) {
		...
		if (data.iSourceXMLField == -1) {
			...
			// get target xml
			String[] source_field_names = data.SourceRowSet.getRowMeta()
					.getFieldNames();
		...
		if (rJoinSource != null) {
			...
			try {
				...
				joinDocument = builder.parse(new InputSource(new StringReader(
						strJoinXML)));
			} catch (Exception e) {
				throw new KettleException(e);
			}

			Node node = data.targetDOM.importNode(joinDocument
					.getDocumentElement(), true);

			if (meta.isComplexJoin()) {
				String strCompareValue = rJoinSource[data.iCompareFieldID]
						.toString();
				String strXPathStatement = data.XPathStatement.replace("?",
						strCompareValue);

				try {
					data.targetNode = (Node) xpath.evaluate(strXPathStatement,
							data.targetDOM, XPathConstants.NODE);
					if (data.targetNode == null) {
						throw new KettleXMLException(
								"XPath statement returned no reuslt ["
										+ strXPathStatement + "]");
					} else {
						appendChildByRoot(node);
					}
				} catch (Exception e) {
					throw new KettleException(e);
				}
	...
	private void appendChildByRoot(Node node) {
		if (meta.isSourceNotIncludeRoot()) {
	...
	public boolean init(StepMetaInterface smi, StepDataInterface sdi) {
		...
		try {
			...
			// See if a main step is supplied: in that case move the
			// corresponding rowset to position 0
			for (int i = 0; i < inputRowSets.size(); i++) {
				RowSet rs = (RowSet) inputRowSets.get(i);
				if (rs.getOriginStepName().equalsIgnoreCase(
						meta.getTargetXMLstep())) {
