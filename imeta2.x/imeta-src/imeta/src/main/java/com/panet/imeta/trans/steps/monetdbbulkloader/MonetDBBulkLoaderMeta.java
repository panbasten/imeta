 ...
 * @author Sven Boden
 */
public class MonetDBBulkLoaderMeta extends BaseStepMeta implements
		StepMetaInterface {
	...
	private void readData(Node stepnode,
		...
		try {
			...
			throw new KettleXMLException(
					Messages
							.getString("MonetDBBulkLoaderMeta.Exception.UnableToReadStepInfoFromXML"), e); //$NON-NLS-1$
		}
	}
	...
	public void setDefault() {
		...
		tableName = Messages
				.getString("MonetDBBulkLoaderMeta.DefaultTableName"); //$NON-NLS-1$
	...
	public String getXML() {
		...
		retval
				.append("    ").append(XMLHandler.addTagValue("mclient_path", mClientPath)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
	...
	public void readRep(Repository rep, long id_step,
		...
		try {
			...
			throw new KettleException(
					Messages
							.getString("MonetDBBulkLoaderMeta.Exception.UnexpectedErrorReadingStepInfoFromRepository"), e); //$NON-NLS-1$
		}
	}
	...
	public void saveRep(Repository rep, long id_transformation, long id_step)
		...
		try {
			...
			rep.saveStepAttribute(id_transformation, id_step,
					"mclient_path", mClientPath); //$NON-NLS-1$
			rep.saveStepAttribute(id_transformation, id_step,
			...
			throw new KettleException(
					Messages
							.getString("MonetDBBulkLoaderMeta.Exception.UnableToSaveStepInfoToRepository") + id_step, e); //$NON-NLS-1$
		}
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (databaseMeta != null) {
			...
			try {
				...
				if (!Const.isEmpty(tableName)) {
					cr = new CheckResult(
							...
							Messages
									.getString("MonetDBBulkLoaderMeta.CheckResult.TableNameOK"), stepMeta); //$NON-NLS-1$
					remarks.add(cr);
					...
					if (r != null) {
						cr = new CheckResult(
								CheckResultInterface.TYPE_RESULT_OK,
								Messages
										.getString("MonetDBBulkLoaderMeta.CheckResult.TableExists"), stepMeta); //$NON-NLS-1$
						remarks.add(cr);
							...
							if (v == null) {
								if (first) {
									...
									error_message += Messages
											.getString("MonetDBBulkLoaderMeta.CheckResult.MissingFieldsToLoadInTargetTable") + Const.CR; //$NON-NLS-1$
								}
								error_found = true;
						...
						if (error_found) {
							...
							cr = new CheckResult(
									CheckResultInterface.TYPE_RESULT_OK,
									Messages
											.getString("MonetDBBulkLoaderMeta.CheckResult.AllFieldsFoundInTargetTable"), stepMeta); //$NON-NLS-1$
						}
						remarks.add(cr);
					} else {
						error_message = Messages
								.getString("MonetDBBulkLoaderMeta.CheckResult.CouldNotReadTableInfo"); //$NON-NLS-1$
						cr = new CheckResult(
								CheckResultInterface.TYPE_RESULT_ERROR,
								error_message, stepMeta);
						remarks.add(cr);
				...
				if (prev != null && prev.size() > 0) {
					cr = new CheckResult(
							...
							Messages
									.getString(
											"MonetDBBulkLoaderMeta.CheckResult.StepReceivingDatas", prev.size() + ""), stepMeta); //$NON-NLS-1$ //$NON-NLS-2$
					remarks.add(cr);
					...
					for (int i = 0; i < fieldStream.length; i++) {
						...
						if (v == null) {
							if (first) {
								first = false;
								error_message += Messages
										.getString("MonetDBBulkLoaderMeta.CheckResult.MissingFieldsInInput") + Const.CR; //$NON-NLS-1$
							}
							error_found = true;
					...
					if (error_found) {
						...
						cr = new CheckResult(
								CheckResultInterface.TYPE_RESULT_OK,
								Messages
										.getString("MonetDBBulkLoaderMeta.CheckResult.AllFieldsFoundInInput"), stepMeta); //$NON-NLS-1$
					}
					remarks.add(cr);
				} else {
					error_message = Messages
							.getString("MonetDBBulkLoaderMeta.CheckResult.MissingFieldsInInput3") + Const.CR; //$NON-NLS-1$
					cr = new CheckResult(
							CheckResultInterface.TYPE_RESULT_ERROR,
							error_message, stepMeta);
					remarks.add(cr);
				...
				error_message = Messages
						.getString("MonetDBBulkLoaderMeta.CheckResult.DatabaseErrorOccurred") + e.getMessage(); //$NON-NLS-1$
			...
			error_message = Messages
					.getString("MonetDBBulkLoaderMeta.CheckResult.InvalidConnection"); //$NON-NLS-1$
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR,
					error_message, stepMeta);
			remarks.add(cr);
		}
		...
		if (input.length > 0) {
			cr = new CheckResult(
					CheckResultInterface.TYPE_RESULT_OK,
					Messages
							.getString("MonetDBBulkLoaderMeta.CheckResult.StepReceivingInfoFromOtherSteps"), stepMeta); //$NON-NLS-1$
			remarks.add(cr);
		} else {
			cr = new CheckResult(
					CheckResultInterface.TYPE_RESULT_ERROR,
					Messages
							.getString("MonetDBBulkLoaderMeta.CheckResult.NoInputError"), stepMeta); //$NON-NLS-1$
			remarks.add(cr);
		}
	}
			...
			if (prev != null && prev.size() > 0) {
				...
				if (!Const.isEmpty(tableName)) {
					...
					try {
						...
						retval
								.setError(Messages
										.getString("MonetDBBulkLoaderMeta.GetSQL.ErrorOccurred") + e.getMessage()); //$NON-NLS-1$
					}
				} else {
					retval
							.setError(Messages
									.getString("MonetDBBulkLoaderMeta.GetSQL.NoTableDefinedOnConnection")); //$NON-NLS-1$
				}
			} else {
				retval
						.setError(Messages
								.getString("MonetDBBulkLoaderMeta.GetSQL.NotReceivingAnyFields")); //$NON-NLS-1$
			}
		} else {
			retval
					.setError(Messages
							.getString("MonetDBBulkLoaderMeta.GetSQL.NoConnectionDefined")); //$NON-NLS-1$
		}
	...
	public StepInterface getStep(StepMeta stepMeta,
			...
			Trans trans) {
		return new MonetDBBulkLoader(stepMeta, stepDataInterface, cnr,
				transMeta, trans);
	}
	...
	public RowMetaInterface getRequiredFields() throws KettleException {
		if (databaseMeta != null) {
			...
			try {
				...
				if (!Const.isEmpty(tableName)) {
					...
					if (db.checkTableExists(schemaTable)) {
						...
						throw new KettleException(
								Messages
										.getString("MonetDBBulkLoaderMeta.Exception.TableNotFound"));
					}
				} else {
					throw new KettleException(
							Messages
									.getString("MonetDBBulkLoaderMeta.Exception.TableNotSpecified"));
				}
			} catch (Exception e) {
				throw new KettleException(
						Messages
								.getString("MonetDBBulkLoaderMeta.Exception.ErrorGettingFields"),
						e);
			} finally {
				db.disconnect();
			}
			...
			throw new KettleException(
					Messages
							.getString("MonetDBBulkLoaderMeta.Exception.ConnectionNotDefined"));
		}
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		mClientPath = BaseStepMeta
				.parameterToString(p.get(id + ".mClientPath"));
		logFile = BaseStepMeta.parameterToString(p.get(id + ".logFile"));
		encoding = BaseStepMeta.parameterToString(p.get(id + ".encoding"));

		String[] fieldTable = p.get(id + "_fieldsToLoad.fieldTable");
		String[] fieldStream = p.get(id + "_fieldsToLoad.fieldStream");
		String[] fieldFormatOk = p.get(id + "_fieldsToLoad.fieldFormatOk");

		this.fieldTable = (fieldTable != null) ? fieldTable : (new String[0]);
		this.fieldStream = (fieldStream != null) ? fieldStream
				: (new String[0]);
		this.fieldFormatOk = (fieldFormatOk != null) ? stringArrayToBooleanArray(
				fieldFormatOk, false)
				: (new boolean[0]);
	}
