...
//	 keyField = rep.getStepAttributeString(id_step, "key_field"); //$NON-NLS-1$
    ...
    public void setInfo(Map<String, String[]> p, String id,
		...
		String[] groupField = p.get(id + "_consistwords.groupField");

		this.groupField = (groupField != null) ? groupField : (new String[0]);
		
		String[] fieldName = p.get(id + "_obwords.fieldName");
		String[] keyValue = p.get(id + "_obwords.keyValue");
		String[] targetName = p.get(id + "_obwords.targetName");
		String[] targetType = p.get(id + "_obwords.targetType");
		String[] targetFormat = p.get(id + "_obwords.targetFormat");
		String[] targetLength = p.get(id + "_obwords.targetLength");
		String[] targetPrecision = p.get(id + "_obwords.targetPrecision");
		String[] targetDecimalSymbol = p.get(id + "_obwords.targetDecimalSymbol");
		String[] targetGroupingSymbol = p.get(id + "_obwords.targetGroupingSymbol");
		String[] targetCurrencySymbol = p.get(id + "_obwords.targetCurrencySymbol");
		String[] targetNullString = p.get(id + "_obwords.targetNullString");
		String[] targetAggregationType = p.get(id + "_obwords.targetAggregationType");
		
		if (fieldName != null && fieldName.length > 0) {
 			this.denormaliserTargetField = new DenormaliserTargetField[fieldName.length];
 			for (int i = 0; i < fieldName.length; i++) {
			 denormaliserTargetField[i] = new DenormaliserTargetField();
			 denormaliserTargetField[i].setFieldName(fieldName[i]); 
			 denormaliserTargetField[i].setKeyValue(keyValue[i]); 
			 denormaliserTargetField[i].setTargetName(targetName[i]); 
			 denormaliserTargetField[i].setTargetType(Const.toInt(targetType[i],0));
			 denormaliserTargetField[i].setTargetFormat(targetFormat[i]); 
			 denormaliserTargetField[i].setTargetLength(Const.toInt(targetLength[i],-1));
			 denormaliserTargetField[i].setTargetPrecision(Const.toInt(targetPrecision[i],-1));
			 denormaliserTargetField[i].setTargetDecimalSymbol(targetDecimalSymbol[i]); 
			 denormaliserTargetField[i].setTargetGroupingSymbol(targetGroupingSymbol[i]); 
			 denormaliserTargetField[i].setTargetCurrencySymbol(targetCurrencySymbol[i]); 
			 denormaliserTargetField[i].setTargetNullString(targetNullString[i]); 
			 denormaliserTargetField[i].setTargetAggregationType(Const.toInt(targetAggregationType[i],-1));
         }
		}else {
			this.denormaliserTargetField = new DenormaliserTargetField[0];
		}
	}
