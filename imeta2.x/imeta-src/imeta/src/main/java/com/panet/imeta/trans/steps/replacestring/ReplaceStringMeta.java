...
public class ReplaceStringMeta extends BaseStepMeta implements
		StepMetaInterface {
	...
	private void readData(Node stepnode,
		...
		try {
			...
			for (int i = 0; i < nrkeys; i++) {
				...
				useRegEx[i] = getCaseSensitiveByCode(Const.NVL(XMLHandler
						.getTagValue(fnode, "use_regex"), ""));
				replaceString[i] = Const.NVL(XMLHandler.getTagValue(fnode,
						"replace_string"), ""); //$NON-NLS-1$
				replaceByString[i] = Const.NVL(XMLHandler.getTagValue(fnode,
						"replace_by_string"), ""); //$NON-NLS-1$
				wholeWord[i] = getWholeWordByCode(Const.NVL(XMLHandler
						.getTagValue(fnode, "whole_word"), ""));
				caseSensitive[i] = getCaseSensitiveByCode(Const.NVL(XMLHandler
						.getTagValue(fnode, "case_sensitive"), ""));

			}
		} catch (Exception e) {
			throw new KettleXMLException(
					Messages
							.getString("ReplaceStringMeta.Exception.UnableToReadStepInfoFromXML"), e); //$NON-NLS-1$
		}
	}
	...
	public String getXML() {
		...
		for (int i = 0; i < fieldInStream.length; i++) {
			...
			retval.append("        ").append(
					XMLHandler.addTagValue("use_regex",
							getUseRegExCode(useRegEx[i])));
			retval
					.append("        ").append(XMLHandler.addTagValue("replace_string", replaceString[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval.append("        ").append(
					XMLHandler.addTagValue("replace_by_string",
							replaceByString[i]));
			retval.append("        ").append(
					XMLHandler.addTagValue("whole_word",
							getWholeWordCode(wholeWord[i])));
			retval.append("        ").append(
					XMLHandler.addTagValue("case_sensitive",
							getCaseSensitiveCode(caseSensitive[i])));
			retval.append("      </field>").append(Const.CR); //$NON-NLS-1$
		}
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		String[] fieldInStream = p.get(id + "_fields.fieldInStream");
		String[] fieldOutStream = p.get(id + "_fields.fieldOutStream");
		String[] useRegEx = p.get(id + "_fields.useRegEx");
		String[] replaceString = p.get(id + "_fields.replaceString");
		String[] replaceByString = p.get(id + "_fields.replaceByString");
		String[] wholeWord = p.get(id + "_fields.wholeWord");
		String[] caseSensitive = p.get(id + "_fields.caseSensitive");

		this.fieldInStream = (fieldInStream != null) ? fieldInStream : (new String[0]);
		this.fieldOutStream = (fieldOutStream != null) ? fieldOutStream : (new String[0]);
		this.useRegEx = stringArrayToIntArray(useRegEx, 0);
		this.replaceString = (replaceString != null) ? replaceString : (new String[0]);
		this.replaceByString = (replaceByString != null) ? replaceByString : (new String[0]);
		this.wholeWord = stringArrayToIntArray(wholeWord, 0);
		this.caseSensitive = stringArrayToIntArray(caseSensitive, 0);
	}
		...
		try {
			...
			for (int i = 0; i < nrkeys; i++) {
				...
				fieldOutStream[i] = Const.NVL(rep.getStepAttributeString(
						id_step, i, "out_stream_name"), ""); //$NON-NLS-1$
				useRegEx[i] = getCaseSensitiveByCode(Const.NVL(rep
						.getStepAttributeString(id_step, i, "use_regex"), ""));
				replaceString[i] = Const.NVL(rep.getStepAttributeString(
						id_step, i, "replace_string"), "");
				replaceByString[i] = Const.NVL(rep.getStepAttributeString(
						id_step, i, "replace_by_string"), "");
				wholeWord[i] = getWholeWordByCode(Const.NVL(rep
						.getStepAttributeString(id_step, i, "whole_world"), ""));
				caseSensitive[i] = getCaseSensitiveByCode(Const.NVL(rep
						.getStepAttributeString(id_step, i, "case_sensitive"),
						""));
			...
			throw new KettleException(
					Messages
							.getString("ReplaceStringMeta.Exception.UnexpectedErrorInReadingStepInfo"), e); //$NON-NLS-1$
		}
	}
	...
	public void saveRep(Repository rep, long id_transformation, long id_step)
		...
		try {
			for (int i = 0; i < fieldInStream.length; i++) {
				...
				rep.saveStepAttribute(id_transformation, id_step, i,
						"use_regex", getUseRegExCode(useRegEx[i]));
				rep.saveStepAttribute(id_transformation, id_step, i,
						"replace_string", replaceString[i]); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i,
						"replace_by_string", replaceByString[i]);
				rep.saveStepAttribute(id_transformation, id_step, i,
						"whole_world", getWholeWordCode(wholeWord[i]));
				rep.saveStepAttribute(id_transformation, id_step, i,
						"case_sensitive",
						getCaseSensitiveCode(caseSensitive[i]));
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (prev == null) {
			error_message += Messages
					.getString("ReplaceStringMeta.CheckResult.NoInputReceived") + Const.CR; //$NON-NLS-1$
			...
			for (int i = 0; i < fieldInStream.length; i++) {
				...
				if (v == null) {
					if (first) {
						first = false;
						error_message += Messages
								.getString("ReplaceStringMeta.CheckResult.MissingInStreamFields") + Const.CR; //$NON-NLS-1$
					}
					error_found = true;
			...
			if (error_found) {
				...
				cr = new CheckResult(
						...
						Messages
								.getString("ReplaceStringMeta.CheckResult.FoundInStreamFields"), stepinfo); //$NON-NLS-1$
			}
			remarks.add(cr);
				...
				if (v != null) {
					if (v.getType() != ValueMeta.TYPE_STRING) {
						if (first) {
							first = false;
							error_message += Messages
									.getString("ReplaceStringMeta.CheckResult.OperationOnNonStringFields") + Const.CR; //$NON-NLS-1$
						}
						error_found = true;
						error_message += "\t\t" + field + Const.CR; //$NON-NLS-1$
					}
			...
			if (error_found) {
				...
				cr = new CheckResult(
						CheckResult.TYPE_RESULT_OK,
						Messages
								.getString("ReplaceStringMeta.CheckResult.AllOperationsOnStringFields"), stepinfo); //$NON-NLS-1$
			}
			remarks.add(cr);
				...
				for (int idx = 0; idx < fieldInStream.length; idx++) {
					if (Const.isEmpty(fieldInStream[idx])) {
						cr = new CheckResult(
								...
								Messages
										.getString(
												"ReplaceStringMeta.CheckResult.InStreamFieldMissing", new Integer(idx + 1).toString()), stepinfo); //$NON-NLS-1$
						remarks.add(cr);
			...
			for (int idx = 0; idx < fieldInStream.length; idx++) {
				for (int jdx = 0; jdx < fieldInStream.length; jdx++) {
					if (fieldInStream[idx].equals(fieldInStream[jdx])
						...
						error_message = Messages
								.getString(
										"ReplaceStringMeta.CheckResult.FieldInputError", fieldInStream[idx]); //$NON-NLS-1$
						cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR,
								error_message, stepinfo);
						remarks.add(cr);
					}
	...
	public StepInterface getStep(StepMeta stepMeta,
			...
			Trans trans) {
		return new ReplaceString(stepMeta, stepDataInterface, cnr, transMeta,
				trans);
	}
