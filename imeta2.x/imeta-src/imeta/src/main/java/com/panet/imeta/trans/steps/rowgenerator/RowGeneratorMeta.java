 ...
 */
@Step(name = "RowGenerator", image = "GEN.png", tooltip = "BaseStep.TypeTooltipDesc.GenerateRows", description = "BaseStep.TypeLongDesc.GenerateRows", category = StepCategory.CATEGORY_INPUT)
public class RowGeneratorMeta extends BaseStepMeta implements StepMetaInterface {
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		this.fieldType = (fieldType != null) ? fieldType : (new String[0]);
		this.fieldFormat = (fieldFormat != null) ? fieldFormat
				: (new String[0]);
		this.fieldLength = (fieldLength != null) ? stringArrayToIntArray(
				fieldLength, -1) : (new int[0]);
		this.fieldPrecision = (fieldPrecision != null) ? stringArrayToIntArray(
				fieldPrecision, -1) : (new int[0]);
		this.currency = (fieldCurrency != null) ? fieldCurrency
				: (new String[0]);
		this.decimal = (fieldDecimal != null) ? fieldDecimal : (new String[0]);
		this.group = (fieldGroup != null) ? fieldGroup : (new String[0]);
		this.value = (fieldNullif != null) ? fieldNullif : (new String[0]);
	}
	...
	public void getFields(RowMetaInterface row, String origin,
		...
		RowMetaAndData rowMetaAndData = RowGenerator.getInstance().buildRow(
				this, remarks, origin);
		if (!remarks.isEmpty()) {
	...
	public void readRep(Repository rep, long id_step,
		...
		try {
			...
			for (int i = 0; i < nrfields; i++) {
				...
				group[i] = rep.getStepAttributeString(id_step, i,
			...
			long longLimit = rep.getStepAttributeInteger(id_step,
					STEP_ATTRIBUTE_LIMIT);
			if (longLimit <= 0) {
				rowLimit = rep.getStepAttributeString(id_step,
						STEP_ATTRIBUTE_LIMIT);
			} else {
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (prev != null && prev.size() > 0) {
			cr = new CheckResult(
					...
					Messages
							.getString("RowGeneratorMeta.CheckResult.NoInputStreamsError"),
					stepMeta);
			remarks.add(cr);
		} else {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, Messages
					.getString("RowGeneratorMeta.CheckResult.NoInputStreamOk"),
					stepMeta);
			remarks.add(cr);
			...
			if (Const.toLong(strLimit, -1L) <= 0) {
				cr = new CheckResult(
						...
						Messages
								.getString("RowGeneratorMeta.CheckResult.WarnNoRows"),
						stepMeta);
				remarks.add(cr);
			} else {
		...
		if (input.length > 0) {
			cr = new CheckResult(
					CheckResultInterface.TYPE_RESULT_ERROR,
					Messages
							.getString("RowGeneratorMeta.CheckResult.NoInputError"),
					stepMeta);
			remarks.add(cr);
		} else {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, Messages
					.getString("RowGeneratorMeta.CheckResult.NoInputOk"),
					stepMeta);
			remarks.add(cr);
		}
	}
