...
public class CalculatorMeta extends BaseStepMeta implements StepMetaInterface
{
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		String[] calcType = p.get(id + "_fields.calcType");
		String[] fieldA = p.get(id + "_fields.fieldA");
		String[] fieldB = p.get(id + "_fields.fieldB");
		String[] fieldC = p.get(id + "_fields.fieldC");
		...
		String[] removedFromResult = p.get(id + "_fields.removedFromResult");
		String[] conversionMask = p.get(id + "_fields.conversionMask");
		String[] decimalSymbol = p.get(id + "_fields.decimalSymbol");
		String[] groupingSymbol = p.get(id + "_fields.groupingSymbol");
		String[] currencySymbol = p.get(id + "_fields.currencySymbol");
        
		if (fieldName != null && fieldName.length > 0) {
			this.calculation = new CalculatorMetaFunction[fieldName.length];
			for (int i = 0; i < fieldName.length; i++) {
				this.calculation[i] = new CalculatorMetaFunction();
				this.calculation[i].setFieldName(fieldName[i]);
				this.calculation[i].setCalcType(Const.toInt(calcType[i], -1));
				this.calculation[i].setFieldA(fieldA[i]);
				this.calculation[i].setFieldB(fieldB[i]);
				this.calculation[i].setFieldC(fieldC[i]);
				this.calculation[i].setValueType(Const.toInt(valueType[i], 0));
				this.calculation[i].setValueLength(Const.toInt(valueLength[i], -1));
				this.calculation[i].setValuePrecision(Const.toInt(valuePrecision[i], -1));
				this.calculation[i].setRemovedFromResult(BaseStepMeta
							.parameterToBoolean(removedFromResult[i]));
				this.calculation[i].setConversionMask(conversionMask[i]);
				this.calculation[i].setDecimalSymbol(decimalSymbol[i]);
				this.calculation[i].setGroupingSymbol(groupingSymbol[i]);
				this.calculation[i].setCurrencySymbol(currencySymbol[i]);
			}
		}else {
