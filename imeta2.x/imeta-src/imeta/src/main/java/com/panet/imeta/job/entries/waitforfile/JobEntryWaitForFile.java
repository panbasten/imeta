 ...
 * 
 */
public class JobEntryWaitForFile extends JobEntryBase implements Cloneable,
		JobEntryInterface {
	...
	public String getXML() {
		...
		retval.append("      ").append(
				XMLHandler.addTagValue("maximum_timeout", maximumTimeout));
		retval.append("      ").append(
				XMLHandler.addTagValue("check_cycle_time", checkCycleTime));
		retval.append("      ").append(
				XMLHandler.addTagValue("success_on_timeout", successOnTimeout));
		retval.append("      ").append(
				XMLHandler.addTagValue("file_size_check", fileSizeCheck));
		retval.append("      ").append(
				XMLHandler.addTagValue("add_filename_result",
						addFilenameToResult));
	...
	public void loadXML(Node entrynode, List<DatabaseMeta> databases,
		...
		try {
			...
			maximumTimeout = XMLHandler.getTagValue(entrynode,
					"maximum_timeout");
			checkCycleTime = XMLHandler.getTagValue(entrynode,
					"check_cycle_time");
			successOnTimeout = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					entrynode, "success_on_timeout"));
			fileSizeCheck = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					entrynode, "file_size_check"));
			addFilenameToResult = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					entrynode, "add_filename_result"));
		} catch (KettleXMLException xe) {
			throw new KettleXMLException(
					"Unable to load job entry of type 'wait for file' from XML node",
					xe);
		}
	}
	...
	public void loadRep(Repository rep, long id_jobentry,
		...
		try {
			...
			maximumTimeout = rep.getJobEntryAttributeString(id_jobentry,
					"maximum_timeout");
			checkCycleTime = rep.getJobEntryAttributeString(id_jobentry,
					"check_cycle_time");
			successOnTimeout = rep.getJobEntryAttributeBoolean(id_jobentry,
					"success_on_timeout");
			fileSizeCheck = rep.getJobEntryAttributeBoolean(id_jobentry,
					"file_size_check");
			addFilenameToResult = rep.getJobEntryAttributeBoolean(id_jobentry,
					"add_filename_result");
	...
	public void saveRep(Repository rep, long id_job) throws KettleException {
		try {
			...
			rep.saveJobEntryAttribute(id_job, getID(), "maximum_timeout",
					maximumTimeout);
			rep.saveJobEntryAttribute(id_job, getID(), "check_cycle_time",
					checkCycleTime);
			rep.saveJobEntryAttribute(id_job, getID(), "success_on_timeout",
					successOnTimeout);
			rep.saveJobEntryAttribute(id_job, getID(), "file_size_check",
					fileSizeCheck);
			rep.saveJobEntryAttribute(id_job, getID(), "add_filename_result",
					addFilenameToResult);
	...
	public Result execute(Result previousResult, int nr, Repository rep,
		...
		if (filename != null) {
			...
			try {
				...
				long iCycleTime = Const.toInt(getRealCheckCycleTime(), Const
						.toInt(DEFAULT_CHECK_CYCLE_TIME, 0));
				...
				if (iMaximumTimeout < 0) {
					...
					if (log.isBasic())
						log.logBasic(toString(),
								"Maximum timeout invalid, reset to "
										+ iMaximumTimeout);
				}
				...
				if (iCycleTime < 1) {
					...
					if (log.isBasic())
						log.logBasic(toString(),
								"Check cycle time invalid, reset to "
										+ iCycleTime);
				}
				...
				if (iMaximumTimeout == 0) {
					if (log.isBasic())
						log.logBasic(toString(),
								"Waiting indefinitely for file ["
										+ realFilename + "]");
				} else {
					if (log.isBasic())
						log.logBasic(toString(), "Waiting " + iMaximumTimeout
								+ " seconds for file [" + realFilename + "]");
				}
				...
				while (continueLoop && !parentJob.isStopped()) {
					...
					if (fileObject.exists()) {
						...
						if (log.isBasic())
							log.logBasic(toString(), "Detected file ["
									+ realFilename + "] within timeout");
						...
						if (addFilenameToResult
							...
							ResultFile resultFile = new ResultFile(
									ResultFile.FILE_TYPE_GENERAL, fileObject,
									parentJob.getJobname(), toString());
							resultFile.setComment(Messages
									.getString("JobWaitForFile.FilenameAdded"));
						...
						if (iMaximumTimeout == 0) {
							...
							if ((now + iCycleTime) < (timeStart + iMaximumTimeout)) {
								...
								sleepTime = iCycleTime
										- ((now + iCycleTime) - (timeStart + iMaximumTimeout));
							}
						}
				...
				if (!parentJob.isStopped() && fileObject.exists()
						&& isFileSizeCheck()) {
					long oldSize = -1;
					long newSize = fileObject.getContent().getSize();

					if (log.isDetailed())
						log.logDetailed(toString(), "File [" + realFilename
								+ "] is " + newSize + " bytes long");
					if (log.isBasic())
						log.logBasic(toString(), "Waiting until file ["
								+ realFilename + "] stops growing for "
								+ iCycleTime + " seconds");
					while (oldSize != newSize && !parentJob.isStopped()) {
						try {
							if (log.isDetailed()) {
								log.logDetailed(toString(), "Sleeping "
										+ iCycleTime
										+ " seconds, waiting for file ["
										+ realFilename + "] to stop growing");
							}
					...
					if (log.isBasic())
						log.logBasic(toString(), "Stopped waiting for file ["
								+ realFilename + "] to stop growing");
				}
				...
				log
						.logBasic(toString(),
								"Exception while waiting for file ["
										+ realFilename + "] to stop growing: "
										+ e.getMessage());
			} finally {
				if (fileObject != null) {
	...
	public void check(List<CheckResultInterface> remarks, JobMeta jobMeta) {
		...
		andValidator().validate(this,
				"maximumTimeout", remarks, putValidators(integerValidator())); //$NON-NLS-1$
		andValidator().validate(this,
				"checkCycleTime", remarks, putValidators(integerValidator())); //$NON-NLS-1$
	}
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		this.addFilenameToResult = BaseStepMeta.parameterToBoolean(p.get(id
				+ ".addFilenameToResult"));
		this.fileSizeCheck = BaseStepMeta.parameterToBoolean(p.get(id
				+ ".fileSizeCheck"));
		this.successOnTimeout = BaseStepMeta.parameterToBoolean(p.get(id
				+ ".successOnTimeout"));
		this.checkCycleTime = BaseStepMeta.parameterToString(p.get(id
				+ ".checkCycleTime"));
		this.filename = BaseStepMeta.parameterToString(p.get(id + ".filename"));
		this.maximumTimeout = BaseStepMeta.parameterToString(p.get(id
				+ ".maximumTimeout"));
	}
