 ...
 */
public class GetPreviousRowFieldMeta extends BaseStepMeta implements
		StepMetaInterface {
	...
	public Object clone() {
		GetPreviousRowFieldMeta retval = (GetPreviousRowFieldMeta) super
				.clone();
	...
	private void readData(Node stepnode) throws KettleXMLException {
		try {
			...
			throw new KettleXMLException(
					Messages
							.getString("GetPreviousRowFieldMeta.Exception.UnableToReadStepInfoFromXML"), e); //$NON-NLS-1$
		}
	}
			...
			if (!Const.isEmpty(fieldOutStream[i])) {
				...
				if (index >= 0) {
					...
					ValueMetaInterface v = new ValueMeta(space
							.environmentSubstitute(fieldOutStream[i]), in
							.getType());
			...
			for (int i = 0; i < fieldInStream.length; i++) {
				...
				if (v == null) {
					if (first) {
						first = false;
						error_message += Messages
								.getString("GetPreviousRowFieldMeta.CheckResult.MissingInStreamFields") + Const.CR; //$NON-NLS-1$
					}
					error_found = true;
			...
			for (int i = 0; i < fieldInStream.length; i++) {
				...
				if (Const.isEmpty(field)) {
					if (first) {
						...
						error_message = Messages
								.getString(
										"GetPreviousRowFieldMeta.CheckResult.OutputFieldEmpty", "" + i) + Const.CR; //$NON-NLS-1$
					}
					error_found = true;
					error_message += "\t\t" + field + Const.CR; //$NON-NLS-1$
				}
	...
	public StepInterface getStep(StepMeta stepMeta,
			...
			Trans trans) {
		return new GetPreviousRowField(stepMeta, stepDataInterface, cnr,
				transMeta, trans);
	}
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		this.fieldInStream = p.get(id + "_fields.input") == null ? new String[0]
				: p.get(id + "_fields.input");
		this.fieldOutStream = p.get(id + "_fields.output") == null ? new String[0]
				: p.get(id + "_fields.output");
	}
}
