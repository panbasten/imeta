...
public class RegexEvalMeta extends BaseStepMeta implements StepMetaInterface {
	...
	private void readData(Node stepnode,
		...
		try {
			...
			throw new KettleXMLException(
					Messages
							.getString("RegexEvalMeta.Exception.UnableToLoadStepInfoFromXML"), e); //$NON-NLS-1$
		}
	}
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		script = BaseStepMeta.parameterToString(p.get(id + ".script"));
		matcher = BaseStepMeta.parameterToString(p.get(id + ".matcher"));
		resultfieldname = BaseStepMeta.parameterToString(p.get(id
				+ ".resultfieldname"));
		usevar = BaseStepMeta.parameterToBoolean(p.get(id + ".usevar"));
		allowcapturegroups = BaseStepMeta.parameterToBoolean(p.get(id
				+ ".allowcapturegroups"));
		canoneq = BaseStepMeta.parameterToBoolean(p.get(id + ".canoneq"));
		caseinsensitive = BaseStepMeta.parameterToBoolean(p.get(id
				+ ".caseinsensitive"));
		comment = BaseStepMeta.parameterToBoolean(p.get(id + ".comment"));
		multiline = BaseStepMeta.parameterToBoolean(p.get(id + ".multiline"));
		dotall = BaseStepMeta.parameterToBoolean(p.get(id + ".dotall"));
		unicode = BaseStepMeta.parameterToBoolean(p.get(id + ".unicode"));
		unix = BaseStepMeta.parameterToBoolean(p.get(id + ".unix"));
...

	{
		try {
			...
			throw new KettleException(
					Messages
							.getString("RegexEvalMeta.Exception.UnexpectedErrorInReadingStepInfo"), e); //$NON-NLS-1$
		}
	}
	...
	public void saveRep(Repository rep, long id_transformation, long id_step)
		...
		try {
			...
			throw new KettleException(
					Messages
							.getString("RegexEvalMeta.Exception.UnableToSaveStepInfo") + id_step, e); //$NON-NLS-1$
		}
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (!Const.isEmpty(resultfieldname)) {
			cr = new CheckResult(CheckResult.TYPE_RESULT_OK, Messages
					.getString("RegexEvalMeta.CheckResult.ResultFieldnameOK"),
					stepMeta);
			remarks.add(cr);
		} else {
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, Messages
					.getString("RegexEvalMeta.CheckResult.NoResultFieldname"),
					stepMeta);
			remarks.add(cr);
		}
