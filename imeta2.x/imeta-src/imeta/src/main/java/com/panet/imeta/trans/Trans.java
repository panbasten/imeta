...
import com.panet.imeta.core.database.map.DatabaseConnectionMap;
import com.panet.imeta.core.exception.KettleDatabaseException;
import com.panet.imeta.core.exception.KettleException;
import com.panet.imeta.core.exception.KettleTransException;
...
import com.panet.imeta.trans.performance.StepPerformanceSnapShot;
import com.panet.imeta.trans.step.BaseStep;
import com.panet.imeta.trans.step.StepDataInterface;
import com.panet.imeta.trans.step.StepErrorMeta;
import com.panet.imeta.trans.step.StepInitThread;
import com.panet.imeta.trans.step.StepInterface;
import com.panet.imeta.trans.step.StepListener;
import com.panet.imeta.trans.step.StepMeta;
import com.panet.imeta.trans.step.StepMetaDataCombi;
...
import com.panet.imeta.www.AddTransServlet;
import com.panet.imeta.www.PrepareExecutionTransServlet;
import com.panet.imeta.www.SlaveServerTransStatus;
import com.panet.imeta.www.SocketRepository;
import com.panet.imeta.www.StartExecutionTransServlet;
import com.panet.imeta.www.WebResult;

/**
 * 此类负责执行“转换”。 加载，实例化，初始化，运行，监控“转换”的执行情况，该转换包括在你提供的TransInfo对象中。
 * 
 * @author Peter Pan
 * 
 */
public class Trans implements VariableSpace, NamedParams {
	...
	/**
	 * 表示这个“转换”是一个再次的转换，这个“转换”曾在该时间执行。 如果replayDate为null，该“转换”不是一个再次转换。
	 */
	...
	public Trans(TransMeta transMeta) {
		...
		if (log.isDetailed())
			log.logDetailed(toString(), Messages
					.getString("Trans.Log.TransformationIsPreloaded")); //$NON-NLS-1$
		if (log.isDebug())
			log
					.logDebug(
							toString(),
	...
	/**
	 * 初始化一个“转换”，从在内存中定义的元数据
	 * 
	 * @param transMeta
	 *            the transformation meta-data to use.
	 */
	public Trans(LogWriterProxy log, TransMeta transMeta) {
		this.log = log;
		this.transMeta = transMeta;
	...
	public Trans(LogWriterProxy log, VariableSpace parentVariableSpace,
			Repository rep, String name, String dirname, String filename)
	 ...
	 * 
	 * @throws KettleException
	 *             in case the transformation could not be prepared
	 *             (initialized)
	 */
	...
	public void prepareExecution(String[] arguments) throws KettleException {
			...
			if (transMeta.getFilename() != null) {
				log
						.logMinimal(
								...
								Messages
										.getString(
												"Trans.Log.DispacthingStartedForFilename", transMeta.getFilename())); //$NON-NLS-1$ //$NON-NLS-2$
			}
		} else {
			log
					.logMinimal(
							...
							Messages
									.getString(
											"Trans.Log.DispacthingStartedForTransformation", transMeta.getName())); //$NON-NLS-1$ //$NON-NLS-2$
		}
			...
			if (log.isDetailed())
				log
						.logDetailed(
								...
								Messages
										.getString(
												"Trans.Log.NumberOfArgumentsDetected", String.valueOf(transMeta.getArguments().length))); //$NON-NLS-1$
		}

		if (isSafeModeEnabled()) {
			if (log.isDetailed())
				log.logDetailed(toString(), Messages.getString(
						"Trans.Log.SafeModeIsEnabled", transMeta.getName())); //$NON-NLS-1$ //$NON-NLS-2$
		}

		if (getReplayDate() != null) {
			SimpleDateFormat df = new SimpleDateFormat(REPLAY_DATE_FORMAT);
			log.logBasic(toString(), Messages
					.getString("Trans.Log.ThisIsAReplayTransformation") //$NON-NLS-1$
					+ df.format(getReplayDate()));
		} else {
			if (log.isDetailed())
				log.logDetailed(toString(), Messages
						.getString("Trans.Log.ThisIsNotAReplayTransformation")); //$NON-NLS-1$
		}
		...
		if (log.isDetailed()) {
			log
					.logDetailed(
							...
							Messages
									.getString(
											"Trans.Log.FoundDefferentSteps", String.valueOf(hopsteps.size()))); //$NON-NLS-1$ //$NON-NLS-2$
			log.logDetailed(toString(), Messages
					.getString("Trans.Log.AllocatingRowsets")); //$NON-NLS-1$
		}
		...
		for (int i = 0; i < hopsteps.size(); i++) {
			...
			for (int n = 0; n < nrTargets; n++) {
				...
				if (nextStep.isMapping())
					continue; // handled and allocated by the mapping step
				// itself.
				...
				if (dispatchType != TYPE_DISP_N_M) {
					for (int c = 0; c < nrCopies; c++) {
						...
						switch (dispatchType) {
						case TYPE_DISP_1_1:
							rowSet.setThreadNameFromToCopy(thisStep.getName(),
									0, nextStep.getName(), 0);
							break;
						case TYPE_DISP_1_N:
							rowSet.setThreadNameFromToCopy(thisStep.getName(),
									0, nextStep.getName(), c);
							break;
						case TYPE_DISP_N_1:
							rowSet.setThreadNameFromToCopy(thisStep.getName(),
									c, nextStep.getName(), 0);
							break;
						case TYPE_DISP_N_N:
							rowSet.setThreadNameFromToCopy(thisStep.getName(),
									c, nextStep.getName(), c);
							break;
						}
						rowsets.add(rowSet);
						if (log.isDetailed())
							log
									.logDetailed(
											...
											Messages
													.getString(
															"Trans.TransformationAllocatedNewRowset", rowSet.toString())); //$NON-NLS-1$ //$NON-NLS-2$
					}
				} else {
					...
					for (int s = 0; s < thisCopies; s++) {
						for (int t = 0; t < nextCopies; t++) {
							RowSet rowSet = new RowSet(transMeta
									.getSizeRowset());
							rowSet.setThreadNameFromToCopy(thisStep.getName(),
									s, nextStep.getName(), t);
							rowsets.add(rowSet);
							if (log.isDetailed())
								log
										.logDetailed(
		...
		if (log.isDetailed())
			log.logDetailed(toString(), Messages
					.getString("Trans.Log.AllocatingStepsAndStepData")); //$NON-NLS-1$
		...
		for (int i = 0; i < hopsteps.size(); i++) {
			...
			if (log.isDebug())
				log
						.logDebug(
								...
								Messages
										.getString(
												"Trans.Log.StepHasNumberRowCopies", String.valueOf(nrCopies))); //$NON-NLS-1$
			...
			for (int c = 0; c < nrCopies; c++) {
				...
				if (!hasStepStarted(stepMeta.getName(), c)) {
					...
					// Allocate the step
					StepInterface step = combi.meta.getStep(stepMeta, data, c,
							transMeta, this);
					...
					((BaseStep) step).setUsingThreadPriorityManagment(transMeta
							.isUsingThreadPriorityManagment());

					// If the step is partitioned, set the partitioning ID and
					// some other things as well...
					if (stepMeta.isPartitioned()) {
						List<String> partitionIDs = stepMeta
								.getStepPartitioningMeta().getPartitionSchema()
								.getPartitionIDs();
						if (partitionIDs != null && partitionIDs.size() > 0) {
							step.setPartitionID(partitionIDs.get(c)); // Pass
							// the
							// partition
							// ID to
							// the
		...
		// Now we need to verify if certain rowsets are not meant to be for
		// error handling...
		...
		// The input rowsets are already in place, so the next step just accepts
		// the rows.
		...
		for (int s = 0; s < steps.size(); s++) {
			...
			if (combi.stepMeta.isDoingErrorHandling()) {
				...
				boolean stop = false;
				for (int rowsetNr = 0; rowsetNr < baseStep.outputRowSets.size()
						&& !stop; rowsetNr++) {
					RowSet outputRowSet = baseStep.outputRowSets.get(rowsetNr);
					if (outputRowSet.getDestinationStepName().equalsIgnoreCase(
		...
		for (int i = 0; i < steps.size(); i++) {
			...
			//
			// If this source step is not partitioned, and the target step is:
			// it means we need to re-partition the incoming data.
			// If both steps are partitioned on the same method and schema, we
			// don't need to re-partition
			// If both steps are partitioned on a different method or schema, we
			// need to re-partition as well.
			...
			if (isThisPartitioned)
				thisPartitionSchema = stepMeta.getStepPartitioningMeta()
						.getPartitionSchema();
			...
			for (int p = 0; p < nrNext; p++) {
				...
				if (nextStep.isPartitioned()) {
					...
					nextStepPartitioningMeta = nextStep
							.getStepPartitioningMeta();
					nextPartitionSchema = nextStepPartitioningMeta
							.getPartitionSchema();
				}
			}

			baseStep
					.setRepartitioning(StepPartitioningMeta.PARTITIONING_METHOD_NONE);

			// If the next step is partitioned differently, set re-partitioning,
			// when running locally.
			//
			if ((!isThisPartitioned && isNextPartitioned)
					|| (isThisPartitioned && isNextPartitioned && !thisPartitionSchema
							.equals(nextPartitionSchema))) {
				baseStep.setRepartitioning(nextStepPartitioningMeta
						.getMethodType());
			}
		}
		...
		if (log.isDetailed())
			log
					.logDetailed(
							...
							Messages
									.getString(
											"Trans.Log.InitialisingSteps", String.valueOf(steps.size()))); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		...
		for (int i = 0; i < steps.size(); i++) {
			...
			// Init all steps at once, but ALL steps need to finish before we
			...
			threads[i].setName("init of " + sid.stepname + "." + sid.copy
					+ " (" + threads[i].getName() + ")");
			threads[i].start();
		}
		...
		for (int i = 0; i < threads.length; i++) {
			try {
				...
				log.logError("Error with init thread: " + ex.getMessage(), ex
						.getMessage());
		...
		for (int i = 0; i < initThreads.length; i++) {
			...
			if (!initThreads[i].isOk()) {
				log.logError(toString(), Messages.getString(
						"Trans.Log.StepFailedToInit", combi.stepname + "."
								+ combi.copy));
				...
				if (log.isDetailed())
					log.logDetailed(toString(), Messages.getString(
							"Trans.Log.StepInitialized", combi.stepname + "."
									+ combi.copy));
			}
		}

		if (!ok) {
			// Halt the other threads as well, signal end-of-the line to the
			// outside world...
			// Also explicitly call dispose() to clean up resources opened
			// during init();
			//
			for (int i = 0; i < initThreads.length; i++) {
				StepMetaDataCombi combi = initThreads[i].getCombi();

				// Dispose will overwrite the status, but we set it back right
				// after this.
			...
			throw new KettleException(Messages
					.getString("Trans.Log.FailToInitializeAtLeastOneStep")); //$NON-NLS-1
		}
	...
	/**
	 * Start the threads prepared by prepareThreads(); Before you start the
	 * threads, you can add RowListeners to them.
	 * 
	 * @throws KettleException
	 *             in case there is a communication error with a remote output
	 *             socket.
	 */
	public void startThreads() throws KettleException {
		...
		for (int i = 0; i < steps.size(); i++) {
			...
			StepListener stepListener = new StepListener() {
				public void stepFinished(Trans trans, StepMeta stepMeta,
						StepInterface step) {
					synchronized (Trans.this) {
						...
						if (nrOfFinishedSteps >= steps.size()) {
							...
							// Grab the performance statistics one last time (if
							// enabled)
							//
						...
						// If a step fails with an error, we want to kill/stop
						// the others too...
						//
						if (step.getErrors() > 0) {
							...
							log
									.logMinimal(
											getName(),
											Messages
													.getString("Trans.Log.TransformationDetectedErrors")); //$NON-NLS-1$ //$NON-NLS-2$
							log
									.logMinimal(
											getName(),
											Messages
													.getString("Trans.Log.TransformationIsKillingTheOtherSteps")); //$NON-NLS-1$
		...
		if (transMeta.isCapturingStepPerformanceSnapShots()) {
			...
			// Set a timer to collect the performance data from the running
			// threads...
			//
			...
			stepPerformanceSnapShotTimer.schedule(timerTask, 100, transMeta
		...
		TransListener transListener = new TransListener() {
			public void transFinished(Trans trans) {
				...
				// First of all, stop the performance snapshot timer if there is
				// is one...
				//
				if (transMeta.isCapturingStepPerformanceSnapShots()
						&& stepPerformanceSnapShotTimer != null) {
		...
		if (transMeta.isCapturingStepPerformanceSnapShots()) {
			...
			for (int i = 0; i < steps.size(); i++) {
				...
				StepPerformanceSnapShot snapShot = new StepPerformanceSnapShot(
						new Date(), stepMeta.getName(), step.getCopy(), step
								.getLinesRead(), step.getLinesWritten(), step
								.getLinesInput(), step.getLinesOutput(), step
								.getLinesUpdated(), step.getLinesRejected(),
						step.getErrors());
				List<StepPerformanceSnapShot> snapShotList = stepPerformanceSnapShots
						.get(step.toString());
				...
				if (snapShotList == null) {
					...
					previous = snapShotList.get(snapShotList.size() - 1); // the
					// last
					// one...
				}
				// Make the difference...
				//
				if (previous != null)
					snapShot.diff(previous, baseStep.rowsetInputSize(),
							baseStep.rowsetOutputSize());
	...
	/**
	 * Call this method after the transformation has finished. Typically, after
	 * ALL the slave transformations in a clustered run have finished.
	 */
	public void cleanup() {
	...
	public void waitUntilFinished() {
		// We do this the simple way: we attach a transformation listener to
		// this transformation...
		//
		try {
			while (!finished.get()) {
			...
			log.logError(toString(), Messages
					.getString("Trans.Log.TransformationError") + e.toString()); //$NON-NLS-1$
	...
	public int getErrors() {
		...
		if (errors > 0)
			log.logError(toString(), Messages
					.getString("Trans.Log.TransformationErrorsDetected")); //$NON-NLS-1$
	...
	public int getEnded() {
		...
		for (int i = 0; i < steps.size(); i++) {
			...
			if ((thr != null && !thr.isAlive()) || // Should normally not be
					// needed anymore, status is
					// kept in data.
					data.getStatus() == StepDataInterface.STATUS_FINISHED || // Finished
					// processing
					data.getStatus() == StepDataInterface.STATUS_HALTED || // Not
					// launching
					// because
					// of
					// init
					// error
					data.getStatus() == StepDataInterface.STATUS_STOPPED // Stopped
			// because
			// of
			// an
			// error
		...
		for (int i = 0; i < steps.size(); i++) {
			...
			if (log.isDebug())
				log
						.logDebug(
								toString(),
								Messages.getString("Trans.Log.LookingAtStep") + thr.getStepname()); //$NON-NLS-1$
				...
				try {
					...
					log
							.logError(
									toString(),
									Messages
											.getString("Trans.Log.TransformationErrors") + e.toString()); //$NON-NLS-1$
					return;
				}
			}
	...
	/**
	 * Ask all steps to stop but don't wait around for it to happen. Special
	 * method for use with mappings.
	 */
				...
				if (thr.getErrors() == 0) {
					log
							.logBasic(
									...
									Messages
											.getString(
													"Trans.Log.ProcessSuccessfullyInfo", thr.getStepname(), "'." + thr.getCopy(), String.valueOf(proc), String.valueOf((proc / seconds)))); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
				} else {
					log
							.logError(
									toString(),
									Messages
											.getString(
													"Trans.Log.ProcessErrorInfo", thr.getStepname(), "'." + thr.getCopy(), String.valueOf(thr.getErrors()), String.valueOf(proc), String.valueOf(proc / seconds))); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$
				}
			} else {
				if (thr.getErrors() == 0) {
					log
							.logBasic(
									...
									Messages
											.getString(
													"Trans.Log.ProcessSuccessfullyInfo", thr.getStepname(), "'." + thr.getCopy(), String.valueOf(proc), seconds != 0 ? String.valueOf((proc / seconds)) : "-")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
				} else {
					log
							.logError(
									toString(),
									Messages
											.getString(
													"Trans.Log.ProcessErrorInfo2", thr.getStepname(), "'." + thr.getCopy(), String.valueOf(thr.getErrors()), String.valueOf(proc), String.valueOf(seconds))); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$
				}
			}
		}
	}
	...
	public RowSet findRowSet(String rowsetname) {
		...
		for (int i = 0; i < rowsets.size(); i++) {
			// log.logDetailed("DIS: looking for RowSet ["+rowsetname+"] in nr
			// "+i+" of "+threads.size()+" threads...");
	...
	public boolean hasStepStarted(String sname, int copy) {
		// log.logDetailed("DIS: Checking wether of not ["+sname+"]."+cnr+" has
		// started!");
		// log.logDetailed("DIS: hasStepStarted() looking in "+threads.size()+"
		// threads");
		for (int i = 0; i < steps.size(); i++) {
			StepMetaDataCombi sid = steps.get(i);
			boolean started = (sid.stepname != null && sid.stepname
		...
		for (int i = 0; i < steps.size(); i++) {
			...
			try {
				...
				log.logError(toString(),
						"Something went wrong while trying to stop the transformation: "
								+ e.toString());
		...
		try {
			...
			log
					.logBasic(
							toString(),
							Messages
									.getString("Trans.Log.TransformationCanBeReplayed") + df.format(currentDate)); //$NON-NLS-1$
			...
			try {
				...
				if (logcon != null) {
					if (transMeta.getLogTable() == null) {
						// It doesn't make sense to start database logging
						// without a table
						// to log to.
						throw new KettleTransException(Messages
								.getString("Trans.Exception.NoLogTableDefined")); //$NON-NLS-1$ //$NON-NLS-2$
					}
					...
					if (log.isDetailed())
						log
								.logDetailed(
										...
										Messages
												.getString(
														"Trans.Log.OpeningLogConnection", "" + transMeta.getLogConnection())); //$NON-NLS-1$ //$NON-NLS-2$
					ldb.connect();
					...
					// Do this first, before anything else to lock the complete
					// table exclusively
					//
					if (transMeta.isBatchIdUsed()) {
						...
						//
						ldb
								.lockTables(new String[] { transMeta
										.getLogTable(), });
						lockedTable = true;

						// Now insert value -1 to create a real write lock
						// blocking the other requests.. FCFS
						//
						String sql = "INSERT INTO "
								+ logcon.quoteField(transMeta.getLogTable())
								+ "(" + logcon.quoteField("ID_BATCH")
								+ ") values (-1)";
						ldb.execStatement(sql);

						// Now this next lookup will stall on the other
						// connections
						//
						Long id_batch = ldb.getNextValue(transMeta
								.getCounters(), transMeta.getLogTable(),
								"ID_BATCH");
						setBatchId(id_batch.longValue());
					}

					//
					// Get the date range from the logging table: from the last
					// end_date to now. (currentDate)
					//
					Object[] lastr = ldb.getLastLogDate(
							transMeta.getLogTable(), transMeta.getName(),
							false, Messages.getString("Trans.Row.Status.End")); //$NON-NLS-1$
						...
						if (log.isDetailed())
							log
									.logDetailed(
											toString(),
											Messages
													.getString("Trans.Log.StartDateFound") + startDate); //$NON-NLS-1$
					}

					//
					// OK, we have a date-range.
					// However, perhaps we need to look at a table before we
					// make a final judment?
					//
					if (transMeta.getMaxDateConnection() != null
							&& transMeta.getMaxDateTable() != null
							&& transMeta.getMaxDateTable().length() > 0
							&& transMeta.getMaxDateField() != null
						...
						if (maxcon != null) {
							...
							try {
								if (log.isDetailed())
									log
											.logDetailed(
													toString(),
													Messages
															.getString("Trans.Log.OpeningMaximumDateConnection")); //$NON-NLS-1$
								maxdb.connect();

								//
								// Determine the endDate by looking at a field
								// in a table...
								//
								...
								if (r1 != null) {
									...
									Date maxvalue = r1.getRowMeta().getDate(
											r1.getData(), 0);
									if (maxvalue != null) {
										if (log.isDetailed())
											log
													.logDetailed(
															toString(),
															Messages
																	.getString("Trans.Log.LastDateFoundOnTheMaxdateConnection") + r1); //$NON-NLS-1$
										endDate.setTime((long) (maxvalue
												.getTime() + (transMeta
												.getMaxDateOffset() * 1000)));
									}
								} else {
									if (log.isDetailed())
										log
												.logDetailed(
														toString(),
														Messages
																.getString("Trans.Log.NoLastDateFoundOnTheMaxdateConnection")); //$NON-NLS-1$
								}
							} catch (KettleException e) {
						...
						if (log.isDetailed())
							log
									.logDetailed(
											toString(),
											Messages
													.getString("Trans.Log.CheckingForMaxDependencyDate")); //$NON-NLS-1$
						//
						// Maybe one of the tables where this transformation is
						// dependent on has changed?
								...
								try {
									...
									if (r1 != null) {
										...
										if (maxvalue != null) {
											...
											if (maxvalue.getTime() > maxdepdate
													.getTime()) {
												maxdepdate = maxvalue;
											}
										} else {
							...
							if (log.isDetailed())
								log
										.logDetailed(
												toString(),
												Messages
														.getString("Trans.Log.Maxdepdate") + (XMLHandler.date2string(maxdepdate))); //$NON-NLS-1$ //$NON-NLS-2$
						}

						// OK, so we now have the maximum depdate;
						// If it is larger, it means we have to read everything
						// back in again.
				...
				// OK, now we have a date-range. See if we need to set a
				// maximum!
				if (transMeta.getMaxDateDifference() > 0.0 && // Do we have a
						// difference
						// specified?
						startDate.getTime() > Const.MIN_DATE.getTime() // Is
				// the
				// startdate
				// >
				// Minimum?
					...
					Date maxdesired = new Date(startDate.getTime()
							+ ((long) transMeta.getMaxDateDifference() * 1000));

					// If this is the case: lower the end-date. Pick up the next
					// 'region' next time around.
					// We do this to limit the workload in a single update
					// session (e.g. for large fact tables)
					//
					if (endDate.compareTo(maxdesired) > 0)
						endDate = maxdesired;
				}

				if (Const.isEmpty(transMeta.getName()) && logcon != null
						&& transMeta.getLogTable() != null) {
					throw new KettleException(
							Messages
									.getString("Trans.Exception.NoTransnameAvailableForLogging"));
				}

				if (logcon != null && transMeta.getLogTable() != null
						&& transMeta.getName() != null) {
					ldb.writeLogRecord(transMeta.getLogTable(), transMeta
							.isBatchIdUsed(), getBatchId(),
							false,
							transMeta.getName(),
							Database.LOG_STATUS_START, //$NON-NLS-1$
							0L, 0L, 0L, 0L, 0L, 0L, startDate, endDate,
							logDate, depDate, currentDate,
							((LogWriterProxy) log).getOperateUser(),
							((LogWriterProxy) log).getIvokeType(), null);
				}
				...
				if (lockedTable) {
					...
					String sql = "DELETE FROM "
							+ logcon.quoteField(transMeta.getLogTable())
							+ " WHERE " + logcon.quoteField("ID_BATCH")
							+ "= -1";
					ldb.execStatement(sql);
				...
				throw new KettleTransException(
						Messages
								.getString(
										"Trans.Exception.ErrorWritingLogRecordToTable", transMeta.getLogTable()), e); //$NON-NLS-1$ //$NON-NLS-2$
			} finally {
				if (ldb != null)
					ldb.disconnect();
			}
			...
			if (transMeta.isLogfieldUsed()) {
				...
				stringAppender.setBuffer(new StringBuffer(Messages
						.getString("Trans.Log.Start")
						+ Const.CR));
			}
		} catch (KettleException e) {
			throw new KettleTransException(
					Messages
							.getString("Trans.Exception.UnableToBeginProcessingTransformation"), e); //$NON-NLS-1$
		}
	}
	...
	public Result getResult() {
		...
		for (int i = 0; i < steps.size(); i++) {
			...
			if (transMeta.getReadStep() != null
					&& rt.getStepname().equals(
							transMeta.getReadStep().getName()))
				result.setNrLinesRead(result.getNrLinesRead()
						+ rt.getLinesRead());
			if (transMeta.getInputStep() != null
					&& rt.getStepname().equals(
							transMeta.getInputStep().getName()))
				result.setNrLinesInput(result.getNrLinesInput()
						+ rt.getLinesInput());
			if (transMeta.getWriteStep() != null
					&& rt.getStepname().equals(
							transMeta.getWriteStep().getName()))
				result.setNrLinesWritten(result.getNrLinesWritten()
						+ rt.getLinesWritten());
			if (transMeta.getOutputStep() != null
					&& rt.getStepname().equals(
							transMeta.getOutputStep().getName()))
				result.setNrLinesOutput(result.getNrLinesOutput()
						+ rt.getLinesOutput());
			if (transMeta.getUpdateStep() != null
					&& rt.getStepname().equals(
							transMeta.getUpdateStep().getName()))
				result.setNrLinesUpdated(result.getNrLinesUpdated()
						+ rt.getLinesUpdated());
			if (transMeta.getRejectedStep() != null
					&& rt.getStepname().equals(
							transMeta.getRejectedStep().getName()))
				result.setNrLinesRejected(result.getNrLinesRejected()
						+ rt.getLinesRejected());
		}
	...
	public boolean endProcessing(String status) throws KettleException {
		...
		if (transMeta.isUsingUniqueConnections()) {
			// Commit or roll back the transaction in the unique database
			// connections...
			// 
		...
		if (logcon != null) {
			...
			try {
				...
				if (!Const.isEmpty(transMeta.getLogTable())) {
					ldb.writeLogRecord(transMeta.getLogTable(), transMeta
							.isBatchIdUsed(), getBatchId(), false, transMeta
							.getName(), status, result.getNrLinesRead(), result
							.getNrLinesWritten(), result.getNrLinesUpdated(),
							result.getNrLinesInput()
									+ result.getNrFilesRetrieved(), result
									.getNrLinesOutput(), result.getNrErrors(),
							startDate, endDate, logDate, depDate, currentDate,
							((LogWriterProxy) log).getOperateUser(),
							((LogWriterProxy) log).getIvokeType(), log_string);
				}

				// Write to the step performance log table...
				//
				if (!Const.isEmpty(transMeta.getStepPerformanceLogTable())
						&& transMeta.isCapturingStepPerformanceSnapShots()) {
					// Loop over the steps...
					//
					RowMetaInterface rowMeta = Database
							.getStepPerformanceLogrecordFields();
					ldb.prepareInsert(rowMeta, transMeta
							.getStepPerformanceLogTable());

					for (String key : stepPerformanceSnapShots.keySet()) {
						List<StepPerformanceSnapShot> snapshots = stepPerformanceSnapShots
								.get(key);
						long seqNr = 1;
						for (StepPerformanceSnapShot snapshot : snapshots) {
							...
							row[outputIndex++] = new Long(snapshot
									.getStepCopy());
							row[outputIndex++] = new Long(snapshot
									.getLinesRead());
							row[outputIndex++] = new Long(snapshot
				...
				throw new KettleException(
						Messages
								.getString("Trans.Exception.ErrorWritingLogRecordToTable") + transMeta.getLogTable() + "]", e); //$NON-NLS-1$ //$NON-NLS-2$
			} finally {
				ldb.disconnect();
			}
		}
	...
	private void closeUniqueDatabaseConnections(Result result) {
		...
		List<Database> databaseList = new ArrayList<Database>(map.getMap()
				.values());
		for (Database database : databaseList) {
			if (database.getConnectionGroup().equals(getThreadName())) {
				try {
						...
						try {
							...
							log
									.logBasic(
											...
											Messages
													.getString(
															"Trans.Exception.TransactionsRolledBackOnConnection",
															database.toString()));
						} catch (Exception e) {
						...
						try {
							...
							log
									.logBasic(
											...
											Messages
													.getString(
															"Trans.Exception.TransactionsCommittedOnConnection",
															database.toString()));
						} catch (Exception e) {
							throw new KettleDatabaseException(
									Messages
					...
					//
					map.removeConnection(database.getConnectionGroup(),
							database.getPartitionId(), database);
				} catch (Exception e) {
					log
							.logError(
									toString(),
									Messages
											.getString(
													"Trans.Exception.ErrorHandlingTransformationTransaction",
													database.toString()), e);
	...
	/**
	 * @param monitored
	 *            Indicate we want to monitor the running transformation
	 */
	...
	/**
	 * Turn on safe mode during running: the transformation will run slower but
	 * with more checking enabled.
	 * 
	...
	/**
	 * @return Returns true if the safe mode is enabled: the transformation will
	 *         run slower but with more checking enabled
	 */
	 ...
	 * @param copynr
	 *            The copynr of the step to produce row for (normally 0 unless
	 *            you have multiple copies running)
	 * @throws KettleException
	 *             in case the thread/step to produce rows for could not be
	 *             found.
	 */
	public RowProducer addRowProducer(String stepname, int copynr)
			throws KettleException {
		StepInterface stepInterface = getStepInterface(stepname, copynr);
		if (stepInterface == null) {
			throw new KettleException("Unable to find thread with name "
					+ stepname + " and copy number " + copynr);
		}
	...
	/**
	 * Finds the StepDataInterface (currently) associated with the specified
	 * step
	 * 
	 * @param stepname
	 *            The name of the step to look for
	...
	public boolean hasHaltedSteps() {
		...
		for (int i = 0; i < steps.size(); i++) {
			StepMetaDataCombi sid = steps.get(i);
			if (sid.data.getStatus() == StepDataInterface.STATUS_HALTED)
				return true;
		}
		return false;
	}
	...
	public static final TransSplitter executeClustered(
	 ...
	 * 
	 * See also : com.panet.imeta.ui.spoon.delegates.SpoonTransformationDelegate
	 * 
	 * @param transSplitter
	 * @param executionConfiguration
	 * @throws KettleException
	 */
	public static final void executeClustered(
			final TransSplitter transSplitter,
			final TransExecutionConfiguration executionConfiguration)
			throws KettleException {
		try {
			...
			// be done on the master...
			{
				...
				if (executionConfiguration.isClusterPosting()) {
					TransConfiguration transConfiguration = new TransConfiguration(
							master, executionConfiguration);
					Map<String, String> variables = transConfiguration
							.getTransExecutionConfiguration().getVariables();
					variables.put(Const.INTERNAL_VARIABLE_CLUSTER_SIZE, Integer
							.toString(slaves.length));
					variables.put(Const.INTERNAL_VARIABLE_CLUSTER_MASTER, "Y");

					String masterReply = masterServer.sendXML(
							transConfiguration.getXML(),
							AddTransServlet.CONTEXT_PATH + "/?xml=Y");
					WebResult webResult = WebResult.fromXMLString(masterReply);
					if (!webResult.getResult().equalsIgnoreCase(
							WebResult.STRING_OK)) {
						throw new KettleException(
			...
			for (int i = 0; i < slaves.length; i++) {
				...
				final TransMeta slaveTrans = (TransMeta) transSplitter
						.getSlaveTransMap().get(slaves[i]);

				if (executionConfiguration.isClusterPosting()) {
					Runnable runnable = new Runnable() {
						public void run() {
							try {
								// Create a copy for local use... We get
								...
								Map<String, String> variables = slaveTransExecutionConfiguration
										.getVariables();
								variables
										.put(
												Const.INTERNAL_VARIABLE_SLAVE_SERVER_NUMBER,
												Integer.toString(index));
								variables
										.put(
												Const.INTERNAL_VARIABLE_SLAVE_SERVER_NAME,
												slaves[index].getName());
								variables.put(
										Const.INTERNAL_VARIABLE_CLUSTER_SIZE,
										Integer.toString(slaves.length));
								variables.put(
										Const.INTERNAL_VARIABLE_CLUSTER_MASTER,
										"N");

								String slaveReply = slaves[index].sendXML(
										transConfiguration.getXML(),
										AddTransServlet.CONTEXT_PATH
												+ "/?xml=Y");
								WebResult webResult = WebResult
										.fromXMLString(slaveReply);
								if (!webResult.getResult().equalsIgnoreCase(
										WebResult.STRING_OK)) {
									throw new KettleException(
			...
			if (executionConfiguration.isClusterPosting()) {
				if (executionConfiguration.isClusterPreparing()) {
					...
					// master...
					{
						String masterReply = masterServer
								.getContentFromServer(PrepareExecutionTransServlet.CONTEXT_PATH
										+ "/?name="
										+ URLEncoder.encode(master.getName(),
												"UTF-8") + "&xml=Y");
						WebResult webResult = WebResult
								.fromXMLString(masterReply);
						if (!webResult.getResult().equalsIgnoreCase(
								WebResult.STRING_OK)) {
							throw new KettleException(
					...
					// Prepare the slaves
					// WG: Should these be threaded like the above
					// initialization?
					for (int i = 0; i < slaves.length; i++) {
						TransMeta slaveTrans = (TransMeta) transSplitter
								.getSlaveTransMap().get(slaves[i]);
						String slaveReply = slaves[i]
								.getContentFromServer(PrepareExecutionTransServlet.CONTEXT_PATH
										+ "/?name="
										+ URLEncoder.encode(slaveTrans
												.getName(), "UTF-8") + "&xml=Y");
						WebResult webResult = WebResult
								.fromXMLString(slaveReply);
						if (!webResult.getResult().equalsIgnoreCase(
				...
				if (executionConfiguration.isClusterStarting()) {
					...
					{
						String masterReply = masterServer
								.getContentFromServer(StartExecutionTransServlet.CONTEXT_PATH
										+ "/?name="
										+ URLEncoder.encode(master.getName(),
												"UTF-8") + "&xml=Y");
						WebResult webResult = WebResult
			...
			throw new KettleException(
					"There was an error during transformation split", e);
		}
	}
	 ...
	 * <br>
	 * When the master and slave transformations have all finished, we should
	 * also run<br>
	 ...
	 * @param parentJob
	 *            the parent job when executed in a job, otherwise just set to
	 *            null
	 * @param sleepTimeSeconds
	 *            the sleep time in seconds in between slave transformation
	 *            status polling
	 * @return the number of errors encountered
	 */
	public static final long monitorClusteredTransformation(
			LogWriterInterface log, String logSubject,
			TransSplitter transSplitter, Job parentJob) {
		return monitorClusteredTransformation(log, logSubject, transSplitter,
				parentJob, 5); // monitor every 5 seconds
	}
	...
	public static final long monitorClusteredTransformation(
			LogWriterInterface log, String logSubject,
			TransSplitter transSplitter, Job parentJob, int sleepTimeSeconds) {
		long errors = 0L;

		//
		// See if the remote transformations have finished.
		// We could just look at the master, but I doubt that that is enough in
		// all situations.
		//
		...
		while (!allFinished && errors == 0
				&& (parentJob == null || !parentJob.isStopped())) {
			allFinished = true;
			errors = 0L;

			// Slaves first...
			//
			for (int s = 0; s < slaveServers.length && allFinished
					&& errors == 0; s++) {
				try {
					SlaveServerTransStatus transStatus = slaveServers[s]
							.getTransStatus(slaves[s].getName());
					if (transStatus.isRunning()) {
						if (log.isDetailed())
							log.logDetailed(logSubject,
									"Slave transformation on '"
											+ slaveServers[s]
											+ "' is still running.");
						allFinished = false;
					} else {
					...
					errors += 1;
					log.logError(logSubject, "Unable to contact slave server '"
							+ slaveServers[s].getName()
							+ "' to check slave transformation : "
							+ e.toString());
				}
			}

			// Check the master too
			if (allFinished && errors == 0 && master != null
					&& master.nrSteps() > 0) {
				try {
					SlaveServerTransStatus transStatus = masterServer
							.getTransStatus(master.getName());
					if (transStatus.isRunning()) {
						if (log.isDetailed())
							log.logDetailed(logSubject,
									"Master transformation is still running.");
						allFinished = false;
					} else {
						if (log.isDetailed())
							log.logDetailed(logSubject,
									"Master transformation has finished.");
					}
					Result result = transStatus.getResult(transSplitter
							.getOriginalTransformation());
					errors += result.getNrErrors();
				} catch (Exception e) {
					errors += 1;
					log.logError(logSubject,
							"Unable to contact master server '"
									+ masterServer.getName()
									+ "' to check master transformation : "
									+ e.toString());
				}
			}
			...
			if ((parentJob != null && parentJob.isStopped()) || errors != 0) {
				...
				for (int s = 0; s < slaveServers.length && allFinished
						&& errors == 0; s++) {
					try {
						WebResult webResult = slaveServers[s]
								.stopTransformation(slaves[s].getName());
						if (!WebResult.STRING_OK.equals(webResult.getResult())) {
							log.logError(logSubject,
									"Unable to stop slave transformation '"
											+ slaves[s].getName() + "' : "
											+ webResult.getMessage());
						}
					} catch (Exception e) {
						errors += 1;
						log.logError(logSubject,
								"Unable to contact slave server '"
										+ slaveServers[s].getName()
				...
				try {
					WebResult webResult = masterServer
							.stopTransformation(master.getName());
					if (!WebResult.STRING_OK.equals(webResult.getResult())) {
						log.logError(logSubject,
								"Unable to stop master transformation '"
										+ masterServer.getName() + "' : "
										+ webResult.getMessage());
					}
				} catch (Exception e) {
			...
			if (!allFinished) {
				...
				if (log.isDetailed())
					log
							.logDetailed(logSubject,
									"Clustered transformation is still running, waiting a few seconds...");
				try {
		...
		log.logMinimal(logSubject,
				"All transformations in the cluster have finished.");

		// All transformations have finished, with or without error.
		// Now run a cleanup on all the transformation on the master and the
		// slaves.
		//
		// Slaves first...
		//
		for (int s = 0; s < slaveServers.length; s++) {
			try {
				WebResult webResult = slaveServers[s]
						.cleanupTransformation(slaves[s].getName());
				if (!WebResult.STRING_OK.equals(webResult.getResult())) {
					log.logError(logSubject,
							"Unable to run clean-up on slave transformation '"
									+ slaves[s].getName() + "' : "
									+ webResult.getMessage());
					errors += 1;
				}
		...
		if (master != null && master.nrSteps() > 0) {
			try {
				WebResult webResult = masterServer.cleanupTransformation(master
						.getName());
				if (!WebResult.STRING_OK.equals(webResult.getResult())) {
					log.logError(logSubject,
							"Unable to run clean-up on master transformation '"
									+ master.getName() + "' : "
									+ webResult.getMessage());
					errors += 1;
				}
				...
				errors += 1;
				log.logError(logSubject, "Unable to contact master server '"
						+ masterServer.getName()
						+ "' to clean up master transformation : "
						+ e.toString());
			}
		}
	...
	public static final Result getClusteredTransformationResult(
			LogWriterInterface log, String logSubject,
			TransSplitter transSplitter, Job parentJob) {
		Result result = new Result();
		//
		...
		for (int s = 0; s < slaveServers.length; s++) {
			try {
				...
				log.logError(logSubject, "Unable to contact slave server '"
						+ slaveServers[s].getName()
						+ "' to get result of slave transformation : "
						+ e.toString());
			}
		}
		...
		if (master != null && master.nrSteps() > 0) {
			try {
				...
				log.logError(logSubject, "Unable to contact master server '"
						+ masterServer.getName()
						+ "' to get result of master transformation : "
						+ e.toString());
			}
		}
	...
	public static void sendXMLToSlaveServer(TransMeta transMeta,
		...
		try {
			...
			//
			String xml = new TransConfiguration(transMeta,
			...
			reply = slaveServer
					.getContentFromServer(PrepareExecutionTransServlet.CONTEXT_PATH
							+ "/?name="
							+ URLEncoder.encode(transMeta.getName(), "UTF-8")
							+ "&xml=Y");
	...
	/**
	 * @return true if the transformation was prepared for execution
	 *         successfully.
	 * @see com.panet.imeta.trans.Trans#prepareExecution(String[])
	 */
	...
	public void setInternalKettleVariables(VariableSpace var) {
		if (transMeta != null && !Const.isEmpty(transMeta.getFilename())) // we
		// have
		// a
		// finename
		// that's
		...
		{
			try {
				FileObject fileObject = KettleVFS.getFileObject(transMeta
						.getFilename());
		...
		variables.setVariable(Const.INTERNAL_VARIABLE_TRANSFORMATION_NAME,
				Const.NVL(transMeta.getName(), ""));

		// TODO PUT THIS INSIDE OF THE "IF"
		// The name of the directory in the repository
		variables.setVariable(
				Const.INTERNAL_VARIABLE_TRANSFORMATION_REPOSITORY_DIRECTORY,
				transMeta.getDirectory() != null ? transMeta.getDirectory()
						.getPath() : "");

		// Here we don't clear the definition of the job specific parameters, as
		// they may come in handy.
		// A transformation can be called from a job and may inherit the job
		// internal variables
	 ...
	 */
	public void setStepPerformanceSnapShots(
	...
	public static void monitorRemoteTransformation(LogWriterInterface log,
			String transName, SlaveServer remoteSlaveServer) {
	...
	public static void monitorRemoteTransformation(LogWriterInterface log,
		...
		while (!allFinished && errors == 0) {
			...
			if (allFinished && errors == 0) {
				try {
					SlaveServerTransStatus transStatus = remoteSlaveServer
							.getTransStatus(transName);
					if (transStatus.isRunning()) {
						if (log.isDetailed())
							log.logDetailed(transName,
									"Remote transformation is still running.");
						allFinished = false;
					} else {
						if (log.isDetailed())
							log.logDetailed(transName,
									"Remote transformation has finished.");
					}
					Result result = transStatus.getResult();
					errors += result.getNrErrors();
				} catch (Exception e) {
					errors += 1;
					log.logError(transName,
							"Unable to contact remote slave server '"
									+ remoteSlaveServer.getName()
									+ "' to check transformation status : "
									+ e.toString());
				}
			}
		...
		//
		try {
			WebResult webResult = remoteSlaveServer
					.cleanupTransformation(transName);
			if (!WebResult.STRING_OK.equals(webResult.getResult())) {
				log.logError(transName,
						"Unable to run clean-up on remote transformation '"
