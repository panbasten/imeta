...
public class DatabaseLookupMeta extends BaseStepMeta implements StepMetaInterface
{
	...
	public static final String[] fieldTypeDesc = new String[] { "",conditionStrings[0],
		 conditionStrings[1], conditionStrings[2],
		 conditionStrings[3], conditionStrings[4],
		 conditionStrings[5], conditionStrings[6],
		 conditionStrings[7], conditionStrings[8],
		 conditionStrings[9]};
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		databaseMeta = DatabaseMeta.findDatabase( (List<DatabaseMeta>) databases, BaseStepMeta.parameterToLong(p
				.get(id + ".connection")));
		cached = BaseStepMeta.parameterToBoolean(p.get(id + ".cached"));  
		loadingAllDataInCache = BaseStepMeta.parameterToBoolean(p.get(id + ".loadingAllDataInCache"));
		...
		orderByClause = BaseStepMeta.parameterToString(p.get(id + ".orderByClause"));         
        failingOnMultipleResults = BaseStepMeta.parameterToBoolean(p.get(id + ".failingOnMultipleResults"));
        eatingRowOnLookupFailure = BaseStepMeta.parameterToBoolean(p.get(id + ".eatingRowOnLookupFailure"));
        
        String[] streamKeyField1 = p.get(id + "_keywords.streamKeyField1");
        String[] tableKeyField = p.get(id + "_keywords.tableKeyField"); 
        String[] keyCondition = p.get(id + "_keywords.keyCondition");
        String[] streamKeyField2 = p.get(id + "_keywords.streamKeyField2");
        
        this.streamKeyField1 = (streamKeyField1 != null) ? streamKeyField1 : (new String[0]);
        this.tableKeyField = (tableKeyField != null) ? tableKeyField : (new String[0]);
        this.keyCondition = (keyCondition != null) ? keyCondition : (new String[0]);
        this.streamKeyField2 = (streamKeyField2 != null) ? streamKeyField2 : (new String[0]);
	
        String[] returnValueField = p.get(id + "_queryvalue.returnValueField");
        String[] returnValueNewName = p.get(id + "_queryvalue.returnValueNewName");
        String[] returnValueDefault = p.get(id + "_queryvalue.returnValueDefault");
        String[] returnValueDefaultType = p.get(id + "_queryvalue.returnValueDefaultType");
        
        this.returnValueField = (returnValueField != null) ? returnValueField : (new String[0]);
        this.returnValueNewName = (returnValueNewName != null) ? returnValueNewName : (new String[0]);
        this.returnValueDefault = (returnValueDefault != null) ? returnValueDefault : (new String[0]);
        this.returnValueDefaultType = (returnValueDefaultType != null) ? 
        		stringArrayToIntArray(returnValueDefaultType, 0) : (new int[0]);
	}
}
