...
public class UnivariateStatsMeta extends BaseStepMeta implements
	 ...
	 *            <code>com.panet.imeta.core.CheckResult</code>
	 * @param transmeta
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		String[] fieldName = p.get(id + "_inputFields.fieldName") == null ? new String[0]
				: p.get(id + "_inputFields.fieldName");
		String[] n = p.get(id + "_inputFields.n") == null ? new String[0] : p
				.get(id + "_inputFields.n");
		String[] mean = p.get(id + "_inputFields.mean") == null ? new String[0]
				: p.get(id + "_inputFields.mean");
		String[] stdDev = p.get(id + "_inputFields.stdDev") == null ? new String[0]
				: p.get(id + "_inputFields.stdDev");
		String[] min = p.get(id + "_inputFields.min") == null ? new String[0]
				: p.get(id + "_inputFields.min");
		String[] max = p.get(id + "_inputFields.max") == null ? new String[0]
				: p.get(id + "_inputFields.max");
		String[] median = p.get(id + "_inputFields.median") == null ? new String[0]
				: p.get(id + "_inputFields.median");
		String[] arbitraryPercentile = p.get(id
				+ "_inputFields.arbitraryPercentile") == null ? new String[0]
				: p.get(id + "_inputFields.arbitraryPercentile");
		String[] interpolatePercentile = p.get(id
				+ "_inputFields.interpolatePercentile") == null ? new String[0]
				: p.get(id + "_inputFields.interpolatePercentile");
		UnivariateStatsMetaFunction[] functions = null;
		// if (null != fieldName && fieldName.length > 0) {
		functions = new UnivariateStatsMetaFunction[fieldName.length];
		for (int i = 0; i < functions.length; i++) {
			functions[i] = new UnivariateStatsMetaFunction(fieldName[i],
					Boolean.parseBoolean(n[i]), Boolean.parseBoolean(mean[i]),
					Boolean.parseBoolean(stdDev[i]), Boolean
							.parseBoolean(min[i]),
					Boolean.parseBoolean(max[i]), Boolean
							.parseBoolean(median[i]), Double
							.parseDouble(arbitraryPercentile[i] == "" ? "0"
									: arbitraryPercentile[i]),

					Boolean.parseBoolean(interpolatePercentile[i]));
		}
		// }
		this.m_stats = functions;
	}
}
