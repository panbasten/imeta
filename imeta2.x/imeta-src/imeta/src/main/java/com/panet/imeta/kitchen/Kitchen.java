...
package com.panet.imeta.kitchen;
...
import com.panet.imeta.pan.CommandLineOption;
	...
	public static void main(String[] a) throws KettleException {
		...
		if (!Const.isEmpty(optionLoglevel)) {
			...
			log.logMinimal(STRING_KITCHEN, Messages.getString(
					"Kitchen.Log.LogLevel", log.getLogLevelLongDesc()));
		}

		if (!Const.isEmpty(optionVersion)) {
			BuildVersion buildVersion = BuildVersion.getInstance();
			log.logBasic("Kitchen", Messages.getString(
					"Kitchen.Log.KettleVersion", buildVersion.getVersion(),
		...
		if (!Const.isEmpty(optionRepname) && !Const.isEmpty(optionUsername)) {
			if (log.isDetailed())
				log.logDetailed(STRING_KITCHEN, Messages
						.getString("Kitchen.Log.RepUsernameSupplied"));
		}

		log.logMinimal(STRING_KITCHEN, Messages
				.getString("Kitchen.Log.Starting"));
		...
		try {
			...
			if (!Const.isEmpty(optionRepname) || !Const.isEmpty(optionFilename)) {
				if (log.isDebug())
					log.logDebug(STRING_KITCHEN, Messages
							.getString("Kitchen.Log.ParsingCommandLine"));
				if (!Const.isEmpty(optionRepname)
						&& !"Y".equalsIgnoreCase(optionNorep.toString())) {
					if (log.isDebug())
						log.logDebug(STRING_KITCHEN, Messages
								.getString("Kitchen.Log.LoadingRep"));

					RepositoriesMeta repsinfo = new RepositoriesMeta(log);
					if (repsinfo.readData()) {
						if (log.isDebug())
							log.logDebug(STRING_KITCHEN, Messages.getString(
							...
							if (log.isDebug())
								log
										.logDebug(
												STRING_KITCHEN,
												Messages
														.getString("Kitchen.Log.Alocate&ConnectRep"));

							repository = new Repository(log, repinfo);
							if (repository.connect("Kitchen commandline")) {
								RepositoryDirectory directory = repository
								...
								if (!Const.isEmpty(optionDirname)) {
									directory = repository.getDirectoryTree()
											.findDirectory(
								...
								if (directory != null) {
									...
									if (log.isDebug())
										log
												.logDebug(
														STRING_KITCHEN,
														Messages
																.getString("Kitchen.Log.CheckUserPass"));

									userinfo = new UserInfo(repository,
									...
									if (userinfo.getID() > 0) {
											...
											if (log.isDebug())
												log
														.logDebug(
																STRING_KITCHEN,
																Messages
																		.getString("Kitchen.Log.LoadingJobInfo"));

											jobMeta = new JobMeta(log,
													repository, optionJobname
															.toString(),
													directory);
											if (log.isDebug())
												log
														.logDebug(
																STRING_KITCHEN,
																Messages
																		.getString("Kitchen.Log.AllocateJob"));

											job = new Job(log, stepLoader,
													repository, jobMeta);
										} else
										// List the jobs in the repository
										if ("Y".equalsIgnoreCase(optionListjobs
												.toString())) {
											if (log.isDebug())
												log
														.logDebug(
																...
																Messages
																		.getString(
																				"Kitchen.Log.GettingLostJobsInDirectory",
																				""
																						+ directory));

											String jobnames[] = repository
													.getJobNames(directory
															.getID());
										...
										System.out
												.println(Messages
														.getString("Kitchen.Error.CanNotVerifyUserPass"));
									...
									System.out
											.println(Messages
													.getString(
															"Kitchen.Error.CanNotFindSuppliedDirectory",
															optionDirname + ""));
								...
								System.out
										.println(Messages
												.getString("Kitchen.Error.CanNotConnectRep"));

							}
						} else {
							System.out.println(Messages
									.getString("Kitchen.Error.NoRepProvided"));

						}
					} else {
						System.out.println(Messages
								.getString("Kitchen.Error.NoRepDefinied"));
				...
				if (ri.readData()) {
					System.out.println(Messages
							.getString("Kitchen.Log.ListRep"));

					for (int i = 0; i < ri.nrRepositories(); i++) {
						RepositoryMeta rinfo = ri.getRepository(i);
						System.out.println("#" + (i + 1) + " : "
								+ rinfo.getName() + " ["
								+ rinfo.getDescription() + "] ");
					}
				} else {
					System.out.println(Messages
							.getString("Kitchen.Error.UnableToReadXMLFile"));
		...
		if (job == null) {
			if (!"Y".equalsIgnoreCase(optionListjobs.toString())
				...
				System.out.println(Messages
						.getString("Kitchen.Error.canNotLoadJob"));
			}
