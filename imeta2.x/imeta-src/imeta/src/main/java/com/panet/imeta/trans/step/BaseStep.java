...
import com.panet.imeta.trans.SlaveStepCopyPartitionDistribution;
import com.panet.imeta.trans.StepLoader;
import com.panet.imeta.trans.StepPlugin;
import com.panet.imeta.trans.StepPluginMeta;
...
public class BaseStep extends Thread implements VariableSpace, StepInterface {
	...
	static {
		...
		synchronized (BaseStep.class) {
			try {
				...
				ConfigManager<?> stepsAnntCfg = KettleConfig.getInstance()
						.getManager("steps-annotation-config");
	...
	/*
	 * public static final StepPluginMeta[] steps = { TODO: port these steps
	 * 
	 * new StepPluginMeta(WebServiceMeta.class, "WebServiceLookup",
	 * Messages.getString("BaseStep.TypeLongDesc.WebServiceLookup"),
	 * Messages.getString("BaseStep.TypeTooltipDesc.WebServiceLookup"),
	 * "WSL.png", CATEGORY_EXPERIMENTAL), new StepPluginMeta(FormulaMeta.class,
	 * "Formula", Messages.getString("BaseStep.TypeLongDesc.Formula"),
	 * Messages.getString("BaseStep.TypeTooltipDesc.Formula"), "FRM.png",
	 * CATEGORY_EXPERIMENTAL), };
	 */

	/**
	 * @deprecated Please use StepCategory.STANDARD_CATEGORIES to get the
	 *             natural order
	 */
	public static final String category_order[] = {
			StepCategory.INPUT.getName(), StepCategory.OUTPUT.getName(),
			StepCategory.LOOKUP.getName(), StepCategory.TRANSFORM.getName(),
			StepCategory.JOINS.getName(), StepCategory.SCRIPTING.getName(),
			StepCategory.DATA_WAREHOUSE.getName(),
			StepCategory.MAPPING.getName(), StepCategory.JOB.getName(),
			StepCategory.INLINE.getName(), StepCategory.EXPERIMENTAL.getName(),
			StepCategory.DEPRECATED.getName(), StepCategory.BULK.getName(), };
	 ...
	 * 
	 * @deprecated please use the supplied getters, setters and
	...
	/**
	 * total sleep time in ns caused by an empty input buffer (previous step is
	 * slow)
	 * 
	 * @deprecated please use the supplied getters, setters and
	 *             increment/decrement methods
	...
	/**
	 * Map of files that are generated or used by this step. After execution,
	 * these can be added to result. The entry to the map is the filename
	 */
	private Map<String, ResultFile> resultFiles;

	/**
	 * Set this to true if you want to have extra checking enabled on the rows
	 * that are entering this step. All too often people send in bugs when it is
	 * really the mixing of different types of rows that is causing the problem.
	 */
	private boolean safeModeEnabled;

	/**
	 * This contains the first row received and will be the reference row. We
	 * used it to perform extra checking: see if we don't get rows with "mixed"
	 * contents.
	 */
	...
	/**
	 * The partition ID at which this step copy runs, or null if this step is
	 * not running partitioned.
	 */
	private String partitionID;

	/**
	 * This field tells the putRow() method to re-partition the incoming data,
	 * See also StepPartitioningMeta.PARTITIONING_METHOD_*
	 */
	...
	/**
	 * The metadata information of the error output row. There is only one per
	 * step so we cache it
	 */
	...
	/**
	 * A list of server sockets that need to be closed during transformation
	 * cleanup.
	 */
	...
	/**
	 * A flag to determine whether or not we are doing local or clustered
	 * (remote) par
	 */
	...
	/**
	 * This is the base step that forms that basis for all steps. You can derive
	 * from this class to implement your own steps.
	 * 
	 * @param stepMeta
	...
	public BaseStep(StepMeta stepMeta, StepDataInterface stepDataInterface,
		...
		if (distributed)
			if (log.isDetailed())
				logDetailed(Messages
						.getString("BaseStep.Log.DistributionActivated")); //$NON-NLS-1$
			else if (log.isDetailed())
				logDetailed(Messages
						.getString("BaseStep.Log.DistributionDeactivated")); //$NON-NLS-1$
	...
	public boolean init(StepMetaInterface smi, StepDataInterface sdi) {
		...
		String slaveNr = transMeta
				.getVariable(Const.INTERNAL_VARIABLE_SLAVE_SERVER_NUMBER);
		String clusterSize = transMeta
				.getVariable(Const.INTERNAL_VARIABLE_CLUSTER_SIZE);
		boolean master = "Y".equalsIgnoreCase(transMeta
				.getVariable(Const.INTERNAL_VARIABLE_CLUSTER_MASTER));
		...
		if (!Const.isEmpty(slaveNr) && !Const.isEmpty(clusterSize) && !master) {
			...
			if (log.isDetailed())
				logDetailed("Running on slave server #" + slaveNr + "/"
						+ clusterSize + ".");
		} else {
			this.slaveNr = 0;
			this.clusterSize = 0;
		...
		if (stepMeta.isPartitioned()) {
			...
			if (partitionDistribution != null
					&& !partitionDistribution.getDistribution().isEmpty()) {
				...
				// Set the partition ID (string) as well as the partition nr
				// [0..size[
				//
				PartitionSchema partitionSchema = stepMeta
						.getStepPartitioningMeta().getPartitionSchema();
				int partitionNr = partitionDistribution.getPartition(
						slaveServerName, partitionSchema.getName(), stepCopyNr);
				if (partitionNr >= 0) {
					String partitionNrString = new DecimalFormat("000")
							.format(partitionNr);
					setVariable(Const.INTERNAL_VARIABLE_STEP_PARTITION_NR,
							partitionNrString);

					if (partitionDistribution.getOriginalPartitionSchemas() != null) {
						// What is the partition schema name?
						//
						String partitionSchemaName = stepMeta
								.getStepPartitioningMeta().getPartitionSchema()
								.getName();

						// Search the original partition schema in the
						// distribution...
						//
						for (PartitionSchema originalPartitionSchema : partitionDistribution
							...
							String slavePartitionSchemaName = TransSplitter
									.createSlavePartitionSchemaName(originalPartitionSchema
											.getName());
							if (slavePartitionSchemaName
									.equals(partitionSchemaName)) {
								PartitionSchema schema = (PartitionSchema) originalPartitionSchema
										.clone();

								// This is the one...
								//
								if (schema.isDynamicallyDefined()) {
									schema.expandPartitionsDynamically(
											this.clusterSize, this);
								}

								String partID = schema.getPartitionIDs().get(
										partitionNr);
								setVariable(
										Const.INTERNAL_VARIABLE_STEP_PARTITION_ID,
										partID);
								break;
							}
						}
				...
				setVariable(Const.INTERNAL_VARIABLE_STEP_PARTITION_NR,
						partitionNrString);
				String partitionID = stepMeta.getStepPartitioningMeta()
						.getPartitionSchema().getPartitionIDs()
						.get(partitionNr);
				setVariable(Const.INTERNAL_VARIABLE_STEP_PARTITION_ID,
						partitionID);
			}
		...
		//
		uniqueStepNrAcrossSlaves = this.slaveNr * getStepMeta().getCopies()
				+ stepcopy;
		uniqueStepCountAcrossSlaves = this.clusterSize <= 1 ? getStepMeta()
				.getCopies() : this.clusterSize * getStepMeta().getCopies();
		if (uniqueStepCountAcrossSlaves == 0)
			uniqueStepCountAcrossSlaves = 1;

		setVariable(Const.INTERNAL_VARIABLE_STEP_UNIQUE_NUMBER, Integer
				.toString(uniqueStepNrAcrossSlaves));
		setVariable(Const.INTERNAL_VARIABLE_STEP_UNIQUE_COUNT, Integer
				.toString(uniqueStepCountAcrossSlaves));
		setVariable(Const.INTERNAL_VARIABLE_STEP_COPYNR, Integer
				.toString(stepcopy));

		// Now that these things have been done, we also need to start a number
		// of server sockets.
		...
		try {
			...
			//
			// boolean isMaster =
			// "Y".equalsIgnoreCase(getVariable(Const.INTERNAL_VARIABLE_CLUSTER_MASTER));
			...
			for (int i = 0; i < stepMeta.getRemoteOutputSteps().size(); i++) {
				...
				// If the step run in multiple copies, we only want to open
				// every socket once.
				// 
				if (stepMeta.getCopies() == 1
						|| (stepMeta.getCopies() > 1 && getCopy() == i)) {
					// Open a server socket to allow the remote output step to
					// connect.
					//
					...
					try {
						...
						if (log.isDetailed())
							logDetailed("Opened a server socket connection to "
									+ copy);
					} catch (Exception e) {
						log.logError(toString(),
								"Unable to open server socket during step initialisation: "
										+ copy.toString(), e);
				...
				if (remoteStep.getServerSocket() != null) {
					try {
						...
						log
								.logError(
										toString(),
										"Unable to close server socket after error during step initialisation",
										e);
					}
				}
			}
		...
		// For the remote input steps to read from, we do the same: make a list
		// and initialize what we can...
		//
		try {
			remoteInputSteps = new ArrayList<RemoteStep>();

			if ((stepMeta.isPartitioned() && getClusterSize() > 1)
					|| stepMeta.getCopies() > 1) {
				// If the step is partitioned or has multiple copies and
				// clustered, we only want to take one remote input step per
				// copy.
				// This is where we make that selection...
				//
				for (int i = 0; i < stepMeta.getRemoteInputSteps().size(); i++) {
					RemoteStep remoteStep = stepMeta.getRemoteInputSteps().get(
							i);
			...
			log
					.logError(
							toString(),
							"Unable to initialize remote input steps during step initialisation",
							e);
			return false;
		}
	...
	public void cleanup() {
		for (ServerSocket serverSocket : serverSockets) {
			try {
				...
				log.logError(toString(),
						"Cleanup: Unable to close server socket ("
	...
	/**
	 * @return Returns the number of lines read from an input source: database,
	 *         file, socket, etc.
	 */
	...
	/**
	 * Increments the number of lines read from an input source: database, file,
	 * socket, etc.
	 * 
	 ...
	 *            the new number of lines read from an input source: database,
	 *            file, socket, etc.
	 */
	...
	/**
	 * @return Returns the number of lines written to an output target:
	 *         database, file, socket, etc.
	 */
	...
	/**
	 * Increments the number of lines written to an output target: database,
	 * file, socket, etc.
	 * 
	 * @return the new incremented value
	 */
	 ...
	 *            the new number of lines written to an output target: database,
	 *            file, socket, etc.
	 */
	...
	/**
	 * @return Returns the number of lines updated in an output target:
	 *         database, file, socket, etc.
	 */
	...
	/**
	 * Increments the number of lines updated in an output target: database,
	 * file, socket, etc.
	 * 
	 * @return the new incremented value
	 */
	...
	/**
	 * @param newLinesOutputValue
	 *            the new number of lines updated in an output target: database,
	 *            file, socket, etc.
	 */
	...
	/**
	 * putRow is used to copy a row, to the alternate rowset(s) This should get
	 * priority over everything else! (synchronized) If distribute is true, a
	 * row is copied only once to the output rowsets, otherwise copies are sent
	 * to each rowset!
	 * 
	 * @param row
	...
	public void putRow(RowMetaInterface rowMeta, Object[] row)
		...
		// Right after the pause loop we have to check if this thread is stopped
		// or not.
		//
		if (stopped.get()) {
			if (log.isDebug())
		...
		// Repartitioning happens when the current step is not partitioned, but
		// the next one is.
		// That means we need to look up the partitioning information in the
		// next step..
		// If there are multiple steps, we need to look at the first (they
		...
		case StepPartitioningMeta.PARTITIONING_METHOD_SPECIAL: {
			if (nextStepPartitioningMeta == null) {
				...
				if (nextSteps.length > 0) {
					nextStepPartitioningMeta = nextSteps[0]
			...
			int partitionNr;
			try {
				partitionNr = nextStepPartitioningMeta.getPartition(rowMeta,
						row);
			...
			if (clusteredPartitioningFirst) {
				...
				// We are only running remotely if both the distribution is
				// there AND if the distribution is actually contains something.
				//
				clusteredPartitioning = transMeta
						.getSlaveStepCopyPartitionDistribution() != null
						&& !transMeta.getSlaveStepCopyPartitionDistribution()
								.getDistribution().isEmpty();
			}

			// OK, we have a SlaveStepCopyPartitionDistribution in the
			// transformation...
			// We want to pre-calculate what rowset we're sending data to for
			// which partition...
			...
			if (clusteredPartitioning) {
				...
				if (partitionNrRowSetList == null) {
					...
					// The distribution is calculated during transformation
					// split
					// The slave-step-copy distribution is passed onto the slave
					// transformation
					//
					SlaveStepCopyPartitionDistribution distribution = transMeta
							.getSlaveStepCopyPartitionDistribution();

					String nextPartitionSchemaName = TransSplitter
							.createPartitionSchemaNameFromTarget(nextStepPartitioningMeta
									.getPartitionSchema().getName());

					for (RowSet outputRowSet : outputRowSets) {
						try {
							// Look at the pre-determined distribution, decided
							// at "transformation split" time.
							//
							int partNr = distribution.getPartition(outputRowSet
									.getRemoteSlaveServerName(),
									nextPartitionSchemaName, outputRowSet
				...
				// Local partitioning...
				// Put the row forward to the next step according to the
				// partition rule.
				//
			...
			if (selectedRowSet == null) {
				logBasic("Target rowset is not available for target partition, partitionNr="
						+ partitionNr);
			}
			...
			if (log.isRowLevel())
				try {
					logRowlevel("Partitioned #" + partitionNr + " to "
							+ selectedRowSet + ", row="
	 ...
	 */
	public void putRowTo(RowMetaInterface rowMeta, Object[] row, RowSet rowSet)
			throws KettleStepException {
		// Are we pausing the step? If so, stall forever...
		//
		while (paused.get() && !stopped.get()) {
	...
	public void putError(RowMetaInterface rowMeta, Object[] row, long nrErrors,
		...
		if (errorRowMeta == null) {
			...
			RowMetaInterface add = stepErrorMeta.getErrorRowMeta(nrErrors,
					errorDescriptions, fieldNames, errorCodes);
			errorRowMeta.addRowMeta(add);
		}

		Object[] errorRowData = RowDataUtil
				.allocateRowData(errorRowMeta.size());
		if (row != null)
	...
	private void verifyRejectionRates() {
		...
		if (stepErrorMeta.getMaxErrors() > 0
				&& getLinesRejected() > stepErrorMeta.getMaxErrors()) {
			logError(Messages.getString("BaseStep.Log.TooManyRejectedRows",
					Long.toString(stepErrorMeta.getMaxErrors()), Long
							.toString(getLinesRejected())));
			setErrors(1L);
			stopAll();
		}

		if (stepErrorMeta.getMaxPercentErrors() > 0
				&& getLinesRejected() > 0
				&& (stepErrorMeta.getMinPercentRows() <= 0 || getLinesRead() >= stepErrorMeta
						.getMinPercentRows())) {
			int pct = (int) (100 * getLinesRejected() / getLinesRead());
			if (pct > stepErrorMeta.getMaxPercentErrors()) {
				logError(Messages.getString(
						"BaseStep.Log.MaxPercentageRejectedReached", Integer
								.toString(pct), Long
								.toString(getLinesRejected()), Long
								.toString(getLinesRead())));
				setErrors(1L);
				stopAll();
			}
		}
	...
	public Object[] getRow() throws KettleException {
		...
		if (stopped.get()) {
			if (log.isDebug())
				logDebug(Messages
						.getString("BaseStep.Log.StopLookingForMoreRows")); //$NON-NLS-1$
			stopAll();
			return null;
		}
		...
		if (blockPointer >= NR_OF_ROWS_IN_BLOCK) {
			...
			// If there is no data, process another NR_OF_ROWS_IN_BLOCK on the
			// next input stream.
			//
		...
		//
		if (isUsingThreadPriorityManagment() && !inputRowSet.isDone()
				&& inputRowSet.size() <= (transMeta.getSizeRowset() >> 6)
				&& !isStopped()) {
			try {
				Thread.sleep(0, 1);
			} catch (InterruptedException e) {
		...
		while (row == null && !isStopped()) {
			...
			if (row != null) {
				...
				// Try once more...
				// If row is still empty and the row set is done, we remove the
				// row set from
		...
		while (row == null && !stopped.get()) {
			// Try the next input row set(s) until we find a row set that still
			// has rows...
		...
		if (row != null) {
			// OK, before we return the row, let's see if we need to check on
			// mixing row compositions...
			// 
			if (safeModeEnabled) {
				...
				if (row.length < inputRowMeta.size()) {
					throw new KettleException(
							"Safe mode check noticed that the length of the row data is smaller ("
									+ row.length
									+ ") than the row metadata size ("
									+ inputRowMeta.size() + ")");
				}
			}
	 ...
	 * This method should be used by steps that don't call getRow() first in
	 * which it is executed automatically. <br>
	 * <b>This method should be called before any data is read from previous
	 * steps.</b> <br>
	...
	protected void openRemoteInputStepSocketsOnce() throws KettleStepException {
		if (!remoteInputSteps.isEmpty()) {
			if (!remoteInputStepsInitialized) {
				...
				// Just be careful in case we're dealing with a partitioned
				// clustered step.
				...
				for (RemoteStep remoteStep : remoteInputSteps) {
					try {
						...
						throw new KettleStepException(
								"Error opening reader socket to remote step '"
										+ remoteStep + "'", e);
					}
				}
	 ...
	 * This method is called in method initBeforeStart() because it needs to
	 * connect to the server sockets (remote steps) as soon as possible to avoid
	 * time-out situations. <br>
	 * This action is executed only once.
	 * 
	 * @throws KettleStepException
	 */
	protected void openRemoteOutputStepSocketsOnce() throws KettleStepException {
		if (!remoteOutputSteps.isEmpty()) {
			if (!remoteOutputStepsInitialized) {
				...
				// Set the current slave target name on all the current output
				// steps (local)
				//
				for (int c = 0; c < outputRowSets.size(); c++) {
					RowSet rowSet = outputRowSets.get(c);
					rowSet
							.setRemoteSlaveServerName(getVariable(Const.INTERNAL_VARIABLE_SLAVE_SERVER_NAME));
				...
				// Start threads: one per remote step to funnel the data
				// through...
				//
				for (int i = 0; i < remoteOutputSteps.size(); i++) {
					...
					try {
						if (remoteStep.getTargetSlaveServerName() == null) {
							throw new KettleStepException(
									"The target slave server name is not defined for remote output step: "
											+ remoteStep);
						}
						RowSet rowSet = remoteStep.openWriterSocket();
						if (log.isDetailed())
							logDetailed("Opened a writer socket to remote step: "
									+ remoteStep);
						outputRowSets.add(rowSet);
					} catch (IOException e) {
						throw new KettleStepException(
								"Error opening writer socket to remote step '"
										+ remoteStep + "'", e);
					}
				}
	...
	protected void safeModeChecking(RowMetaInterface row)
		...
		if (inputReferenceRow == null) {
			...
			for (int i = 0; i < fieldnames.length - 1; i++) {
				if (fieldnames[i].equals(fieldnames[i + 1])) {
					throw new KettleRowException(Messages.getString(
	...
	public static void safeModeChecking(RowMetaInterface referenceRowMeta,
			RowMetaInterface rowMeta) throws KettleRowException {
		...
		if (referenceRowMeta.size() != rowMeta.size()) {
			throw new KettleRowException(Messages.getString(
					"BaseStep.SafeMode.Exception.VaryingSize", ""
							+ referenceRowMeta.size(), "" + rowMeta.size(),
					rowMeta.toString()));
		} else {
			// Check field by field for the position of the names...
			for (int i = 0; i < referenceRowMeta.size(); i++) {
				ValueMetaInterface referenceValue = referenceRowMeta
						.getValueMeta(i);
				ValueMetaInterface compareValue = rowMeta.getValueMeta(i);

				if (!referenceValue.getName().equalsIgnoreCase(
						compareValue.getName())) {
					throw new KettleRowException(Messages.getString(
							"BaseStep.SafeMode.Exception.MixingLayout", ""
									+ (i + 1), referenceValue.getName() + " "
									+ referenceValue.toStringMeta(),
							compareValue.getName() + " "
	...
	public Object[] getRowFrom(RowSet rowSet) throws KettleStepException {
		...
		// We'll only do that if the previous step has not ended...
		//
		if (isUsingThreadPriorityManagment() && !rowSet.isDone()
				&& rowSet.size() <= (transMeta.getSizeRowset() >> 6)
				&& !isStopped()) {
			try {
				Thread.sleep(0, 1);
			} catch (InterruptedException e) {
	...
	public RowSet findInputRowSet(String from, int fromcopy, String to,
			int tocopy) {
	...
	public RowSet findOutputRowSet(String from, int fromcopy, String to,
			int tocopy) {
	...
	public void setOutputDone() {
		if (log.isDebug())
			logDebug(Messages
					.getString(
							"BaseStep.Log.OutputDone", String.valueOf(outputRowSets.size()))); //$NON-NLS-1$ //$NON-NLS-2$
	 ...
	 * This steps keeps it's own list of rowsets (etc.) to prevent it from
	 * having to search every time.
	 */
	public void dispatch() {
		if (transMeta == null) { // for preview reasons, no dispatching is
			// done!
			return;
		}
		...
		if (log.isDetailed())
			logDetailed(Messages
					.getString("BaseStep.Log.StartingBuffersAllocation")); //$NON-NLS-1$

		// How many next steps are there? 0, 1 or more??
		// How many steps do we send output to?
		List<StepMeta> previousSteps = transMeta.findPreviousSteps(stepMeta,
				true);
		...
		for (int i = 0; i < previousSteps.size(); i++) {
			...
			// Looking at the previous step, you can have either 1 rowset to
			// look at or more then one.
			...
			for (int c = 0; c < nrCopies; c++) {
				...
				case Trans.TYPE_DISP_1_1:
					rowSet = trans.findRowSet(prevSteps[i].getName(), 0,
							stepname, 0);
					break;
				...
				case Trans.TYPE_DISP_N_1:
					rowSet = trans.findRowSet(prevSteps[i].getName(), c,
							stepname, 0);
					break;
				case Trans.TYPE_DISP_N_N:
					rowSet = trans.findRowSet(prevSteps[i].getName(),
							getCopy(), stepname, getCopy());
					break;
				...
				if (rowSet != null) {
					...
					if (log.isDetailed())
						logDetailed(Messages
								.getString(
										"BaseStep.Log.FoundInputRowset", rowSet.getName())); //$NON-NLS-1$ //$NON-NLS-2$
				} else {
					if (!prevSteps[i].isMapping() && !stepMeta.isMapping()) {
						logError(Messages
								.getString("BaseStep.Log.UnableToFindInputRowset")); //$NON-NLS-1$
						setErrors(1);
						stopAll();
						return;
					}
		...
		for (int i = 0; i < nrOutput; i++) {
			...
			for (int c = 0; c < nrCopies; c++) {
				...
				switch (dispatchType) {
				case Trans.TYPE_DISP_1_1:
					rowSet = trans.findRowSet(stepname, 0, nextSteps[i]
							.getName(), 0);
					break;
				case Trans.TYPE_DISP_1_N:
					rowSet = trans.findRowSet(stepname, 0, nextSteps[i]
					...
					break;
				case Trans.TYPE_DISP_N_1:
					rowSet = trans.findRowSet(stepname, getCopy(), nextSteps[i]
							.getName(), 0);
					break;
				case Trans.TYPE_DISP_N_N:
					rowSet = trans.findRowSet(stepname, getCopy(), nextSteps[i]
				...
				if (rowSet != null) {
					...
					if (log.isDetailed())
						logDetailed(Messages
								.getString(
										"BaseStep.Log.FoundOutputRowset", rowSet.getName())); //$NON-NLS-1$ //$NON-NLS-2$
				} else {
					if (!stepMeta.isMapping() && !nextSteps[i].isMapping()) {
						logError(Messages
								.getString("BaseStep.Log.UnableToFindOutputRowset")); //$NON-NLS-1$
						setErrors(1);
						stopAll();
						return;
					}
	...
	public void markStop() {
		...
		// Call all the attached listeners and notify the outside world that the
		// step has finished.
		//
	...
	public RowMetaAndData buildLog(String sname, int copynr, long lines_read,
			long lines_written, long lines_updated, long lines_skipped,
			long errors, Date start_date, Date end_date) {
		...
		r
				.addValueMeta(new ValueMeta(
						Messages.getString("BaseStep.ColumnName.Stepname"), ValueMetaInterface.TYPE_STRING)); //$NON-NLS-1$
		data[nr] = sname;
		nr++;

		r
				.addValueMeta(new ValueMeta(
						Messages.getString("BaseStep.ColumnName.Copy"), ValueMetaInterface.TYPE_NUMBER)); //$NON-NLS-1$
		data[nr] = new Double(copynr);
		nr++;

		r
				.addValueMeta(new ValueMeta(
						Messages.getString("BaseStep.ColumnName.LinesReaded"), ValueMetaInterface.TYPE_NUMBER)); //$NON-NLS-1$
		data[nr] = new Double(lines_read);
		nr++;

		r
				.addValueMeta(new ValueMeta(
						Messages.getString("BaseStep.ColumnName.LinesWritten"), ValueMetaInterface.TYPE_NUMBER)); //$NON-NLS-1$
		data[nr] = new Double(lines_written);
		nr++;

		r
				.addValueMeta(new ValueMeta(
						Messages.getString("BaseStep.ColumnName.LinesUpdated"), ValueMetaInterface.TYPE_NUMBER)); //$NON-NLS-1$
		data[nr] = new Double(lines_updated);
		nr++;

		r
				.addValueMeta(new ValueMeta(
						Messages.getString("BaseStep.ColumnName.LinesSkipped"), ValueMetaInterface.TYPE_NUMBER)); //$NON-NLS-1$
		data[nr] = new Double(lines_skipped);
		nr++;

		r
				.addValueMeta(new ValueMeta(
						Messages.getString("BaseStep.ColumnName.Errors"), ValueMetaInterface.TYPE_NUMBER)); //$NON-NLS-1$
		data[nr] = new Double(errors);
		nr++;

		r
				.addValueMeta(new ValueMeta(
						"start_date", ValueMetaInterface.TYPE_DATE)); //$NON-NLS-1$
		data[nr] = start_date;
		nr++;
	...
	public static final RowMetaInterface getLogFields(String comm) {
		...
		ValueMetaInterface sname = new ValueMeta(
				Messages.getString("BaseStep.ColumnName.Stepname"), ValueMetaInterface.TYPE_STRING); //$NON-NLS-1$ //$NON-NLS-2$
		sname.setLength(256);
		...
		r
				.addValueMeta(new ValueMeta(
						Messages.getString("BaseStep.ColumnName.StartDate"), ValueMetaInterface.TYPE_DATE)); //$NON-NLS-1$
		r
				.addValueMeta(new ValueMeta(
						Messages.getString("BaseStep.ColumnName.EndDate"), ValueMetaInterface.TYPE_DATE)); //$NON-NLS-1$
	...
	/**
	 * Perform actions to stop a running step. This can be stopping running SQL
	 * queries (cancel), etc. Default it doesn't do anything.
	 * 
	 * @param stepDataInterface
	 *            The interface to the step data containing the connections,
	 *            resultsets, open files, etc.
	...
	/**
	 * Stops running operations This method is deprecated, please use the method
	 * specifying the metadata and data interfaces.
	 * 
	 * @deprecated
	 */
	...
	public void logSummary() {
		synchronized (statusCountersLock) {
			...
			if (li > 0 || lo > 0 || lr > 0 || lw > 0 || lu > 0 || lj > 0
					|| errors > 0)
				logBasic(Messages.getString("BaseStep.Log.SummaryInfo", String
						.valueOf(li), String.valueOf(lo), String.valueOf(lr),
						String.valueOf(lw), String.valueOf(lw), String
								.valueOf(errors + lj)));
			else
				logDetailed(Messages.getString("BaseStep.Log.SummaryInfo",
						String.valueOf(li), String.valueOf(lo), String
								.valueOf(lr), String.valueOf(lw), String
								.valueOf(lw), String.valueOf(errors + lj)));
		}
	}
	...
	/**
	 * @return Returns true is this step is running in safe mode, with extra
	 *         checking enabled...
	 */
	 ...
	 *            set to true is this step has to be running in safe mode, with
	...
	public int getStatus() {
		...
		if (sdi != null) {
			if (sdi.getStatus() == StepDataInterface.STATUS_DISPOSED
					&& !isAlive())
				return StepDataInterface.STATUS_FINISHED;
	...
	protected boolean checkFeedback(long lines) {
		return getTransMeta().isFeedbackShown() && (lines > 0)
				&& (getTransMeta().getFeedbackSize() > 0)
	...
	/**
	 * @return a unique step number across all slave servers: slaveNr * nrCopies
	 *         + copyNr
	 */
	...
	/**
	 * This method is executed by Trans right before the threads start and right
	...
	public static void runStepThread(StepInterface stepInterface,
			StepMetaInterface meta, StepDataInterface data) {
		LogWriter log = LogWriter.getInstance();
		try {
			if (log.isDetailed())
				log.logDetailed(stepInterface.toString(), Messages
						.getString("System.Log.StartingToRun")); //$NON-NLS-1$

			while (stepInterface.processRow(meta, data)
					&& !stepInterface.isStopped())
				;
		} catch (Throwable t) {
			try {
				// check for OOME
				if (t instanceof OutOfMemoryError) {
					// Handle this different with as less overhead as possible
					// to get an error message in the log.
					// Otherwise it crashes likely with another OOME in
					// Me$$ages.getString() and does not log
					// nor call the setErrors() and stopAll() below.
					log.logError(stepInterface.toString(),
							"UnexpectedError: " + t.toString()); //$NON-NLS-1$
				} else {
					log.logError(stepInterface.toString(), Messages
							.getString("System.Log.UnexpectedError") + " : "); //$NON-NLS-1$ //$NON-NLS-2$
				}
				log
						.logError(stepInterface.toString(), Const
								.getStackTracker(t));
			...
			try {
				...
				if (li > 0 || lo > 0 || lr > 0 || lw > 0 || lu > 0 || lj > 0
						|| e > 0)
					log.logBasic(stepInterface.toString(), Messages.getString(
							"BaseStep.Log.SummaryInfo", String.valueOf(li),
							String.valueOf(lo), String.valueOf(lr), String
									.valueOf(lw), String.valueOf(lw), String
									.valueOf(e + lj)));
				else
					log.logDetailed(stepInterface.toString(), Messages
							.getString("BaseStep.Log.SummaryInfo", String
									.valueOf(li), String.valueOf(lo), String
									.valueOf(lr), String.valueOf(lw), String
									.valueOf(lw), String.valueOf(e + lj)));
			} catch (Throwable t) {
				// it's likely an OOME, thus no overhead by
				// Me$$ages.getString(), see above
				log.logError(stepInterface.toString(),
						"UnexpectedError: " + t.toString()); //$NON-NLS-1$
			} finally {
