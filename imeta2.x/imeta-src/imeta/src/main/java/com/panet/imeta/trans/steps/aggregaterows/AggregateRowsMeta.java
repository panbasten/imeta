 ...
 * 
 */
public class AggregateRowsMeta extends BaseStepMeta implements
		StepMetaInterface {
	...
	public final static String aggregateTypeDesc[] = {
			...
			Messages
					.getString("AggregateRowsMeta.AggregateTypeDesc.FIRST_NULL"), //$NON-NLS-1$
	...
	private void readData(Node stepnode) throws KettleXMLException {
		try {
			...
			throw new KettleXMLException(
					Messages
							.getString("AggregateRowsMeta.Exception.UnableToLoadStepInfo"), e); //$NON-NLS-1$
		}
	}
	...
	public void setDefault() {
		...
		for (i = 0; i < nrfields; i++) {
			fieldName[i] = Messages
					.getString("AggregateRowsMeta.Fieldname.Label"); //$NON-NLS-1$
			fieldNewName[i] = Messages
					.getString("AggregateRowsMeta.NewName.Label"); //$NON-NLS-1$
	...
	public String getXML() {
		...
		for (int i = 0; i < fieldName.length; i++) {
			...
			retval
					.append("        ").append(XMLHandler.addTagValue("rename", fieldNewName[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval
	...
	public void readRep(Repository rep, long id_step,
		...
		try {
			...
			for (int i = 0; i < nrfields; i++) {
				...
				fieldNewName[i] = rep.getStepAttributeString(id_step, i,
			...
			throw new KettleException(
					Messages
							.getString("AggregateRowsMeta.Exception.UnexpectedErrorWhileReadingStepInfo"), e); //$NON-NLS-1$
		}
	...
	public void saveRep(Repository rep, long id_transformation, long id_step)
		...
		try {
			for (int i = 0; i < fieldName.length; i++) {
				...
				rep.saveStepAttribute(id_transformation, id_step, i,
						"field_rename", fieldNewName[i]); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i,
						"field_type", getTypeDesc(aggregateType[i])); //$NON-NLS-1$
			}
		} catch (KettleException e) {
			throw new KettleException(
					Messages
							.getString("AggregateRowsMeta.Exception.UnableToSaveStepInfo") + id_step, e); //$NON-NLS-1$
		}
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (fieldName.length > 0) {
			...
			message = Messages
					.getString("AggregateRowsMeta.CheckResult.FieldsNotFound.DialogMessage") + Const.CR; //$NON-NLS-1$
			for (int i = 0; i < fieldName.length; i++) {
			...
			if (error_found) {
				cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, message,
						stepMeta);
			} else {
				message = Messages
						.getString("AggregateRowsMeta.CheckResult.AllFieldsOK.DialogMessage"); //$NON-NLS-1$
				cr = new CheckResult(CheckResult.TYPE_RESULT_OK, message,
						stepMeta);
			}
			remarks.add(cr);
			...
			message = Messages
					.getString("AggregateRowsMeta.CheckResult.IgnoredFields.DialogMessage") + Const.CR; //$NON-NLS-1$
			error_found = false;
			...
			if (error_found) {
				cr = new CheckResult(CheckResult.TYPE_RESULT_COMMENT, message,
						stepMeta);
			} else {
				message = Messages
						.getString("AggregateRowsMeta.CheckResult.AllFieldsUsed.DialogMessage"); //$NON-NLS-1$
				cr = new CheckResult(CheckResult.TYPE_RESULT_OK, message,
						stepMeta);
			}
			remarks.add(cr);
			...
			message = Messages
					.getString("AggregateRowsMeta.CheckResult.NothingSpecified.DialogMessage"); //$NON-NLS-1$
			cr = new CheckResult(CheckResult.TYPE_RESULT_WARNING, message,
					stepMeta);
			remarks.add(cr);
		}
		...
		if (input.length > 0) {
			cr = new CheckResult(
					...
					Messages
							.getString("AggregateRowsMeta.CheckResult.StepReceiveInfo.DialogMessage"), stepMeta); //$NON-NLS-1$
			remarks.add(cr);
		} else {
			cr = new CheckResult(
					CheckResult.TYPE_RESULT_ERROR,
					Messages
							.getString("AggregateRowsMeta.CheckResult.NoInputReceived.DialogMessage"), stepMeta); //$NON-NLS-1$
			remarks.add(cr);
		}
	...
	public StepInterface getStep(StepMeta stepMeta,
			...
			Trans trans) {
		return new AggregateRows(stepMeta, stepDataInterface, cnr, transMeta,
				trans);
	}
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		this.fieldNewName = p.get(id + "_fields.fieldNewName") == null ? new String[0]
				: p.get(id + "_fields.fieldNewName");
		this.aggregateType = p.get(id + "_fields.aggregateType") == null ? new int[0]
				: BaseStepMeta.stringArrayToIntArray(p.get(id
						+ "_fields.aggregateType"), 0);
	}
}
