...
public class XMLInputSaxField implements Cloneable {
	...
	public final static String trimTypeDesc[] = { "none", "left", "right",
			"both" };
	...
	public String getXML() {
		...
		retval += "        "
				+ XMLHandler.addTagValue("trim_type", getTrimTypeDesc());
		retval += "        " + XMLHandler.addTagValue("repeat", isRepeated());

		retval += "        <positions>";
		for (int i = 0; i < fieldPosition.length; i++) {
			retval += XMLHandler.addTagValue("position", fieldPosition[i]
					.toString(), false);
		}
	...
	public Object clone() {
		try {
			...
			if (fieldPosition != null) {
				...
				for (int i = 0; i < fieldPosition.length; i++) {
					positions[i] = (XMLInputSaxFieldPosition) fieldPosition[i]
							.clone();
				}
	...
	public void setFieldPosition(String encoded) throws KettleException {
		try {
			if (Const.isEmpty(encoded)) {
				fieldPosition = new XMLInputSaxFieldPosition[0];
			} else {
			...
			throw new KettleException(
					"Unable to parse the field positions because of an error"
							+ Const.CR
							+ "Please use E=element or A=attribute in a comma separated list (code: "
							+ encoded + ")", e);
		}
	}
	...
	public boolean equals(Object arg) {
		if (arg instanceof XMLInputSaxField) {
			...
			if (this.name.equals(f.getName())
					&& this.getFieldPositionsCode().equals(
							f.getFieldPositionsCode())) {
				return true;
			} else {
				return false;
			}
