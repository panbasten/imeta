...
public class SortRowsMeta extends BaseStepMeta implements StepMetaInterface {
	...
	public static final String STEP_ATTRIBUTE_SORT_SIZE = "sort_size";
	public static final String STEP_ATTRIBUTE_FREE_MEMORY = "free_memory";
	public static final String STEP_ATTRIBUTE_COMPRESS = "compress";
	public static final String STEP_ATTRIBUTE_COMPRESS_VARIABLE = "compress_variable";
	public static final String STEP_ATTRIBUTE_UNIQUE_ROWS = "unique_rows";
	// directory = rep.getStepAttributeString(id_step, "directory");
	// prefix = rep.getStepAttributeString(id_step, "prefix");
	// int sortSizeInt = (int) rep.getStepAttributeInteger(id_step,
	// "sort_size");
	// if (sortSizeInt>0) {
	...
	// compressFilesVariable = rep.getStepAttributeString(id_step,
	// "compress_variable");
	//    
	// onlyPassingUniqueRows = rep.getStepAttributeBoolean(id_step,
	// "unique_rows");
	...
	private void readData(Node stepnode) throws KettleXMLException {
		try {
			...
			compressFiles = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "compress"));
			compressFilesVariable = XMLHandler.getTagValue(stepnode,
					"compress_variable");
			onlyPassingUniqueRows = "Y".equalsIgnoreCase(XMLHandler
					.getTagValue(stepnode, "unique_rows"));
			...
			for (int i = 0; i < nrfields; i++) {
				...
				caseSensitive[i] = Const.isEmpty(sens)
						|| "Y".equalsIgnoreCase(sens);
			}
		} catch (Exception e) {
	...
	public String getXML() {
		...
		retval.append("      ")
				.append(XMLHandler.addTagValue("prefix", prefix));
		retval.append("      ").append(
				XMLHandler.addTagValue("sort_size", sortSize));
		retval.append("      ").append(
				XMLHandler.addTagValue("free_memory", freeMemoryLimit));
		retval.append("      ").append(
				XMLHandler.addTagValue("compress", compressFiles));
		retval.append("      ").append(
				XMLHandler.addTagValue("compress_variable",
						compressFilesVariable));
		retval.append("      ").append(
				XMLHandler.addTagValue("unique_rows", onlyPassingUniqueRows));
		...
		for (int i = 0; i < fieldName.length; i++) {
			...
			retval.append("        ").append(
					XMLHandler.addTagValue("case_sensitive", caseSensitive[i]));
			retval.append("      </field>").append(Const.CR);
		}
		retval.append("    </fields>").append(Const.CR);
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		sortSize = BaseStepMeta.parameterToString(p.get(id + ".sortSize"));
		freeMemoryLimit = BaseStepMeta.parameterToString(p.get(id
				+ ".freeMemoryLimit"));
		compressFiles = BaseStepMeta.parameterToBoolean(p.get(id
				+ ".compressFiles"));
		compressFilesVariable = BaseStepMeta.parameterToString(p.get(id
				+ ".compressFilesVariable"));
		onlyPassingUniqueRows = BaseStepMeta.parameterToBoolean(p.get(id
				+ ".onlyPassingUniqueRows"));

		String[] fieldName = p.get(id + "_fields.fieldName");
		String[] ascending = p.get(id + "_fields.ascending");
		String[] caseSensitive = p.get(id + "_fields.caseSensitive");

		this.fieldName = (fieldName != null) ? fieldName : (new String[0]);
		this.ascending = stringArrayToBooleanArray(ascending, false);
		this.caseSensitive = stringArrayToBooleanArray(caseSensitive, false);
	}
	...
	public void readRep(Repository rep, long id_step,
		...
		try {
			...
			int sortSizeInt = (int) rep.getStepAttributeInteger(id_step,
					"sort_size");
			if (sortSizeInt > 0) {
				sortSize = Integer.toString(sortSizeInt); // For backward
				// compatibility
			} else {
				sortSize = rep.getStepAttributeString(id_step, "sort_size");
			}
			freeMemoryLimit = rep
					.getStepAttributeString(id_step, "free_memory");
			...
			for (int i = 0; i < nrfields; i++) {
				...
				ascending[i] = rep.getStepAttributeBoolean(id_step, i,
						"field_ascending");
				caseSensitive[i] = rep.getStepAttributeBoolean(id_step, i,
						"field_case_sensitive", true);
			}
		} catch (Exception e) {
	...
	public void saveRep(Repository rep, long id_transformation, long id_step)
			throws KettleException {
		try {
			rep.saveStepAttribute(id_transformation, id_step, "directory",
					directory);
			rep.saveStepAttribute(id_transformation, id_step, "prefix", prefix);
			rep.saveStepAttribute(id_transformation, id_step, "sort_size",
					sortSize);
			rep.saveStepAttribute(id_transformation, id_step, "free_memory",
					freeMemoryLimit);
			rep.saveStepAttribute(id_transformation, id_step, "compress",
					compressFiles);
			rep.saveStepAttribute(id_transformation, id_step,
					"compress_variable", compressFilesVariable);
			rep.saveStepAttribute(id_transformation, id_step, "unique_rows",
					onlyPassingUniqueRows);
			...
			for (int i = 0; i < fieldName.length; i++) {
				...
				rep.saveStepAttribute(id_transformation, id_step, i,
						"field_case_sensitive", caseSensitive[i]);
			}
		} catch (Exception e) {
			throw new KettleException(
	...
	public void getFields(RowMetaInterface inputRowMeta, String name,
		...
		for (int i = 0; i < fieldName.length; i++) {
			...
			if (idx >= 0) {
				...
				// If so we want to automatically convert them to the normal
				// storage type.
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (prev != null && prev.size() > 0) {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, Messages
					.getString("SortRowsMeta.CheckResult.FieldsReceived", ""
							+ prev.size()), stepMeta);
			remarks.add(cr);
			...
			if (error_found) {
				error_message = Messages.getString(
						"SortRowsMeta.CheckResult.SortKeysNotFound",
						error_message);
				...
				if (fieldName.length > 0) {
					cr = new CheckResult(
							...
							Messages
									.getString("SortRowsMeta.CheckResult.AllSortKeysFound"),
							stepMeta);
					remarks.add(cr);
				} else {
					cr = new CheckResult(
							...
							Messages
									.getString("SortRowsMeta.CheckResult.NoSortKeysEntered"),
							stepMeta);
					remarks.add(cr);
				}
			}
			...
			if (f.exists()) {
				if (f.isDirectory()) {
					cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK,
							Messages.getString(
									"SortRowsMeta.CheckResult.DirectoryExists",
					...
					cr = new CheckResult(
							...
							Messages
									.getString(
											"SortRowsMeta.CheckResult.ExistsButNoDirectory",
											realDirectory), stepMeta);
					remarks.add(cr);
				}
			} else {
				cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR,
						Messages.getString(
								"SortRowsMeta.CheckResult.DirectoryNotExists",
								realDirectory), stepMeta);
				remarks.add(cr);
			}
		} else {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR,
					Messages.getString("SortRowsMeta.CheckResult.NoFields"),
					stepMeta);
			remarks.add(cr);
		}
		...
		if (input.length > 0) {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, Messages
					.getString("SortRowsMeta.CheckResult.ExpectedInputOk"),
					stepMeta);
			remarks.add(cr);
		} else {
			cr = new CheckResult(
					CheckResultInterface.TYPE_RESULT_ERROR,
					Messages
							.getString("SortRowsMeta.CheckResult.ExpectedInputError"),
					stepMeta);
			remarks.add(cr);
		}
	}
