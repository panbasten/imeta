...
public class SynchronizeAfterMergeMeta extends BaseStepMeta implements StepMetaInterface
{
	...
	public String getXML()
	{
		...
		for (int i=0;i<updateLookup.length;i++)
		{
			...
			retval.append("        ").append(XMLHandler.addTagValue("update", update[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval.append("      </value>").append(Const.CR); //$NON-NLS-1$
		}
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		performLookup = BaseStepMeta.parameterToBoolean(p.get(id + ".performLookup"));
		operationOrderField = BaseStepMeta.parameterToString(p.get(id + ".operationOrderField"));
		OrderInsert = BaseStepMeta.parameterToString(p.get(id + ".OrderInsert"));
		OrderUpdate = BaseStepMeta.parameterToString(p.get(id + ".OrderUpdate"));
		OrderDelete = BaseStepMeta.parameterToString(p.get(id + ".OrderDelete"));
		
		String[] keyStream = p.get(id + "_keys.keyStream");
		String[] keyCondition = p.get(id + "_keys.keyCondition");
		String[] keyLookup = p.get(id + "_keys.keyLookup");
		String[] keyStream2 = p.get(id + "_keys.keyStream2");
		...
		String[] updateLookup = p.get(id + "_values.updateLookup");
		String[] updateStream = p.get(id + "_values.updateStream");
		String[] update = p.get(id + "_values.update");
		...
		throws KettleException
	{
		try
		{
			...
			for (int i=0;i<updateLookup.length;i++)
			{
				...
				rep.saveStepAttribute(id_transformation, id_step, i, "value_update",  update[i]); //$NON-NLS-1$
			}
