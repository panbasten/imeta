...
import com.panet.imeta.core.BlockingRowSet;
...
import com.panet.imeta.trans.step.RowListener;
import com.panet.imeta.trans.step.StepDataInterface;
import com.panet.imeta.trans.step.StepIOMeta;
import com.panet.imeta.trans.step.StepIOMetaInterface;
import com.panet.imeta.trans.step.StepListener;
import com.panet.imeta.trans.step.StepMeta;
import com.panet.imeta.trans.step.StepMetaInterface;
import com.panet.imeta.trans.step.errorhandling.Stream;
import com.panet.imeta.trans.step.errorhandling.StreamIcon;
import com.panet.imeta.trans.step.errorhandling.StreamInterface.StreamType;
...
public abstract class TransformClassBase {
	...
	public TransformClassBase(UserDefinedJavaClass parent,
		...
		try {
			data.inputRowMeta = getTransMeta().getPrevStepFields(getStepMeta())
					.clone();
			data.outputRowMeta = getTransMeta().getThisStepFields(
					getStepMeta(), null, data.inputRowMeta.clone());
			...
			for (StepDefinition stepDefinition : meta.getInfoStepDefinitions()) {
				if (stepDefinition.tag != null
						&& stepDefinition.stepMeta != null
						&& stepDefinition.stepMeta.getName() != null) {
					data.infoMap.put(stepDefinition.tag,
							stepDefinition.stepMeta.getName());
				}
			}

			data.targetMap = new HashMap<String, String>();
			for (StepDefinition stepDefinition : meta
					.getTargetStepDefinitions()) {
				if (stepDefinition.tag != null
						&& stepDefinition.stepMeta != null
						&& stepDefinition.stepMeta.getName() != null) {
					data.targetMap.put(stepDefinition.tag,
							stepDefinition.stepMeta.getName());
				}
			}
	...
	public void putError(RowMetaInterface rowMeta, Object[] row, long nrErrors,
			...
			throws KettleStepException {
		parent.putErrorImpl(rowMeta, row, nrErrors, errorDescriptions,
				fieldNames, errorCodes);
	}
	...
	public static void getFields(boolean clearResultFields,
			RowMetaInterface row, String originStepname,
			RowMetaInterface[] info, StepMeta nextStep, VariableSpace space,
			List<?> fields) throws KettleStepException {
	...
	public static StepIOMetaInterface getStepIOMeta(
		...
		StepIOMetaInterface ioMeta = new StepIOMeta(true, true, true, false,
				true, true);

		for (StepDefinition stepDefinition : meta.getInfoStepDefinitions()) {
			ioMeta.addStream(new Stream(StreamType.INFO,
					stepDefinition.stepMeta, stepDefinition.description,
					StreamIcon.INFO, null));
		}
		for (StepDefinition stepDefinition : meta.getTargetStepDefinitions()) {
			ioMeta.addStream(new Stream(StreamType.TARGET,
					stepDefinition.stepMeta, stepDefinition.description,
	...
	public FieldHelper get(Fields type, String name) throws KettleStepException {
		...
		case Info:
			...
			if (fh == null) {
				RowMetaInterface rmi = getTransMeta().getPrevInfoFields(
						getStepname());
				try {
		...
		default:
			throw new KettleStepException(Messages.getString(
					"TransformClassBase.Exception.InvalidFieldsType", type
							.name(), name));
		}
		return fh;
	}
