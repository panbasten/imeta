...
public class GPBulkLoaderMeta extends BaseStepMeta implements StepMetaInterface
{
	...
	public static final int NR_DATE_MASK_DATE           = 0;
	public static final int NR_DATE_MASK_DATETIME       = 1;
	
	public static final String[] fieldTypeDesc = new String[] { "",
		DATE_MASK_DATE, DATE_MASK_DATETIME
		 };
	...
	public void setInfo(Map<String, String[]> p, String id,
		...
		databaseMeta = DatabaseMeta.findDatabase(
				(List<DatabaseMeta>) databases, BaseStepMeta.parameterToLong(p
						.get(id + ".connection")));
 		maxErrors = BaseStepMeta.parameterToInt(p.get(id + ".maxErrors"));
        schemaName = BaseStepMeta.parameterToString(p.get(id + ".schemaName"));
		tableName = BaseStepMeta.parameterToString(p.get(id + ".tableName"));
		loadMethod = BaseStepMeta.parameterToString(p.get(id + ".loadMethod"));  
		loadAction = BaseStepMeta.parameterToString(p.get(id + ".loadAction"));  
		PsqlPath = BaseStepMeta.parameterToString(p.get(id + ".PsqlPath"));  
		controlFile = BaseStepMeta.parameterToString(p.get(id + ".controlFile")); 
		dataFile = BaseStepMeta.parameterToString(p.get(id + ".dataFile"));    
		logFile = BaseStepMeta.parameterToString(p.get(id + ".logFile"));      
		eraseFiles = BaseStepMeta.parameterToBoolean(p.get(id + ".eraseFiles"));   
		encoding = BaseStepMeta.parameterToString(p.get(id + ".encoding"));
		dbNameOverride = BaseStepMeta.parameterToString(p.get(id + ".dbNameOverride"));
		
		String[] fieldTable = p.get(id + "_fieldsToLoad.fieldTable");
		String[] fieldStream = p.get(id + "_fieldsToLoad.fieldStream");
		String[] dateMask = p.get(id + "_fieldsToLoad.dateMask");
		
		this.fieldTable = (fieldTable != null) ? fieldTable : (new String[0]);
		this.fieldStream = (fieldStream != null) ? fieldStream : (new String[0]);
		this.dateMask = (dateMask != null) ? dateMask : (new String[0]);
	}	
}
