...
public class SetVariableDialog extends BaseStepDialog implements
		StepDialogInterface {
	/**
	 * 列式表单
	 */
	...
	public SetVariableDialog(StepMeta stepMeta, TransMeta transMeta) {
	...
	public JSONObject open() throws ImetaException {
		try {
			...
			SetVariableMeta step = (SetVariableMeta)super.getStep();
			// 得到form
			...
			this.fields.addHeaders(new GridHeaderDataMeta[] {
					new GridHeaderDataMeta(id + "_fields.fieldId", "#",
							...
							fieldMeta,
					new GridHeaderDataMeta(id + "_fields.variableName", "变量名", null, false, 100),
				    (new GridHeaderDataMeta(id + "_fields.variableType",
							"变量活动类型", null, false, 200))
							.setOptions(super
									.getOptionsByStringArrayWithNumberValue(
											SetVariableMeta.fieldTypeDesc,
											false)),
					new GridHeaderDataMeta(id + "_fields.defaultValue", "默认值", null, false, 60)		
			});
			this.fields.setHasBottomBar(true);
			this.fields.setHasAdd(true, true,
					"jQuery.imeta.steps.setvariable.btn.fieldsAdd");
			...
			if (field != null && field.length > 0)
				for (int i = 0; i < field.length; i++) {
					this.fields.addRow(new Object[] {
							...
							new GridCellDataMeta(null,
									step.getVariableName()[i],
									GridCellDataMeta.CELL_TYPE_INPUT),
							new GridCellDataMeta(null, String.valueOf(step.getVariableType()[i]),
									GridCellDataMeta.CELL_TYPE_SELECT),
							new GridCellDataMeta(null, step.getDefaultValue()[i],
									GridCellDataMeta.CELL_TYPE_INPUT)	
					});
				}
			...
			columnFormMeta.putFieldsetsFootButtons(new ButtonMeta[] {super.getOkBtn(), super.getCancelBtn(),
					super.getGetfields("jQuery.imeta.steps.setvariable.btn.getfields")});
