...
public class UserDefinedJavaClassCodeSnippits {
		...
		try {
			Document doc = XMLHandler.loadXMLFile(
					UserDefinedJavaClassCodeSnippits.class
							.getResourceAsStream(strFileName), null, false,
					false);
			buildSnippitList(doc);
		} catch (Exception e) {
			throw new KettleXMLException(
					"Unable to read java values help file from file ["
							+ strFileName + "]", e);
		}
	}
	...
	public enum Category {
		COMMON(
				Messages
						.getString("UserDefinedJavaClassCodeSnippits.categories.COMMON")), STATUS(
				Messages
						.getString("UserDefinedJavaClassCodeSnippits.categories.STATUS")), LOGGING(
				Messages
						.getString("UserDefinedJavaClassCodeSnippits.categories.LOGGING")), LISTENERS(
				Messages
						.getString("UserDefinedJavaClassCodeSnippits.categories.LISTENERS")), ROW(
				Messages
						.getString("UserDefinedJavaClassCodeSnippits.categories.ROW")), OTHER(
				Messages
						.getString("UserDefinedJavaClassCodeSnippits.categories.OTHER"));
	...
	private void buildSnippitList(Document doc) {
		...
		for (Node node : nodes) {
			...
			if (oldSnippit != null) {
				log.logError(toString(), "Multiple code snippits for name: "
						+ snippit.name);
			}
		}
	}
