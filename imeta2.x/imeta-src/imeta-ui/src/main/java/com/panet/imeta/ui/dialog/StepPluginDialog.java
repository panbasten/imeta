...
public class StepPluginDialog {
	...
	private LabelGridMeta StepPluginList;
	public StepPluginDialog() throws KettleException {
	}
	...
	public JSONObject open() throws ImetaException {
		try {
			...
			this.id = "StepPluginList";

			// 得到form
			this.columnFormDataMeta = new ColumnFormDataMeta(id, null);
			this.columnFormMeta = new ColumnFormMeta(columnFormDataMeta);
			this.StepPluginList = new LabelGridMeta(id + "_StepPluginList", "可以使用的步骤和步骤插件", 400);
			this.StepPluginList.addHeaders(new GridHeaderDataMeta[] {
					...
					new GridHeaderDataMeta(this.id + "JarfilesList", "Jar文件类表", null,false, 100),
					new GridHeaderDataMeta(this.id + "IconFilename", "图标文件", null, false,80),
					new GridHeaderDataMeta(this.id + "Classname", "类名", null,false, 260),
					new GridHeaderDataMeta(this.id + "Category", "步骤类别", null,false, 100),
					new GridHeaderDataMeta(this.id + "ErrorHelpFile", "错误帮助文件", null, false,100),
			...
			for (int i=0;i<pluginInformation.size();i++) {
				...
				this.StepPluginList.addRow(rowDataMeta);
			}
			this.StepPluginList.setSingle(true);

			this.columnFormMeta
					.putFieldsetsContent(new BaseFormMeta[] { this.StepPluginList });

			cArr.add(this.columnFormMeta.getFormJo());

			rtn.put("items", cArr);

			rtn.put("title", "步骤插件列表");
			rtn.put("id", this.id);
