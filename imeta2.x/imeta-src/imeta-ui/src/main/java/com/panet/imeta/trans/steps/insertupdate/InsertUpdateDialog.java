...
public class InsertUpdateDialog extends BaseStepDialog implements
		StepDialogInterface {
	/**
	 * 列式表单
	 */
	 ...
	 * 不执行更新
	 */
	private LabelInputMeta updateBypassed;
	...
	public InsertUpdateDialog(StepMeta stepMeta, TransMeta transMeta) {
	...
	public JSONObject open() throws ImetaException {
		try {
			...
			InsertUpdateMeta step = (InsertUpdateMeta) super.getStep();
			...
			this.schemaName = new LabelInputMeta(id + ".schemaName", "目的模式",
					null, null, "目的模式必须填写", step.getSchemaName(), null,
			...
			this.commitSize = new LabelInputMeta(id + ".commitSize", "提交的大小",
					null, null, "Commit size必须填写", String.valueOf(step
							.getCommitSize()), null, ValidateForm.getInstance()
							.setRequired(false));

			this.commitSize.setSingle(true);

			// 不执行任何更新
			this.updateBypassed = new LabelInputMeta(id + ".updateBypassed",
					"不执行更新", null, null, null, String.valueOf(step
							.isUpdateBypassed()),
					InputDataMeta.INPUT_TYPE_CHECKBOX, ValidateForm
							.getInstance().setRequired(false));
			this.updateBypassed.setSingle(false);

			// 得到用来查询的关键字
			this.keywords = new LabelGridMeta(id + "_keywords", null, 150);

			GridHeaderDataMeta conditionTypeDataMeta = new GridHeaderDataMeta(
					id + "_keywords.keyCondition", "比较符", null, false, 100);
			conditionTypeDataMeta.setOptions(super.getOptionsByList(super
					.getSqlConditions(), true));

			this.keywords.addHeaders(new GridHeaderDataMeta[] {
					new GridHeaderDataMeta(id + "_keywords.fieldId", "#",
							GridHeaderDataMeta.HEADER_TYPE_NUMBER, false, 50),
					new GridHeaderDataMeta(id + "_keywords.keyLookup", "表字段",
							null, false, 100),
					conditionTypeDataMeta,
					new GridHeaderDataMeta(id + "_keywords.keyStream",
							"流里的字段1", null, false, 100),
					new GridHeaderDataMeta(id + "_keywords.keyStream2",
							"流里的字段2", null, false, 100) });
			...
			this.keywords.setHasAdd(true, true,
					"jQuery.imeta.steps.insertupdate.btn.keywordsAdd");
			this.keywords.setHasDelete(true, true,
					"jQuery.imeta.parameter.fieldsDelete");

			DivMeta div = new DivMeta(new NullSimpleFormDataMeta(), true);
			super.getGetfields("jQuery.imeta.steps.insertupdate.btn.getfields")
					.appendTo(div);

			// 得到更新字段

			this.refreshwords = new LabelGridMeta(id + "_refreshwords", null,
					150);

			GridHeaderDataMeta fieldBooleanTypeDataMeta = new GridHeaderDataMeta(
					id + "_refreshwords.update", "更新", null, false, 100);
			...
			this.refreshwords.addHeaders(new GridHeaderDataMeta[] {
					new GridHeaderDataMeta(id + "_refreshwords.fieldId", "#",
							null, false, 50),
					new GridHeaderDataMeta(id + "_refreshwords.updateLookup",
							"表字段", null, false, 100),
					new GridHeaderDataMeta(id + "_refreshwords.updateStream",
							"流里的字段", null, false, 100),
			...
			this.refreshwords.setHasAdd(true, true,
					"jQuery.imeta.steps.insertupdate.btn.refreshwordsAdd");
			this.refreshwords.setHasDelete(true, true,
					"jQuery.imeta.parameter.fieldsDelete");

			this.getrefreshwords = new ButtonMeta(id + ".btn.getrefreshwords",
					id + ".btn.getrefreshwords", "获取-更新字段", "获取-更新字段");
			this.getrefreshwords
					.addClick("jQuery.imeta.steps.insertupdate.btn.getRefreshFields");
			this.getrefreshwords.putProperty("roType",
					RepositoryObject.STRING_OBJECT_TYPE_TRANSFORMATION);
			this.getrefreshwords.putProperty("roName", super.getTransMeta()
					.getName());
			this.getrefreshwords
			...
			columnFormMeta.putFieldsetsContent(new BaseFormMeta[] { this.name,
					...
					this.commitSize, this.updateBypassed, this.keywords, div,
					this.refreshwords, this.getrefreshwords });
