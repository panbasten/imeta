...
import com.plywet.imeta.core.xml.XMLHandler;
import com.plywet.imeta.utils.Const;

/**
 * This class defines the location of a transformation, job or schema in the
 * repository. That means that it's just an extra parameter for recognizing a
 * transformation, job or schema. It allows for sub-directories by linking back
 * to itself.
 * 
 * TODO: This class is referenced in a large amount of Interfaces. We should
...
public class RepositoryDirectory implements RepositoryDirectoryInterface {
	...
	/**
	 * True to show this directory in UIs. Not necessarily persisted. Each repo
	 * impl decides whether to mark each dir as visible.
	 */
	...
	public RepositoryDirectory(RepositoryDirectoryInterface parent,
		...
		this.children = new ArrayList<RepositoryDirectoryInterface>(); // default:
		// no
		// subdirectories...
	 ...
	 * by the RepositoryDirectory.DIRECTORY_SEPARATOR property (slash).
	 * 
	...
	public RepositoryDirectory findDirectory(String path[]) {
		...
		if (directoryPath.length == 1
				&& directoryPath[0].equalsIgnoreCase(getName())) {
			return this;
		} else
		...
		if (directoryPath.length >= 1) {
			...
			for (int i = 0; i < directoryPath.length; i++) {
				RepositoryDirectory directory = follow
						.findChild(directoryPath[i]);
				if (directory == null)
					return null;
				follow = directory;
			}
			...
			/*
			 * for (int i=0;i<getNrSubdirectories();i++) { RepositoryDirectory
			 * subdir = getSubdirectory(i); if
			 * (subdir.getDirectoryName().equalsIgnoreCase(directoryPath[0])) {
			 * if (directoryPath.length==1) return subdir; // we arrived at the
			 * destination...
			 * 
			 * // Copy the path exception the highest level, we go down one...
			 * String subpath[] = new String[directoryPath.length-1]; for (int
			 * x=0;x<subpath.length;x++) subpath[x]=directoryPath[x+1];
			 * 
			 * // Perhaps the rest of the path is the same too?
			 * RepositoryDirectory look = subdir.findDirectory(subpath); if
